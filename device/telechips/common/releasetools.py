...
def Install_Splash_Img(info):
    try:
        splash_img = info.input_zip.read("RADIO/splash.img")
    except KeyError:
        print "warning: no splash image in input target_files; not flashing splash image"
        return

    info.script.Print("Writing splash image...")
    common.ZipWriteStr(info.output_zip, "splash.img", splash_img)
    info.script.WriteRawImage("/splash", "splash.img")


def Insert_Reboot_Cmd(info):
    info.script.AppendExtra(('''
					assert(telechips.reboot( 0, 0, 0, 0, 0, 0 ));
					''' % locals()).lstrip())
