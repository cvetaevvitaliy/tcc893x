...
public class VoicemailContentProvider extends ContentProvider
    ...
    private ContactsDatabaseHelper mVoicemailContentDbHelper;
    private ContactsDatabaseHelper mVoicemailStatusDbHelper;
    
	public void reinit() {
		Context context = context();

		if (mVoicemailContentTable == null) {
			mVoicemailContentDbHelper = getDatabaseHelper(context);
			mVoicemailContentTable = new VoicemailContentTable(Tables.CALLS,
					context, getDatabaseHelper(context), this,
					createCallLogInsertionHelper(context));
		}

		if (mVoicemailStatusTable == null) {
			mVoicemailStatusDbHelper = getDatabaseHelper(context);
			mVoicemailStatusTable = new VoicemailStatusTable(
					Tables.VOICEMAIL_STATUS, context,
    ...
    public boolean onCreate() {
        ...
        mVoicemailContentTable = new VoicemailContentTable(Tables.CALLS, context,
        		mVoicemailContentDbHelper, this, createCallLogInsertionHelper(context));
        mVoicemailStatusDbHelper = getDatabaseHelper(context);
        mVoicemailStatusTable = new VoicemailStatusTable(Tables.VOICEMAIL_STATUS, context,
        		mVoicemailStatusDbHelper, this);
        ...
        this.getContext().registerReceiver(new BroadcastReceiver() {
            ...
            public void onReceive(Context context1, Intent intent) {
                 ...
                 if (Intent.ACTION_PRE_QUICKBOOT.equals(action)) {
                	 if( mVoicemailContentTable != null) {
                		 mVoicemailContentDbHelper.close();
                		 mVoicemailContentDbHelper = null;
                		 mVoicemailContentTable = null;
                	 }
                	 
                	 if( mVoicemailStatusTable != null) {
                		 mVoicemailStatusDbHelper.close();
                		 mVoicemailStatusDbHelper = null;
                		 mVoicemailStatusTable = null;
                	 }
    ...
    private VoicemailTable.Delegate getTableDelegate(UriData uriData) {
        switch (uriData.getUriType()) {
            ...
            case STATUS_ID:
            	if(mVoicemailStatusTable == null) {
            		Log.e("VoicemailContentProvider", Log.getStackTraceString(new Throwable()));
            		reinit();
            	}
