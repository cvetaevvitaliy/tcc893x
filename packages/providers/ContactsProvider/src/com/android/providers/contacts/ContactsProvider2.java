...
public class ContactsProvider2 extends AbstractContactsProvider
	...
	private void reinitHelpers() {
		...
		if (inProfileMode()) {
			mDbHelper.set(getProfileDatabaseHelper());
			...
			mDbHelper.set(getContactsDatabaseHelper());
	...
	private ProfileDatabaseHelper getProfileDatabaseHelper() {
		if (mProfileHelper == null) {
	...
	private ContactsDatabaseHelper getContactsDatabaseHelper() {
		if (mContactsHelper == null) {
			Log.e(TAG, Log.getStackTraceString(new Throwable()));
			mContactsHelper = getDatabaseHelper(getContext());
		}
	...
	private ThreadLocal<ContactsDatabaseHelper> getDbHelper() {
		if( mDbHelper.get() == null ) {
			reinitHelpers();
		}
    ...
    private void initForDefaultLocale() {
        ...
        mLegacyApiSupport = new LegacyApiSupport(context, getContactsDatabaseHelper(), this,
                mGlobalSearchSupport);
        mCurrentLocale = getLocale();
        mNameSplitter = getContactsDatabaseHelper().createNameSplitter(mCurrentLocale);
        mNameLookupBuilder = new StructuredNameLookupBuilder(mNameSplitter);
        mPostalSplitter = new PostalSplitter(mCurrentLocale);
        mCommonNicknameCache = new CommonNicknameCache(getContactsDatabaseHelper().getReadableDatabase());
        ContactLocaleUtils.setLocale(mCurrentLocale);
        mContactAggregator = new ContactAggregator(this, getContactsDatabaseHelper(),
                createPhotoPriorityResolver(context), mNameSplitter, mCommonNicknameCache);
        mContactAggregator.setEnabled(SystemProperties.getBoolean(AGGREGATE_CONTACTS, true));
        mProfileAggregator = new ProfileAggregator(this, getProfileDatabaseHelper(),
                createPhotoPriorityResolver(context), mNameSplitter, mCommonNicknameCache);
        ...
        mContactsPhotoStore = new PhotoStore(getContext().getFilesDir(), getContactsDatabaseHelper());
        mProfilePhotoStore = new PhotoStore(new File(getContext().getFilesDir(), "profile"),
                getProfileDatabaseHelper());

        mDataRowHandlers = new HashMap<String, DataRowHandler>();
        initDataRowHandlers(mDataRowHandlers, getContactsDatabaseHelper(), mContactAggregator,
                mContactsPhotoStore);
        mProfileDataRowHandlers = new HashMap<String, DataRowHandler>();
        initDataRowHandlers(mProfileDataRowHandlers, getProfileDatabaseHelper(), mProfileAggregator,
                mProfilePhotoStore);
    ...
    protected void performBackgroundTask(int task, Object arg) {
        ...
        switch (task) {
            ...
            case BACKGROUND_TASK_CLEAN_DELETE_LOG: {
                final SQLiteDatabase db = getDbHelper().get().getWritableDatabase();
    ...
    protected void updateLocaleInBackground() {
        ...
        if (!needsToUpdateLocaleData(prefs, currentLocale,
                        getContactsDatabaseHelper(), getProfileDatabaseHelper())) {
            return;
        }

        int providerStatus = mProviderStatus;
        setProviderStatus(ProviderStatus.STATUS_CHANGING_LOCALE);
        getContactsDatabaseHelper().setLocale(currentLocale);
        getProfileDatabaseHelper().setLocale(currentLocale);
    ...
    private void changeLocaleInBackground() {
        ...
        SQLiteDatabase db = getContactsDatabaseHelper().getWritableDatabase();
        SQLiteDatabase profileDb = getProfileDatabaseHelper().getWritableDatabase();
        db.beginTransaction();
    ...
    private void updateProviderStatus() {
        ...
        if (mContactsAccountCount == 0) {
            boolean isContactsEmpty = DatabaseUtils.queryIsEmpty(getContactsDatabaseHelper().getReadableDatabase(), Tables.CONTACTS);
            long profileNum = DatabaseUtils.queryNumEntries(getProfileDatabaseHelper().getReadableDatabase(),
    ...
    protected void cleanupPhotoStore() {
        ...
        long photoMimeTypeId = getDbHelper().get().getMimeTypeId(Photo.CONTENT_ITEM_TYPE);
    ...
    void wipeData() {
        invalidateFastScrollingIndexCache();
        getContactsDatabaseHelper().wipeData();
        getProfileDatabaseHelper().wipeData();
    ...
    private void switchToProfileMode() {
        ...
        getDbHelper().set(getProfileDatabaseHelper());
        mTransactionContext.set(mProfileTransactionContext);
        mAggregator.set(mProfileAggregator);
        mPhotoStore.set(mProfilePhotoStore);
        mInProfileMode.set(true);
    ...
    private void switchToContactMode() {
        ...
        getDbHelper().set(getContactsDatabaseHelper());
        mTransactionContext.set(mContactTransactionContext);
        mAggregator.set(mContactAggregator);
        mPhotoStore.set(mContactsPhotoStore);
        mInProfileMode.set(false);
    ...
    protected void onCommitTransactionInternal(boolean forProfile) {
        ...
        if (mVisibleTouched) {
            ...
            getDbHelper().get().updateAllVisible();
    ...
    protected void onRollbackTransactionInternal(boolean forProfile) {
        ...
        getDbHelper().get().invalidateAllCache();
    }
    ...
    private void flushTransactionalChanges() {
        ...
        for (long rawContactId : mTransactionContext.get().getInsertedRawContactIds()) {
            getDbHelper().get().updateRawContactDisplayName(db, rawContactId);
        ...
        for (Map.Entry<Long, Object> entry : mTransactionContext.get().getUpdatedSyncStates()) {
            ...
            if (getDbHelper().get().getSyncState().update(db, id, entry.getValue()) <= 0) {
    ...
    public DataRowHandler getDataRowHandler(final String mimeType) {
        ...
        if (handler == null) {
            handler = new DataRowHandlerForCustomMimetype(
                    getContext(), getContactsDatabaseHelper(), mContactAggregator, mimeType);
    ...
    public DataRowHandler getDataRowHandlerForProfile(final String mimeType) {
        ...
        if (handler == null) {
            handler = new DataRowHandlerForCustomMimetype(
                    getContext(), getProfileDatabaseHelper(), mProfileAggregator, mimeType);
    ...
    protected Uri insertInTransaction(Uri uri, ContentValues values) {
        ...
        switch (match) {
            ...
            case PROFILE_SYNCSTATE:
                id = getDbHelper().get().getSyncState().insert(db, values);
                break;
    ...
    private Account resolveAccount(Uri uri, ContentValues values) throws IllegalArgumentException {
        ...
        if (partialUri || partialValues) {
            ...
            throw new IllegalArgumentException(getDbHelper().get().exceptionMessage(
    ...
    private long resolveAccountIdInTransaction(Uri uri, ContentValues values) {
        return getDbHelper().get().getOrCreateAccountIdInTransaction(
    ...
    private Long findGroupByRawContactId(String selection, long rawContactId) {
        final SQLiteDatabase db = getDbHelper().get().getReadableDatabase();
    ...
    private void insertDataGroupMembership(long rawContactId, long groupId) {
        ...
        groupMembershipValues.put(DataColumns.MIMETYPE_ID,
                getDbHelper().get().getMimeTypeId(GroupMembership.CONTENT_ITEM_TYPE));
        final SQLiteDatabase db = getDbHelper().get().getWritableDatabase();
    ...
    private void deleteDataGroupMembership(long rawContactId, long groupId) {
        final String[] selectionArgs = {
                Long.toString(getDbHelper().get().getMimeTypeId(GroupMembership.CONTENT_ITEM_TYPE)),
    ...
    private long insertData(ContentValues values, boolean callerIsSyncAdapter) {
        ...
        if (packageName != null) {
            mValues.put(DataColumns.PACKAGE_ID, getDbHelper().get().getPackageId(packageName));
        }
        ...
        mValues.put(DataColumns.MIMETYPE_ID, getDbHelper().get().getMimeTypeId(mimeType));
    ...
    private long insertGroup(Uri uri, ContentValues values, boolean callerIsSyncAdapter) {
        ...
        final long accountId = getDbHelper().get().getOrCreateAccountIdInTransaction(
                resolveAccountWithDataSet(uri, mValues));
        ...
        if (packageName != null) {
            mValues.put(GroupsColumns.PACKAGE_ID, getDbHelper().get().getPackageId(packageName));
        }
    ...
    public long insertStatusUpdate(ContentValues values) {
        ...
        if (dataId != null) {
            ...
            String mimeTypeIdIm = String.valueOf(getDbHelper().get().getMimeTypeIdForIm());
            if (matchEmail) {
                String mimeTypeIdEmail = String.valueOf(getDbHelper().get().getMimeTypeIdForEmail());
        ...
        if (values.containsKey(StatusUpdates.STATUS)) {
            ...
            if (TextUtils.isEmpty(status)) {
                getDbHelper().get().deleteStatusUpdate(dataId);
            } else {
                Long timestamp = values.getAsLong(StatusUpdates.STATUS_TIMESTAMP);
                if (timestamp != null) {
                    getDbHelper().get().replaceStatusUpdate(dataId, timestamp, status, resPackage,
                            iconResourceId, labelResourceId);
                } else {
                    getDbHelper().get().insertStatusUpdate(dataId, status, resPackage, iconResourceId,
                            labelResourceId);
                }
    ...
    protected int deleteInTransaction(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case SYNCSTATE:
            case PROFILE_SYNCSTATE:
                return getDbHelper().get().getSyncState().delete(db, selection,
                        selectionArgs);
            ...
            case SYNCSTATE_ID: {
                ...
                return getDbHelper().get().getSyncState().delete(db, selectionWithId,
                        selectionArgs);
            }
            ...
            case PROFILE_SYNCSTATE_ID: {
                String selectionWithId =
                        (SyncStateContract.Columns._ID + "=" + ContentUris.parseId(uri) + " ")
                        + (selection == null ? "" : " AND (" + selection + ")");
                return getProfileDatabaseHelper().getSyncState().delete(db, selectionWithId,
                        selectionArgs);
            }
            ...
            case PROFILE_RAW_CONTACTS_ID: {
                ...
                return deleteRawContact(rawContactId, getDbHelper().get().getContactId(rawContactId),
                        callerIsSyncAdapter);
            }
    ...
    public int deleteGroup(Uri uri, long groupId, boolean callerIsSyncAdapter) {
        final SQLiteDatabase db = getDbHelper().get().getWritableDatabase();
        mGroupIdCache.clear();
        final long groupMembershipMimetypeId = getDbHelper().get()
    ...
    protected int updateInTransaction(Uri uri, ContentValues values, String selection,
        ...
        switch(match) {
            case SYNCSTATE:
            case PROFILE_SYNCSTATE:
                return getDbHelper().get().getSyncState().update(db, values,
            ...
            case PROFILE_SYNCSTATE_ID: {
                ...
                String selectionWithId =
                        (SyncStateContract.Columns._ID + "=" + ContentUris.parseId(uri) + " ")
                        + (selection == null ? "" : " AND (" + selection + ")");
                return getProfileDatabaseHelper().getSyncState().update(db, values,
                        selectionWithId, selectionArgs);
            }
    ...
    private int updateGroups(Uri uri, ContentValues originalValues, String selectionWithId,
        ...
        final SQLiteDatabase db = getDbHelper().get().getWritableDatabase();
        final ContactsDatabaseHelper dbHelper = getDbHelper().get();
        ...
        if (count != 0) {
            ...
            if (flagExists(values, RawContacts.NAME_VERIFIED)) {
                ...
                if (flagIsSet(values, RawContacts.NAME_VERIFIED)) {
                    getDbHelper().get().resetNameVerifiedForOtherRawContacts(rawContactId);
                }
    ...
    private DirectoryInfo getDirectoryAuthority(String directoryId) {
        synchronized (mDirectoryCache) {
            if (!mDirectoryCacheValid) {
                ...
                SQLiteDatabase db = getDbHelper().get().getReadableDatabase();
    ...
    protected Cursor queryLocal(final Uri uri, final String[] projection, String selection,
        ...
        switch (match) {
            case SYNCSTATE:
            case PROFILE_SYNCSTATE:
                return getDbHelper().get().getSyncState().query(db, projection, selection,
                        selectionArgs, sortOrder);
            ...
            case CONTACTS_STREQUENT: {
                ...
                if (phoneOnly) {
                    ...
                    final long phoneMimeTypeId =
                            getDbHelper().get().getMimeTypeId(Phone.CONTENT_ITEM_TYPE);
                    final long sipMimeTypeId =
                            getDbHelper().get().getMimeTypeId(SipAddress.CONTENT_ITEM_TYPE);
            ...
            case CALLABLES: {
                final String mimeTypeIsPhoneExpression =
                        DataColumns.MIMETYPE_ID + "=" + getDbHelper().get().getMimeTypeIdForPhone();
                final String mimeTypeIsSipExpression =
                        DataColumns.MIMETYPE_ID + "=" + getDbHelper().get().getMimeTypeIdForSip();
                setTablesAndProjectionMapForData(qb, uri, projection, false);
                if (match == CALLABLES) {
            ...
            case EMAILS: {
                ...
                qb.appendWhere(" AND " + DataColumns.MIMETYPE_ID + " = "
                        + getDbHelper().get().getMimeTypeIdForEmail());
            ...
            case EMAILS_LOOKUP: {
                ...
                if (uri.getPathSegments().size() > 2) {
                    ...
                    String address = getDbHelper().get().extractAddressFromEmailAddress(email);
            ...
            case EMAILS_FILTER: {
                ...
                if (filterParam == null) {
                    ...
                    sb.append(
                            ...
                            " FROM " + Tables.DATA +
                            " WHERE " + DataColumns.MIMETYPE_ID + "=");
                    sb.append(getDbHelper().get().getMimeTypeIdForEmail());
                    sb.append(" AND " + Data.DATA1 + " LIKE ");
                    DatabaseUtils.appendEscapedSQLString(sb, filterParam + '%');
            ...
            case CONTACTABLES_FILTER: {
                ...
                sb.append(getDbHelper().get().getMimeTypeIdForEmail());
                sb.append(",");
                sb.append(getDbHelper().get().getMimeTypeIdForPhone());
                sb.append("))");
            ...
            case POSTALS: {
                setTablesAndProjectionMapForData(qb, uri, projection, false);
                qb.appendWhere(" AND " + DataColumns.MIMETYPE_ID + " = "
                        + getDbHelper().get().getMimeTypeIdForStructuredPostal());
            ...
            case PHONE_LOOKUP: {
                ...
                if (uri.getBooleanQueryParameter(PhoneLookup.QUERY_PARAMETER_SIP_ADDRESS, false)) {
                    ...
                    StringBuilder sb = new StringBuilder();
                    selectionArgs = getDbHelper().get().buildSipContactQuery(sb, sipAddress);
                    ...
                    String numberE164 = PhoneNumberUtils.formatNumberToE164(number,
                            getDbHelper().get().getCurrentCountryIso());
                    String normalizedNumber =
                            PhoneNumberUtils.normalizeNumber(number);
                    getDbHelper().get().buildPhoneLookupAndContactQuery(
                    ...
                    try {
                        if (cursor.getCount() > 0) {
                            ...
                            // numbers
                            getDbHelper().get().buildFallbackPhoneLookupAndContactQuery(qb, number);
            ...
            case SETTINGS: {
                ...
                final String groupMembershipMimetypeId = Long.toString(getDbHelper().get()
                        .getMimeTypeId(GroupMembership.CONTENT_ITEM_TYPE));
                if (projection != null && projection.length != 0 &&
                        getDbHelper().get().isInProjection(projection, Settings.UNGROUPED_COUNT)) {
                    selectionArgs = insertSelectionArg(selectionArgs, groupMembershipMimetypeId);
                }
                if (projection != null && projection.length != 0 &&
                        getDbHelper().get().isInProjection(
    ...
    public void appendSearchIndexJoin(StringBuilder sb, String filter,
        ...
        if (filter.indexOf('@') != -1) {
            emailAddress = getDbHelper().get().extractAddressFromEmailAddress(filter);
    ...
    private void appendLocalDirectoryAndAccountSelectionIfNeeded(SQLiteQueryBuilder qb,
        ...
        if (validAccount) {
            final Long accountId = getDbHelper().get().getAccountIdOrNull(accountWithDataSet);
            if (accountId == null) {
                // No such account.
                sb.setLength(0);
                sb.append("(1=2)");
    ...
    private AssetFileDescriptor openAssetFileInner(Uri uri, String mode)
        ...
        final SQLiteDatabase db = getDbHelper().get().getDatabase(writing);
        ...
        switch (match) {
            ...
            default:
                throw new FileNotFoundException(getDbHelper().get().exceptionMessage(
    ...
    private AssetFileDescriptor openPhotoAssetFile(SQLiteDatabase db, Uri uri, String mode,
            ...
            throws FileNotFoundException {
        if (!"r".equals(mode)) {
            throw new FileNotFoundException(getDbHelper().get().exceptionMessage("Mode " + mode
                    + " not supported.", uri));
        }
        ...
        switch (match) {
            ...
            case DATA_ID:
                ...
                if (ContactsContract.isProfileId(id)) {
                    return getProfileDatabaseHelper().getDataMimeType(id);
                } else {
                    return getContactsDatabaseHelper().getDataMimeType(id);
                }
            case PHONES:
    ...
    private class StructuredNameLookupBuilder extends NameLookupBuilder {
        ...
        protected void insertNameLookup(long rawContactId, long dataId, int lookupType,
                String name) {
            getDbHelper().get().insertNameLookup(rawContactId, dataId, lookupType, name);
        }
    ...
    protected boolean isAggregationUpgradeNeeded() {
        ...
        int version = Integer.parseInt(getContactsDatabaseHelper().getProperty(
    ...
    protected void upgradeAggregationAlgorithmInBackground() {
        ...
        try {
            ...
            try {
                ...
                switchToContactMode();
                db = getContactsDatabaseHelper().getWritableDatabase();
            ...
            try {
                final SQLiteDatabase db =  getContactsDatabaseHelper().getWritableDatabase();
                db.beginTransaction();
                try {
                    updateAggregationAlgorithmVersion();
                    db.setTransactionSuccessful();
    ...
    private void updateAggregationAlgorithmVersion() {
        getContactsDatabaseHelper().setProperty(DbProperties.AGGREGATION_ALGORITHM,
