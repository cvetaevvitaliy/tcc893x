...
public class LauncherProvider extends ContentProvider {
    ...
    public Uri insert(Uri uri, ContentValues initialValues) {
        ...
        SQLiteDatabase db = getOpenHelper().getWritableDatabase();
        final long rowId = dbInsertAndCheck(getOpenHelper(), db, args.table, null, initialValues);
    ...
    public int bulkInsert(Uri uri, ContentValues[] values) {
        ...
        try {
            ...
            for (int i = 0; i < numValues; i++) {
                if (dbInsertAndCheck(getOpenHelper(), db, args.table, null, values[i]) < 0) {
                    return 0;
                }
            }
    ...
    public long generateNewId() {
        return getOpenHelper().generateNewId();
    }
    ...
    synchronized public void loadDefaultFavoritesIfNecessary(int origWorkspaceResId) {
        ...
        if (sp.getBoolean(DB_CREATED_BUT_DEFAULT_WORKSPACE_NOT_LOADED, false)) {
            ...
            getOpenHelper().loadFavorites(getOpenHelper().getWritableDatabase(), workspaceResId);
            editor.commit();
        }
    }
