...
#define BLUETOOTH_DEVICE "/dev/tcc_bt_dev"
...
static jboolean setClockLimit(JNIEnv* env, jobject obj) {
	ALOGE("%s", __FUNCTION__);
	int fd = -1;
	int set;

		fd = open(BLUETOOTH_DEVICE, O_RDWR);
		if(fd<0){
			ALOGE("Can't open tcc_bt_dev");
			return JNI_FALSE;
		}
		else
	      {
             	set = 1;
	             ioctl(fd, IOCTL_BT_DEV_CLOCK_LIMIT, &set);
      		      close(fd);
      	      		return JNI_TRUE;
      		}
	return JNI_FALSE;
...
static jboolean releaseClockLimit(JNIEnv* env, jobject obj) {
    ALOGE("%s", __FUNCTION__);
    int fd = -1;
    int set;
...
static JNINativeMethod sMethods[] = {
    ...
    {"setClockLimit", "()Z",  (void*) setClockLimit},
    {"releaseClockLimit", "()Z",  (void*) releaseClockLimit},
