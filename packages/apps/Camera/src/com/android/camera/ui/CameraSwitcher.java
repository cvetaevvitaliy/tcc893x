...
public class CameraSwitcher extends RotateImageView
    ...
    private void initPopup() {
        ...
        //20130108 ysseung   Case of single-camera, do not support panorama-shot. - start - 
        int drawUI;
        int n = android.hardware.Camera.getNumberOfCameras();
        Log.d(TAG, "Get number of cameras = " + n);
        if(n == 1) {
            drawUI = n + 1;
        } else {
            drawUI = n - 1;
        }
        //20130108 ysseung   Case of single-camera, do not support panorama-shot. - end -
        //for (int i = mDrawIds.length - 1; i >= 0; i--) {
        for (int i = mDrawIds.length - drawUI; i >= 0; i--) {
