...
public class SoundSettings extends SettingsPreferenceFragment implements
    ...
    private static final int FALLBACK_SPDIF_SETTING_VALUE = 0;

    private static final String KEY_SPDIF_SETTING = "spdif_setting";
    ...
    private ListPreference mSPDIFSetting;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mAudio_active = "true".equals(SystemProperties.get("ro.system.audio_active"));
        ...
        mSPDIFSetting = (ListPreference) findPreference(KEY_SPDIF_SETTING);
        if(mSPDIFSetting == null)
                Log.d(TAG, "mSPDIFSetting is null");
                Log.d( TAG, "SPDIF_SETTING = "+Settings.System.getInt(resolver, Settings.System.SPDIF_SETTING, 1) );

        mSPDIFSetting.setValue( String.valueOf(Settings.System.getInt(
                resolver, Settings.System.SPDIF_SETTING, FALLBACK_SPDIF_SETTING_VALUE)) );
        mSPDIFSetting.setOnPreferenceChangeListener(this);

        // SPDIF Enabled
        if(Settings.System.getInt(resolver, Settings.System.SPDIF_SETTING, 0) != 0) {
            mSoundEffects.setEnabled(false);
            Settings.System.putInt(resolver, Settings.System.SOUND_EFFECTS_ENABLED, 0);
        }
        ...
        if(!mAudio_active)
        {
                getPreferenceScreen().removePreference(findPreference(KEY_SPDIF_SETTING));
        }

        // Remove ringtone, touch sound, screen lock sound if its stb
        if ("1".equals(SystemProperties.get("tcc.solution.mbox"))) {
    ...
    public void onResume() {
		...
		updateSPDIFSettingSummary(mSPDIFSetting.getValue());
        lookupRingtoneNames();
    ...
    private void updateSPDIFSettingSummary(Object value) {
        CharSequence[] summaries = getResources().getTextArray(R.array.spdif_setting_summaries);
        CharSequence[] values = mSPDIFSetting.getEntryValues();
        for (int i=0; i<values.length; i++) {
            if (values[i].equals(value)) {
                mSPDIFSetting.setSummary(summaries[i]);
                break;
            }
        }
    }
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
		...
		if (KEY_SPDIF_SETTING.equals(key)) {
			 int value = Integer.parseInt((String) objValue);
			 int value_old;
			 try {
				 // S: For SPDIF
				 value_old = SystemProperties.getInt("persist.sys.spdif_setting", 1);
				 if((value == 2 || value_old == 2) || (value == 3) || (value == 4) ) {	 // SPDIF Planet 20120203
				 ...
				 Settings.System.putInt(getContentResolver(),
						 Settings.System.SPDIF_SETTING, value);
				 updateSPDIFSettingSummary(objValue);
				 ...
				 // Music playback disable
				 AudioManager audiomanager = (AudioManager) getSystemService(Context.AUDIO_SERVICE);
				 if(value == 1) {
					 audiomanager.setSPDIFPCMOn(true);
				 }
				 else {
					 audiomanager.setSPDIFPCMOn(false);
				 }
				 // E: For SPDIF
		
			} catch (NumberFormatException e) {
				 Log.e(TAG, "could not persist screen spdif setting", e);
			 }
		 }
