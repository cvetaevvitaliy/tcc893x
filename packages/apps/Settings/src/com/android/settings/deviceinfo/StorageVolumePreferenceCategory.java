...
public class StorageVolumePreferenceCategory extends PreferenceCategory {
    ...
    private Preference mFormatPreferenceExternal;
    ...
    public void init() {
        ...
        final boolean isRemovable = mVolume != null ? mVolume.isRemovable() : false;
				...
				if (mVolume != null)
					volumePath = mVolume.getPath();
				/*
        boolean isRemovable = false;
				String volumePath = null;
				if (mVolume != null) {
					volumePath = mVolume.getPath();
					if (volumePath.equals("/mnt/sdcard") ||
							volumePath.equals("/mnt/ext_sd"))
						isRemovable = true;
        ...
        boolean allowFormatExternal = false;
				if (mVolume != null) {
					if (volumePath.equals("/mnt/sdcard"))
					allowFormat = true;
					if (volumePath.equals("/mnt/ext_sd") ||
							volumePath.equals("/storage/sdcard0") ||
							volumePath.equals("/storage/sdcard1"))
					allowFormatExternal = true;
				}
					
				if (mVolume != null)
					Log.d("allowFormat", "ksjung - mVolume.getPath() : " + mVolume.getPath());
        if (allowFormat) {
            ...
            addPreference(mFormatPreference);
				...
				if (allowFormatExternal) {
            mFormatPreferenceExternal = new Preference(context);
            mFormatPreferenceExternal.setTitle(R.string.sd_format);
            mFormatPreferenceExternal.setSummary(R.string.sd_format_summary);
            addPreference(mFormatPreferenceExternal);
        }
    ...
    private void updatePreferencesFromState() {
        ...
        final String state = mStorageManager.getVolumeState(mVolume.getPath());
				Log.d(TAG, "ksjung - mVolume.getPath() : " + mVolume.getPath());
				//Log.d(TAG, "ksjung - mVolume.getStorageId() : " + mVolume.getStorageId());

        if (Environment.MEDIA_MOUNTED_READ_ONLY.equals(state)) {
            mItemAvailable.setTitle(R.string.memory_available_read_only);
						if (mFormatPreferenceExternal != null) {
                removePreference(mFormatPreferenceExternal);
            }
        } else {
		...
		if (mFormatPreferenceExternal != null) {
		  mFormatPreferenceExternal.setEnabled(true);
		  mFormatPreferenceExternal.setTitle(R.string.sd_format);
		  mFormatPreferenceExternal.setSummary(R.string.sd_format_summary);
		  addPreference(mFormatPreferenceExternal);
		}
        ...
        if (mUsbConnected && (UsbManager.USB_FUNCTION_MTP.equals(mUsbFunction) ||
            ...
            if (mFormatPreferenceExternal != null) {
                mFormatPreferenceExternal.setEnabled(false);
                mFormatPreferenceExternal.setSummary(mResources.getString(R.string.mtp_ptp_mode_summary));
            }
        } else if (mFormatPreference != null) {
            mFormatPreference.setEnabled(true);
            mFormatPreference.setSummary(mResources.getString(R.string.sd_format_summary));
        } else if (mFormatPreferenceExternal != null) {
            mFormatPreferenceExternal.setEnabled(true);
            mFormatPreferenceExternal.setSummary(mResources.getString(R.string.sd_format_summary));
        }


				String testVolume = mVolume.getPath();
				String usbVolumeRoute0 = "/storage/usb0";
				String usbVolumeRoute1 = "/storage/usb1";
				if (testVolume.equals(usbVolumeRoute0) ||
						testVolume.equals(usbVolumeRoute1)) {
/*
				if ((mVolume.getPath() == "/storage/usb0") ||
						(mVolume.getPath() == "/storage/usb1")) {
						*/
				  //Log.d(TAG, "ksjung -  : " + mVolume.getPath());
        if (mFormatPreference != null) 
					removePreference(mFormatPreference);
				if (mFormatPreferenceExternal != null) 
					removePreference(mFormatPreferenceExternal);
    ...
    public Intent intentForClick(Preference pref) {
        ...
        if (pref == mFormatPreference) {
            ...
            Log.d("putExtra", "ksjung, pref == mFormatPreference- mVolume.getPath() : " + mVolume.getPath());
        } else if (pref == mFormatPreferenceExternal) {
            intent = new Intent(Intent.ACTION_VIEW);
            intent.setClass(getContext(), com.android.settings.MediaFormat.class);
            intent.putExtra(StorageVolume.EXTRA_STORAGE_VOLUME, mVolume);
            Log.d("putExtra", "ksjung, pref == mFormatPreferenceExternal- mVolume.getPath() : " + mVolume.getPath());
