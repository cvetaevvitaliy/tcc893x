...
public class WirelessSettings extends RestrictedSettingsFragment
    ...
    private static final String KEY_NFS_SETTINGS = "nfs_settings";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // Remove Airplane mode if its stb
        if ("1".equals(SystemProperties.get("tcc.solution.mbox"))) {
            getPreferenceScreen().removePreference(mAirplaneModePreference);
        }

        // Remove NFS if its not available
        Preference nfs = getPreferenceScreen().findPreference(KEY_NFS_SETTINGS);
        if (SystemProperties.get("tcc.networkfilesystem.support").equals("enable") == false) {
            getPreferenceScreen().removePreference(nfs);
        }
        ...
        // pn544
        else if(SystemProperties.get("ro.hardware.nfc").equals("true") == false){
            getPreferenceScreen().removePreference(nfc);
            getPreferenceScreen().removePreference(androidBeam);
            mNfcEnabler = null;
        }
