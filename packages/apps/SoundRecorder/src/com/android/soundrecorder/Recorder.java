...
import android.os.StatFs;   // B090183
...
public class Recorder implements OnCompletionListener, OnErrorListener {
    ...
    // S: B090183
    public static final boolean RECORDING_CODEC_MP3 = true;
    public static final boolean RECORDING_CODEC_AMR = false;

    boolean mCodecType = RECORDING_CODEC_AMR;
    boolean mCodecTypePrev = RECORDING_CODEC_AMR;

    public static final boolean AUDIO_INPUT_PORT_MIC  = false;
    public static final boolean AUDIO_INPUT_PORT_LINE = true;

    boolean mAudioInputPort = AUDIO_INPUT_PORT_MIC;
    // E: B090183
    ...
    // S: telechips code
    public void setCodecType(boolean type) {
        mCodecType = type;
    }

    public void setAudioInputPort(boolean port) {
        mAudioInputPort = port;
    }
    // E: telechips code
    ...
    public void startRecording(int outputfileformat, String extension, Context context) {
        ...
        // S: B090183
        if(mCodecType != mCodecTypePrev) {
            mCodecTypePrev = mCodecType;
...
/* S: telechips code --- */ 
        ...
        mMaxFileSize = (long)stat.getAvailableBlocks() * (long)stat.getBlockSize();

        if(mAudioInputPort == AUDIO_INPUT_PORT_MIC) {
            mRecorder.setAudioSource(MediaRecorder.AudioSource.MIC);
        }
        else {
            mRecorder.setAudioSource(MediaRecorder.AudioSource.LINE_IN);
        }

        if(mCodecType == RECORDING_CODEC_AMR) {
            mRecorder.setOutputFormat(outputfileformat);
            mRecorder.setAudioEncoder(MediaRecorder.AudioEncoder.AMR_NB);
            mRecorder.setOutputFile(mSampleFile.getAbsolutePath());
        }
            ...
            mRecorder.setOutputFormat(outputfileformat);
            mRecorder.setAudioEncoder(MediaRecorder.AudioEncoder.MP3);
            mRecorder.setAudioChannels(2);
            mRecorder.setAudioSamplingRate(44100);
            mRecorder.setAudioEncodingBitRate(256000);
            mRecorder.setOutputFile(mSampleFile.getAbsolutePath());
            mRecorder.setMaxFileSize(mMaxFileSize);
            Log.d("Recorder", "maxFileSize = " + mMaxFileSize);
        }
/* E: telechips code */
