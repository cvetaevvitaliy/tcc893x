...
static int nfc_jni_initialize(struct nfc_jni_native_data *nat) {
   ...
   {
force_download:
       TRACE("skip firmware download firmware_update_info %c 0x%x" , 
                                                    caps.psDevCapabilities.firmware_update_info,
                                                    caps.psDevCapabilities.firmware_update_info
                                                    );
       #if 0 // PN544 PN544_NFC 우선 막음.
...
static jboolean com_android_nfc_NfcManager_initialize(JNIEnv *e, jobject o)
{
   ...
   property_get("test.nfc.mode", value, "X");
   if(value[0] == 'I' ||value[0] == 'i'){
       nat->p2p_initiator_modes = phNfc_eP2P_ALL;
       nat->p2p_target_modes = 0x0; // All passive except 106, active
       ALOGD(" =============> P2P mode : Initiator");
    }else if(value[0] == 'T' ||value[0] == 't'){
       nat->p2p_initiator_modes = phNfc_eDefaultP2PMode;
       nat->p2p_target_modes = 0x0E; // All passive except 106, active
       ALOGD(" =============>  P2P mode : Target");
    }else{
       nat->p2p_initiator_modes = phNfc_eP2P_ALL;
       nat->p2p_target_modes = 0x0E; // All passive except 106, active
    }
