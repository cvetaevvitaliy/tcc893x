...
public class NfcService implements DeviceHostListener {
    ...
    public NfcService(Application nfcApplication) {
        if(SystemProperties.get("ro.hardware.nfc").equals("true") == false){
            return;
        }
        ...
        if(DBG) Log.d(TAG,"  NfcService AddService");
    ...
    void applyRouting(boolean force) {
        synchronized (this) {
            ...
            try {
                ...
                if(DBG) //  
                    Log.d(TAG, " applyRouting 1 mScreenState " + mScreenState);
                if (mDeviceHost.enablePN544Quirks() && mScreenState == SCREEN_STATE_OFF) {
                     ...
                     */
                    if(DBG) //  
                        Log.d(TAG, " applyRouting 1.1 POLLING_MODE " + POLLING_MODE +
                            " force " + force + " mNfcPollingEnabled " + mNfcPollingEnabled);
                    ...
                    if(DBG) //  
                        Log.d(TAG, " applyRouting 1.2 mEeRoutingState " + mEeRoutingState +
                            " force " + force + " mNfcPollingEnabled " + mNfcPollingEnabled);
                ...
                if(DBG) //  
                    Log.d(TAG, " applyRouting 3 mNfcPollingEnabled " + mNfcPollingEnabled + 
                        " force " + force + 
                        " mNfceeRouteEnabled " + mNfceeRouteEnabled +
    ...
    final class NfcServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_MOCK_NDEF: {
                    if(DBG) Log.d(TAG," handleMessage MSG_MOCK_NDEF"); // 
                ...
                case MSG_LLCP_LINK_ACTIVATION:
                    if(DBG) Log.d(TAG," handleMessage MSG_LLCP_LINK_ACTIVATION"); // 
                    if (mIsDebugBuild) {
                ...
                case MSG_LLCP_LINK_DEACTIVATED:
                    if(DBG) Log.d(TAG," handleMessage MSG_LLCP_LINK_DEACTIVATED"); // 
                    if (mIsDebugBuild) {
        ...
        private boolean llcpActivated(NfcDepEndpoint device) {
            ...
            if (device.getMode() == NfcDepEndpoint.MODE_P2P_TARGET) {
                ...
                if(DBG) Toast.makeText(mContext,"Remote :  P2P Target",Toast.LENGTH_LONG).show(); // 
                if (device.connect()) {
                ...
                if(DBG) Toast.makeText(mContext,"Remote :  P2P Initiator",Toast.LENGTH_LONG).show(); // 
                /* Check LLCP compliancy */
                if (mDeviceHost.doCheckLlcp()) {
                    /* Activate LLCP Link */
                    if (mDeviceHost.doActivateLlcp()) {
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(
                ...
                if(DBG) // 
                    Log.d(TAG," mReceiver NativeNfcManager.INTERNAL_TARGET_DESELECTED_ACTION");
                ...
                if (action.equals(Intent.ACTION_SCREEN_OFF)) {
                    if(DBG) // 
                        Log.d(TAG," mReceiver ACTION_SCREEN_OFF");
                    screenState = SCREEN_STATE_OFF;
                } else if (action.equals(Intent.ACTION_SCREEN_ON)) {
                    if(DBG) // 
                        Log.d(TAG," mReceiver ACTION_SCREEN_ON");
                    screenState = mKeyguard.isKeyguardLocked() ?
                            SCREEN_STATE_ON_LOCKED : SCREEN_STATE_ON_UNLOCKED;
                } else if (action.equals(Intent.ACTION_USER_PRESENT)) {
                    if(DBG) // 
                        Log.d(TAG," mReceiver ACTION_USER_PRESENT");
                ...
                if(DBG) // 
                    Log.d(TAG," mReceiver ACTION_AIRPLANE_MODE_CHANGED isAirplaneModeOn " + isAirplaneModeOn);
