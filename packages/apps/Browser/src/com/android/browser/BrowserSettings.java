...
public class BrowserSettings implements OnSharedPreferenceChangeListener,
    ...
    private void syncSetting(WebSettings settings) {
		...
		// User agent Sync.
		String prefUserAgent = mPrefs.getString(PREF_USER_AGENT, "defaults");        
		String[] ctxUserAgents = mContext.getResources().getStringArray(R.array.user_agents);
		for(int i = 0; i < ctxUserAgents.length; i++) {
			if(prefUserAgent.equals(ctxUserAgents[i])) {
		        Log.v(TAG, "sync. ua: " + ctxUserAgents[i]);
                mUserAgentIndex = i;
			}
		}
        ...
        if (ua != null) {
            Log.d(TAG, "UA string " + ua);
            settings.setUserAgentString(ua);
        } else {
            settings.setUserAgentString(USER_AGENTS[getUserAgent()]);
        }
        String ua_str = settings.getUserAgentString();
        // if is iOS, disable plugins
        if ( IPAD_USERAGENT.equals(ua_str) || IPHONE_USERAGENT.equals(ua_str))
        {
            settings.setPluginState(PluginState.OFF);
            Log.d(TAG, "Turn off plugins");
        }
    ...
    public int getUserAgent() {
        ...
        //   return 0;
        //}
        //return mUserAgentIndex == 0 ? Integer.parseInt(mPrefs.getString(PREF_USER_AGENT, "0")) : mUserAgentIndex;
        if (mUserAgentIndex != 0) {
            Log.d(TAG, "getUserAgent: mUserAgentIndex - " + mUserAgentIndex);
        }
        return mUserAgentIndex;
    }
