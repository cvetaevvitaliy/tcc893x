 ...
 */
package com.android.browser.ua;
...
class UserAgentPreference extends ListPreference {

    private static final String TAG = "userAgentPreference";
    ...
    public UserAgentPreference(Context context, AttributeSet attrs) {
        ...
        String[] userAgents = res.getStringArray(R.array.user_agents);
        for (int i = 0; i < userAgents.length; i++) {
        	String name = userAgents[i];
        	entryValues.add(name);
          entries.add(name);
          Log.d(TAG,"agent name="+name);
        }
				//ken add end
