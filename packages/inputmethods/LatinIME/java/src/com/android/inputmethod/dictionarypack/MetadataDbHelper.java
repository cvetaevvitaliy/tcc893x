...
public class MetadataDbHelper extends SQLiteOpenHelper {
	...
	{
		if (null != sInstanceMap)
		{
			MetadataDbHelper helper;
			String client_id;
			
			for (Map.Entry entry : sInstanceMap.entrySet()) {
				client_id = (String) entry.getKey();
				helper = sInstanceMap.get(client_id);
				if (null != helper) {
					helper.close();
					helper = null;
		            sInstanceMap.put(client_id, helper);
		        }	            
	        }
		}
	}
