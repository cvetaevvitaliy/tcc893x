...
abstract public class ExpandableBinaryDictionary extends Dictionary {
	...
	private boolean m_need_RunGC = false;
	private boolean m_mindsBlockByGC = false;
	private boolean m_load_available = true;
    /**
        ...
        mContext.registerReceiver(new BroadcastReceiver() {
            ...
            public void onReceive(Context context1, Intent intent) {
                 ...
                 if (Intent.ACTION_PRE_QUICKBOOT.equals(action)) {
					...
					if( m_need_RunGC )
						runGCIfRequired(m_mindsBlockByGC);
    ...
    private BinaryDictionary getCurrentBinaryDictionary() {
    	if( mLoadFlag && mBinaryDictionary == null ) {
    ...
    protected void runGCIfRequired(final boolean mindsBlockByGC) {
		...
		if ( m_load_available == false ) {
			m_mindsBlockByGC = mindsBlockByGC;
			m_need_RunGC = true;
			return;
		}
        getExecutor(mFilename).execute(new Runnable() {
    ...
    private void runGCIfRequiredInternalLocked(final boolean mindsBlockByGC) {
        ...
        if (getCurrentBinaryDictionary().needsToRunGC(mindsBlockByGC)) {
            if (setIsRegeneratingIfNotRegenerating()) {
                ...
                getExecutor(mFilename).executePrioritized(new Runnable() {
                    ...
                    public void run() {
                        try {
                            getCurrentBinaryDictionary().flushWithGC();
                        } finally {
        ...
        getExecutor(mFilename).execute(new Runnable() {
            ...
            public void run() {
                if (ENABLE_BINARY_DICTIONARY_DYNAMIC_UPDATE) {
                    ...
                    getCurrentBinaryDictionary().addUnigramWord(word, frequency);
                } else {
                    // TODO: Remove.
    ...
    protected void addBigramDynamically(final String word0, final String word1,
        ...
        getExecutor(mFilename).execute(new Runnable() {
            ...
            public void run() {
                if (ENABLE_BINARY_DICTIONARY_DYNAMIC_UPDATE) {
                    runGCIfRequiredInternalLocked(true /* mindsBlockByGC */);
                    getCurrentBinaryDictionary().addBigramWords(word0, word1, frequency);
                } else {
                    // TODO: Remove.
    ...
    protected void removeBigramDynamically(final String word0, final String word1) {
        ...
        getExecutor(mFilename).execute(new Runnable() {
            ...
            public void run() {
                if (ENABLE_BINARY_DICTIONARY_DYNAMIC_UPDATE) {
                    runGCIfRequiredInternalLocked(true /* mindsBlockByGC */);
                    getCurrentBinaryDictionary().removeBigramWords(word0, word1);
                } else {
                    // TODO: Remove.
    ...
    public ArrayList<SuggestedWordInfo> getSuggestionsWithSessionId(final WordComposer composer,
        ...
        getExecutor(mFilename).executePrioritized(new Runnable() {
            ...
            public void run() {
                if (ENABLE_BINARY_DICTIONARY_DYNAMIC_UPDATE) {
                    if (getCurrentBinaryDictionary() == null) {
                        holder.set(null);
                        return;
                    }
                    final ArrayList<SuggestedWordInfo> binarySuggestion =
                            getCurrentBinaryDictionary().getSuggestionsWithSessionId(composer, prevWord,
                    ...
                    if (getCurrentBinaryDictionary() != null && !mIsUpdatable) {
                        final ArrayList<SuggestedWordInfo> binarySuggestion =
                                getCurrentBinaryDictionary().getSuggestionsWithSessionId(composer, prevWord,
    ...
    protected boolean isValidWordLocked(final String word) {
        if (getCurrentBinaryDictionary() == null) return false;
        return getCurrentBinaryDictionary().isValidWord(word);
    }

    protected boolean isValidBigramLocked(final String word1, final String word2) {
        if (getCurrentBinaryDictionary() == null) return false;
        return getCurrentBinaryDictionary().isValidBigram(word1, word2);
    }
    ...
    private void writeBinaryDictionary() {
        ...
        if (needsToReloadBeforeWriting()) {
            ...
            if (ENABLE_BINARY_DICTIONARY_DYNAMIC_UPDATE) {
                if (getCurrentBinaryDictionary() == null || !getCurrentBinaryDictionary().isValidDictionary()) {
                    final File file = new File(mContext.getFilesDir(), mFilename);
                    BinaryDictionary.createEmptyDictFile(file.getAbsolutePath(),
                            DICTIONARY_FORMAT_VERSION, getHeaderAttributeMap());
                } else {
                    if (getCurrentBinaryDictionary().needsToRunGC(false /* mindsBlockByGC */)) {
                        getCurrentBinaryDictionary().flushWithGC();
                    } else {
                        getCurrentBinaryDictionary().flush();
                    }
                }
            } else {
                mDictionaryWriter.write(mFilename, getHeaderAttributeMap());
    ...
    private boolean isReloadRequired() {
        return getCurrentBinaryDictionary() == null || mPerInstanceDictionaryUpdateController.isOutOfDate();
    }
        ...
        getExecutor(mFilename).execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    } else if (getCurrentBinaryDictionary() == null ||
        ...
        getExecutor(mFilename).executePrioritized(new Runnable() {
            ...
            public void run() {
                if (mDictType == Dictionary.TYPE_USER_HISTORY) {
                    if (ENABLE_BINARY_DICTIONARY_DYNAMIC_UPDATE) {
                        holder.set(getCurrentBinaryDictionary().isValidWord(word));
                    } else {
