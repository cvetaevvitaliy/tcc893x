...
StagefrightPlayer::StagefrightPlayer() {
    ...
    ALOGI("load TCCAwesomePlayer");
    mPlayer = new TCCAwesomePlayer();
#else
    ALOGI("load AwesomePlayer");
    mPlayer = new AwesomePlayer();
#endif

    mPlayer->setListener(this);
    mPlayStreamingMedia = false;
}
...
status_t StagefrightPlayer::stop() {
    ALOGV("stop");
#if _USE_TCC_AWESOMEPLAYER_
    if (mPlayStreamingMedia) {
        mPlayStreamingMedia = false;
        return mPlayer->stop();
    }
#endif
    return mPlayer->pause(); // what's the difference?
}
