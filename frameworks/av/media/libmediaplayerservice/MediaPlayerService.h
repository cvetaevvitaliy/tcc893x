...
#include <binder/MemoryDealer.h> // INTERNAL_SUBTITLE_INCLUDE

#include <system/audio.h>

#if defined(TCC_SPDIF_BITSTREAM)
#include <hardware_legacy/SPDIFHardwareInterface.h>     // Telechips, B090183
#endif
...
struct tcc_audio_track_cblk_t
{
    ...
    uint16_t    flags;
                ...
                tcc_audio_track_cblk_t();

    size_t      read(void **read_buffer, size_t size);
    void        write(size_t size);
    void        release(size_t size);
    void*       getWriteBufferPos();
    size_t      writableBuffer();
    size_t      prepareBuffer();

    bool        tccMemAlloc(size_t size);
    void        tccMemFree();
};
#endif
...
class MediaPlayerService : public BnMediaPlayerService
{
...
#if defined(TCC_SPDIF_BITSTREAM)
    class AudioOutputSpdif : public MediaPlayerBase::AudioSink
    {
    public:
                                AudioOutputSpdif(int sessionId, int uid);	// SPDIF Kitkat_mr1.1 Enable Planet_20140313
        virtual                 ~AudioOutputSpdif();

        virtual bool            ready() const { return mSpdifHardware != NULL; }
        virtual bool            realtime() const { return true; }
        virtual ssize_t         bufferSize() const;
        virtual ssize_t         frameCount() const;
        virtual ssize_t         channelCount() const;
        ...
        virtual uint32_t        latency() const;
        virtual float           msecsPerFrame() const;
        virtual status_t        getPosition(uint32_t *position) const { return INVALID_OPERATION; }
		virtual status_t        getFramesWritten(uint32_t *frameswritten) const;
        virtual int             getSessionId() const;
        ...
        virtual audio_stream_type_t getAudioStreamType() const { return mStreamType; }	// SPDIF Kitkat_mr1.1 Enable Planet_20140313
        ...
        virtual status_t        setEQ(int eqtype, int eqmode, int data0, int data1, int data2, int data3, int data4) { return NO_ERROR;}
        virtual status_t        setPlayRate(int rate) { return NO_ERROR; }
        virtual status_t        setSRSParam(int srsType, int srsParamType, int srsParamVlaue) { return NO_ERROR; }

		 		void            setNextOutput(const sp<AudioOutputSpdif>& nextOutput);
                void            switchToNextOutput();
        virtual bool            needsTrailingPadding() { return mNextOutput == NULL; }
    ...
    private:
        ...
        SPDIFHardwareInterface* mSpdifHardware;

		sp<AudioOutputSpdif>	mNextOutput;
        AudioCallback           mCallback;
        void *                  mCallbackCookie;
        audio_stream_type_t     mStreamType;	// SPDIF Kitkat_mr1.1 Enable Planet_20140313
        float                   mMsecsPerFrame;
        uint32_t                mLatency;
        ...
        int                     mSessionId;
        float                   mSendLevel;
        int                     mUid;			// SPDIF Kitkat_mr1.1 Enable Planet_20140313
...
private:
    ...
    class Client : public BnMediaPlayer {
        ...
        virtual status_t        setVsync(bool vsync);
...
#if defined(TCC_SPDIF_BITSTREAM)
                    sp<AudioOutputSpdif>        mAudioOutputSpdif;  // Telechips, B090183
                    int                         mAudioModeClientNum;// Planet 20130930 HBR Mode(Audio and be played.)
                    int                         mAudioModeAudioType;// Planet 20130930 HBR Mode(Audio and be played.)
#endif
                    pid_t                       mPid;
                    status_t                    mStatus;
                    bool                        mLoop;
