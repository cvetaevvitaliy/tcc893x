...
#include <autoconf.h>  //20130516 swhwang USB Camera Kernel FEATURE for CTS TEST
...
status_t StagefrightRecorder::setParamVideoRotation(int32_t degrees) {
    ...
    ALOGV("setParamVideoRotation : mRotationDegrees = %d",mRotationDegrees);
    return OK;
}
...
status_t StagefrightRecorder::start() {
    ...
    switch (mOutputFormat) {
        ...
        case OUTPUT_FORMAT_MPEG2TS:
            ...
            break;
        case OUTPUT_FORMAT_TCC_MP3:
            status = startMP3Recording();
            break;
...
sp<MediaSource> StagefrightRecorder::createAudioSource() {
    ...
    switch (mAudioEncoder) {
        ...
        case AUDIO_ENCODER_MP3:
            mime = MEDIA_MIMETYPE_AUDIO_MPEG_TCC;
            break;
        default:
...
status_t StagefrightRecorder::startMP3Recording() {
    CHECK(mOutputFormat == OUTPUT_FORMAT_TCC_MP3);

    ALOGE("startMP3Recording, mAudioEncoder(%d) mAudioChannels(%d) mSampleRate(%d)",
		mAudioEncoder, mAudioChannels, mSampleRate);

    if (mAudioEncoder != AUDIO_ENCODER_MP3) {
        ALOGE("Invalid encoder %d used for MP3 recording",
                mAudioEncoder);
        return BAD_VALUE;
    }
    
    if (mSampleRate > 48000) {
        ALOGE("Invalid sampling rate %d used for MP3 recording",
                mSampleRate);
        return BAD_VALUE;
    }
	
    if (mAudioChannels > 2) {	
        ALOGE("Invalid number of audio channels %d used for MP3 recording",
                mAudioChannels);
        return BAD_VALUE;
    }
...
void StagefrightRecorder::clipVideoFrameRate() {
    ...
    } else if (mFrameRate > maxFrameRate && mFrameRate != -1) {
        ALOGW("Intended video encoding frame rate (%d fps) is too large"
             " and will be set to (%d fps)", mFrameRate, maxFrameRate);
        mFrameRate = maxFrameRate;
    }
	if(mFrameRate<3)mFrameRate=3; //for RecorderRandomAction CTS test(android.media.cts.MediaRandomTest)
}
...
void StagefrightRecorder::clipVideoBitRate() {
    ...
    if (mVideoBitRate < minBitRate && mVideoBitRate != -1) {
        ALOGW("Intended video encoding bit rate (%d bps) is too small"
             " and will be set to (%d bps)", mVideoBitRate, minBitRate);
        mVideoBitRate = minBitRate;
    } else if (mVideoBitRate > maxBitRate && mVideoBitRate != -1) {
...
void StagefrightRecorder::clipVideoFrameWidth() {
    ...
    if (mVideoWidth < minFrameWidth && mVideoWidth != -1) {
        ALOGW("Intended video encoding frame width (%d) is too small"
             " and will be set to (%d)", mVideoWidth, minFrameWidth);
        mVideoWidth = minFrameWidth;
    } else if (mVideoWidth > maxFrameWidth && mVideoWidth != -1) {
...
status_t StagefrightRecorder::checkVideoEncoderCapabilities(
    ...
    *supportsCameraSourceMetaDataMode = codecs.size() > 0;
    ...
    if (!mCaptureTimeLapse) {
       ...
       /* 20130429 swhwang, added for Support USB Camera Frame */
	#if defined(CONFIG_USB_VIDEO_CLASS)
		
		if (mCamera == 0)		mCam = Camera::connect(mCameraId,mClientName, mClientUid);
		else 	      			mCam = Camera::create(mCamera);
		
		CameraParameters params(mCam->getParameters()); 
		
		if(params.getUsbCameraSupported()){
			 mFrameRate = params.getPreviewFrameRate();
		} else {
			clipVideoFrameRate();
		}
	        clipVideoFrameWidth();
...
void StagefrightRecorder::clipVideoFrameHeight() {
    ...
    if (mVideoHeight != -1 && mVideoHeight < minFrameHeight) {
        ALOGW("Intended video encoding frame height (%d) is too small"
             " and will be set to (%d)", mVideoHeight, minFrameHeight);
        mVideoHeight = minFrameHeight;
    } else if (mVideoHeight != -1 && mVideoHeight > maxFrameHeight) {
...
#ifdef TCC_OMX
    sp<MediaSource> encoder = TCCOMXCodec::Create(
#else
