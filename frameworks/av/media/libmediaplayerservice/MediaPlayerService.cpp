...
#if defined(TCC_SPDIF_BITSTREAM)
status_t MediaPlayerService::AudioOutputSpdif::dump(int fd, const Vector<String16>& args) const
{
    const size_t SIZE = 256;
    char buffer[SIZE];
    String8 result;
...
bool MediaPlayerService::setVsync()
{
    return true;
    ...
    property_get("persist.sys.output_mode", value, "0");
...
void MediaPlayerService::clearVsync()
{
    return;
...
// Planet 20130930 HBR Mode(Audio and be played.) Start
    ...
    if ( strcmp(value, "4") == 0 ) {
        mAudioModeClientNum = -1;
        mAudioModeAudioType = -1;
        ALOGE("~~~!!! Client constructor clientNum:%d HdmiAudioType:%d !!!~~~",mAudioModeClientNum, mAudioModeAudioType);
    }
#endif
...
// Planet 20130930 HBR Mode(Audio and be played.) Start
    ...
    if ( strcmp(value, "4") == 0 ) {
        ...
        ALOGE("~~~!!! Client destructor clientNum:%d HdmiAudioType:%d !!!~~~",mAudioModeClientNum, mAudioModeAudioType);
    }
#endif
// Planet 20130930 HBR Mode(Audio and be played.) End
        ...
        player_type playerType)
{
    ...
    if (!p->hardwareOutput()) {
        #if defined(TCC_SPDIF_BITSTREAM)
            ALOGD("!!!! [AudioOutput]MediaPlayerService::Client::setDataSource(url) %d", playerType);
        	char value[PROPERTY_VALUE_MAX];
        	property_get("persist.sys.spdif_setting", value, "");
			property_set("tcc.hdmi.audio_type", "0");
        	if ( strcmp(value, "4") == 0 )
        	{
                ALOGE("!!!! setDataSource() TrueHD");
                mAudioOutputSpdif = new AudioOutputSpdif(mAudioSessionId, IPCThreadState::self()->getCallingUid());		// SPDIF Kitkat_mr1.1 Enable Planet_20140313
                static_cast<MediaPlayerInterface*>(p.get())->setAudioSink(mAudioOutputSpdif);
            }
        	else if ( strcmp(value, "3") == 0 )
        	{
                ALOGE("!!!! setDataSource() LPCM (Multichannel)");
                mAudioOutputSpdif = new AudioOutputSpdif(mAudioSessionId, IPCThreadState::self()->getCallingUid());		// SPDIF Kitkat_mr1.1 Enable Planet_20140313
                static_cast<MediaPlayerInterface*>(p.get())->setAudioSink(mAudioOutputSpdif);
            }
        	else if ( strcmp(value, "2") == 0 )
        	{
                ALOGE("!!!! setDataSource() SPDIF");
                mAudioOutputSpdif = new AudioOutputSpdif(mAudioSessionId, IPCThreadState::self()->getCallingUid());		// SPDIF Kitkat_mr1.1 Enable Planet_20140313
                static_cast<MediaPlayerInterface*>(p.get())->setAudioSink(mAudioOutputSpdif);
            }
            else {
                ALOGD("!!!! setDataSource() Default");
        ...
        const char *url, const KeyedVector<String8, String8> *headers)
{
    ...
    // Planet 20130930 HBR Mode(Audio and be played.) Start
        ...
        property_get("persist.sys.spdif_setting", value, "");
        property_get("tcc.hdmi.audio_type", mode, "");

        if ( strcmp(value, "4") == 0 ) {
            mAudioModeClientNum = mConnId;
            mAudioModeAudioType = atoi(mode);
            ALOGE("~~~!!! setDataSource clientNum:%d AudioType:%d !!!~~~", mAudioModeClientNum, mAudioModeAudioType);
            return mStatus;
        }
    #endif
    // Planet 20130930 HBR Mode(Audio and be played.) End
...
status_t MediaPlayerService::Client::setVsync(bool vsync) {
    ALOGV("setVsync : %d", vsync);
    if (vsync) {
        mVsync = mService->setVsync();
        if (!mVsync) {
        ...
        const sp<IGraphicBufferProducer>& bufferProducer)
{
    ...
    if (bufferProducer != NULL) {
        ...
        const bool WORKAROUD_FOR_YOUKU_APK = true;
        int retry_count = 1000; 

        anw = new Surface(bufferProducer, true /* controlledByApp */);

		// HEES: It's a kind of dummy loop to avoid using 'goto' statement..
		for (;;) {
		...
		if (err != OK && err != -22) {
            ...
            if (WORKAROUD_FOR_YOUKU_APK) {
                // ZzaU: To retry when "already connected" error happens.
                if (err == -22 && retry_count > 0) {
                    if (retry_count % 100 == 0) {
                        ALOGI("################### retry(%d) to connect SurfaceTexture!!", retry_count); 
                    }
                    retry_count--;
                    usleep(10*1000); // 10ms
...
// Planet 20130930 HBR Mode(Audio and be played.) Start
    ...
    if ( strcmp(value, "4") == 0 ) {
        ALOGE("~~~!!! Start() ClientNum:%d, AudioType:%d !!!~~~~",mAudioModeClientNum, mAudioModeAudioType);
        if(mAudioModeAudioType != 0) {
            mode[0] = (char)mAudioModeAudioType;
            mode[0] += '0';
            property_set("tcc.hdmi.audio_type", mode);
        }
        property_get("tcc.hdmi.audio_type", mode, "");
        ALOGE("~~~!!! Start() tcc.hdmi.audio_type:%c !!!~~~", mode[0]);
        return p->start();
    }
#endif
// Planet 20130930 HBR Mode(Audio and be played.) End
...
// Planet 20130930 HBR Mode(Audio and be played.) Start
    ...
    if ( strcmp(value, "4") == 0 ) { 
        mAudioModeClientNum = -1;
        mAudioModeAudioType = -1;
        ALOGE("~~~!!! Stop() ClientNum:%d, HdmiAudioType:%d !!!~~~", mAudioModeClientNum, mAudioModeAudioType); 
    }
#endif
// Planet 20130930 HBR Mode(Audio and be played.) End
        ...
        void* cookie, int msg, int ext1, int ext2, const Parcel *obj)
{
    ...
    sp<IMediaPlayerClient> c;
    {
        ...
        if (msg == MEDIA_PLAYBACK_COMPLETE && client->mNextClient != NULL) {
            ...
            // Planet 20130930 HBR Mode(Audio and be played.) Start
#if defined(TCC_SPDIF_BITSTREAM)
            if (client->mAudioOutputSpdif != NULL) {
                client->mAudioOutputSpdif->switchToNextOutput();
                client->mAudioOutputSpdif->close();
...
////////////////////////////////////////////////////////////////////////////////
// SPDIF/Multichannel(DAI)
////////////////////////////////////////////////////////////////////////////////
#if defined(TCC_SPDIF_BITSTREAM)
#undef LOG_TAG
#define LOG_TAG "AudioSink_SPDIF"
//#define AUDIOOUTPUTSPDIF_DEBUG_ON

#ifdef AUDIOOUTPUTSPDIF_DEBUG_ON
#undef SPDIF_LOG
#define SPDIF_LOG(fmt,arg...)   ALOGE(fmt,##arg)
#undef SPDIF_THREAD_LOG
#define SPDIF_THREAD_LOG(fmt,arg...)   ALOGE(fmt,##arg)
#else
#undef SPDIF_LOG
#define SPDIF_LOG
#undef SPDIF_THREAD_LOG
#define SPDIF_THREAD_LOG
#endif


////////////////////////////////////////////////////////////////////////////////
// =========================================================================
tcc_audio_track_cblk_t::tcc_audio_track_cblk_t()
    : lock(Mutex::SHARED),
      readPos(0), writePos(0), user(0), server(0),
      loopStart(0), loopEnd(0), loopCount(0),
      buffer(NULL), bufferSize(0), flags(0)
{
}

bool tcc_audio_track_cblk_t::tccMemAlloc(size_t size)
{
    if(buffer != NULL) {
        free(buffer);
    }
    ...
    if(buffer != NULL) {
        bufferSize = size;
		//Not thread safe values, when pass-through audio track , let's initialize values
		readPos = 0;
		writePos = 0;
		user = 0;
		server = 0;
...
void tcc_audio_track_cblk_t::tccMemFree()
{
    if(buffer != NULL) {
        free(buffer);
		buffer = NULL;
...
size_t tcc_audio_track_cblk_t::read(void **read_buffer, size_t size)
{
    if(prepareBuffer() < size)
        return 0;
    else {
        *read_buffer = buffer + readPos;
        if(readPos + size > bufferSize)
            SPDIF_THREAD_LOG("~~~~~  tcc_audio_track_cblk_t::read   error.....  %p, 0x%x , 0x%x", buffer, readPos, size);

        return size;
    }
}

void tcc_audio_track_cblk_t::write(size_t size)
{
    server += size/1024;
    writePos += size;
...
void tcc_audio_track_cblk_t::release(size_t size)
{
    user += size/1024;
    readPos += size;
...
void* tcc_audio_track_cblk_t::getWriteBufferPos()
{
    return buffer + writePos;
}


size_t tcc_audio_track_cblk_t::writableBuffer()
{
    if(writePos > readPos)
...
size_t tcc_audio_track_cblk_t::prepareBuffer()
{
    if(writePos > readPos)
...
// TCCFillThread
//////////////////////////////////////////////////////////////////////////////////////////////////////////
struct TCCFillThread : public Thread 
{
    TCCFillThread(const wp<MediaPlayerBase::AudioSink> &sink,
                   MediaPlayerBase::AudioSink::AudioCallback cb,
                   void *cookie,
                   tcc_audio_track_cblk_t *buffer,
                   int channelCount);

protected:
    virtual ~TCCFillThread();
...
private:
    ...
    MediaPlayerBase::AudioSink::AudioCallback mCallback;
    void *mCookie;
    tcc_audio_track_cblk_t *mSharedBuffer;
    tcc_audio_track_cblk_t mTempBuffer;
    size_t mBufferSize;
    int mAudioOutputMode;
    int mChannelCount;
	int mSetLPCMBuffer;

    TCCFillThread(const TCCFillThread &);
    TCCFillThread &operator=(const TCCFillThread &);
};

TCCFillThread::TCCFillThread(
        const wp<MediaPlayerBase::AudioSink> &sink,
        MediaPlayerBase::AudioSink::AudioCallback cb,
        void *cookie,
        tcc_audio_track_cblk_t *buffer,
...
{
	char value_out_enable[PROPERTY_VALUE_MAX];
	property_get("persist.sys.spdif_setting", value_out_enable, "");

    if (value_out_enable[0] == '3') {
        mAudioOutputMode = 3;
    }
    ...
    if(value_out_enable[0] == '4' && hdmi_audio[0]=='3'){
    	 mSetLPCMBuffer = 1;
    }

    SPDIF_THREAD_LOG("!!~~ TCCFillThread::TCCFillThread()");
}

TCCFillThread::~TCCFillThread() 
{
    mSharedBuffer->tccMemFree();

    if(mAudioOutputMode == 3 || mSetLPCMBuffer==1) 
        mTempBuffer.tccMemFree();

    SPDIF_THREAD_LOG("TCCFillThread is killed");
}

bool TCCFillThread::threadLoop() {
    int i;
    sp<MediaPlayerBase::AudioSink> sink = mSink.promote();
    ...
    if((mAudioOutputMode == 3 || mSetLPCMBuffer ==1) && (mChannelCount > 2) && (mChannelCount < 8)) {
        if (mBufferSize == 0) {
            mBufferSize = sink->bufferSize();
            mTempBuffer.tccMemAlloc(mBufferSize*2);
            mSharedBuffer->tccMemAlloc(mBufferSize*2*4/3);

            SPDIF_THREAD_LOG("~~~~ TCCFillThread::threadLoop()    mBufferSize[%x]", mBufferSize*2);
        }
        if(mSharedBuffer->writableBuffer()) {
            size_t actualSize, reqSize;
            char *inbuffer  = (char *)mTempBuffer.getWriteBufferPos();
            char *outbuffer = (char *)mSharedBuffer->getWriteBufferPos();

            reqSize = mSharedBuffer->writableBuffer() * 3 / 4;
            actualSize = (*mCallback)(sink.get(), mTempBuffer.getWriteBufferPos(), reqSize, mCookie, MediaPlayerBase::AudioSink::CB_EVENT_FILL_BUFFER);
 
            for (i = 0;i < actualSize/12;i++) {
                memmove(&outbuffer[16*i], inbuffer + 12*i, 12);
                memset(&outbuffer[16*i + 12], 0, 4);
            }
            actualSize = actualSize * 4 / 3;

            mSharedBuffer->write(actualSize);
        }
        else
        {
            usleep(10000);
        ...
        if (mBufferSize == 0) {
            mBufferSize = sink->bufferSize();
            mSharedBuffer->tccMemAlloc(mBufferSize*2);

            SPDIF_THREAD_LOG("~~~~ TCCFillThread::threadLoop()    mBufferSize[%x]", mBufferSize*2);
        }

        if(mSharedBuffer->writableBuffer()) {
            size_t actualSize = (*mCallback)(sink.get(), mSharedBuffer->getWriteBufferPos(), mSharedBuffer->writableBuffer(), mCookie, MediaPlayerBase::AudioSink::CB_EVENT_FILL_BUFFER);
            mSharedBuffer->write(actualSize);
        }
        else
        {
...
// TCCCallbackThread
//////////////////////////////////////////////////////////////////////////////////////////////////////////
struct TCCCallbackThread : public Thread {
    TCCCallbackThread(const wp<MediaPlayerBase::AudioSink> &sink,
                   MediaPlayerBase::AudioSink::AudioCallback cb,
                   void *cookie,
                   tcc_audio_track_cblk_t *buffer,
                   int channelCount);
...
private:
    ...
    void *mBuffer;
    size_t mBufferSize;
    tcc_audio_track_cblk_t *mReadBuffer;
    int mAudioOutputMode;
    int mChannelCount;
	int mSetLPCMBuffer;

    TCCCallbackThread(const TCCCallbackThread &);
    TCCCallbackThread &operator=(const TCCCallbackThread &);
};

TCCCallbackThread::TCCCallbackThread(
        const wp<MediaPlayerBase::AudioSink> &sink,
        MediaPlayerBase::AudioSink::AudioCallback cb,
        void *cookie,
        tcc_audio_track_cblk_t *buffer,
    ...
    : mSink(sink),
      ...
      mChannelCount(channelCount)
{
    SPDIF_THREAD_LOG("~~~~ TCCCallbackThread::TCCCallbackThread()");
...
TCCCallbackThread::~TCCCallbackThread() {
    SPDIF_THREAD_LOG("TCCCallbackThread is killed");
}

bool TCCCallbackThread::threadLoop() 
{
    sp<MediaPlayerBase::AudioSink> sink = mSink.promote();
    ...
    if (mBufferSize == 0) {
        if((mAudioOutputMode == 3 || mSetLPCMBuffer==1) && (mChannelCount > 2) && (mChannelCount < 8)) {
            mBufferSize = sink->bufferSize() * 4 / 3;
        }
        else {
            mBufferSize = sink->bufferSize();
        }
    ...
    size_t actualSize = mReadBuffer->read(&mBuffer, mBufferSize);

    if (actualSize > 0) {
        SPDIF_THREAD_LOG("~~~~ TCCCallbackThread::threadLoop(1)    actualSize[%x] mBuffer[%p]", actualSize, mBuffer);
        sink->write(mBuffer, actualSize);
        mReadBuffer->release(actualSize);
        SPDIF_THREAD_LOG("~~~~ TCCCallbackThread::threadLoop(2)    actualSize[%x] mBuffer[%p]", actualSize, mBuffer);
    }
...
MediaPlayerService::AudioOutputSpdif::AudioOutputSpdif(int sessionId, int uid)	// SPDIF Kitkat_mr1.1 Enable Planet_20140313
    : mCallback(NULL),
      mCallbackCookie(NULL),
      ...
      mUid(uid)
{
    ...
    if      (value[0] == '3')   mAudioOutputMode = 3;
    else if (value[0] == '4')   mAudioOutputMode = 4;
    else                        mAudioOutputMode = 2;
...
//    mLatency = 1.e3 / (float) sampleRate;
//    mLatency = (bufferSize() * 1000) / (sampleRate * 2 * 2);
    mMsecsPerFrame = 1.e3 / (float) 44100;
    mLatency = (bufferSize() * 1000) / (44100 * 2 * 2);
...
MediaPlayerService::AudioOutputSpdif::~AudioOutputSpdif()
{
    SPDIF_LOG("AudioOutputSpdif::~~AudioOutputSpdif");
    close();
}

void MediaPlayerService::AudioOutputSpdif::setMinBufferCount()
{
    SPDIF_LOG("AudioOutputSpdif::setMinBufferCount");
    char value[PROPERTY_VALUE_MAX];
    if (property_get("ro.kernel.qemu", value, 0)) {
        //mIsOnEmulator = true;
        //mMinBufferCount = 12;  // to prevent systematic buffer underrun for emulator
...
bool MediaPlayerService::AudioOutputSpdif::isOnEmulator()
{
    SPDIF_LOG("AudioOutputSpdif::isOnEmulator");
    setMinBufferCount();
    return false;
}

int MediaPlayerService::AudioOutputSpdif::getMinBufferCount()
{
    SPDIF_LOG("AudioOutputSpdif::getMinBufferCount");
    setMinBufferCount();
    return 12;
}

ssize_t MediaPlayerService::AudioOutputSpdif::bufferSize() const
{
    SPDIF_LOG("AudioOutputSpdif::bufferSize()");
    if(mSpdifHardware) {

    	if (mAudioOutputMode == 3) {
            if (mSampleRate > 190000)
                return (16*1024*2*mChannelCount);

            return (8*1024*2*mChannelCount);
        }
    	else if (mAudioOutputMode == 4){
			char hdmi_audio[PROPERTY_VALUE_MAX];
			memset(hdmi_audio, NULL, PROPERTY_VALUE_MAX);
			property_get("tcc.hdmi.audio_type", hdmi_audio, "0");
...
ssize_t MediaPlayerService::AudioOutputSpdif::frameCount() const
{
    SPDIF_LOG("AudioOutputSpdif::frameCount()");
    if(mSpdifHardware)  {
    	if (mAudioOutputMode == 3) {
            if (mSampleRate > 190000)
                return (16*1024);
...
ssize_t MediaPlayerService::AudioOutputSpdif::channelCount() const
{
    SPDIF_LOG("AudioOutputSpdif::channelCount ()");
    if(mSpdifHardware) {
    	if      (mAudioOutputMode == 3)    return mChannelCount;
        else if (mAudioOutputMode == 4){
			char hdmi_audio[PROPERTY_VALUE_MAX];
			memset(hdmi_audio, NULL, PROPERTY_VALUE_MAX);
			property_get("tcc.hdmi.audio_type", hdmi_audio, "0");
			
			if(hdmi_audio[0]=='1' || hdmi_audio[0]=='2')	return 2;
			else if(hdmi_audio[0]=='3')	return mChannelCount;
			else	return 8;
        }
...
ssize_t MediaPlayerService::AudioOutputSpdif::frameSize() const
{
    SPDIF_LOG("AudioOutputSpdif::frameSize (PCM16 x 2ch = 4)");
    if(mSpdifHardware) {
    	if      (mAudioOutputMode == 3)    return (mChannelCount*2);
        else if (mAudioOutputMode == 4){
			char hdmi_audio[PROPERTY_VALUE_MAX];
			memset(hdmi_audio, NULL, PROPERTY_VALUE_MAX);
			property_get("tcc.hdmi.audio_type", hdmi_audio, "0");
...
uint32_t MediaPlayerService::AudioOutputSpdif::latency () const
{
    SPDIF_LOG("AudioOutputSpdif::latency(%d)", mLatency);
    return mLatency;
}

float MediaPlayerService::AudioOutputSpdif::msecsPerFrame() const
{
    SPDIF_LOG("AudioOutputSpdif::msecsPerFrame(%d)", mMsecsPerFrame);
    return mMsecsPerFrame;
}

status_t MediaPlayerService::AudioOutputSpdif::getFramesWritten(uint32_t *frameswritten) const
{
    //if (mTrack == 0) return NO_INIT;
    //*frameswritten = mBytesWritten / frameSize();
    return OK;
...
status_t MediaPlayerService::AudioOutputSpdif::open(
        uint32_t sampleRate, int channelCount, audio_channel_mask_t channelMask,
        audio_format_t format, int bufferCount,
        AudioCallback cb, void *cookie, audio_output_flags_t flags,
        const audio_offload_info_t *offloadInfo)
{
    SPDIF_LOG("!!!! AudioOutputSpdif::open(%u, %d, %d, %d)", sampleRate, channelCount, format, bufferCount);

    mSpdifHardware = createSPDIFHardware();

    if (mAudioOutputMode == 4)      // HBR
    {
		char hdmi_audio[PROPERTY_VALUE_MAX];
		memset(hdmi_audio, NULL, PROPERTY_VALUE_MAX);
		property_get("tcc.hdmi.audio_type", hdmi_audio, "0");

		// audio_type 0:normal, 1:ddp, 2:ac3 or dts 3:lpcm
		if(hdmi_audio[0]=='1') {
			mSampleRate = sampleRate *4;
			if(mSampleRate>192000)
				mSampleRate = 192000;
				
			mChannelCount	= 2;
			mLatency = (bufferSize() * 1000 *2) / (sampleRate * 8 * 2);
		}
		else if(hdmi_audio[0]=='2') {
			mSampleRate   = sampleRate;
			mChannelCount	= 2;
			mLatency = (bufferSize() * 1000 *2) / (mSampleRate * 2 * 2) + 400; //400ms hdmiaudiosetting - start delay
		}
		else if(hdmi_audio[0]=='3') {
			mSampleRate   = sampleRate;
			mChannelCount = channelCount;
			mLatency = (bufferSize() * 1000) / (mSampleRate * 8 * 2) + 300;
	    }
	    else {
        mSampleRate   = 192000;
        mChannelCount = 8;
			mLatency = (bufferSize() * 1000*2) / (mSampleRate * 8 * 2);
		}
		
        mFormat       = format;
		
		ALOGD("AudioOutputSpdif open: DAI HBR mSampleRate %d mChannels %d",mSampleRate,mChannelCount);
		...
		if(mAudioOutputMode == 2 )
			mLatency = (bufferSize() * 1000 *2) / (mSampleRate *2 * 2);
		else
			mLatency = (bufferSize() * 1000) / (mSampleRate * 8 * 2);
    }

    mMsecsPerFrame = 1.e3 / (float) mSampleRate;
    //mLatency = (bufferSize() * 1000) / (mSampleRate * 8 * 2);

    if(mSpdifHardware) {
        if(cb != NULL) {
            mReadThread     = new TCCFillThread(this, cb, cookie, &mSharedBuffer, channelCount);
            mCallbackThread = new TCCCallbackThread(this, cb, cookie, &mSharedBuffer, channelCount);
        }
        return NO_ERROR;
    }
...
status_t MediaPlayerService::AudioOutputSpdif::start()
{
    SPDIF_LOG("!!~~ AudioOutputSpdif::start    %d", mSampleRate);
    // Open spdif device
    if(mSpdifHardware) {
        mSpdifHardware->setSampleRate(mSampleRate);
        mSpdifHardware->setChannelCount(mChannelCount);
        mSpdifHardware->setFrameSize(mChannelCount*2);
        mSpdifHardware->start();

        if(mReadThread != NULL)
            mReadThread->run("AudioOutputSpdif ReadThread");
        if(mCallbackThread != NULL)
            mCallbackThread->run("AudioOutputSpdif callback");
    }
    return OK;
}

ssize_t MediaPlayerService::AudioOutputSpdif::write(const void* buffer, size_t size)
{
    //SPDIF_LOG("!!~~ AudioOutputSpdif::write buffer=0x%p, size[%d]", buffer, size);
    if(mSpdifHardware)
        return mSpdifHardware->write(buffer, size);

    return 0;
}

void MediaPlayerService::AudioOutputSpdif::stop()
{
    SPDIF_LOG("!!~~ AudioOutputSpdif::stop in");
    // Close spdif device
    if(mSpdifHardware) { 
        if(mReadThread != NULL) {
            mReadThread->requestExitAndWait();
        }
        if(mCallbackThread != NULL) {
            mCallbackThread->requestExitAndWait();
        }
    ...
    SPDIF_LOG("!!~~ AudioOutputSpdif::stop out");
}

void MediaPlayerService::AudioOutputSpdif::flush()
{
    SPDIF_LOG("!!~~ AudioOutputSpdif::flush");
    if(mSpdifHardware) mSpdifHardware->flush();
}

void MediaPlayerService::AudioOutputSpdif::pause()
{
    SPDIF_LOG("!!~~ AudioOutputSpdif::pause");
    if(mSpdifHardware) {
        if(mReadThread != NULL) {
            mReadThread->requestExitAndWait();
        }
        ...
        mSpdifHardware->pause();
    }
}

void MediaPlayerService::AudioOutputSpdif::close()
{
    SPDIF_LOG("AudioOutputSpdif::close");
    if(mSpdifHardware) {
        if(mReadThread != NULL) {
            mReadThread->requestExitAndWait();
        }
...
// Planet 20130930 HBR Mode(Audio and be played.) Start
void MediaPlayerService::AudioOutputSpdif::switchToNextOutput() {
    ALOGV("switchToNextOutput");
    if (mNextOutput != NULL) {
        mNextOutput->mSampleRate= mSampleRate;
        mNextOutput->mMsecsPerFrame = mMsecsPerFrame;
        mNextOutput->mChannelCount= mChannelCount;
        mNextOutput->mLatency = mLatency;
    }
}
// Planet 20130930 HBR Mode(Audio and be played.) End

status_t MediaPlayerService::AudioOutputSpdif::setPlaybackRatePermille(int32_t ratePermille)
{
    ALOGV("setPlaybackRatePermille(%d)", ratePermille);
    status_t res = NO_ERROR;
    //if (mTrack) {
...
status_t MediaPlayerService::AudioOutputSpdif::setAuxEffectSendLevel(float level)
{
    ALOGV("setAuxEffectSendLevel(%f)", level);
    //mSendLevel = level;
    //if (mTrack) {
...
status_t MediaPlayerService::AudioOutputSpdif::attachAuxEffect(int effectId)
{
    ALOGV("attachAuxEffect(%d)", effectId);
    //mAuxEffectId = effectId;
    //if (mTrack) {
...
int MediaPlayerService::AudioOutputSpdif::getSessionId() const
{
    return mSessionId;
}

// static
void MediaPlayerService::AudioOutputSpdif::CallbackWrapper(
        int event, void *cookie, void *info) {
    if (event != AudioTrack::EVENT_MORE_DATA) {
        return;
    }
    SPDIF_LOG("AudioOutputSpdif::CallbackWrapper in...");

    AudioOutputSpdif *me = (AudioOutputSpdif *)cookie;
    AudioTrack::Buffer *buffer = (AudioTrack::Buffer *)info;

    //(*me->mCallback)(
    //        me, buffer->raw, buffer->size, me->mCallbackCookie);
    SPDIF_LOG("AudioOutputSpdif::CallbackWrapper out...");
}
#endif
