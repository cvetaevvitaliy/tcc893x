  ...
  public:
    virtual float scoreFactory(const sp<IMediaPlayer>& client,
        ...
        if (!strncasecmp("http://", url, 7)
            ...
            char value[PROPERTY_VALUE_MAX];
            char* def_player_tag = _STAGEFRIGHT_HTTPLIVE_ ? "sf" : "nu";
            property_get("tcc.hls.player", value, def_player_tag);

            size_t len = strlen(url);
            if (len >= 5 && !strcasecmp(".m3u8", &url[len - 5])) {
                if (!strncasecmp("sf", value, 2)) {
                    return kOurScore;
                }
            }
        ...
        if (!strncasecmp("rtsp://", url, 7)) {
            char value[PROPERTY_VALUE_MAX];
            char* def_player_tag = _STAGEFRIGHT_RTSP_ ? "sf" : "nu";
            if(strstr(url, "wfd1.0"))
                property_get("tcc.rtsp.player", value, "sf");
            else
                property_get("tcc.rtsp.player", value, "nu");
