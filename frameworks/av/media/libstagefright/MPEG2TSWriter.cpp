                ...
                if (mStreamType == 0x0f && mAACCodecSpecificData == NULL) {
                    ...
                    memcpy(mAACCodecSpecificData->data(),
                           (const uint8_t *)buffer->data()
                            + buffer->range_offset(),
                           buffer->range_length());
 				 msg->post();  //2013 04 13 swhwang, After mAACCodecSpecificData init , Sound Source Post 
...
//        ALOGV("MPEG2Writer->Start, Source Index = %d ",i);

        notify->setInt32("source-index", i);

        mSources.editItemAt(i)->start(notify);
		
	    usleep(START_DELAY);  //2013 04 13 swhwang, added Source start init delay for support MPEG2TS Format
    }
...
#if 0//2013 04 13 swhwang : JB Func, replaced Gb Func.(fix ver,).. JB Func. can cause frame drop. 
    ...
    switch (msg->what()) {
        case kWhatSourceNotify:
        {
            ...
            if (what == SourceInfo::kNotifyReachedEOS
                ...
                if (minTimeUs < 0) {
                    ALOGE("not a all tracks have valid data.");
                    break;
                }
