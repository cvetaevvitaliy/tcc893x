...
#define BUFFER_DELAY_DEBUG 0 // 1
#define BD_MSG(msg...) if(BUFFER_DELAY_DEBUG) {ALOGD( msg);}
#define BI_MSG(msg...) if(BUFFER_DELAY_DEBUG) {ALOGI( msg);}
#define BE_MSG(msg...) if(BUFFER_DELAY_DEBUG) {ALOGE( msg);}
#define BW_MSG(msg...) if(BUFFER_DELAY_DEBUG) {ALOGW( msg);}
...
#ifdef BUFFER_DELAY
    for(int idx = 0; idx < ADELAY_MAX; idx++)
...
status_t ACodec::configureOutputBuffersFromNativeWindow(
    ...
    err = native_window_set_usage(
            mNativeWindow.get(),
            usage | GRALLOC_USAGE_HW_TEXTURE | GRALLOC_USAGE_EXTERNAL_DISP | GRALLOC_USAGE_PRIVATE_2); //ZzaU :: for flash player
...
status_t ACodec::setComponentRole(
    ...
    static const MimeToRole kMimeToRole[] = {
        ...
        { MEDIA_MIMETYPE_AUDIO_AAC_TCC,
            "audio_decoder.tcc_aac", "audio_encoder.aac" },
    };
...
status_t ACodec::configureCodec(
    ...
    } else if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_AAC) || !strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_AAC_TCC)) {
        int32_t numChannels, sampleRate;
        if (!msg->findInt32("channel-count", &numChannels)
                || !msg->findInt32("sample-rate", &sampleRate)) {
            err = INVALID_OPERATION;
...
#ifdef BUFFER_DELAY
    if( !strncmp("OMX.TCC.", mComponentName.c_str(), 8) &&  strncmp(mComponentName.c_str(), "OMX.TCC.google", 14) )
        mTCCVideo = true;
#endif
...
bool ACodec::BaseState::onOMXFillBufferDone(
	...
	if( mCodec->mTCCVideo ){
	    BD_MSG("[%s] onOMXFillBufferDone %p time %lld us, flags = 0x%08lx",
    ...
    switch (mode) {
        ...
        case RESUBMIT_BUFFERS:
        {
            if (rangeLength == 0 && !(flags & OMX_BUFFERFLAG_EOS)) {
                ...
                if( mCodec->mTCCVideo ){
                    BD_MSG("1 [%s] calling fillBuffer %p",
...
void ACodec::BaseState::onOutputBufferDrained(const sp<AMessage> &msg) {
    ...
    if (mCodec->mNativeWindow != NULL
        ...
        if ((err = mCodec->mNativeWindow->queueBuffer(
            ...
            if( mCodec->mTCCVideo ){
                BD_MSG("[%s] queuing Buffer %p to Surface", mCodec->mComponentName.c_str(), info->mBufferID);
            }
        } else {
            mCodec->signalError(OMX_ErrorUndefined, err);
            info->mStatus = BufferInfo::OWNED_BY_US;
            if( mCodec->mTCCVideo ){
                BE_MSG("[%s] failed queuing Buffer %p to Surface", mCodec->mComponentName.c_str(), info->mBufferID);
            }
        }
    } else {
        info->mStatus = BufferInfo::OWNED_BY_US;
        if( mCodec->mTCCVideo ){
            BE_MSG("[%s] failed queuing Buffer %p to Surface :: no-NativeWindo or no-rendered(%d)", mCodec->mComponentName.c_str(), info->mBufferID, render);
        }
    }
    ...
    switch (mode) {
        case KEEP_BUFFERS:
        {
            ...
            if (info->mStatus == BufferInfo::OWNED_BY_NATIVE_WINDOW) {
                ...
                if( mCodec->mTCCVideo )
                {
                    if( info != NULL)
                    {
                        BI_MSG("[%s] KEEP_BUFFERS :: calling dequeueBufferFromNativeWindow %p", mCodec->mComponentName.c_str(), info->mBufferID);

                        for(int idx = 0; idx < (ADELAY_CNT -1); idx++)
                            mCodec->mLast_dequed[idx] = mCodec->mLast_dequed[idx+1];
                        mCodec->mLast_dequed[ADELAY_CNT -1] = info;
                    }
                }
#endif
            }
        ...
        case RESUBMIT_BUFFERS:
        {
            if (!mCodec->mPortEOS[kPortIndexOutput]) {
                ...
                if( mCodec->mTCCVideo )
                {
                    BufferInfo *prev = mCodec->mLast_dequed[0];
                    ...
                    if ( info->mStatus == BufferInfo::OWNED_BY_NATIVE_WINDOW ) {
                        ...
                        BI_MSG("[%s] RESUBMIT_BUFFERS :: calling dequeueBufferFromNativeWindow %p", mCodec->mComponentName.c_str(), info->mBufferID);
                        ...
                        if( prev == NULL ){
                            BE_MSG("Buffer-Delay :: buffer return => [0x%x, 0x%x, 0x%x]", mCodec->mLast_dequed[0], mCodec->mLast_dequed[1], mCodec->mLast_dequed[2]);
                            return;
                        }
                        else
                        {
                            if( prev->mStatus != BufferInfo::OWNED_BY_US ){
                                for(int idx = 0; idx < ADELAY_CNT -1; idx++)
                                    mCodec->mLast_dequed[idx] = NULL;
                                BE_MSG("Buffer-Delay :: buffer error for seek 0x%x(%d) => [0x%x, 0x%x, 0x%x]", prev, prev->mStatus, mCodec->mLast_dequed[0], mCodec->mLast_dequed[1], mCodec->mLast_dequed[2]);
                                return;
                            }
                            info = prev;
                        }
                ...
                if (info != NULL) {
                    if( mCodec->mTCCVideo ){
                        BI_MSG("2 [%s] calling fillBuffer %p",
                            mCodec->mComponentName.c_str(), info->mBufferID);
					}
                    else{
	                    ALOGV("[%s] calling fillBuffer %p",
...
void ACodec::ExecutingState::submitRegularOutputBuffers() {
    ...
    if( mCodec->mTCCVideo )
    {
        ...
        if(nCnt_FreeOutBuffer < (ADELAY_CNT-1))
            return;
        ...
        for (size_t i = 0; i < mCodec->mBuffers[kPortIndexOutput].size(); ++i) {
            ...
            if( mCodec->mLast_dequed[0] != NULL )
            {
                if( mCodec->mLast_dequed[0]->mGraphicBuffer->handle == info->mGraphicBuffer->handle )
                    continue;
            }

            if( mCodec->mLast_dequed[1] != NULL )
            {
                if( mCodec->mLast_dequed[1]->mGraphicBuffer->handle == info->mGraphicBuffer->handle )
                    continue;
            }

            if( mCodec->mLast_dequed[2] != NULL )
            {
                if( mCodec->mLast_dequed[2]->mGraphicBuffer->handle == info->mGraphicBuffer->handle )
                    continue;
            }

    		if( mCodec->mTCCVideo ){
                BI_MSG("3 [%s] calling fillBuffer %p",
    	             mCodec->mComponentName.c_str(), info->mBufferID);
    		}
            ...
            nCnt_FreeOutBuffer--;
            if( mCodec->mLast_dequed[0] != NULL || mCodec->mLast_dequed[1] != NULL || mCodec->mLast_dequed[2] != NULL )
            {
                if( nCnt_FreeOutBuffer < (ADELAY_CNT-1) )
                    return;
            }
