...
static int64_t kBufferFilledEventTimeOutNs = 3000000000LL;
...
sp<MediaSource> OMXCodec::Create(
    ...
    int32_t streamingMediaType;
    if (meta->findInt32(
                kKeyIsStreamingMedia,
                &streamingMediaType)
            && streamingMediaType) {
        flags |= kIgnoreBufferFilledTimeOut;
        if (streamingMediaType == MediaSource::kHttpMediaSource) {
            flags |= kSourceIsFromHttp;
        } else if (streamingMediaType == MediaSource::kRtspMediaSource) {
            flags |= kSourceIsFromRtsp;
        } else if( streamingMediaType == MediaSource::kWFDMediaSource) {
            flags |= kSourceIsFromWFD;
        }
		ALOGW("[%s] Ignore BufferFilled TimeOut", mime);
    }
...
#if 0 // Telechips, ZzaU :: for SecureInput Buffer.
        AString tmp;
...
status_t OMXCodec::configureCodec(const sp<MetaData> &meta) {
    ...
    if (!(mFlags & kIgnoreCodecSpecificData)) {
        ...
        if (meta->findData(kKeyESDS, &type, &data, &size)) {
            ...
            const char* pc_key_mime_type;
            CHECK(meta->findCString(kKeyMIMEType, &pc_key_mime_type));
            if (!strcasecmp(pc_key_mime_type, MEDIA_MIMETYPE_AUDIO_MPEG_TCC)) {
                if (!mIsEncoder)
		...
		// TELECHIPS, SSG
		else if (meta->findData(kKeyTCCCodecInfo, &type, &data, &size)) {
			if(!mIsEncoder)
		        addCodecSpecificData(data, size);
		}
    }
	...
	// TELECHIPS, SSG
	else if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_MPEG_TCC, mMIME)) {
		int32_t numChannels, sampleRate;
		CHECK(meta->findInt32(kKeyChannelCount, &numChannels));
		CHECK(meta->findInt32(kKeySampleRate, &sampleRate));
        if (mIsEncoder) {
		...
		status_t err = setMP3Format(numChannels, sampleRate, bitRate);
        if (err != OK) {
            CODEC_LOGE("setMP3Format() failed (err = %d)", err);
            return err;
        }
	} else if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_AAC_TCC, mMIME)) {
		int32_t numChannels, sampleRate;
		CHECK(meta->findInt32(kKeyChannelCount, &numChannels));
		CHECK(meta->findInt32(kKeySampleRate, &sampleRate));

		status_t err = setTCCAACFormat(numChannels, sampleRate);
        if (err != OK) {
            CODEC_LOGE("setTCCAACFormat() failed (err = %d)", err);
            return err;
        }
	} else if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_VORBIS_TCC, mMIME)) {
		int32_t numChannels, sampleRate;
		CHECK(meta->findInt32(kKeyChannelCount, &numChannels));
		CHECK(meta->findInt32(kKeySampleRate, &sampleRate));

		status_t err = setVORBISFormat(numChannels, sampleRate);
        if (err != OK) {
		...
		status_t err = setFLACFormat(numChannels, sampleRate);
        if (err != OK) {
		...
		status_t err = setAPEFormat(numChannels, sampleRate);
        if (err != OK) {
		...
		status_t err = setWMAFormat(numChannels, sampleRate);
        if (err != OK) {
            CODEC_LOGE("setWMAFormat() failed (err = %d)", err);
            return err;
        }
	} else if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_AC3, mMIME)
		|| !strcasecmp(MEDIA_MIMETYPE_AUDIO_DDP, mMIME)
		|| !strcasecmp(MEDIA_MIMETYPE_AUDIO_TRUEHD, mMIME)) {
		int32_t numChannels, sampleRate;
		CHECK(meta->findInt32(kKeyChannelCount, &numChannels));
		CHECK(meta->findInt32(kKeySampleRate, &sampleRate));
		...
		|| !strcasecmp(MEDIA_MIMETYPE_AUDIO_DTSHD, mMIME)) {
		int32_t numChannels, sampleRate;
		CHECK(meta->findInt32(kKeyChannelCount, &numChannels));
		CHECK(meta->findInt32(kKeySampleRate, &sampleRate));

		status_t err = setDTSFormat(numChannels, sampleRate);
        if (err != OK) {
		...
		status_t err = setMP2Format(numChannels, sampleRate);
        if (err != OK) {
		...
		status_t err = setRAFormat(numChannels, sampleRate);
        if (err != OK) {
            CODEC_LOGE("setRAFormat() failed (err = %d)", err);
            return err;
        }
	} else if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_PCM, mMIME)) {
		int32_t numChannels, sampleRate;
		CHECK(meta->findInt32(kKeyChannelCount, &numChannels));
		CHECK(meta->findInt32(kKeySampleRate, &sampleRate));

		status_t err = setPCMFormat(numChannels, sampleRate);
        if (err != OK) {
            CODEC_LOGE("setPCMFormat() failed (err = %d)", err);
            return err;
        }
	}
	...
	// TELECHIPS, SSG
    if ((mFlags & kClientNeedsFramebuffer)
            && !strncmp(mComponentName, "OMX.TCC.", 8)
			&& !strncmp(mMIME, "video/", 6)
			&& !mIsEncoder) {
        OMX_INDEXTYPE index;
        ...
        if (err != OK) {
            CODEC_LOGE("setConfig('OMX.TCC.index.ThumbnailMode') "
                       "returned error 0x%08x", err);
...
status_t OMXCodec::setVideoOutputFormat(
	...
	// TELECHIPS, SSG
        ...
        compressionFormat = OMX_VIDEO_CodingWMV_1_2;
...
OMXCodec::OMXCodec(
    ...
    : mOMX(omx),
      ...
      mSignalledEOS(false),
	  mIsStarted(false), // TELECHIPS, SSG
      ...
      mSkipCutBuffer(NULL),
      mIsSourceUnderrun(false),
      mIsSyncFrameDelayed(false),
      mLeftOverBuffer(NULL),
      mIsWaitSeekRequest(false),
      mPaused(false),
      mExtractorType(MediaExtractor::UNASSIGNED),
      mNativeWindow(
      ...
      mPlayMode(0),
      mIsReadDone(false),
      mIsSkimmingDone(false){
    mPortStatus[kPortIndexInput] = ENABLED;
    mPortStatus[kPortIndexOutput] = ENABLED;

    setComponentRole();

    if(mFlags & kSourceIsFromWFD)
        mWFDModeEnable = OMX_TRUE;
    else
        mWFDModeEnable = OMX_FALSE;

#ifdef S_BUFFER_DELAY
    for(int idx = 0; idx < DELAY_CNT; idx++)
        mLast_dequed[idx] = NULL;

    if(mIsVideo && !strncmp(componentName, "OMX.TCC.", 8) && strncmp(componentName, "OMX.TCC.google", 14)){
        mIsTCCVideo = true;
        ALOGI("Enabled buffer delay for tcc-video");
    }
    else{
        mIsTCCVideo = false;
    }
...
status_t OMXCodec::allocateBuffersOnPort(OMX_U32 portIndex) {
    ...
    // Telechips, ZzaU :: for SecureInput Buffer.
    if (portIndex == kPortIndexInput && (mFlags & kUseSecureInputBuffers))
    {
        OMX_INDEXTYPE index;

        status_t err = mOMX->getExtensionIndex( mNode, "OMX.TCC.index.InputBuffer.SecureMode", &index);

        if (err != OK) {
            return err;
        }

        OMX_BOOL enable = mWFDModeEnable ? OMX_TRUE : OMX_FALSE;
        err = mOMX->setConfig(mNode, index, &enable, sizeof(enable));

        if (err != OK) {
            CODEC_LOGE("setConfig('OMX.TCC.index.InputBuffer.SecureMode') returned error 0x%08x", err);
            return err;
        }
    }
...
#else
    // When calculating total buffer size,
    // nBufferSize must be rounded up to multiple of page size to avoid memory allocation failure by MemoryDealer.
    // This code was applied to Gingerbread/ICS, but it has not been applied to JB yet.
    const size_t pagesize = 4096;
    size_t bufSize = ((def.nBufferSize + pagesize-1) & ~(pagesize-1));
    ...
    switch (msg.type) {
        ...
        case omx_message::FILL_BUFFER_DONE:
        {
            ...
            if (mPortStatus[kPortIndexOutput] == DISABLING) {
                ...
                if ((msg.u.extended_buffer_data.flags & OMX_BUFFERFLAG_MISSING_TIMESTAMP) &&
					!strncmp(mMIME, "audio/", 6)) {
                    buffer->meta_data()->setInt32(kKeyTimeGoesMissing, 1);
                }

                if (mTargetTimeUs >= 0) {
            #if 0 //ZzaU :: to prevent video-editor faulting when inserting transtion into clip.
                  // In case of android seek method, they ignore frames which have less timestamp than seek time.
                  // So, this check function don't have meanning.
        ...
        case OMX_EventPortSettingsChanged:
        {
            ...
            if (data2 == 0 || data2 == OMX_IndexParamPortDefinition) {
                ...
                if (data1 == kPortIndexOutput) {
                    if (mFlags & kIgnoreBufferFilledTimeOut) {
                        if (!mFilledBuffers.empty()) {
							CODEC_LOGW("mFilledBuffers is NOT empty..");
                ...
                if (data2 == OMX_IndexConfigCommonOutputCrop &&
                    ...
                    if (OK == mOMX->getConfig(
                        ...
                        ALOGI("Get OMX_IndexConfigScale: 0x%lx/0x%lx",
    ...
    for (size_t i = buffers->size(); i-- > 0;) {
        ...
        if (mFlags & kIgnoreBufferFilledTimeOut) {
            if (info->mStatus != OWNED_BY_US
                && info->mStatus != OWNED_BY_NATIVE_WINDOW) {
                CODEC_LOGW("change status(%d) for freeing buffer %p on port %ld", 
                           info->mStatus, info->mBuffer, portIndex);
				continue;
            }
        }
...
void OMXCodec::fillOutputBuffers() {
    ...
    Vector<BufferInfo> *buffers = &mPortBuffers[kPortIndexOutput];
    size_t buff_size = buffers->size();

    if (!mIsEncoder) {
        // setup output buffer max for skimming
        if(mPlayMode & 0x40000000)
            buff_size = OUTPUT_BUFFER_MAX_FOR_SKIMMING;
        else
    ...
    for (size_t i = 0; i < buff_size; ++i) {
        ...
        if (info->mStatus == OWNED_BY_US) {
            ...
            if(mWFDModeEnable == OMX_TRUE) 
            {
                ALOGI("First Outputbuffer is drained.");
                break;
            }
        }
    }
...
void OMXCodec::drainInputBuffers() {
    ...
    if (mFlags & kUseSecureInputBuffers) {
        ...
        for (size_t i = 0; i < buffers->size(); ++i) {
            ...
            if(mWFDModeEnable == OMX_TRUE) 
            {
                ALOGI("First Inputbuffer is drained.");
                break;
            }
...
bool OMXCodec::drainInputBuffer(BufferInfo *info) {
    ...
    if (mCodecSpecificDataIndex < mCodecSpecificData.size()) {
		...
		// TELECHIPS
		if (!mSource->getFormat()->findInt32(kKeyMediaExtractor, &mExtractorType)) {
			mExtractorType = MediaExtractor::UNASSIGNED;
		}

		if ((mExtractorType == MediaExtractor::MPEG2TS) 
                && (!strncasecmp(mMIME, "video/", 6))) {
        ...
        status_t err = mOMX->emptyBuffer(
                ...
                OMX_BUFFERFLAG_ENDOFFRAME | OMX_BUFFERFLAG_CODECCONFIG | mExtractorType,
                0);
        CHECK_EQ(err, (status_t)OK);
    ...
    bool isBFrameSkip = false;
    bool isIFrameOnly = false;
    int32_t timeNotFound = 0;
    int32_t packetDiscon = 0;
    ...
    for (;;) {
        ...
        if (!mIsEncoder) {
            if (mIsReadDone)
            {
                options.setPlayRate(mPlayRate);
                if(mPlayMode & 0x40000000)
                    isIFrameOnly = true;
                else
                    isIFrameOnly = false;
            }
            if (mPlayMode & 0x80000000)
                isBFrameSkip = true;
            else
                isBFrameSkip = false;
        }
        ...
        //Skimming
        if (!mIsEncoder) {
            if (err == INFO_SKIMMING_DONE) {
                signalEOS = true;
                mSignalledEOS = true;
                err = ERROR_END_OF_STREAM;
                mIsSkimmingDone = true;
                break;
            }
        }

        if (err != OK) {
            if (err == INFO_RETRY && (mFlags & kIgnoreBufferFilledTimeOut)) {
                mIsSourceUnderrun = true;
                if (isSyncFrame) {
                    mIsSyncFrameDelayed = true;
                }
                mBufferFilled.signal();
                break;
            }

            if( err == WAIT_SEEK_REQUEST ) {
                DLOGD("[WAIT-step02] drainInputBuffer() - returned WAIT_SEEK_REQUEST (%d)", err);
                mIsWaitSeekRequest = true;
                mBufferFilled.signal();
                break;
            }
        ...
        mIsSourceUnderrun = false;
        if (mIsSyncFrameDelayed) {
            isSyncFrame = mIsSyncFrameDelayed;
            mIsSyncFrameDelayed = false;
        }
        ...
        if (srcBuffer->range_length() > remainingBytes) {
            if (offset == 0) {
                ...
                // TELECHIPS, Hees
                if (!strcmp(mComponentName, "OMX.google.aac.decoder") 
                        && (mFlags & kSourceIsFromRtsp)) {
                    CODEC_LOGE(
                            "Codec's input buffers are too small to accomodate "
                            "buffer read from source. So, abandon it (info->mSize = %d, srcLength = %d)",
        ...
        int32_t level = 0;
        char temp_WFD[PROPERTY_VALUE_MAX];
        property_get("tcc.wfd.display.level", temp_WFD, "0");
        level = atoi(temp_WFD);

        property_get("tcc.wfd.display.level.enable", temp_WFD, "0");

        if ( mWFDModeEnable  && atoi(temp_WFD) == 1 && level == 4) {
            CHECK(srcBuffer->meta_data()->findInt32(kKeyPacketDisCont, &packetDiscon));
        }

#if 0
        CHECK(lastBufferTimeUs >= 0);
#else
        if ((mFlags & kSourceIsFromRtsp) && (lastBufferTimeUs < 0)) {
            // RTSP might set the negative keyTime for some environmental reasons. 
            lastBufferTimeUs = 0;
        }
#endif
        if (mIsEncoder && mIsVideo) {
        ...
        if (offset == 0) {
            ...
            if (!srcBuffer->meta_data()->findInt32(kKeyTimeGoesMissing, &timeNotFound)) {
                timeNotFound = 0;
            }
        }
	...
	// TELECHIPS, SSG
	if (!mIsEncoder) {
		if (isSyncFrame == true || !mIsStarted) {
			mIsStarted = true;
			flags |= OMX_BUFFERFLAG_SYNCFRAME;
		}
		//Skimming
		if (isBFrameSkip) {
			flags |= OMX_BUFFERFLAG_BFRAME_SKIP;
		}

		if (isIFrameOnly) {
			flags |= OMX_BUFFERFLAG_IFRAME_ONLY;
		}

        if (timeNotFound) {
			flags |= OMX_BUFFERFLAG_MISSING_TIMESTAMP;
        }

        if (packetDiscon == 1) {
            ALOGI("[WFD] POC error Lv4");
            flags |= OMX_BUFFERFLAG_SYNCFRAME;
        }
	}
...
status_t OMXCodec::waitForBufferFilled_l() {
    ...
    if (mIsEncoder || (mFlags & kIgnoreBufferFilledTimeOut)) {
        ...
        // for video encoding.
		// In addition, we adopt the same rule to an input frame from n/w data source. 
...
// ===========================================================================
// TELECHIPS, SSG, block start
status_t OMXCodec::setMP3Format(int32_t numChannels, int32_t sampleRate, int32_t bitRate) {

    ALOGE("setMP3Format(%d, %d, %d)", numChannels, sampleRate, bitRate);
    ...
    if (mIsEncoder) {
        ...
        InitOMXParams(&profile);
        profile.nPortIndex = kPortIndexOutput;
        CHECK_EQ(mOMX->getParameter(mNode, OMX_IndexParamAudioMp3,
                &profile, sizeof(profile)), (status_t)OK);
        profile.nChannels = numChannels;
        profile.eChannelMode = (numChannels == 1?
                OMX_AUDIO_ChannelModeMono: OMX_AUDIO_ChannelModeStereo);
        ...
        CHECK_EQ(mOMX->setParameter(mNode, OMX_IndexParamAudioMp3,
                &profile, sizeof(profile)), (status_t)OK);
    }
	else {
		OMX_AUDIO_PARAM_MP3TYPE profile;
		...
		status_t err = mOMX->getParameter(
				mNode, OMX_IndexParamAudioMp3, &profile, sizeof(profile));
		CHECK_EQ(err, (status_t)OK);
        ...
        if (err != OK) {
            CODEC_LOGE("setParameter('OMX_IndexParamAudioMp3') failed (err = %d)", err);
            return err;
        }
	}
...
status_t OMXCodec::setTCCAACFormat(int32_t numChannels, int32_t sampleRate) {
	OMX_AUDIO_PARAM_AACPROFILETYPE profile;
	InitOMXParams(&profile);
	profile.nPortIndex = kPortIndexInput;
...
status_t OMXCodec::setVORBISFormat(int32_t numChannels, int32_t sampleRate) {
	OMX_AUDIO_PARAM_VORBISTYPE profile;
	InitOMXParams(&profile);
	profile.nPortIndex = kPortIndexInput;

	status_t err = mOMX->getParameter(
			mNode, OMX_IndexParamAudioVorbis, &profile, sizeof(profile));
	CHECK_EQ(err, (status_t)OK);
	...
	if (err != OK) {
		CODEC_LOGE("setParameter('OMX_IndexParamAudioVorbis') failed (err = %d)", err);
		return err;
	}
...
status_t OMXCodec::setFLACFormat(int32_t numChannels, int32_t sampleRate) {
	...
	if (err != OK) {
		CODEC_LOGE("setParameter('OMX_IndexParamAudioFlac') failed (err = %d)", err);
		return err;
	}
...
status_t OMXCodec::setAPEFormat(int32_t numChannels, int32_t sampleRate) {
	...
	if (err != OK) {
		CODEC_LOGE("setParameter('OMX_IndexParamAudioAPE') failed (err = %d)", err);
		return err;
	}
...
status_t OMXCodec::setAC3Format(int32_t numChannels, int32_t sampleRate) {
	...
	status_t err = mOMX->getParameter(
			mNode, OMX_IndexParamAudioAC3, &profile, sizeof(profile));
	CHECK_EQ(err, (status_t)OK);
	...
	if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_AC3, mMIME)) {
		profile.eFormat = OMX_AUDIO_AC3FormatAC3;
	} else if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_TRUEHD, mMIME)) {
		profile.eFormat = OMX_AUDIO_AC3FormatTRUEHD;
	} else if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_DDP, mMIME)) {
		profile.eFormat = OMX_AUDIO_AC3FormatDDP;
	} else {
		profile.eFormat = OMX_AUDIO_AC3FormatUnused;
	}
	...
	if (err != OK) {
		CODEC_LOGE("setParameter('OMX_IndexParamAudioAC3') failed (err = %d)", err);
		return err;
	}
...
status_t OMXCodec::setDTSFormat(int32_t numChannels, int32_t sampleRate) {
	OMX_AUDIO_PARAM_DTSTYPE profile;
	InitOMXParams(&profile);
	profile.nPortIndex = kPortIndexInput;

	status_t err = mOMX->getParameter(
			mNode, OMX_IndexParamAudioDTS, &profile, sizeof(profile));
	CHECK_EQ(err, (status_t)OK);

	profile.nChannels = numChannels;
	profile.nSamplingRate = sampleRate;

	if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_DTS, mMIME)) {
		profile.eFormat = OMX_AUDIO_DTSFormatDTS;
	} else if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_DTSHD, mMIME)) {
		profile.eFormat = OMX_AUDIO_DTSFormatDTSHD;
	} else {
		profile.eFormat = OMX_AUDIO_DTSFormatUnused;
	}
	...
	if (err != OK) {
		CODEC_LOGE("setParameter('OMX_IndexParamAudioDTS') failed (err = %d)", err);
		return err;
	}
...
status_t OMXCodec::setMP2Format(int32_t numChannels, int32_t sampleRate) {
	OMX_AUDIO_PARAM_MP2TYPE profile;
	InitOMXParams(&profile);
	profile.nPortIndex = kPortIndexInput;

	status_t err = mOMX->getParameter(
			mNode, OMX_IndexParamAudioMp2, &profile, sizeof(profile));
	CHECK_EQ(err, (status_t)OK);
	...
	if (err != OK) {
		CODEC_LOGE("setParameter('OMX_IndexParamAudioMp2') failed (err = %d)", err);
		return err;
	}
...
status_t OMXCodec::setRAFormat(int32_t numChannels, int32_t sampleRate) {
...
status_t OMXCodec::setPCMFormat(int32_t numChannels, int32_t sampleRate) {
	OMX_AUDIO_PARAM_PCMMODETYPE profile;
	InitOMXParams(&profile);
	profile.nPortIndex = kPortIndexInput;

	status_t err = mOMX->getParameter(
			mNode, OMX_IndexParamAudioPcm, &profile, sizeof(profile));
	CHECK_EQ(err, (status_t)OK);
	...
	if (err != OK) {
		CODEC_LOGE("setParameter('OMX_IndexParamAudioPcm') failed (err = %d)", err);
		return err;
	}

	return OK;
}
// TELECHIPS, SSG, block end
// ==========================================================================
...
status_t OMXCodec::read(
    ...
    *buffer = NULL;
    ...
    if(!mIsEncoder)
    {
        ...
        {
            mPlayRate = playRate;
            //Get current playmode from Awesome player
            //then if it is change from previous condition
            //let omx flush remaining input and output buffers.
            if(mPlayMode != (mPlayRate & 0xC0000000))
            {
                mPlayMode = mPlayRate & 0xC0000000;
                if(mPlayMode & 0x40000000)
                    kBufferFilledEventTimeOutNs = 30000000000LL;//wait for 30 sec.
                else
                    kBufferFilledEventTimeOutNs = 3000000000LL;//wait for 3 sec.
            }
            mIsReadDone = true;
        }
    }
    ...
    if (seeking) {
        ...
        while (mSeekTimeUs >= 0 && !mIsWaitSeekRequest && !mIsSourceUnderrun) {
            if ((err = waitForBufferFilled_l()) != OK) {
                return err;
            }
        }
    ...
    while (mState != ERROR && !mNoMoreOutputData && !mIsWaitSeekRequest && !mIsSourceUnderrun && mFilledBuffers.empty()) {
        if ((err = waitForBufferFilled_l()) != OK) {
            return err;
        }
    }
    ...
    if (mFilledBuffers.empty()) {
		if (mIsSourceUnderrun) {
            mIsSourceUnderrun = false;
			return INFO_RETRY;
		}

		if( mIsWaitSeekRequest == true ) {
			DLOGD("[WAIT-step03] OMXCodec::read() - received flag");
			mIsWaitSeekRequest = false;
    ...
    for (size_t i = 0; i < buffers->size(); ++i) {
        ...
        if (info->mMediaBuffer == buffer) {
            ...
            if (buffer->graphicBuffer() == 0) {
                ...
                if( !mIsEncoder && mIsTCCVideo && (atoi(prop_value) != 1))
                {
                    BufferInfo *prev = mLast_dequed[0];
                    ...
                    for(int idx = 0; idx < (DELAY_CNT -1); idx++)
                        mLast_dequed[idx] = mLast_dequed[idx+1];
                    mLast_dequed[DELAY_CNT -1] = nextBufInfo;

                    if( prev == NULL ){
                        ALOGV("Buffer-Delay :: buffer return => [0x%x, 0x%x, 0x%x]", mLast_dequed[0], mLast_dequed[1], mLast_dequed[2]);
                        return;
                    }
                    else
                    {
                        if( prev->mStatus != OWNED_BY_US ){
                            for(int idx = 0; idx < DELAY_CNT -1; idx++)
                                mLast_dequed[idx] = NULL;
                            ALOGV("Buffer-Delay :: buffer error for seek 0x%x(%d) => [0x%x, 0x%x, 0x%x]", prev, prev->mStatus, mLast_dequed[0], mLast_dequed[1], mLast_dequed[2]);
                            return;
                        }
                        nextBufInfo = prev;
                    }
                    
                    ALOGV("Buffer-Delay :: buffer proc 0x%x => [0x%x, 0x%x, 0x%x]", nextBufInfo, mLast_dequed[0], mLast_dequed[1], mLast_dequed[2]);
...
void OMXCodec::initOutputFormat(const sp<MetaData> &inputFormat) {
    ...
    if (mIsEncoder) {
        ...
        //TELECHIPS, ZzaU :: for V2IP
        if(mIsEncoder)
        {
            CODEC_LOGV(" TELECHIPS, ZzaU :: kKeyNodeId");
    ...
    switch (def.eDomain) {
        ...
        case OMX_PortDomainAudio:
        {
            ...
            }else if (audio_def->eEncoding == OMX_AUDIO_CodingMP3) {
                mOutputFormat->setCString(
                        kKeyMIMEType, MEDIA_MIMETYPE_AUDIO_MPEG_TCC);
                int32_t numChannels, sampleRate, bitRate;
                inputFormat->findInt32(kKeyChannelCount, &numChannels);
                inputFormat->findInt32(kKeySampleRate, &sampleRate);
                inputFormat->findInt32(kKeyBitRate, &bitRate);
