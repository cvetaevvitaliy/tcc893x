...
void WifiDisplaySource::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        case kWhatStart:
        {
            ALOGD("onMessageReceived: kWhatStart");
...
status_t WifiDisplaySource::onReceiveM3Response(
        int32_t sessionID, const sp<ParsedMessage> &msg) {
    int32_t statusCode;
    ALOGD("onReceiveM3Response msg = %s len=%d", msg->getContent(), strlen(msg->getContent()));
...
void WifiDisplaySource::sendErrorResponse(
        ...
        int32_t cseq) {
    ALOGE("%s: %s", __FUNCTION__,errorDetail);
