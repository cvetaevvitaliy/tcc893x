...
void AudioPlayer::pauseAfterFillBuffer(bool playPendingSamples) {
    CHECK(mStarted);

    Mutex::Autolock autoSerializer(mSerializer);

    while (mFillBufferDone == false) {
        mCondition.wait(mSerializer);
    }
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
    if(mWFDSinkPlayMode)
       result = (mNumFramesPlayed * 1000000) / mSampleRate;
    else
#endif
       result = -mLatencyUs + (mNumFramesPlayed * 1000000) / mSampleRate;
