...
#include <sys/time.h>
#include <media/stagefright/TCCOMXCodec.h>
#endif
...
static int64_t kLowWaterMarkUs = 1000000ll; // android: 2secs
static int64_t kHighWaterMarkUs = 3000000ll; // android : 5secs

static const size_t kLowWaterMarkBytes = 40000;
static const size_t kHighWaterMarkBytes = 200000;

#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
static int64_t kPauseWaterMarkUs = 500000ll;  // 0.5 secs
static int64_t kRTSPResumeWaterMarkUs = 3000000ll;  // 3.0 secs
static int64_t kHLSResumeWaterMarkUs = 3000000ll;  // 3.0 secs
static int64_t kDLNAResumeWaterMarkUs = 2000000ll;  // 2.0 secs
static int64_t kResumeWaterMarkUs = 5000000ll;  // 5.0 secs
static int64_t kPrerollWaterMarkUs = 2000000ll;  // 2.0 secs

static int64_t kPrefillWaitTimeUs = 60000000ll; // 1 min.
static int64_t kEntryConnectionTimeUs = 120000000ll; // 2 min.
#endif
...
AwesomePlayer::AwesomePlayer()
    : mQueueStarted(false),
      ...
      mHTTPTimeoutSecs(-1),
      mCheckEOSPossibility(false),
      mEOSReached(false),
      mEOSFinalized(false),
      mNumKeepingUnderrunNearEOS(-1),
      mSeekAsyncDonePending(false),
      mBufferingUpdateType(NOTIFY_ON_RUNNING),
      mHasItsOwnCache(false),
      mUseAnotherCachedSource(false),
      mSeekable(true),
      mSeekInvokable(false),
      mStreamingMode(NONE),
#endif
      mTextDriver(NULL),
      mOffloadAudio(false),
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
    updateBufferingParamsFromSystemProperty();
    mBufferingEvent = NULL;
#else
    mBufferingEventDelayUs = 1000000ll;
    mBufferingEvent = new AwesomeEvent(this, &AwesomePlayer::onBufferingUpdate);
#endif
    mBufferingEventPending = false;

#ifdef _USE_TCC_AWESOMEPLAYER_
    mVideoLagEvent = NULL;
#else
...
AwesomePlayer::~AwesomePlayer() {
    ...
    mClient.disconnect();
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
    property_set("tcc.video.lplayback.mode", "0");
    property_set("tcc.media.wfd.sink.run", "0");
#endif
}
...
status_t AwesomePlayer::setDataSource_l(
    ...
    #if _USE_TCC_AWESOMEPLAYER_
        getApplicationSpecificHeaders();
    #endif
    }

#if _USE_TCC_AWESOMEPLAYER_
    addHTTPSpecificHeadersIfRequired(HTTPBase::HTTP_GET_AUTHORIZATION |
                                     HTTPBase::HTTP_GET_CONNECTION);
#endif

    ALOGI("setDataSource_l('%s')", (!(mFlags & INCOGNITO)) ? mUri.string() : "URL suppressed");
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
    property_set("tcc.video.lplayback.mode", (!strncmp(mUri.string(), "/", 1)) ? "1" : "0");
#endif
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
    if(mFileSource != NULL){
        property_set("tcc.video.lplayback.mode", "1");
    }
#endif
...
status_t AwesomePlayer::setDataSource_l(
    ...
    mExtractorType = extractor->getMyType();
...
void AwesomePlayer::notifyListener_l(int msg, int ext1, int ext2) {
    if ((mListener != NULL) && !mAudioTearDown) {
        ...
        #if _USE_TCC_AWESOMEPLAYER_
            // APK work-around : LETV_v1.4, POOQ
            if (msg == MEDIA_SEEK_COMPLETE 
                && (isStreamingRTSP() || isStreamingHTTP())) {
                if (mExtractor != NULL) {
                    mExtractor->cancelSeekIfNecessary(false);
                }
                listener->sendEvent(MEDIA_INFO, MEDIA_INFO_BUFFERING_END, 0);
            }
        #endif
        }
    }
...
bool AwesomePlayer::getCachedDuration_l(int64_t *durationUs, bool *eos
    ...
    if (mCachedSource != NULL && getBitrate(&bitrate) && (bitrate > 0)) {
        ...
        ssize_t cachedDataRemaining = mCachedSource->approxDataRemaining(&finalStatus);
        *durationUs = cachedDataRemaining * 8000000ll / bitrate;
    #ifdef _TCC_PROPRIETARY_DEPENDENCIES_
        if (isCacheFull && finalStatus == ERROR_BUFFER_TOO_SMALL) {
            *isCacheFull = true;
            finalStatus = OK;
        }
    #endif
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
void AwesomePlayer::onBufferingUpdateRtsp() {
    Mutex::Autolock autoLock(mLock);
    if (!mBufferingEventPending) {
        return;
    }
    ...
    CHECK(mCachedSource == NULL);
    CHECK(mWVMExtractor == NULL);
    if (!isStreamingRTSP()) {
        return;
    }
    ...
    if (getCachedDuration_l(&cachedDurationUs, &eos, &isCacheFull)) {
        if (mFlags & CACHE_UNDERRUN) {
            ALOGD("cachedDurationUs = %.2f secs, eos=%d", cachedDurationUs / 1E6, eos);
        }
        int64_t bufferEndTimeUs = 0;
        if (mCheckEOSPossibility) {
            bufferEndTimeUs = mRTSPController->getBufferEndTimeUs();
            mEOSReached = (((mDurationUs < bufferEndTimeUs) || ((mDurationUs > bufferEndTimeUs)
                        && (mDurationUs - bufferEndTimeUs < mPauseWaterMarkUs))) && mSeekable);
            if (mEOSReached) {
                ALOGD("cachedDurationUs = %.2f secs, eos = %d, bufferEndTimeUs = %.2f secs, duration = %.2f secs",
                     cachedDurationUs / 1E6, eos, bufferEndTimeUs / 1E6, mDurationUs / 1E6);
            }
        }

        if (mBufferingUpdateType == NOTIFY_ON_RUNNING && mSeekable && (mDurationUs > 0)) {
            int percentage = 100.0 * (double)(mRTSPController->getLastBufferTime()) / mDurationUs;
            if (percentage > 100) {
                percentage = 100;
            }
            ALOGV("percentage %d", percentage);

            notifyListener_l(MEDIA_BUFFERING_UPDATE, percentage);
        }

        if ((mFlags & PLAYING) && !eos
                && (cachedDurationUs < mPauseWaterMarkUs)) {
            if (mEOSReached) {
                if (cachedDurationUs == 0
                    || (mNumKeepingUnderrunNearEOS++ > (mPauseWaterMarkUs / mBufferingEventDelayUs))) {
                    ALOGW("set rtsp abort by force after %lld us",
                        mNumKeepingUnderrunNearEOS * mBufferingEventDelayUs);
                    mEOSFinalized = true;
                    if (mBufferingUpdateType == NOTIFY_ON_RUNNING && mSeekable) {
                        notifyListener_l(MEDIA_BUFFERING_UPDATE, 100);
                    }
                }
            } else {
                ALOGI("cache is running low (%.2f secs < %.2f secs) , pausing.",
                     cachedDurationUs / 1E6, mPauseWaterMarkUs / 1E6);
                modifyFlags(CACHE_UNDERRUN, SET);
                pause_l();
                ensureCacheIsFetching_l();
                sendCacheStats();
                notifyListener_l(MEDIA_INFO, MEDIA_INFO_BUFFERING_START);
                if (mBufferingUpdateType == NOTIFY_ON_UNDERRUN) {
                    ALOGD("percentage 0");
                    notifyListener_l(MEDIA_BUFFERING_UPDATE, 0);
                }
            }
        } else if (mEOSReached || eos || cachedDurationUs > mResumeWaterMarkUs) {
            if (mFlags & CACHE_UNDERRUN) {
                ALOGI("cache has filled up (%.2f secs > %.2f secs), resuming.",
                     cachedDurationUs / 1E6, mResumeWaterMarkUs / 1E6);
                if (mBufferingUpdateType == NOTIFY_ON_UNDERRUN) {
                    ALOGD("percentage 100");
                    notifyListener_l(MEDIA_BUFFERING_UPDATE, 100);
                }
                modifyFlags(CACHE_UNDERRUN, CLEAR);
                play_l();
        ...
        } else if (!(mFlags & PLAYING) && (mFlags & CACHE_UNDERRUN)) {
            if (isCacheFull) {
                ALOGI("Cache full (%.2f secs), resuming.",
                      cachedDurationUs / 1E6);
                modifyFlags(CACHE_UNDERRUN, CLEAR);
                play_l();
                mNumKeepingUnderrunNearEOS = -1;
                ...
                notifyListener_l(MEDIA_INFO, MEDIA_INFO_BUFFERING_END);
            } else if (mBufferingUpdateType == NOTIFY_ON_UNDERRUN) {
                int percentage = 100.0 * (double)cachedDurationUs / mResumeWaterMarkUs;
                if (percentage > 100) {
                    percentage = 100;
                }
                ALOGD("percentage %d", percentage);
                notifyListener_l(MEDIA_BUFFERING_UPDATE, percentage);
            }
        }
    }

    if (mFlags & (PLAYING | PREPARING | PREPARED | CACHE_UNDERRUN)) {
        postBufferingEvent_l();
    }
}

void AwesomePlayer::updateBufferingParamsFromSystemProperty() {
    char value[PROPERTY_VALUE_MAX];

    // Prefill
    property_get("tcc.http.entry.connection.msec", value, "0");
    int64_t settingValue = atoi(value);
    mEntryConnectionTimeUs = (settingValue <= 0) ? kEntryConnectionTimeUs : settingValue * 1000;

    // Prefill
    property_get("tcc.http.prefill.buffering.msec", value, "0");
    settingValue = atoi(value);
    mPrefillWaitTimeUs = (settingValue <= 0) ? kPrefillWaitTimeUs : settingValue * 1000;

    // Preroll 
    property_get("tcc.http.preroll.buffering.msec", value, "0");
    settingValue = atoi(value);
    mPrerollWaterMarkUs = (settingValue < 0) ? kPrerollWaterMarkUs : settingValue * 1000;

    // Pause
    memset(value, 0, PROPERTY_VALUE_MAX);
    property_get("tcc.http.pause.buffering.msec", value, "0");
    settingValue = atoi(value);
    mPauseWaterMarkUs = (settingValue == 0) ? kPauseWaterMarkUs : settingValue * 1000;

    // Resume - HTTP
    memset(value, 0, PROPERTY_VALUE_MAX);
    property_get("tcc.http.resume.buffering.msec", value, "0");
    settingValue = atoi(value);
    mResumeWaterMarkUs = (settingValue == 0) ? kResumeWaterMarkUs : settingValue * 1000;

    // Low caching bytes 
    memset(value, 0, PROPERTY_VALUE_MAX);
    property_get("tcc.http.low.buffering.bytes", value, "0");
	settingValue = atoi(value);
    mLowWaterMarkBytes = (settingValue == 0) ? kLowWaterMarkBytes : settingValue;

    // High caching bytes 
    memset(value, 0, PROPERTY_VALUE_MAX);
    property_get("tcc.http.high.buffering.bytes", value, "0");
	settingValue = atoi(value);
    mHighWaterMarkBytes = (settingValue == 0) ? kHighWaterMarkBytes : settingValue;

    // Etc.
    memset(value, 0, PROPERTY_VALUE_MAX);
    property_get("tcc.bufevent.delay.msec", value, "1000");
    mBufferingEventDelayUs = atoi(value) * 1000;
    ...
    memset(value, 0, PROPERTY_VALUE_MAX);
    property_get("tcc.prefill.mbyte", value, "0");
    mPrefillCacheSize = atoi(value) * 1024 * 1024;
}

void AwesomePlayer::showBufferingParams() {
    ALOGD("Buffering Params :");
    ALOGV("Prefill : %.2f sec", mPrefillWaitTimeUs / 1E6);
    ALOGD("Preroll : %.2f sec", mPrerollWaterMarkUs / 1E6);
    ALOGD("Pause   : %.2f sec", mPauseWaterMarkUs / 1E6);
    ALOGD("Resume  : %.2f sec", mResumeWaterMarkUs / 1E6);
}
#endif
...
void AwesomePlayer::notifyVideoSize_l() {
    ...
    char value[PROPERTY_VALUE_MAX];
    if (property_get("tcc.solution.mbox", value, NULL)) {
        if (!strcmp(value, "1")) {
            rotationDegrees = 0;
        }
    }
...
#if _USE_TCC_AWESOMEPLAYER_
    if (isStreamingHTTP() && !(mStreamingMode & HLS)) {
        teardown();
        return OK;
    }
...
#if _USE_TCC_AWESOMEPLAYER_
        || (mFlags & PLAYING_READY)
#endif
        || (mFlags & CACHE_UNDERRUN);
}

#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
bool AwesomePlayer::isSeekable() const {
    return mSeekable | mSeekInvokable;
}
#endif
...
#if _USE_TCC_AWESOMEPLAYER_
    if (isStreamingHTTP()) {
        // Work-around: this would guarantee the safer exchange between
        // inline(fullscreen) and fullscreen(inline) during the playback
        // (iff media extractor performs seek operation thru its own inner thread).

        //if (mExtractor != NULL && mExtractor->getMyType() == MediaExtractor::TCC)
        {
            if (native == NULL && mLastVideoTimeUs >= 0) {
                if (wasPlaying) {
                    modifyFlags(PLAYING, SET);
                }
                return OK;
...
#if _USE_TCC_AWESOMEPLAYER_
    else if (isStreamingRTSP()) {
		int64_t normalPlayTimeUs = mRTSPController->getNormalPlayTimeUs();
        *positionUs = (mSeekAsyncDonePending || normalPlayTimeUs < 0) 
                    ? mSeekTimeUs : normalPlayTimeUs;
    }
#endif
	else if (mVideoSource != NULL
...
status_t AwesomePlayer::seekTo(int64_t timeUs) {
    ...
    #if _USE_TCC_AWESOMEPLAYER_
        if (mCachedSource != NULL) {
            ALOGI("seekTo: timeUs (%.2f sec.)", timeUs/1E6);
            if (mExtractor != NULL) {
                mExtractor->cancelSeekIfNecessary(true);
            }
            mCachedSource->cancelSeekIfNecessary();
        }
    #endif
        Mutex::Autolock autoLock(mLock);
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
// static
void AwesomePlayer::OnSeekDoneWrapper(void *cookie) {
    static_cast<AwesomePlayer *>(cookie)->onSeekDone();
}

void AwesomePlayer::onSeekDone() {
    ALOGI("onSeekDone");
    mNumKeepingUnderrunNearEOS = -1;
    mSeekAsyncDonePending = false;

    mSeekedCondition.signal();
    ALOGI("onSeekDone: mSeekTimeUs %lld", mSeekTimeUs);
}

// static
void AwesomePlayer::OnSeekSuccessDoneWrapper(void *cookie) {
    static_cast<AwesomePlayer *>(cookie)->onSeekSuccessDone();
}

void AwesomePlayer::onSeekSuccessDone() {
    int64_t respondedTimeUs;
    if (isStreamingRTSP()) {
        respondedTimeUs = mRTSPController->getSeekTime();
    } else {
        respondedTimeUs = mSeekTimeUs;
    }
    ...
    mSeekedCondition.signal();
    ALOGI("OnSeekSuccessDone: mSeekTimeUs %lld, respondedTimeUs = %lld", 
            mSeekTimeUs, respondedTimeUs);
}
#endif
...
void AwesomePlayer::seekAudioIfNecessary_l() {
    if (mSeeking != NO_SEEK && mAudioPlayer != NULL) {
        if (mVideoSource == NULL) {
        ...
        if (mDecryptHandle != NULL) {
            ...
            if (mAudioPlayer != NULL && (mFlags & AUDIO_RUNNING)) {
                mAudioPlayer->pauseAfterFillBuffer();
                modifyFlags(AUDIO_RUNNING, CLEAR);
            }
            mAudioSource->pause();
        }
...
status_t AwesomePlayer::initDecoders(bool video, bool audio) {
    if (video && mVideoTrack != NULL && mVideoSource == NULL) {
        status_t err = initVideoDecoder();
    ...
    if (audio && mAudioTrack != NULL && mAudioSource == NULL) {
...
#if _USE_TCC_AWESOMEPLAYER_
    if (mStreamingMode & WFD){
        ALOGI("Awesomeplayer mStreamingMode is kSourceIsFromWFD");
        flags |= OMXCodec::kSourceIsFromWFD;
    }
#endif
...
#if _USE_TCC_AWESOMEPLAYER_
    if (mStreamingMode & WFD){
        ALOGI("Create TCCOMXCodec for WFD Sink....."); //Update for Release 13.12.11
        mVideoSource = TCCOMXCodec::Create(
...
void AwesomePlayer::postBufferingEvent_l() {
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
    if (mBufferingEvent == NULL) {
        int64_t settingValue = 0;
        char httpStreamProp[PROPERTY_VALUE_MAX];

        property_get("tcc.indicate.buff.percentage.at", httpStreamProp, "running");
        if (!strcmp(httpStreamProp, "underrun")) {
            mBufferingUpdateType = NOTIFY_ON_UNDERRUN;
            ALOGI("notify buffering percentage iff underrunned");
        }
        memset(httpStreamProp, 0, PROPERTY_VALUE_MAX);

        if (mStreamingMode & RTSP) {
            mPauseWaterMarkUs = 1000000ll;

            property_get("tcc.http.resume.rtsp.msec", httpStreamProp, "0");
            settingValue = atoi(httpStreamProp);
            mResumeWaterMarkUs = (settingValue == 0) 
                ? kRTSPResumeWaterMarkUs : settingValue * 1000;

            CHECK(mResumeWaterMarkUs >= 1000000ll); 
            mPauseWaterMarkUs = (mResumeWaterMarkUs == 1000000ll) ? 500000ll : 1000000ll;

            if (mBufferingEventDelayUs <= 1000000ll) {
                mBufferingEventDelayUs = 500000ll;
            }

            if (mCheckEOSPossibility && !mSeekable) {
                mCheckEOSPossibility = false;
            }
            mBufferingEvent = new AwesomeEvent(this,
                    &AwesomePlayer::onBufferingUpdateRtsp);
        } else if (mStreamingMode & HLS) {
            property_get("tcc.http.resume.hls.msec", httpStreamProp, "0");
            settingValue = atoi(httpStreamProp);
            mResumeWaterMarkUs = (settingValue == 0)
                ? kHLSResumeWaterMarkUs : settingValue * 1000;
            mBufferingEvent = new AwesomeEvent(this,
                    &AwesomePlayer::onBufferingUpdateHls);
        } else if ((mStreamingMode & HTTP) && !(mStreamingMode & WVM)) {
            if (mStreamingMode & DLNA) {
                property_get("tcc.http.resume.dlna.msec", httpStreamProp, "0");
                settingValue = atoi(httpStreamProp);
                mResumeWaterMarkUs = (settingValue == 0) 
                    ? kDLNAResumeWaterMarkUs : settingValue * 1000;
            }

            CHECK(mResumeWaterMarkUs > 1000000ll); 
            if (!isSeekable() && mPauseWaterMarkUs < 1000000ll) {
                ALOGW("re-adjust pause watermark %.2f sec -> 1.0 sec", mPauseWaterMarkUs / 1E6);
                mPauseWaterMarkUs = 1000000ll;
            }
            ...
            mBufferingEvent = new AwesomeEvent(this, (mExtractor->countTracks() > 1)
                    ? &AwesomePlayer::onBufferingUpdateHttp : &AwesomePlayer::onBufferingUpdate);

        } else if (mStreamingMode & WFD) {
            mBufferingEventDelayUs = 30000ll;
            mBufferingEvent = new AwesomeEvent(this,
                    &AwesomePlayer::onBufferingUpdateWFD);
        } else {
            mBufferingEvent = new AwesomeEvent(this,
                    &AwesomePlayer::onBufferingUpdate);
        }
    ...
    mQueue.postEventWithDelay(mBufferingEvent, mBufferingEventDelayUs);
}
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
status_t AwesomePlayer::finishSetDataSource() {
    status_t err = OK;
#if _USE_TCC_AWESOMEPLAYER_
    if (!strncasecmp("rtsp://", mUri.string(), 7)) {
        if(strstr(mUri.string(), "wfd1.0")) {
            modifyStreamingMode(WFD, ASSIGN);
            return finishSetWFDSource_l();
        }

        modifyStreamingMode(RTSP, ASSIGN);

        char value[PROPERTY_VALUE_MAX];
		memset(value, 0, PROPERTY_VALUE_MAX);
		property_get("tcc.media.rtsp.eos", value, "1");
		mCheckEOSPossibility = (atoi(value));

		memset(value, 0, PROPERTY_VALUE_MAX);
        property_get("tcc.media.rtsp.seekend", value, "2");
        mSeekToEndTimeMarginUs = (double)atoi(value) * 1E6;

        return finishSetRTSPSource_l();
    }

    if (!strncasecmp("http://", mUri.string(), 7)
            || !strncasecmp("https://", mUri.string(), 8)) {

        modifyStreamingMode(HTTP, SET);

        if (!strncasecmp("https://", mUri.string(), 8)) {
            modifyFlags(LEGACY_HTTPIMPL, CLEAR);
        } else {
            char value[PROPERTY_VALUE_MAX];
            property_get("tcc.media.http.mgmt", value, "mp");
            if (!strcmp(value, "mp")) {
                modifyFlags(LEGACY_HTTPIMPL, SET);
            }
		}

        if (strstr(mUri.string(), "m3u8")
                || !strncasecmp(mUri.getPathExtension(), ".m3u", 4)
                || strstr(mUri.string(), "hls")) {
            modifyStreamingMode(HLS, SET);
            err = finishSetHLSSource_l();
            if (err == INFO_URL_HPS) {
                mSeekable = false;
            } else if (err == (status_t)BAD_TYPE) {
                modifyStreamingMode(HLS, CLEAR);
            } else {
                return err;
            }
        }
...
status_t AwesomePlayer::finishSetDataSource_l() {
    ...
    if (!strncasecmp("widevine://", mUri.string(), 11)) {
        ...
        mUri = newURI;
    #if _USE_TCC_AWESOMEPLAYER_
        modifyStreamingMode(HTTP | WVM, SET);
        modifyFlags(LEGACY_HTTPIMPL, CLEAR);
    #endif
    }
    ...
    if (
            ...
            !mHasItsOwnCache && 
#endif
            (!strncasecmp("http://", mUri.string(), 7)
            || !strncasecmp("https://", mUri.string(), 8)
            || isWidevineStreaming)) {

        mConnectingDataSource = HTTPBase::Create(
#if _USE_TCC_AWESOMEPLAYER_
		((mFlags & LEGACY_HTTPIMPL)
                    ? HTTPBase::kFlagLegacy : 0) |
#endif
                ((mFlags & INCOGNITO)
                    ? HTTPBase::kFlagIncognito : 0)
#if _USE_TCC_AWESOMEPLAYER_
		, mHTTPTimeoutSecs
#endif
		);
    ...
    #if _USE_TCC_AWESOMEPLAYER_
        if (setPlayerBeforeConnection(&disconnectAtHighwatermark) != OK) {
            return UNKNOWN_ERROR;
        }
        ...
        int64_t nowUs = 0;
        int64_t timeoutUs = 0;
        int64_t profileStartTimeUs = GetNowUs();

        do {
            mLock.unlock();
            err = mConnectingDataSource->connect(mUri, &mUriHeaders);
            if (needToChangeDataSource(err)) {
			    return err;
            }
            mLock.lock();
            ...
            nowUs = GetNowUs();
            timeoutUs = profileStartTimeUs + mEntryConnectionTimeUs;
            usleep(200000);
        } while (nowUs < timeoutUs); 
    #else
    ...
    #if _USE_TCC_AWESOMEPLAYER_
        if (setPlayerAfterConnection(&disconnectAtHighwatermark) != OK) {
            return UNKNOWN_ERROR;
        }

        bool useMoreHttpSession = false;
    #endif
        ...
        #if _USE_TCC_AWESOMEPLAYER_
            err = connectAuxSessionIfRequested(useMoreHttpSession);
            if (err == (status_t)PREPARE_CANCELLED) {
                ALOGE("finishSetDataSource_l: done with stream dump and finish player ..");
                // let the player stop and do nothing.
                return UNKNOWN_ERROR;
            }

            mCachedSource = NuCacheBase::Create(
                    mUseAnotherCachedSource ? NuCacheBase::kAnotherImpl : NuCacheBase::kDefaultImpl,
                    mConnectingDataSource,
                    cacheConfig.isEmpty() ? NULL : cacheConfig.string(),
                    disconnectAtHighwatermark, 
                    useMoreHttpSession ? NuCachedSource2::EXTRA_CONNECTING : 0);
        #else
        ...
        #if _USE_TCC_AWESOMEPLAYER_
            if (!mSeekable) {
                dataSource->setFlags(DataSource::kHttpByteRangeDenied);
            }
            if (mSeekInvokable) {
               dataSource->setFlags(DataSource::kHttpTimeRangeRequestable);
            }
        #endif
        } else {
		...
		ALOGI("MIME response from server %s", contentType.string());

    #if _USE_TCC_AWESOMEPLAYER_
        if (!useMoreHttpSession)
    #endif
        mConnectingDataSource.clear();
        ...
        if (strncasecmp(contentType.string(), "audio/", 6)) {
            ...
            if (mCachedSource != NULL) {
            ...
            #if (_USE_TCC_AWESOMEPLAYER_ == 0)
                static const
            #endif
                size_t kMinBytesForSniffing = 192 * 1024;

            #if _USE_TCC_AWESOMEPLAYER_
                profileStartTimeUs = GetNowUs();
            #endif
                off64_t metaDataSize =
                            #if _USE_TCC_AWESOMEPLAYER_
                                mUseAnotherCachedSource ? kMinBytesForSniffing :
                            #endif
                                -1ll;
                ...
                for (;;) {
                ...
                #if _USE_TCC_AWESOMEPLAYER_
                    off64_t lastBytePos = -1;
                    if (mProfilingMode) {
                        cachedDataRemaining =
                            mCachedSource->approxDataRemaining(&finalStatus, &lastBytePos);
					} else
                #endif
                    cachedDataRemaining =
                ...
                #if _USE_TCC_AWESOMEPLAYER_
                    if (finalStatus != OK || (mFlags & PREPARE_CANCELLED)) {
						break;
					}

                    if (useMoreHttpSession) {
                        // we'll wait more until aux. stream is fulfilled.
                        err = disconnectAuxSession(&useMoreHttpSession, cachedDataRemaining);
                        if (err != OK) {
                            mConnectingDataSource.clear();
                            return err;
                        }
                        
                        // the server allowed extra connection to connect with byte-ranged offset 
                        // but we double-check if range request turns to be disabled or not.
                        if (mSeekable == false) {
                            ALOGW("http byte range request is denied");
                            dataSource->setFlags(DataSource::kHttpByteRangeDenied);
                        }
                    } else if (metaDataSize >= 0 && cachedDataRemaining >= metaDataSize) {
                        break;
                    }
                #else
                    if (finalStatus != OK
                    ...
                    ALOGD("now cached %d bytes of data, metaDataSize %lld", cachedDataRemaining, metaDataSize);
                    ...
                    if (metaDataSize < 0
                        ...
                        if (meta == NULL
                            ...
                            metaDataSize = 
                            #if _USE_TCC_AWESOMEPLAYER_
                                (!mSeekable || !mSeekInvokable) ? kLowWaterMarkBytes :
                            #endif
                                kHighWaterMarkBytes;
                        }

                    #if _USE_TCC_AWESOMEPLAYER_
                        if (mPrefillCacheSize > 0) {
                            metaDataSize = mPrefillCacheSize;
                        }
                    #endif

                        CHECK_GE(metaDataSize, 0ll);
                        ALOGI("metaDataSize = %lld bytes", metaDataSize);
                    }

                #if _USE_TCC_AWESOMEPLAYER_
                    nowUs = GetNowUs();
                    timeoutUs = profileStartTimeUs + mPrefillWaitTimeUs;
                    if (nowUs > timeoutUs && mPrefillCacheSize == 0) {
                        ALOGE("Timeout to prefill the cache");
                        teardown();
                        modifyFlags(PREPARE_CANCELLED, SET);
                        break;
                    }
                    ...
                    if (mProfilingMode) {
                       ALOGD("[PROFILE] cache w offset %10lld, r offset %10lld, avail %10d",
                           lastBytePos, lastBytePos - cachedDataRemaining, cachedDataRemaining);

                       usleep(mBufferingEventDelayUs);
                    } else
                #endif 
                    usleep(200000);
                }
    ...
    #if _USE_TCC_AWESOMEPLAYER_
        else if (useMoreHttpSession) {
            mConnectingDataSource.clear();
        }

        changeMIMEIfNecessary(sniffedMIME, contentType);
    #endif
    } else {
    ...
    #if _USE_TCC_AWESOMEPLAYER_
        dataSource->setExternalSourceSpecificInfos(mSourceSpecificInfos);
    #endif
        extractor = MediaExtractor::Create(
                dataSource, sniffedMIME.empty() ? NULL : sniffedMIME.c_str()
            #if _USE_TCC_AWESOMEPLAYER_
                , false, dataSource->getMIMEType().string()
            #endif
                );

        if (extractor == NULL) {
            ALOGE("nullified extractor");
            return UNKNOWN_ERROR;
        }
    ...
    if (err == ERROR_UNSUPPORTED) {
        err = changeDataSourceIfNecessary(dataSource, extractor, sniffedMIME.c_str());
    }
#endif
    ...
    if (err != OK) {
        ...
        ALOGE("setDataSource_l returns error %d", err);
        return err;
    }
...
void AwesomePlayer::abortPrepare(status_t err) {
	ALOGE("abortPrepare: err = %08x", err);
    CHECK(err != OK);
...
void AwesomePlayer::beginPrepareAsync_l() {
...
#if _USE_TCC_AWESOMEPLAYER_
    if (mStreamingMode & HLS) {
        char value[PROPERTY_VALUE_MAX];
        property_get("tcc.hls.prepare.opt", value, "0");
        if (atoi(value) != 0) {
            finishAsyncPrepare_l();
            modifyFlags(PLAYING_READY, SET);
            postBufferingEvent_l();
            return;
        }
    }
#endif
    status_t err = initDecoders();
    if (err != OK) {
        abortPrepare(err);
        return;
    }
    ...
    if (isStreamingHTTP()
    #if _USE_TCC_AWESOMEPLAYER_
     || isStreamingRTSP() || isStreamingWFD()
    #endif
	) {
        postBufferingEvent_l();
    } else {
...
status_t AwesomePlayer::getParameter(int key, Parcel *reply) {
    ...
    case KEY_PARAMETER_AUDIO_CHANNEL_COUNT:
        ...
        return OK;
    #ifdef _TCC_PROPRIETARY_DEPENDENCIES_
    case KEY_PARAMETER_TOTAL_AUDIO_TRACK_COUNT: // TCCVideoPlayer
        {
            reply->writeInt32(1);
        }
        return OK;
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
bool AwesomePlayer::isStreamingRTSP() const {
    return mRTSPController != NULL;
}
#endif
...
bool AwesomePlayer::isStreamingHTTP() const {
    ...
    #ifdef _TCC_PROPRIETARY_DEPENDENCIES_
        || (mCachedSource == NULL && mConnectingDataSource != NULL)
    #endif
        || mWVMExtractor != NULL;
}
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
status_t AwesomePlayer::setVideoSynchronousMode(int32_t mode) {
	/*
    if (mNativeWindow != NULL) {
		ALOGV("setVideoSynchronousMode %d", mode);
        status_t err = native_window_set_synchronous_mode(
                mNativeWindow.get(), mode);
        if (err != OK) {
            ALOGW("Failed to set sync mode: %d", err);
        }
    }
    */
    return OK;
...
void AwesomePlayer::modifyStreamingMode(unsigned value, FlagMode mode) {
    switch (mode) {
        case SET:
            mStreamingMode |= value;
            break;
        case CLEAR:
            mStreamingMode &= ~value;
            break;
