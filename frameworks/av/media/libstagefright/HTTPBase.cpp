...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
        , int32_t connTimeoutSecs, int32_t recvTimeoutSecs
#endif
) {
	ALOGV("Create: flags(%d)", flags);
#if SF_HTTP_AVAILABLE
    if (flags & kFlagLegacy) {
        return new NuHTTPDataSource(
                        connTimeoutSecs, recvTimeoutSecs < 0 ? connTimeoutSecs : recvTimeoutSecs, 
                        (flags & kFlagFakeUserAgent));
	}
#endif
#if CHROMIUM_AVAILABLE
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
status_t HTTPBase::connectMore(uint32_t type) {
	return INVALID_OPERATION;
}
#endif
