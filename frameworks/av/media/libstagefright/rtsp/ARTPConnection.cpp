...
ARTPConnection::ARTPConnection(uint32_t flags)
    : mFlags(flags),
      ...
      mLastReceiverReportTimeUs(-1) {
        ...
        mSubSource = new TCCARTPConnection(this);
        mDelegateClass = new TCCARTPConnectionDelegate<TCCARTPConnection>();
#else
        mSubSource = this;
        mDelegateClass = new TCCARTPConnectionDelegate<ARTPConnection>();
#endif
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
        err = mDelegateClass->parseRTP((void *)s, buffer);
#else
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
status_t ARTPConnection::parseRTP( void*_s, const sp<ABuffer> &buffer)
#else
status_t ARTPConnection::parseRTP(StreamInfo* s, const sp<ABuffer> &buffer)
#endif
{
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
    StreamInfo *s = (StreamInfo *) _s;
#endif
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
    sp<ARTPSource> source = findSource((void*)s, srcId);
#else
...
status_t ARTPConnection::parseRTCP(StreamInfo *s, const sp<ABuffer> &buffer) {
    ...
    while (size > 0) {
        ...
        switch (data[1]) {
            ...
            {
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
                mDelegateClass->parseSR((void *)s, data, headerLength);
#else
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
    sp<ARTPSource> source = findSource((void*)s, id);
#else
...
status_t ARTPConnection::parseSR(
          void *_s, const uint8_t *data, size_t size)
#else
status_t ARTPConnection::parseSR(
        StreamInfo* s, const uint8_t *data, size_t size)
#endif
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
    sp<ARTPSource> ARTPConnection::findSource(void *_info, uint32_t srcId)
#else
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
    StreamInfo* info = (StreamInfo *)_info;
#endif
...
void ARTPConnection::onTimeReset() {
    mDelegateClass->onTimeReset();
}

void ARTPConnection::onRTPBufferClear() {
    mDelegateClass->onRTPBufferClear();
}
}  // namespace android
