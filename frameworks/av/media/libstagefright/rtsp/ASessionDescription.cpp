...
ASessionDescription::ASessionDescription()
    ...
    mSubSource = new TCCASessionDescription(this);
    mDelegateClass = new TCCASessionDescriptionDelegate<TCCASessionDescription>();
#else
    mSubSource = this;
    mDelegateClass = new TCCASessionDescriptionDelegate<ASessionDescription>();
#endif
    mDelegateClass->setSubClass(mSubSource);
#endif
}
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
bool ASessionDescription::setToD(const void *data, size_t size) {
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
    return mDelegateClass->setTo(data, size);
#else
    mIsValid = parse(data, size);
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
bool ASessionDescription::parseD(const void *data, size_t size) {
    mTracks.clear();
    mFormats.clear();
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
    return mDelegateClass->parse(data, size);
#else
    mTracks.clear();
    mFormats.clear();
...
bool ASessionDescription::getDurationUs(int64_t *durationUs) const {
    *durationUs = 0;
    ...
    float from, to;
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
    if (!mDelegateClass->parseNTPRange(value.c_str() + 4, &from, &to)) {
        return false;
    }
#else
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
// static
bool ASessionDescription::parseNTPRangeD(
        const char *s, float *npt1, float *npt2) {
    return mDelegateClass->parseNTPRange(s, npt1, npt2);
}
#endif
...
#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
void ASessionDescription::getNTPRangeD(float *npt1, float *npt2) {
    mDelegateClass->getNTPRange(npt1, npt2);
}

bool ASessionDescription::getFrameRateD(uint32_t *framerate) {
    return mDelegateClass->getFrameRate(framerate);
}

void ASessionDescription::getNTPRange(float *npt1, float *npt2) {
    npt1 = 0;
    npt2 = 0;
}

bool ASessionDescription::getFrameRate(uint32_t *framerate){
    *framerate = 0;
    return false;
}
#endif
