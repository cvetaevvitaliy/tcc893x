...
ARTPSource::ARTPSource(
    ...
    if (!strncmp(desc.c_str(), "H264/", 5)) {
    ...
    #ifdef _TCC_PROPRIETARY_DEPENDENCIES_
        ALOGV("Constructor: AAVCAssembler - id(%d)", id);
        mPacketType = VIDEO_PACKET;
    #endif
    } else if (!strncmp(desc.c_str(), "MP4A-LATM/", 10)) {
        mAssembler = new AMPEG4AudioAssembler(notify, params);
    #ifdef _TCC_PROPRIETARY_DEPENDENCIES_
        ALOGV("Constructor: AMPEG4AudioAssembler - id(%d)", id);
        mPacketType = AUDIO_PACKET;
    #endif
        ...
        mIssueFIRRequests = true;
    #ifdef _TCC_PROPRIETARY_DEPENDENCIES_
        ALOGV("Constructor: AMPEG4ElementaryAssembler - id(%d)", id);
        if(!strncmp(desc.c_str(), "MP4V-ES/", 8) )
            mPacketType = VIDEO_PACKET;
        else
            mPacketType = AUDIO_PACKET;
    #endif
