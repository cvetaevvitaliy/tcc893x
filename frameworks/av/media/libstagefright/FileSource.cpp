...
status_t FileSource::getFilePath(char* pszPath, int ilengthMax) {
    if (mFd < 0) {
                return NO_INIT;
    }
        ...
        sprintf(fdstr, "/proc/self/fd/%d", mFd);
        length = readlink(fdstr, pszPath, ilengthMax);
        if( length < 0 ) {
                ALOGE("link read failed (FD: %s)", fdstr);
                return UNKNOWN_ERROR;
        }
        pszPath[length] = NULL;
...
String8 FileSource::getFilePath() {
    if (mFd < 0) {
        return String8("");
    }

        const int FILESRC_PATH_MAX = 1024;
    int length;
    char fdstr[128];
    char pszPath[FILESRC_PATH_MAX];

    sprintf(fdstr, "/proc/self/fd/%d", mFd);
    length = readlink(fdstr, pszPath, FILESRC_PATH_MAX);
    if (length < 0) {
        ALOGE("link read failed (FD: %s)", fdstr);
        return String8("");
    }
