...
enum {
    ...
    SET_EFFECT_ENABLED,
    SET_FORCE_SAMPLING_RATE,		// Planet 20130205 setSamplingRate
...
public:
	...
	virtual status_t setForceSamplingRate(uint32_t rate)
    {
        Parcel data, reply;
        data.writeInterfaceToken(IAudioPolicyService::getInterfaceDescriptor());
        data.writeInt32(rate);
        remote()->transact(SET_FORCE_SAMPLING_RATE, data, &reply);
        return static_cast <status_t> (reply.readInt32());
    }
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch (code) {
        ...
        // Planet 20130205 setSamplingRate Start
		case SET_FORCE_SAMPLING_RATE: {
            CHECK_INTERFACE(IAudioPolicyService, data, reply);
            uint32_t rate = (uint32_t)data.readInt32();
            reply->writeInt32(static_cast <uint32_t>(setForceSamplingRate(rate)));
            return NO_ERROR;
        } break;
		// Planet 20130205 setSamplingRate End
