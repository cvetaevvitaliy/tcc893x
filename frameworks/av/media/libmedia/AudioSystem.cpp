...
status_t AudioSystem::setVoipMode(int mode)
{
    if (mode > 1) return BAD_VALUE;
    const sp<IAudioFlinger>& af = AudioSystem::get_audio_flinger();
    if (af == 0) 
    {
        ALOGE("setVoipMode error: PERMISSION_DENIED");
        return PERMISSION_DENIED;
    }
    return af->setVoipMode(mode);
}

status_t AudioSystem::setVoipPid(int mode, int pid)
{
    if (mode > 1) return BAD_VALUE;
    const sp<IAudioFlinger>& af = AudioSystem::get_audio_flinger();
    if (af == 0) return PERMISSION_DENIED;
    return af->setVoipPid(mode, pid);
}
...
status_t AudioSystem::setForceSamplingRate(uint32_t rate)
{
    const sp<IAudioPolicyService>& aps = AudioSystem::get_audio_policy_service();
    ALOGV("AudioPolicyService %s", aps == 0 ? "PERMISSION_DENIED":"SUCCESS");
    if (aps == 0) return PERMISSION_DENIED;
    return aps->setForceSamplingRate(rate);
}
