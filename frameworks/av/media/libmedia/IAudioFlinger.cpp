...
enum {
    ...
    SET_VOIP_MODE,      // VoipAudio Planet 20121212
    SET_VOIP_PID,       // VoipAudio Planet 20121212
    SET_LOW_RAM_DEVICE,
};
...
public:
    ...
    virtual status_t setVoipMode(int mode)
    {
        Parcel data, reply;
        data.writeInterfaceToken(IAudioFlinger::getInterfaceDescriptor());
        data.writeInt32(mode);
        remote()->transact(SET_VOIP_MODE, data, &reply);
        return reply.readInt32();
    }

    virtual status_t setVoipPid(int mode, int pid)
    {
        Parcel data, reply;
        data.writeInterfaceToken(IAudioFlinger::getInterfaceDescriptor());
        data.writeInt32(mode);
        data.writeInt32(pid);
        remote()->transact(SET_VOIP_PID, data, &reply);
        return reply.readInt32();
    }
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch (code) {
        ...
        case SET_VOIP_MODE: {
            ...
            reply->writeInt32(setVoipMode(mode));
            return NO_ERROR;
        } break;
        case SET_VOIP_PID: {
            CHECK_INTERFACE(IAudioFlinger, data, reply);
            ...
            reply->writeInt32(setVoipPid(mode, pid));
            return NO_ERROR;
        } break;
        // VoipAudio Planet 20121212 End
