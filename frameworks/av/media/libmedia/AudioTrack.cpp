...
uint32_t AudioTrack::latency_ex()
{
    int emptied = mFrameCount - mProxy->getFramesFilled();

    //ALOGI("%s: emptied = %d", __func__, emptied);

    if (mSampleRate == 0)
        return mLatency;
    return mLatency - ((1000*emptied) / mSampleRate);
}
