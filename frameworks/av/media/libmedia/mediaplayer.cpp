...
status_t MediaPlayer::setVsync(bool vsync)
{
    ALOGV("MediaPlayer::setVsync(%d)", vsync);
    mVsync = vsync;
    return NO_ERROR;
}
...
status_t MediaPlayer::setParameter(int key, const Parcel& request)
{
    ...
    ALOGD("MediaPlayer::setParameter(%d)", key);
    Mutex::Autolock _l(mLock);
    if (mPlayer != NULL) {
...
void MediaPlayer::notify(int msg, int ext1, int ext2, const Parcel *obj)
{
    ...
    case MEDIA_INFO:
        ...
        if (ext1 == MEDIA_INFO_VIDEO_SEEK_LAGGING) {
            mNoQueuedSeek = true;
        } else if (ext1 == MEDIA_INFO_BUFFERING_START) {
            mAutoPaused = true;
