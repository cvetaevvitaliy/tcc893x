...
#define ENABLE_WIFI_DISPLAY
#ifdef ENABLE_WIFI_DISPLAY
...
#ifdef ENABLE_WIFI_DISPLAY
static sp<InterfaceClient> gClient = NULL;
bool gWifiDisplay = false;
//bool gStreamState = false;
static unsigned int currtime = 0;
static unsigned int lasttime = 0;

static unsigned int TCC_OS_GetTickCount(void)
{
...
#define mBitShift 2 // FIXME
		...
		if(gClient == NULL)
		{
			char wifi_display[PROPERTY_VALUE_MAX];

			property_get("tcc.wifi.display", wifi_display, "0");
			if(!strcmp(wifi_display,"1") )
			{
			    gWifiDisplay = true;
			}
			else if(gWifiDisplay == true)
			{
			    gWifiDisplay = false;
			}

			if(gWifiDisplay == true)
			{
			    if (gClient == NULL) {
                    gClient = new InterfaceClient();
			    }
			}
		}

        if ( (gClient != NULL) && (gWifiDisplay == true) ) {
            char wifi_display[PROPERTY_VALUE_MAX];
            property_get("tcc.wifi.display.connect", wifi_display, "0");
            if(!strcmp(wifi_display,"0") )
            {
                gWifiDisplay = false;
                gClient.clear();
                gClient = NULL;
            ...
            {
                currtime = TCC_OS_GetTickCount();
                lasttime = currtime;
                if (count << mBitShift != 0) {
                    gClient->WriteAudioFrame((unsigned char *)mMixBuffer,
                                                        count << mBitShift,
                                                        currtime, NULL);
                }
            }
...
bool AudioFlinger::PlaybackThread::threadLoop()
{
    ...
    nsecs_t checkTime;  // VoipAudio Planet 20121212
    ...
    while (!exitPending())
    {
        ...
        if(mAudioFlinger->mVoipMode)
        {
            nsecs_t temp_time;
            int ret;
            int i;

            temp_time = systemTime();
            if(temp_time > checkTime)
            {
                checkTime = temp_time+kStandbyTimeVoipInNsecs;
                for(i=0; i<PID_LIST_MAX; i++)
                {
                    if(mAudioFlinger->mVoipPidList[i])
                    {
                        char statline[128];

                        sprintf(statline, "/proc/%d/cmdline", mAudioFlinger->mVoipPidList[i]);
                        ret = open(statline, O_RDONLY);

                        //ret = kill(mAudioFlinger->mVoipPidList[i], 0);

                        //ALOGV("check pid %d , ret : %d", mAudioFlinger->mVoipPidList[i], ret);
                        if(ret < 0)
                            mAudioFlinger->mVoipPidList[i] = 0;
                        else
                        {
                            close(ret);
                            break;
                ...
                if(i== PID_LIST_MAX)
                {
                    ALOGV("setVoipMode changed to 0 because voipHardware is not working");                    
                    mAudioFlinger->setVoipMode(0);
                }
            }                
        }
...
bool AudioFlinger::RecordThread::threadLoop()
{
    ...
    nsecs_t lastWarning = 0;
    nsecs_t checkTime = 0;  // VoipAudio Planet 20121212
            ...
            if(temp_time > checkTime)
            {
                ...
                for(i=0; i<PID_LIST_MAX; i++)
                {
                    if(mAudioFlinger->mVoipPidList[i])
                    {
                        ...
                        ALOGV("check2 pid %d, ret : %d", mAudioFlinger->mVoipPidList[i], ret);
                        if(ret < 0)
                            mAudioFlinger->mVoipPidList[i] = 0;
                        else
                        {
