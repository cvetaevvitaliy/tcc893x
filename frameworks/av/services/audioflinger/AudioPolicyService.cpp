...
status_t AudioPolicyService::setForceSamplingRate(uint32_t rate)
{
	if (mpAudioPolicy == NULL) {
        return NO_INIT;
    }
	
	//LOGE("~~~~~~!!!!! AudioPolicyService.cpp setForceSamplingRate() : %d !!!!!~~~~~~", rate);
    return mpAudioPolicy->set_force_sampling_rate(mpAudioPolicy, rate);
}
