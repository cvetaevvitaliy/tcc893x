...
#include <hardware_legacy/VoipHardwareInterface.h>  // VoipAudio Planet 20121212
...
AudioFlinger::AudioFlinger()
    : BnAudioFlinger(),
      ...
      mVoipMode(0),         // VoipAudio Planet 20121212
      mAudioStreamOut(0),   // VoipAudio Planet 20121212
      mAudioStreamIn(0)     // VoipAudio Planet 20121212
{
...
status_t AudioFlinger::setVoipMode(int mode)
{
    int ret;
    
    ALOGV("setVoipMode(%d)", mode);
#if 0
    if(mode == mVoipMode)
    {
        return 0;
    ...
    if(mAudioStreamIn)
    {
        if(mAudioStreamIn->stream->common.set_voip_mode)
        {
            ret = mAudioStreamIn->stream->common.set_voip_mode(&mAudioStreamIn->stream->common, mode);
            if(ret < 0)
            {
                ALOGE("error : mAudioStreamIn->setVoipMode(%d)", mode);
                return -1;
            }
        }
    }
    ...
    {
        ALOGE("mAudioStreamIn is NULL");
        
    }

    if(mAudioStreamOut)
    {
        if(mAudioStreamOut->stream->common.set_voip_mode)
        {
            ret = mAudioStreamOut->stream->common.set_voip_mode(&mAudioStreamOut->stream->common, mode);
            if(ret < 0)
            {
                 ALOGE("error : mAudioStreamOut->setVoipMode(%d)", mode);
                 if(mAudioStreamIn)
                     mAudioStreamIn->stream->common.set_voip_mode(&mAudioStreamOut->stream->common, mVoipMode);
                 return -1;
            }
        }
    }
    ...
    else
    {
        ...
        mVoipMode = 0;        
        memset(mVoipPidList, 0, sizeof(mVoipPidList));
    }
...
status_t AudioFlinger::setVoipPid(int mode, int pid)
{
    int ret;
    int i;
    char statline[128];

    ALOGV("setVoipPid(%d, %d) " , mode, pid);

    if(mode)
    {
        for(i=0; i<PID_LIST_MAX; i++)
        {
            if(mVoipPidList[i] == 0)
            {
                mVoipPidList[i] = pid;
        ...
        for(i=0; i<PID_LIST_MAX; i++)
        {
            sprintf(statline, "/proc/%d/cmdline", mVoipPidList[i]);
            ret = open("/proc/%d/cmdline", O_RDONLY);
        ...
        ALOGE("Cannot register pid for Voip");
        return -1;
    }
    else
    {
        ...
        ALOGE("Cannot unregister pid for Voip because pid(%d) is not alive", pid);
        return -1;
    }
                                           ...
                                           const audio_offload_info_t *offloadInfo)
{
    ...
    if (status == NO_ERROR && outStream != NULL) {
        ...
        mAudioStreamOut = output;   // VoipAudio Planet 20121212
        return id;
    }
...
status_t AudioFlinger::closeOutput_nonvirtual(audio_io_handle_t output)
{
    ...
    mAudioStreamOut = NULL;     // VoipAudio Planet 20121212
    return NO_ERROR;
}
                                          ...
                                          audio_channel_mask_t *pChannelMask)
{
    ...
    if (status == NO_ERROR && inStream != NULL) {
        ...
        mAudioStreamIn = input; // VoipAudio Planet 20121212
        return id;
    }
...
status_t AudioFlinger::closeInput_nonvirtual(audio_io_handle_t input)
{
    ...
    mAudioStreamIn = NULL;  // VoipAudio Planet 20121212
    return NO_ERROR;
}
