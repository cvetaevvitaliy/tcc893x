...
//In ICS, minimum 2 video buffers are reqired.
//in gingerbread, however, just 1 video buffer is reqired for minimum.
//#define OUTPUT_BUFFER_MAX_FOR_SKIMMING	1
#define OUTPUT_BUFFER_MAX_FOR_SKIMMING	2

#define S_BUFFER_DELAY // Telechips, ZzaU :: for inline view of web-player.
#ifdef S_BUFFER_DELAY
#define DELAY_CNT 3 // Caution: MAX Count is 3.
#endif
...
struct OMXCodec : public MediaSource,
    ...
    enum CreationFlags {
        ...
        // Ignore buffer filled timeout
        kIgnoreBufferFilledTimeOut = 512,

        // Input is from ..
        kSourceIsFromHttp = 1024,
        kSourceIsFromRtsp = 2048,
        kSourceIsFromWFD  = 4096,
...
//private: // modified by TELECHIPS, ZzaU for V2IP

    // Make sure mLock is accessible to OMXCodecObserver
    friend class OMXCodecObserver;

    // Call this with mLock hold
    virtual void on_message(const omx_message &msg); // modified by TELECHIPS, ZzaU for V2IP
    ...
    int32_t mPlayMode;
    bool mIsReadDone;
    bool mIsSkimmingDone;

    // TELECHIPS, streaming case
	bool mIsSourceUnderrun;
	bool mIsSyncFrameDelayed;

	bool mIsWaitSeekRequest; // TELECHIPS, AVG

    MediaBuffer *mLeftOverBuffer;

    Mutex mLock;
    Condition mAsyncCompletion;
    int mIsLock; //TELECHIPS, ZzaU for V2IP

    bool mPaused;

    int32_t mExtractorType;
    bool mWFDModeEnable;
    ...
    // TELECHIPS, SSG
    status_t setMP3Format(int32_t numChannels, int32_t sampleRate, int32_t bitRate);
    status_t setTCCAACFormat(int32_t numChannels, int32_t sampleRate);
    status_t setVORBISFormat(int32_t numChannels, int32_t sampleRate);
    status_t setFLACFormat(int32_t numChannels, int32_t sampleRate);
    status_t setAPEFormat(int32_t numChannels, int32_t sampleRate);
    status_t setWMAFormat(int32_t numChannels, int32_t sampleRate);
    status_t setAC3Format(int32_t numChannels, int32_t sampleRate);
    status_t setDTSFormat(int32_t numChannels, int32_t sampleRate);
    status_t setMP2Format(int32_t numChannels, int32_t sampleRate);
    status_t setRAFormat(int32_t numChannels, int32_t sampleRate);
    status_t setPCMFormat(int32_t numChannels, int32_t sampleRate);
    ...
    virtual status_t waitForBufferFilled_l(); // modified by TELECHIPS, ZzaU for V2IP
