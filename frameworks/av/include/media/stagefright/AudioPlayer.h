...
public:
	...
	virtual void signalToIgnoreBufferFilled() { return; } // TELECHIPS
	virtual void teardown() { return; } // TELECHIPS

    virtual status_t start(bool sourceAlreadyStarted = false);

    virtual void pause(bool playPendingSamples = false);
	void pauseAfterFillBuffer(bool playPendingSamples = false); // TELECHIPS, HEES
    ...
    virtual status_t seekTo(int64_t time_us);	// modified by TELECHIPS, AVG
...
protected: // TELECHIPS, SSG
    ...
    virtual size_t fillBuffer(void *data, size_t size); // TELECHIPS, SSG
	...
	int mSpdifFlag;	// AudioOutput update Planet 20121120

#ifdef _TCC_PROPRIETARY_DEPENDENCIES_
	int mWFDSinkPlayMode;
	VoipHardwareInterface *mPlaybackHandle;
#endif
