...
status_t SensorEventQueue::setCalibration(Sensor const* sensor,int32_t sensors, float x, float y, float z) const {
    return mSensorEventConnection->setCalibration(sensor->getHandle(),sensors,x,y,z);
}

status_t SensorEventQueue::startCalibration(Sensor const* sensor,int32_t sensors) const {
    return mSensorEventConnection->startCalibration(sensor->getHandle(),sensors);
}
