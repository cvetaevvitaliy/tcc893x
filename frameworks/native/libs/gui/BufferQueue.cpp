    ...
    { // scope for the lock
        ...
        if (mQueue.empty()) {
            ...
            if (front->mIsDroppable) {
                ...
                ALOGE("Frame is dropped because mQueue is full. mSlots[%d] = TS %d ms", buf, (unsigned int)(timestamp/1000000));
		...
		mCurrentQueuedTextureBuf = mSlots[buf].mGraphicBuffer;
        #endif
...
#ifdef TCC_SKIP_UI_UPDATE_DURING_VIDEO
sp<GraphicBuffer> BufferQueue::getCurrentQueuedBuffer() const {
    Mutex::Autolock lock(mMutex);
	
    return mCurrentQueuedTextureBuf;
}
#endif
