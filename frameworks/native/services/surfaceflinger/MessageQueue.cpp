...
void MessageQueue::Handler::signalVideo() {
    if ((android_atomic_or(eventMaskVideo, &mEventMask) & eventMaskVideo) == 0) {
        mQueue.mLooper->sendMessage(this, Message(MessageQueue::VIDEO));
    }
}

void MessageQueue::Handler::signalUI() {
    if ((android_atomic_or(eventMaskUI, &mEventMask) & eventMaskUI) == 0) {
        mQueue.mLooper->sendMessage(this, Message(MessageQueue::UI));
    }
}
#endif
...
void MessageQueue::Handler::handleMessage(const Message& message) {
    switch (message.what) {
		...
		#ifdef TCC_SKIP_UI_UPDATE_DURING_VIDEO
		case VIDEO:
			android_atomic_and(~eventMaskVideo, &mEventMask);
			mQueue.mFlinger->onMessageReceived(message.what);
			break;
		case UI:
			android_atomic_and(~eventMaskUI, &mEventMask);
			mQueue.mFlinger->onMessageReceived(message.what);
			break;
		#endif
        case TRANSACTION:
...
#ifdef TCC_SKIP_UI_UPDATE_DURING_VIDEO
void MessageQueue::refreshVideo() {
    //mEvents->requestNextVsync();
    mHandler->signalVideo();
}

void MessageQueue::refreshUI() {
    //mEvents->requestNextVsync();
    mHandler->signalUI();
}
#endif
