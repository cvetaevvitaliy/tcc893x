...
#if defined(HAVE_HDMI_OUTPUT) || defined(HAVE_TV_ANALOG_OUTPUT)
extern int extend_display_init();
extern int extend_display_deinit();
#endif//
...
void SurfaceFlinger::bootFinished()
{
    ...
    bool roQBEnable = false;
    bool bBootWithQuickboot = false;
    bool bPrebultLauncher = false;
	...
	if (property_get("ro.QB.enable", property, NULL) > 0) {
		if (strcmp(property, "true") || strcmp(property, "1")) {
			roQBEnable = true;
		}

		if (roQBEnable == true) {
			if (property_get("tcc.QB.boot.with", property, NULL) > 0) {
				if (strcmp(property, "snasphot")
						|| strcmp(property, "hibernate")) {
					bBootWithQuickboot = true;
				}
			}

			if (property_get("ro.QB.builtin.launcher", property, NULL) > 0) {
				if (strcmp(property, "true") || strcmp(property, "1")) {
					bPrebultLauncher = true;
				}
			}
		}
	}

	if (roQBEnable && bPrebultLauncher) {
		property_set("tcc.bootanim.exit", "1");

		if (bBootWithQuickboot) {
...
#ifdef TCC_SKIP_UI_UPDATE_DURING_VIDEO
void SurfaceFlinger::signalVideoLayerUpdate() {
    mEventQueue.refreshVideo();
}

void SurfaceFlinger::signalUILayerUpdate() {
    mEventQueue.refreshUI();
}

void SurfaceFlinger::handleMessageRefresh(int isVideo) {
    ATRACE_CALL();
    ...
    setUpHWComposer();
    doDebugFlashRegions();
    doComposition(isVideo);
    postComposition();
}

void SurfaceFlinger::doComposition(int isVideo) {
    ATRACE_CALL();
    ...
    for (size_t dpy=0 ; dpy<mDisplays.size() ; dpy++) {
        ...
        if (hw->canDraw()) {
            ...
            doDisplayComposition(hw, dirtyRegion,isVideo);
...
void SurfaceFlinger::doDisplayComposition(const sp<const DisplayDevice>& hw, const Region& inDirtyRegion,int isVideo)
{
    ...
    if (CC_LIKELY(!mDaltonize)) {
		if(!isVideo) doComposeSurfaces(hw, dirtyRegion);
    } else {
...
void SurfaceFlinger::onMessageReceived(int32_t what) {
    ...
    #ifdef TCC_SKIP_UI_UPDATE_DURING_VIDEO
    case MessageQueue::VIDEO:
    case MessageQueue::REFRESH:
	    
		HWComposer& hwc(getHwComposer());
		if(what==MessageQueue::VIDEO){
			mVideoUpdate =1;
			hwc.setVideoFlag(1);
		}
		else{
			hwc.setInvalidateFlag(1);
			mVideoUpdate =0;
		}

		if(hwc.getRoutineForVideoFlag())
        	handleMessageRefresh(mVideoUpdate);
        else
...
void SurfaceFlinger::doComposeSurfaces(const sp<const DisplayDevice>& hw, const Region& dirty)
{
	...
	// checking the navigation bar state
	char value[PROPERTY_VALUE_MAX];
	property_get("qemu.hw.mainkeys", value, "0");
	uint32_t HWC_FRAMEBUFFER_CNT = 0;
	uint32_t HWC_FRAMEBBUFFER_CNT_CHECK;
	HWC_FRAMEBBUFFER_CNT_CHECK = atoi(value);
	...
	// if there is no HWC_FRAMEBUFFER, clear UI when video play in hdmi 
	
	 if(HWC_FRAMEBBUFFER_CNT_CHECK){
			for (size_t i=0 ; i<count ; ++i) {				
				const sp<Layer>& layer(layers[i]);
				const Region clip(dirty.intersect(tr.transform(layer->visibleRegion)));
				
				if((HWC_FRAMEBBUFFER_CNT_CHECK != 0) && (hw->getDisplayType() == DisplayDevice::DISPLAY_PRIMARY))
				{
					if(HWC_FRAMEBUFFER_CNT == 0)
					{
						layer->draw(hw, clip);
						layer->clearWithOpenGL(hw,clip);
						//ALOGE("@@@ HWC_DISABLE HWC_FRAMEBUFFER_CNT=%d",HWC_FRAMEBUFFER_CNT);
					}
				}		
			}
		}
