...
//Telechips, ZzaU
status_t HWComposer::updateFramebufferTarget(int32_t id, const sp<Fence>& acquireFence) {
    DisplayData& disp(mDisplayData[id]);
    if (!disp.framebufferTarget) {
        ALOGE("HWComposer: framebufferTarget is null");
        return NO_ERROR;
    ...
    if (id == HWC_DISPLAY_PRIMARY){
        //ALOGV(int32_t id)("HWComposer: framebufferTarget for primary");
        mHwc->fbTargetUpdate(mHwc, disp.framebufferTarget->handle, NULL,disp.list,NULL);
    }
    else if(id == HWC_DISPLAY_EXTERNAL){
        //ALOGV("HWComposer: framebufferTarget for external");
        mHwc->fbTargetUpdate(mHwc, NULL, disp.framebufferTarget->handle,NULL,disp.list);
    }
    
#ifdef TCC_SKIP_UI_UPDATE_DURING_VIDEO
	if (disp.list) {
		disp.list->flags &= ~ HWC_FB_FOR_VIDEO;
	}
#endif
            ...
            if (disp.list) {
                for (size_t i=0 ; i<disp.list->numHwLayers ; i++) {
                    ...
                    if (l.flags & HWC_SKIP_LAYER) {
                        if( l.compositionType != HWC_DISABLE ) //Telechips, ZzaU
...
bool HWComposer::hasGlesComposition(int32_t id) const {
    ...
    if (!mHwc || uint32_t(id)>31 || !mAllocatedDisplayIDs.hasBit(id) || ( atoi(value) != 0 ))
        return true;
	        ...
	        for (size_t i=0 ; i<mNumDisplays ; i++) {
	            ...
	            if (disp.list) {
	                if(getVideoFlag()){
						...
						//	ALOGE("Now Video Frame but got Framebuffer ");
							disp.list->flags |= HWC_FB_FOR_VIDEO;
	                	}
	                	else{
	                		disp.list->flags |= HWC_SKIP_MALI_FOR_VIDEO;
	                	}
	                }
	                else if(getInvalidateFlag())
	                {
	                	disp.list->flags |= HWC_FB_FOR_VIDEO;
	                }
	            }
...
int HWComposer::fbPost(int32_t id,
    ...
    if (mHwc && hwcHasApiVersion(mHwc, HWC_DEVICE_API_VERSION_1_1)) {
        ...
        if ( err == NO_ERROR ) {
            updateFramebufferTarget(id, acquireFence);
        }
...
#ifdef TCC_SKIP_UI_UPDATE_DURING_VIDEO
void HWComposer::setVideoFlag(int32_t isVideo)
{
	//Mutex::Autolock _l(mLock);
	mVideoFlag = isVideo;
}

int32_t HWComposer::getVideoFlag()
{
	return (mVideoFlag /*&& mNumOVLayers>0*/);
}

void HWComposer::setInvalidateFlag(int32_t isInvalidate)
{
	//Mutex::Autolock _l(mLock);
	mInvalidateFlag = isInvalidate;
}

int32_t HWComposer::getInvalidateFlag()
{
	return (mInvalidateFlag);
}

void HWComposer::setRoutineForVideoFlag(int32_t isRoutineForVideo)
{
	//Mutex::Autolock _l(mLock);
	mRoutineForVideoFlag = isRoutineForVideo;
}

int32_t HWComposer::getRoutineForVideoFlag()
{
	return mRoutineForVideoFlag;
}

int32_t HWComposer::checkRoutineForVideoFlag()
{
	char value[PROPERTY_VALUE_MAX];
	property_get("tcc.video.surface.enable", value, "0");
	return  atoi(value);
}
#endif
// ---------------------------------------------------------------------------
