...
public:
	...
	status_t reset_fbt(); //Telechips: ZzaU
	...
	void setVideoFlag(int32_t isVideo);
	int32_t getVideoFlag();
	void setInvalidateFlag(int32_t isInvalidate);
	int32_t getInvalidateFlag();
	int32_t getRoutineForVideoFlag();
	int32_t checkRoutineForVideoFlag();
	void setRoutineForVideoFlag(int32_t isRoutineForVideo);
	#endif
    // Events handling ---------------------------------------------------------
...
private:
    ...
    status_t updateFramebufferTarget(int32_t id, const sp<Fence>& acquireFence); //Telechips, ZzaU
...
#ifdef TCC_SKIP_UI_UPDATE_DURING_VIDEO
	int32_t			mVideoFlag;
	int32_t			mInvalidateFlag;
    int32_t                         mRoutineForVideoFlag;
#endif
