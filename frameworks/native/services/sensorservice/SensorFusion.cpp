...
status_t SensorFusion::setCalibration(void* ident,float x, float y, float z) {
    mSensorDevice.setCalibration(ident, mAcc.getHandle(), x,y,z);
    return NO_ERROR;
}

status_t SensorFusion::startCalibration(void* ident) {
    mSensorDevice.startCalibration(ident, mAcc.getHandle());
    return NO_ERROR;
}
