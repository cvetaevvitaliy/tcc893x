...
status_t SensorService::setCalibration(const sp<SensorEventConnection>& connection,
        int handle, int sensors, float x, float y, float z)
{
    if (mInitCheck != NO_ERROR)
        return mInitCheck;
    ...
    return sensor->setCalibration(connection.get(), handle,x,y,z);
}

status_t SensorService::startCalibration(const sp<SensorEventConnection>& connection,
        int handle, int sensors)
{
    if (mInitCheck != NO_ERROR)
        return mInitCheck;
    ...
    SensorInterface* sensor = mSensorMap.valueFor(handle);
    if (!sensor) return BAD_VALUE;
    return sensor->startCalibration(connection.get(), handle);
}
...
status_t SensorService::SensorEventConnection::setCalibration(
        int handle,int sensor, float x, float y, float z)
{
    //LOGI(" SensorService::SensorEventConnection::setCalibration  sensor=%d  %f %f %f",sensor,x,y,z);
    return mService->setCalibration(this, handle,sensor, x, y, z);
}

status_t SensorService::SensorEventConnection::startCalibration(
        int handle,int sensor)
{
    //LOGI(" SensorService::SensorEventConnection::setCalibration  sensor=%d  %f %f %f",sensor);
    return mService->startCalibration(this, handle,sensor);
}
