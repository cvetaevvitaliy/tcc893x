...
public:
    ...
    status_t setCalibration(Sensor const* sensor,int32_t sensors, float x, float y, float z) const;
    status_t startCalibration(Sensor const* sensor,int32_t sensors) const;

    // these are here only to support SensorManager.java
    status_t enableSensor(int32_t handle, int32_t samplingPeriodUs, int maxBatchReportLatencyUs,
                          int reservedFlags) const;
    status_t disableSensor(int32_t handle) const;
    status_t setCalibration(int32_t handle,int32_t sensors, float x, float y, float z) const;
    status_t startCalibration(int32_t handle,int32_t sensors) const;
