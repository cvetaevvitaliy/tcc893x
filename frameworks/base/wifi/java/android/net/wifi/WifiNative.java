...
public class WifiNative {
    ...
    public boolean scan(int type) {
        if(!SystemProperties.get("tcc.wifi.wfd.sinkapp").equals("1") ) { // for improve p2p connection
   ...
   /**
    ...
    public String p2pConnect(WifiP2pConfig config, boolean joinExistingGroup) {
        ...
        Log.d(mTAG, "joinExistingGroup: " + joinExistingGroup);
        ...
        if (joinExistingGroup) {
            ...
            String intent = SystemProperties.get("tcc.wifi.p2p.go_intent","-1");

            if(intent == "-1")
            args.add("go_intent=" + groupOwnerIntent);
            else
                args.add("go_intent=" + Integer.parseInt(intent));
        }

        String opch = SystemProperties.get("tcc.wifi.p2p.operating.channel","0");
        if(opch != "0") {
            args.add("freq="+Integer.parseInt(opch));
        }
    ...
    public boolean p2pGroupAdd(boolean persistent) {
        Log.d(mTAG, "p2pProvisionDiscovery persistent = " + persistent);
        if(SystemProperties.get("tcc.wifi.p2p.persistent").equals("1")) {
        if (persistent) {
    ...
    public String p2pSetListenChannel(int channel) {
        if(channel == 1 || channel == 6 || channel == 11)
            return doStringCommand("p2p_set listen_channel " + channel);
        return null;
    }
