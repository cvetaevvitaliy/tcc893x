...
public class WifiP2pService extends IWifiP2pManager.Stub {
    ...
    private boolean mSendToAppDialog;
    private final WifiP2pInfo mWifiP2pInfo = new WifiP2pInfo();
    private String mWpsPin; // telechips
    private boolean bPersistentConnection=false;
    private boolean mWpsMethodPinDisplay = false;
    /**
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.P2P_GROUP_FORMATION_FAILURE_EVENT:
                    break;
                case WifiP2pManager.SET_LISTEN_CHANNEL:
                    int channel = (int) message.arg1;
                    mWifiNative.p2pSetListenChannel(channel);
                    break;
                case WifiP2pManager.REQUEST_TEMP_DISCONNECT_WIFI:
                    mWifiChannel.sendMessage(WifiP2pService.DISCONNECT_WIFI_REQUEST, 1);
                    break;   
                case WifiP2pManager.REQUEST_TEMP_RECONNECT_WIFI:
                    mWifiChannel.sendMessage(WifiP2pService.DISCONNECT_WIFI_REQUEST, 0);
                    break;
                case WifiP2pManager.REQUEST_P2P_FLUSH:
            ...
            switch (message.what) {
                ...
                case WifiMonitor.P2P_GO_NEGOTIATION_REQUEST_EVENT:
                    ...
                    if (!sendConnectNoticeToApp(mPeers.get(mSavedPeerConfig.deviceAddress),
                            mSavedPeerConfig)) {
                        mSendToAppDialog = false;
                    }else
                        mSendToAppDialog = true;

                    if(mSavedPeerConfig.wfd_ctrl_port > 0) {
                        setWifiP2pInfoWfdRtspPort(mSavedPeerConfig.wfd_ctrl_port);
                    } else{
                        WifiP2pDevice dev = mPeers.get(mSavedPeerConfig.deviceAddress);
                        if(dev!=null && dev.wfdInfo!=null)
                        setWifiP2pInfoWfdRtspPort(dev.wfdInfo.getControlPort());   
                    }
                    transitionTo(mUserAuthorizingNegotiationRequestState);
                    break;
                case WifiMonitor.P2P_PERSISTENT_REQUEST_EVENT:
                    logd("P2P_PERSISTENT_REQUEST");
                    WifiP2pDevice device = (WifiP2pDevice) message.obj;
                    bPersistentConnection = true;

                    WifiP2pDevice dev = mPeers.get(device.deviceAddress);
                    if(dev!=null && dev.wfdInfo!=null) 
                        setWifiP2pInfoWfdRtspPort(dev.wfdInfo.getControlPort());
                    else
                      loge("dev is null");                      

                    sendConnectNoticeToApp(null, null);
                    transitionTo(mGroupNegotiationState);
                    break;
                ...
                case WifiMonitor.P2P_PROV_DISC_PBC_REQ_EVENT:
                    ...
                    device = mPeers.get(provDisc.device.deviceAddress);
                    if(device!=null && device.wfdInfo!=null) 
                        setWifiP2pInfoWfdRtspPort(device.wfdInfo.getControlPort());
                    else
                      loge("dev is null");       
                ...
                case WifiMonitor.P2P_PROV_DISC_SHOW_PIN_EVENT:
                    ...
                    mWpsPin = provDisc.pin;
                    sendShowPinReqToFrontApp(provDisc.pin);
                    break;
                case WifiP2pManager.CREATE_GROUP:
                ...
                case WifiMonitor.P2P_GROUP_STARTED_EVENT:
                    ...
                    config = new WifiP2pConfig();
                    config.setMessage(WifiP2pManager.P2P_GROUP_STARTED);
                    sendMesssageToApp(null, config);                    
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.P2P_GO_NEGOTIATION_SUCCESS_EVENT:
                    // nofity to application
                    config = new WifiP2pConfig();
                    config.setMessage(WifiP2pManager.P2P_GO_NEG_SUCCESS);
                    sendMesssageToApp(null, config);
                    break;
                case WifiMonitor.P2P_GROUP_FORMATION_SUCCESS_EVENT:
                    if (DBG) logd(getName() + " go success");
                    config = new WifiP2pConfig();
                    config.setMessage(WifiP2pManager.P2P_GROUP_FORMATION_SUCCESS);
                    sendMesssageToApp(null, config);                    
                    break;
                case WifiMonitor.P2P_GROUP_STARTED_EVENT:
                    mGroup = (WifiP2pGroup) message.obj;
                    ...
                    // get WFD RTSP port                 
                    logd("go = " + mGroup.isGroupOwner() + " deviceAddress = " + mGroup.getOwner().deviceAddress);
                    if(!mGroup.isGroupOwner()) {
                        WifiP2pDevice dev = mPeers.get(mGroup.getOwner().deviceAddress);
                        if(dev!=null && dev.wfdInfo!=null) 
                            setWifiP2pInfoWfdRtspPort(dev.wfdInfo.getControlPort());
                        else
                          loge("mPeers.get("+mGroup.getOwner().deviceAddress+") is null");                      
                    }
    ...
    class FrequencyConflictState extends State {
        ...
        private void notifyFrequencyConflict() {
            ...
            if(SystemProperties.get("tcc.wifi.wfd.sinkapp").equals("1") ) {
            	 sendMessage(DROP_WIFI_USER_ACCEPT);
            	 sendMessage(DISCONNECT_WIFI_RESPONSE);
            	 return;
            }else { 
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.P2P_GO_NEGOTIATION_FAILURE_EVENT:
                    config = new WifiP2pConfig();
                    config.setMessage(WifiP2pManager.P2P_GO_NEG_FAIL);
                    sendMesssageToApp(null, config);
                    transitionTo(mInactiveState);
                    break;
                case WifiMonitor.P2P_GROUP_REMOVED_EVENT:
                    config = new WifiP2pConfig();
                    config.setMessage(WifiP2pManager.P2P_GROUP_REMOVED);
                    sendMesssageToApp(null, config);
                    transitionTo(mInactiveState);
                    break;                    
                case WifiMonitor.P2P_GROUP_FORMATION_FAILURE_EVENT:
                    ...
                    config = new WifiP2pConfig();
                    config.setMessage(WifiP2pManager.P2P_GROUP_FORMATION_FAIL);
                    sendMesssageToApp(null, config);
                    transitionTo(mInactiveState);
                    break;
                case DROP_WIFI_USER_REJECT:
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiP2pManager.START_WPS:
                    ...
                    break;
                case WifiP2pManager.CONNECT:
                    if(SystemProperties.get("tcc.wifi.display.sink.connect").equals("1")) {
                        break;
                    }
                    WifiP2pConfig config = (WifiP2pConfig) message.obj;
                    if (isConfigInvalid(config)) {
                ...
                case WifiMonitor.P2P_PROV_DISC_SHOW_PIN_EVENT:
                    ...
                    dev = mPeers.get(provDisc.device.deviceAddress);
                    if(dev!=null && dev.wfdInfo!=null) 
                        setWifiP2pInfoWfdRtspPort(dev.wfdInfo.getControlPort());
                    transitionTo(mUserAuthorizingJoinState);
                    break;
    ...
    private void p2pConnectWithPinDisplay(WifiP2pConfig config) {
        ...
        loge("p2pConnectWithPinDisplay: mJoinExistingGroup: " + mJoinExistingGroup);
...
*/      
        if(mJoinExistingGroup) // for P2P-INVITATION_RECEIVED
            pin = mWifiNative.p2pConnect(config, mJoinExistingGroup);
        else
            pin = mWifiNative.p2pConnect(config, dev.isGroupOwner());
    ...
    private void handleGroupCreationFailure() {
        ...
        if(mSavedPeerConfig.deviceAddress.length() == 0)
            return;
    ...
    private boolean sendConnectNoticeToApp(WifiP2pDevice dev, WifiP2pConfig config) {
        ...
        /* don't use auto pbc property from Kitkat
        if(SystemProperties.get("tcc.wifi.p2p.auto_pbc").equals("0") && bPersistentConnection==false)
            return false;
        */
        
        bPersistentConnection = false;
    ...
    private boolean sendMesssageToApp(WifiP2pDevice dev, WifiP2pConfig config) {
        logd("sendMesssageToApp msg = " + config.message);
        ...
        Message msg = Message.obtain();
        msg.what = WifiP2pManager.MESSAGE_SEND;
        Bundle bundle = new Bundle();
        bundle.putParcelable(WifiP2pManager.P2P_DEV_BUNDLE_KEY, dev);
        bundle.putParcelable(WifiP2pManager.P2P_CONFIG_BUNDLE_KEY, config);
        msg.setData(bundle);
    ...
    private boolean sendDialogMsgToFrontApp(Message msg) {
        logd("sendDialogMsgToFrontApp msg : " + msg);
    ...
    private boolean setDialogListenerApp(Messenger m,
        ...
        logd("setDialogListenerApp package : " + appPkgName);
    ...
    private boolean isForegroundApp(String pkgName) {
        ...
        if (tasks.size() == 0) {
            loge("isForegroundApp package : " + pkgName);
            return false;
        }
        
        loge("tasks.get(0).baseActivity : " + tasks.get(0).baseActivity.getPackageName());
    ...
    private void setWifiP2pInfoWfdRtspPort(int port) {
        Slog.d(TAG,"setWifiP2pInfoWfdRtspPort = " + port);
        if(port>0)
            mWifiP2pInfo.mWfdRtspPort = port;
    }

    private void notifyWpsPbcOverLap() {
        Resources r = Resources.getSystem();
        AlertDialog dialog = new AlertDialog.Builder(mContext)
            .setTitle("Connection error")
            .setMessage("Detect WPS-PBC-OVERLAP, please try to connect later")
            .setPositiveButton(r.getString(R.string.ok), null)
            .create();
        dialog.getWindow().setType(WindowManager.LayoutParams.TYPE_SYSTEM_ALERT);
        dialog.show();
