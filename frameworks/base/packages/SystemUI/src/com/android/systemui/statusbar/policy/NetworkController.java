...
public class NetworkController extends BroadcastReceiver implements DemoMode {
    ...
    // nfs
    boolean mNfsConnected;
    int mNfsIconId = 0;
    int mLastNfsIconId = -1;
    String mContentDescriptionENfs;
    ...
    // NetworkFileSystem
    ArrayList<ImageView> mNfsIconViews = new ArrayList<ImageView>();
    private static final int[] sNfsImages = new int [] {
        R.drawable.nfs_established,
        R.drawable.connect_no,
        R.drawable.connect_creating
    };
    public interface SignalCluster {
        ...
        void setEthernetIndicators(boolean visible, int strengthIcon, int activityIcon,
                String contentDescription);
        void setNfsIndicators(boolean visible, int strengthIcon, int activityIcon,
                String contentDescription);
    }
    ...
    public NetworkController(Context context) {
        ...
        filter.addAction(NetworkFileSystemManager.NFS_STATE_CHANGED_ACTION);
		filter.addAction(Intent.ACTION_RELOAD_QUICKBOOT);
    ...
    public void refreshSignalCluster(SignalCluster cluster) {
        ...
        cluster.setNfsIndicators(
                mNfsConnected,
                mNfsIconId,
                mNfsIconId,
                mContentDescriptionNfs);
    }
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(WifiManager.RSSI_CHANGED_ACTION)
            ...
            updateEth(intent);
            refreshViews();
        } else if (action.equals(NetworkFileSystemManager.NFS_STATE_CHANGED_ACTION)) {
            updateNfs(intent);
            refreshViews();
		...
		else if (SystemProperties.getBoolean("ro.QB.enable", false) && Intent.ACTION_RELOAD_QUICKBOOT.equals(action)) {
			refreshLocale();
			updateAirplaneMode();
            refreshViews();
        }
    ...
    private final void updateEth(Intent intent) {
        final int event = intent.getIntExtra(EthernetManager.EXTRA_ETH_EVENT, EthernetStateTracker.EVENT_INTERFACE_CONFIGURATION_FAILED);
        boolean visible = true;
        ...
        mEthernetConnected = visible;
    }

    private final void updateNfs(Intent intent) {
          final int event = intent.getIntExtra(NetworkFileSystemManager.EXTRA_NFS_STATE, NetworkFileSystemManager.EVENT_NFS_UNKNOWN);
          boolean visible = true;
          mContentDescriptionNfs = "";

          Log.d(TAG," updateNfs event " + event);
          switch (event) {
                 case NetworkFileSystemManager.EVENT_NFS_MOUNTED:
                        mNfsIconId = sNfsImages[0];
                        visible = true;
                        break;
                 case NetworkFileSystemManager.EVENT_NFS_UNMOUNTED:
                        mNfsIconId = 0;
                        visible = false;
                        break;
                 default:
                        mNfsIconId = sNfsImages[2];
                        visible = true;
                        break;
          }
          mNfsConnected = visible;
    }
