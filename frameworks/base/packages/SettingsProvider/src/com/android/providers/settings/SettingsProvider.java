...
public class SettingsProvider extends ContentProvider {
    ...
    private static SettingsCache oldSecureCache = null;
    private static SettingsCache oldSystemCache = null;
    ...
    private boolean mFullyPopulateCacheNotify = false;
    private boolean mDBDetach = false;
    private boolean mDBDetaching = false;
    
    private ArrayList<Integer> mKeyBackupList = new ArrayList<Integer>();
    ...
    private class SettingsFileObserver extends FileObserver {
        ...
        public void onEvent(int event, String path) {
        	AtomicInteger knownMutation = sKnownMutationsInFlight.get(mUserHandle);
        	
        	if( knownMutation == null || mDBDetach )
        		return;
        	
            int modsInFlight = knownMutation.get();
            if (modsInFlight > 0) {
            ...
            if( !mDBDetach ) {
	            Log.d(TAG, "User " + mUserHandle + " updating our caches for " + mPath + " mDBDetach::" + mDBDetach);
    ...
    private DatabaseHelper getOpenHelpers(int userHandle) {
    	if( mDBDetach && !mDBDetaching ) {
    		//mDBDetach = false;
    		if( !SystemProperties.get("tcc.hibernate.property", "").equals("reloaded")) {
    			Log.e(TAG, "setting database is deteached!=======================\n" + Log.getStackTraceString(new Throwable()));
    		}
    	}
    	
    	return mOpenHelpers.get(userHandle);
    }
    ...
    private void establishDbTracking(int userHandle) {
        ...
        synchronized (this) {
            dbhelper = getOpenHelpers(userHandle);
            if (dbhelper == null) {
    ...
    private void fullyPopulateCaches(final int userHandle) {
        ...
        DatabaseHelper dbHelper = getOpenHelpers(userHandle);
    ...
    private DatabaseHelper getOrEstablishDatabase(int callingUser) {
        ...
        try {
            DatabaseHelper dbHelper = getOpenHelpers(callingUser);
            if (null == dbHelper) {
                establishDbTracking(callingUser);
                dbHelper = getOpenHelpers(callingUser);
            }
            return dbHelper;
        } finally {
            Binder.restoreCallingIdentity(oldId);
    ...
    public Bundle call(String method, String request, Bundle args) {
    	...
    	if( mDBDetach && !mDBDetaching ) {
    		...
    		while(!SystemProperties.get("tcc.hibernate.property", "").equals("reloaded")) {
    			try {
					Thread.sleep(200);
    ...
    public int delete(Uri url, String where, String[] whereArgs) {
    	...
    	if( url.getPathSegments().size() == 1 ) {
	    	if( "detach".equals(url.getPathSegments().get(0))){
	    		synchronized (this) {
	    			if( mDBDetach )
	    				return 0;
					...
					for (int i = 0; i < mOpenHelpers.size(); i++) {
						int userHandle = mOpenHelpers.keyAt(0);
						mKeyBackupList.add(userHandle);
						
						DatabaseHelper db = getOpenHelpers(userHandle);
						db.close();
						
						SettingsCache cache = sSystemCaches.get(userHandle);
						cache.evictAll();
						
						cache = sSecureCaches.get(userHandle);
						cache.evictAll();
						
						onUserRemoved(userHandle);
					}
					
					Settings.System.initNameValueCache();
					Settings.Secure.initNameValueCache();
    ...
    private void reload() {
        if (mDBDetach) {
            ...
            while (mKeyBackupList.size() > 0) {
                int userHandle = mKeyBackupList.get(0);
                establishDbTracking(userHandle);
                mKeyBackupList.remove(0);
            }

            sendNotify(Uri.parse("content://settings/secure"), UserHandle.USER_OWNER);
            sendNotify(Uri.parse("content://settings/system"), UserHandle.USER_OWNER);
            sendNotify(Uri.parse("content://settings/global"), UserHandle.USER_OWNER);
        }
    }
    ...
    public int update(Uri url, ContentValues initialValues, String where, String[] whereArgs) {
    	...
    	if( url.getPathSegments().size() == 1 ) {
	    	if( "reload".equals(url.getPathSegments().get(0))){
	    		reload();
	    		return 0;
	    	}
    	}
