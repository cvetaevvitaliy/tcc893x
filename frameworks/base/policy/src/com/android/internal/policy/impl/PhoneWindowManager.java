...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            resolver.registerContentObserver(Settings.System.getUriFor(
                    ...
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.System.getUriFor(
                    Settings.System.OUTPUT_MODE), false, this);
            updateSettings();
        }
    ...
    private final Runnable mPowerLongPress = new Runnable() {
        ...
        public void run() {
            ...
            int shutdown_active = SystemProperties.getInt("tcc.shutdown.active",1);
    ...
    public void setDisplayOutput() {
		...
		int HdmiResolution = SystemProperties.getInt("persist.sys.hdmi_resolution", 1);
		int HdmiMode = SystemProperties.getInt("persist.sys.hdmi_mode", 1);
		int CompositeMode = SystemProperties.getInt("persist.sys.composite_mode", 1);
		int ComponentMode = SystemProperties.getInt("persist.sys.component_mode", 1);
		int OutputChangeMode = SystemProperties.getInt("tcc.output.change.mode", 1);
		...
		switch(OutputMode)
		{
			...
			case 1:
				...
				if(HdmiMode == 0) 
				{
					...
					{
						HdmiMode = 1;			/* DVI Mode */
						HdmiResolution = 8;		/* 640x480 */
						OutputMode = 1;			/* HDMI Output */
					}
					else
					{
						HdmiMode = 0;			/* DVI Mode */
				...
				{
					HdmiMode = 0;			/* HDMI Mode */
					HdmiResolution = 8;		/* 640x480 */
					OutputMode = 2;			/* Composite Output */
					CompositeMode = 0;		/* Composite NTSC */
				}
				break;
			...
			case 2:
				...
				if(CompositeMode == 0) 
				{
					CompositeMode = 1;	/* Composite PAL */
				}
				/* Composite PAL */
				else 
				{
					OutputMode = 3;		/* Component Output */
					ComponentMode = 1;	/* Component 1080i */
				}
				break;
		...
		Log.d(TAG, "HdmiResolution = " + HdmiResolution);
		Log.d(TAG, "CompositeMode = " + CompositeMode);
		Log.d(TAG, "ComponentMode = " + ComponentMode);
		Log.d(TAG, "OutputChangeMode = " + OutputChangeMode);
		
		Settings.System.putInt(mContext.getContentResolver(), "output_mode", OutputMode);
		Settings.System.putInt(mContext.getContentResolver(), "hdmi_resolution", HdmiResolution);
		Settings.System.putInt(mContext.getContentResolver(), "hdmi_mode", HdmiMode);
		Settings.System.putInt(mContext.getContentResolver(), "composite_mode", CompositeMode);
		Settings.System.putInt(mContext.getContentResolver(), "component_resolution", ComponentMode);
    }

	public void setOutputResizeMode(int keyCode) {
		int OutputMode = SystemProperties.getInt("persist.sys.output_mode", 1);
		int OutputResize_x = 0, OutputResize_y = 0;

		switch(OutputMode)
		{
			case 0:	// LCD
			case 1:	// HDMI
				OutputResize_x = SystemProperties.getInt("persist.sys.hdmi_resize_x", 1);
				OutputResize_y = SystemProperties.getInt("persist.sys.hdmi_resize_y", 1);
				break;
			case 2:	//Composite
				OutputResize_x = SystemProperties.getInt("persist.sys.composite_resize_x", 1);
				OutputResize_y = SystemProperties.getInt("persist.sys.composite_resize_y", 1);
				break;
			case 3:
				OutputResize_x = SystemProperties.getInt("persist.sys.component_resize_x", 1);
				OutputResize_y = SystemProperties.getInt("persist.sys.component_resize_y", 1);
				break;
		}
		...
		Log.d(TAG, "OutputResize_x = " + OutputResize_x + " OutputResize_y = " + OutputResize_y);
		
		switch(OutputMode)
		{
			case 0:	// LCD
			case 1:	// HDMI
				SystemProperties.set("persist.sys.hdmi_resize_x", Integer.toString(OutputResize_x));
				SystemProperties.set("persist.sys.hdmi_resize_y", Integer.toString(OutputResize_y));
				break;
			case 2:	//Composite
				SystemProperties.set("persist.sys.composite_resize_x", Integer.toString(OutputResize_x));
				SystemProperties.set("persist.sys.composite_resize_y", Integer.toString(OutputResize_y));
				break;
			case 3:
				SystemProperties.set("persist.sys.component_resize_x", Integer.toString(OutputResize_x));
				SystemProperties.set("persist.sys.component_resize_y", Integer.toString(OutputResize_y));
				break;
		}
	}
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (((keyCode == KeyEvent.KEYCODE_F3) && !down)||((keyCode == KeyEvent.KEYCODE_FUNCTION) && !down)) {
            setDisplayOutput();
        }
        if (((keyCode == KeyEvent.KEYCODE_F4) && !down)||(keyCode == KeyEvent.KEYCODE_F5) && !down){
            setOutputResizeMode(keyCode);
        }
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
          ...
          if ((SystemProperties.getInt("tcc.solution.mbox", 1) == 1) && (SystemProperties.getInt("persist.sys.auto_resolution", 1) == 0)) {
	     return -1;
	   } else {
            if (!down) {
                ...
                if (event instanceof MotionEvent
                    ...
                    if (motionEvent.getAction() == MotionEvent.ACTION_DOWN) {
                        ...
                        synchronized (mWindowManagerFuncs.getWindowManagerLock()) {
                            ...
                            if (SystemProperties.get("tcc.solution.mbox", "0").equals("0")) {
    ...
    public void beginLayoutLw(boolean isDefaultDisplay, int displayWidth, int displayHeight,
        ...
        if (isDefaultDisplay) {
            ...
            } else if (mHideNavFakeWindow == null && !SystemProperties.get("tcc.wifi.display.sink.connect", "0").equals("1")) {
            ...
            if(SystemProperties.get("tcc.wifi.display.sink.connect", "0").equals("1"))
                navVisible = false;
    ...
    void initializeHdmiState() {
        ...
        if (SystemProperties.get("tcc.solution.mbox", "0").equals("1")) {
            plugged = true;
        } else if (new File("/sys/devices/virtual/switch/hdmi/state").exists()) {
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_POWER: {
                ...
                if (down) {
			     ...
			     boolean mTccSolutionMbox = SystemProperties.get("tcc.solution.mbox.sleep", "0").equals("1");
			     boolean mExtenddisplayPlugIn = SystemProperties.get("persist.sys.hdmi_detected", "0").equals("1");

				if(mExtenddisplayPlugIn && (mTccSolutionMbox != true))
					result = (result & ~ACTION_WAKE_UP) ;
				else
					result = (result & ~ACTION_WAKE_UP) | ACTION_GO_TO_SLEEP;

				if(mTccSolutionMbox == true)
				{
    ...
    public int rotationForOrientationLw(int orientation, int lastRotation) {
        ...
        synchronized (mLock) {
            ...
            if (!SystemProperties.get("persist.sys.app_rotation", "0").equals("0")) {
                return mDemoHdmiRotation;
            }
            ...
            switch (orientation) {
                ...
                default:
                    ...
                    // int HdmiDetected = SystemProperties.getInt("persist.sys.hdmi_detected", 0);
                    // if(HdmiDetected != 0)
                    //    return mLandscapeRotation;
