...
public abstract class WindowOrientationListener {
    ...
    final class SensorEventListenerImpl implements SensorEventListener {
        ...
        private boolean mBlock180Rotation = false;
        private final int ROTATION_0 = 0;
        private final int ROTATION_90 = 1;
        private final int ROTATION_180 = 2;        
        ...
        public SensorEventListenerImpl(WindowOrientationListener orientationListener) {
            ...
            resetLocked();
Log.d("dw_debug","SensorEventListnerImpl() called");
            boolean block180 = SystemProperties.get("ro.rotation.block180", "false").equals("true");
            if(LOG)
                Log.d(TAG, " rotation block180 "+ block180);
            if(block180 == true){
                mBlock180Rotation = true;
            }
        }
            ...
            synchronized (mLock) {
                ...
                if (!skipSample) {
                    ...
                    if (magnitude < NEAR_ZERO_MAGNITUDE) {
                        ...
                        int nearestRotation;
                        final boolean block180Rotation = mBlock180Rotation;
                        if(block180Rotation == true)
                        {
Log.d("dw_debug","In SensorChanged, block180Rotation is true");
                            int rotation= mProposedRotation;
                            boolean invalidRotation = false;
                            if(rotation < ROTATION_0 || rotation >= (ROTATION_270 + 1) || rotation == ROTATION_180){
                                invalidRotation = true;
                                rotation = (orientationAngle + 60) / 120;
                                if (rotation == 2) {
                            ...
                            int thresholdRanges[][] = THRESHOLDS[rotation];
                            int row = -1;
                            for (int i = 0; i < thresholdRanges.length; i++) {
                                if (orientationAngle >= thresholdRanges[i][0] && orientationAngle < thresholdRanges[i][1]) {
                                    row = i;
                                    break;
                                }
                            }
                            if (row == -1) 
                                nearestRotation = rotation; // no matching transition
                            else
                                nearestRotation = ROTATE_TO[rotation][row];
                        }
                        else
                        {
