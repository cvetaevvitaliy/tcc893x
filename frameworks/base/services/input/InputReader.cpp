...
static unsigned int output_mode;
static unsigned int hdmi_resolution;
static unsigned int hdmi_mode;
static unsigned int component_resolution;
static int mCustomKey1 = 0;
static int mCustomKey2 = 0;
...
static void synthesizeButtonKeys(InputReaderContext* context, int32_t action,
    ...
    synthesizeButtonKey(context, action, when, deviceId, source, policyFlags,
            lastButtonState, currentButtonState,
            AMOTION_EVENT_BUTTON_CUSTOM_KEY1, mCustomKey1);
    synthesizeButtonKey(context, action, when, deviceId, source, policyFlags,
            lastButtonState, currentButtonState,
            AMOTION_EVENT_BUTTON_CUSTOM_KEY2, mCustomKey2);
}
...
uint32_t CursorButtonAccumulator::getButtonState() const {
    ...
    if (mBtnRight) {
        property_get ("persist.sys.right_mouse", value, "0");
        mCustomKey1 = atoi(value);
        if (mCustomKey1 == 0)
            result |= AMOTION_EVENT_BUTTON_SECONDARY;
        else
            result |= AMOTION_EVENT_BUTTON_CUSTOM_KEY1;
    }
    if (mBtnMiddle) {
        property_get ("persist.sys.middle_mouse", value, "0");
        mCustomKey2 = atoi(value);
        if (mCustomKey2 == 0)
            result |= AMOTION_EVENT_BUTTON_TERTIARY;
        else
            result |= AMOTION_EVENT_BUTTON_CUSTOM_KEY2;
    }
...
KeyboardInputMapper::KeyboardInputMapper(InputDevice* device,
    ...
    if (!strcmp(getDeviceName().string(), "telechips_remote_controller")) { // telechips remocon controller
        mIsRemoteController = true;
    } else {
        mIsRemoteController = false;
    }
}
...
void KeyboardInputMapper::processKey(nsecs_t when, bool down, int32_t keyCode,
    ...
    if (mIsRemoteController) {
        if (virtualPointerController(when, policyFlags, keyCode, down, downTime)) {
            return;
        }
    }
...
bool KeyboardInputMapper::virtualPointerController(nsecs_t when, uint32_t policyFlags,
        int32_t keyCode, bool down, nsecs_t downTime) {
    ...
    if (!mPointerController->isShown()) { // If pointer is enable
        return false;
    }
    ...
    case AKEYCODE_DPAD_UP:
        if (down) mPointerController->move(0, -20);
        cursorChanged = true;
        break;
    ...
    case AKEYCODE_DPAD_RIGHT:
        if (down) mPointerController->move(20, 0);
        cursorChanged = true;
        break;
    ...
    case AKEYCODE_ENTER:
        ...
        uint32_t displayId = ADISPLAY_ID_DEFAULT;
        NotifyMotionArgs args(when, getDeviceId(), AINPUT_SOURCE_MOUSE, policyFlags,
                down ? AMOTION_EVENT_ACTION_DOWN : AMOTION_EVENT_ACTION_UP,
                0, metaState, AMOTION_EVENT_BUTTON_PRIMARY, 0, displayId, 1,
                &pointerProperties, &pointerCoords, 1.0f, 1.0f, downTime);
...
void CursorInputMapper::sync(nsecs_t when) {
	...
	if(output == 1){
		...
		int mode = atoi(value);
		if((output_mode != output) || (hdmi_resolution != resolution || hdmi_mode != mode)){
			output_mode = output;
			hdmi_resolution = resolution;
			hdmi_mode = mode;
			mPointerController = getPolicy()->changePointerController(getDeviceId());
		}
	} else if(output == 2){
		...
		int resolution = atoi(value);
		if((output_mode != output) || (component_resolution != resolution)){
			output_mode = output;
...
#if 0
    int32_t keyCode = 0, scanCode = 0, newMetaState = 0;
    if ((currentButtonState & AMOTION_EVENT_BUTTON_SECONDARY) != (lastButtonState & AMOTION_EVENT_BUTTON_SECONDARY))
    {
        keyCode = AKEYCODE_BACK;
        scanCode = 1;
        newMetaState = 0;
    }
    if ((currentButtonState & AMOTION_EVENT_BUTTON_TERTIARY) != (lastButtonState & AMOTION_EVENT_BUTTON_TERTIARY))
    {
...
void TouchInputMapper::configureSurface(nsecs_t when, bool* outResetNeeded) {
    ...
    if (viewportChanged) {
		...
		property_set("persist.sys.display_orientation", "90");
                break;
            case DISPLAY_ORIENTATION_180:
		...
		property_set("persist.sys.display_orientation", "180");
                break;
            case DISPLAY_ORIENTATION_270:
                naturalLogicalWidth = mViewport.logicalBottom - mViewport.logicalTop;
                naturalLogicalHeight = mViewport.logicalRight - mViewport.logicalLeft;
                ...
                naturalDeviceWidth = mViewport.deviceHeight;
                naturalDeviceHeight = mViewport.deviceWidth;
		property_set("persist.sys.display_orientation", "270");
                break;
            ...
            default:
                ...
                naturalDeviceWidth = mViewport.deviceWidth;
                naturalDeviceHeight = mViewport.deviceHeight;
		property_set("persist.sys.display_orientation", "0");
                break;
            }
