...
size_t EventHub::getEvents(int timeoutMillis, RawEvent* buffer, size_t bufferSize) {
    ...
    for (;;) {
        ...
        while (mPendingEventIndex < mPendingEventCount) {
            ...
            if (eventItem.events & EPOLLIN) {
#ifdef HAVE_TSLIB
//		LOGE("Inside EventHub.cpp with mFDs[i].fd=%d \n", fd);
		if (mTS != NULL) {
			if (device->fd != mTS->fd ) {
				...
				char ts_property[PROPERTY_VALUE_MAX];
				property_get("tcc.calibration.state",ts_property,NULL);
				...
				if(strcmp(ts_property, "done") == 0)
				{
					...
					property_set("tcc.calibration.state","over");
				}
	    ...
	    /*if (samp.ev[numOfEventsSent].type == EV_KEY && device->keyMap.haveKeyLayout()) {
               status_t err = device->keyMap.keyLayoutMap->mapKey(samp.ev[numOfEventsSent].code,
                            &event->keyCode, &event->flags);
                   ALOGV("iev.code=%d keyCode=%d flags=0x%08x err=%d\n",
                              samp.ev[numOfEventsSent].code, event->keyCode, event->flags, err);
            }*/
