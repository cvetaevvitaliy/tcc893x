...
#ifdef KERNEL_DRAW_CURSOR
    mKernelController = new KernelController(looper);
    mKernelPointerChanged = false;
#endif
...
void PointerController::move(float deltaX, float deltaY) {
    ...
    if (isBox) {
        switch (mLocked.displayOrientation) {
        case DISPLAY_ORIENTATION_90:
            setPositionLocked(mLocked.pointerX + deltaY, mLocked.pointerY - deltaX);
            break;
...
bool PointerController::isShown() {
...
#ifdef KERNEL_DRAW_CURSOR
void PointerController::updateKernelPointerLocked() {
    mKernelController->openTransaction();

    if (isBox) {
        switch (mLocked.displayOrientation) {
            case DISPLAY_ORIENTATION_90:
                mKernelController->setPosition(mLocked.displayWidth - mLocked.pointerY, mLocked.pointerX);
                break;
            case DISPLAY_ORIENTATION_180:
                mKernelController->setPosition(mLocked.displayWidth - mLocked.pointerX, mLocked.displayHeight - mLocked.pointerY);
                break;
            case DISPLAY_ORIENTATION_270:
                mKernelController->setPosition(mLocked.pointerY, mLocked.displayHeight - mLocked.pointerX);
                break;
            default:
                mKernelController->setPosition(mLocked.pointerX, mLocked.pointerY);
                break;
        }
    } else {
        mKernelController->setPosition(mLocked.pointerX, mLocked.pointerY);
    ...
    if (mLocked.pointerAlpha > 0) {
        mKernelController->setVisible(true);
    } else {
        mKernelController->setVisible(false);
    }

    if (mKernelPointerChanged) {
        mKernelController->setIcon(mLocked.pointerIcon);
        mKernelPointerChanged = false;
    }

    mKernelController->closeTransaction();
}
#endif
...
#ifdef KERNEL_DRAW_CURSOR
    char value[PROPERTY_VALUE_MAX];
    int output_mode, ui_mode, display_path, mvc_enable;

    property_get ("persist.sys.output_mode", value, "");
    output_mode = atoi(value);
    property_get ("tcc.3d.ui.mode", value, "");
    ui_mode = atoi(value);
    property_get ("tcc.video.mvc.mode", value, "");
    mvc_enable = atoi(value);
 
   if (output_mode == 2) {
        property_get ("tcc.composite.mouse.path", value, "");
        display_path = atoi(value);
    } else {
        display_path = 1;
    }

    if ((display_path == 1) && (ui_mode == 0) && (mvc_enable == 0)) {
        mLocked.pointerSprite->setVisible(false);
        updateKernelPointerLocked();
        return;
    }
    mKernelController->setVisible(false);
#endif
    ...
    if (isBox) {
        switch (mLocked.displayOrientation) {
            case DISPLAY_ORIENTATION_90:
                mLocked.pointerSprite->setPosition(mLocked.displayWidth - mLocked.pointerY, mLocked.pointerX);
                break;
            case DISPLAY_ORIENTATION_180:
                mLocked.pointerSprite->setPosition(mLocked.displayWidth - mLocked.pointerX, mLocked.displayHeight - mLocked.pointerY);
                break;
            case DISPLAY_ORIENTATION_270:
                mLocked.pointerSprite->setPosition(mLocked.pointerY, mLocked.displayHeight - mLocked.pointerX);
                break;
            default:
