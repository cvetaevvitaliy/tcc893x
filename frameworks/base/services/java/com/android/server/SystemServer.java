...
import java.io.File;
import static android.provider.Settings.System.SPDIF_SETTING;
...
class ServerThread {
    ...
    public void initAndLoop() {
       ...
       // NetworkFileSystemService nfs = null;
        ...
        if (factoryTest != SystemServer.FACTORY_TEST_LOW_LEVEL) {
            ...
            if (!disableNetwork) {
               ...
               try {
				...
				try {
					Slog.i(TAG, "NetworkFileSystem Service");
					ServiceManager.addService("networkfilesystem", new NetworkFileSystemService(context));
				} catch (Throwable e) {
					reportWtf("starting NetworkFileSystem Service", e);
				}
            ...
            try {
                Slog.i(TAG, "Video Thumb Service");
                ServiceManager.addService("videothumb", new VideoThumbService(context));
            } catch (Throwable e) {
                reportWtf("starting Video Thumb", e);
            }

            try {
                Slog.i(TAG, "NetworkFileSystem Service");
                ServiceManager.addService("networkfilesystem", (IBinder) new NetworkFileSystemService(context));
            } catch (Throwable e) {
                reportWtf("starting NetworkFileSystem Service", e);
            }
        ...
        final BeforeQBSystemCallback2 beforeCallback = new BeforeQBSystemCallback2(contextF, mContentResolver);
        final AfterQBSystemCallback1 afterCallback1 = new AfterQBSystemCallback1(contextF, mContentResolver);
        final AfterQBSystemCallback2 afterCallback2 = new AfterQBSystemCallback2(contextF, mContentResolver);
        final AfterQBSystemCallback3 afterCallback3 = new AfterQBSystemCallback3(contextF, mContentResolver);
