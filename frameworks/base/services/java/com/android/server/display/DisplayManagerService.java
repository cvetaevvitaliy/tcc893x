...
public final class DisplayManagerService extends IDisplayManager.Stub {
	...
	mPowerManager = (PowerManager) context.getSystemService(Context.POWER_SERVICE);
        ...
        mWakelock = mPowerManager.newWakeLock(PowerManager.SCREEN_BRIGHT_WAKE_LOCK, "*hdmi_test*");
        ...
        mHandler.sendEmptyMessage(MSG_REGISTER_ADDITIONAL_DISPLAY_ADAPTERS);
	        ...
	        mContext.registerReceiver(new BroadcastReceiver() {
				...
				if(device.getDisplayDeviceInfoLocked().type == Display.TYPE_HDMI) {
					registerPowerMangerLocked(true);
					break;
				}
	                }
            		}
    ...
    private void registerPowerMangerLocked(boolean locked) {
        if (DEBUG) {
            Slog.d(TAG, "registerPowerMangerLocked: locked: " + locked + "  mSystemRead: " + mSystemReady);
        }

	if(mSystemReady == false)
		return;
    ...
    private void handleDisplayDeviceRemovedLocked(DisplayDevice device) {
	    ...
	    if(device.getDisplayDeviceInfoLocked().type == Display.TYPE_HDMI) {
		  registerPowerMangerLocked(false);
	    }
