...
final class WifiDisplayController implements DumpUtils.Dump {
    ...
    private void updateWfdEnableState() {
        Slog.d(TAG, "updateWfdEnableState");
            ...
            if (!mWfdEnabled && !mWfdEnabling) {
                ...
                mWifiP2pManager.setWFDInfo(mWifiP2pChannel, wfdInfo, new ActionListener() {
                    ...
                    public void onFailure(int reason) {
                        if (DEBUG) {
                            Slog.e(TAG, "Failed to set WFD info with reason " + reason + ".");
                        }
                        mWfdEnabling = false;
                    }
                });
    ...
    private void tryDiscoverPeers() {
        mWifiP2pManager.discoverPeers(mWifiP2pChannel, new ActionListener() {
            ...
            public void onFailure(int reason) {
                if (DEBUG) {
                    Slog.e(TAG, "Discover peers failed with reason " + reason + ".");
                }
    ...
    private void connect(final WifiP2pDevice device) {
		...
		if(SystemProperties.get("tcc.wifi.wfd.sinkapp").equals("1")) {
            Slog.e(TAG, "sink app executed");
			return;
		}

        SystemProperties.set("tcc.wifi.display.connect", "-1"); //FIXME:
    ...
    private void disconnect() {
        ...
        mDesiredDevice = null;
        SystemProperties.set("tcc.wifi.display.connect","0");
    ...
    private void updateConnection() {
        ...
        if (mCancelingDevice != null) {
            Slog.i(TAG, "canceling device is not null");
            return; // wait for asynchronous callback
        }
        ...
        if (mDesiredDevice == null) {
            if (mWifiDisplayCertMode) {
                mListener.onDisplaySessionInfo(getSessionInfo(mConnectedDeviceGroupInfo, 1234));
            }
            unadvertiseDisplay();
            return; // done
        }
...
*/
            ...
            mHandler.post( new Runnable() {
                ...
                public void run() {
                    for(i=0; i<rtspTimeout; i++) {
                        try {
                            Slog.d(TAG, "wait for rtsp connection");
                            Thread.sleep(1000);
                            /* this property sets in telechips wfd_source service */
                            if(SystemProperties.get("tcc.wifi.display.connect").equals("1")) {
                                Slog.i(TAG, "rtsp connected");
                                ...
                                if (mConnectedDevice == oldDevice && !mRemoteDisplayConnected) {
                                    ...
                                    if (mWifiDisplayCertMode) {
                                        mListener.onDisplaySessionInfo(
                                                //getSessionInfo(mConnectedDeviceGroupInfo, session));
                                                getSessionInfo(mConnectedDeviceGroupInfo, 1234));
                                    }

                                    final WifiDisplay display = createWifiDisplay(mConnectedDevice);
                                    //advertiseDisplay(display, surface, width, height, flags);
                                    advertiseDisplay(display, new Surface(), 0, 0, 1);
                                }
            ...
            if (mDesiredDevice != null || mWifiDisplayCertMode) {
                mWifiP2pManager.requestGroupInfo(mWifiP2pChannel, new GroupInfoListener() {
                    ...
                    public void onGroupInfoAvailable(WifiP2pGroup info) {
                        ...
                        /* telechips, for Miracast Marvel source
    ...
    private final Runnable mRtspTimeout = new Runnable() {
        ...
        public void run() {
            if (mConnectedDevice != null
                ...
                Slog.e(TAG, "Timed out waiting for Wifi display RTSP connection after "
    ...
    private void handleConnectionFailure(boolean timeoutOccurred) {
        Slog.e(TAG, "Wifi display connection failed!");
            ...
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (display != null) {
                        ...
                        if ( (surface != null && surface != oldSurface) || flags == 1) { // telechips
                            Slog.i(TAG, "onDisplayConnected");
    ...
    private static WifiDisplay createWifiDisplay(WifiP2pDevice device) {
        return new WifiDisplay(device.deviceAddress, device.deviceName, null,
                true, device.wfdInfo.isSessionAvailable(), false,device.wfdInfo.getDeviceType(), device.wfdInfo.getControlPort());
    }
