...
public final class PowerManagerService extends IPowerManager.Stub
	...
	mMediaBox = SystemProperties.get("tcc.solution.mbox", "0").equals("1");
    ...
    private void updatePowerStateLocked() {
        ...
        boolean iExtenddisplayDetect = SystemProperties.get("tcc.extenddisplay.detected", "0").equals("1");

		
        int iWFDConnected = 0;
        String WFDConnected = SystemProperties.get("tcc.wifi.display.connect");
        if(WFDConnected.length()!=0) {
            iWFDConnected = Integer.parseInt(WFDConnected);
            if(iWFDConnected == -1) // connecting
                iWFDConnected = 1;
        } else
            iWFDConnected = 0;


	if(iExtenddisplayDetect == true)
	{		
		if(mDisplayPowerRequest.screenState == DisplayPowerRequest.SCREEN_STATE_DIM) 
		{	
			//setting LCD backlight state change from SCREEN_STATE_DIM to SCREEN_STATE_BRIGHT
			mDisplayPowerRequest.screenState = DisplayPowerRequest.SCREEN_STATE_BRIGHT;

			// Update dreams and display power state.
			updateDreamLocked(1);
			updateDisplayPowerStateLocked(1);
            }
	}	
        if ((!mSystemReady || mDirty == 0) 
			|| ((mTccSolutionMbox == false) &&  (( iWFDConnected == 1) || (iExtenddisplayDetect == true))))
        {               
            return;
        }
    ...
    private void handleUserActivityTimeout() { // runs on handler thread
        synchronized (mLock) {
            ...
            int iWFDConnected = 0;
            String WFDConnected = SystemProperties.get("tcc.wifi.display.connect");
		boolean iExtenddisplayDetect = false;
		iExtenddisplayDetect = SystemProperties.get("tcc.extenddisplay.detected", "0").equals("1");
    	    ...
    	    if((iWFDConnected == 1)|| (iExtenddisplayDetect == true))	{
                	return;
	    }
	...
	boolean iExtenddisplayDetect = SystemProperties.get("tcc.extenddisplay.detected", "0").equals("1");
        ...
        if ((mWakeLockSummary & WAKE_LOCK_SCREEN_BRIGHT) != 0
                ...
                || !mBootCompleted || (iExtenddisplayDetect ==true)) {
