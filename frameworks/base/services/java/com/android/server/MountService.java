...
class MountService extends IMountService.Stub
    ...
    private void handleSystemReady() {
		...
		while (scanning_idle_msec < 1500 && wait_msec < (10 * 60 * 1000)) {
			int count;
			String[] paths;
			String[] states;
			if (isMediaScannerScanning()) {
				// if (DEBUG_EVENTS) Slog.e(TAG,
				// "scanning_idle_msec reset because of media scanner scanning");
				scanning_idle_msec = 0;
			}
			...
			for (int i = 0; i < count; i++) {
				...
				if (state.equals(Environment.MEDIA_UNMOUNTED)
					...
					// if (DEBUG_EVENTS) Slog.e(TAG,
					// "scanning_idle_msec reset because of that path::" + path
					// + " is " + state);
					scanning_idle_msec = 0;
				}
			}
			...
			wait_msec += 300;
			scanning_idle_msec += 300;

			// if (DEBUG_EVENTS) Slog.i(TAG, "scanning_idle_msec::" +
			// scanning_idle_msec);
		}
    ...
    private void notifyVolumeStateChange(String label, String path, int oldState, int newState) {
        ...
        if (DEBUG_EVENTS) Slog.i(TAG, "notifyVolumeStateChange::" + state + "newState::" + newState + " " + path);
    ...
    private int doUSBControl(boolean on) {

       int rc = StorageResultCode.OperationSucceeded;

       if (DEBUG_EVENTS) Slog.i(TAG, "doUSBTrunOn: USB Port Enabled. ");
       try {
          if (on) 
             mConnector.execute("volume", "enable", "/sys/class/android_usb/android0/enable");
          else
             mConnector.execute("volume", "disable", "/sys/class/android_usb/android0/enable");                      
       } catch (NativeDaemonConnectorException e) {    
    ...
    private int doFormatVolume(String path, String type) {
        try {
            mConnector.execute("volume", "format", path, type);
            return StorageResultCode.OperationSucceeded;
        } catch (NativeDaemonConnectorException e) {
            int code = e.getCode();
            if (code == VoldResponseCode.OpFailedNoMedia) {
    ...
    public void setUsbMassStorageEnabled(boolean enable) {
       ...
       Slog.d(TAG, "======================================");
       Slog.d(TAG, "Usb Mass Storage Enable" + " " + enable);
       Slog.d(TAG, "======================================");
       // TODO: Add support for multiple share methods
       ...
       String switching  = SystemProperties.get("ro.usb.switch");
       if (switching.equals("ums")) {
          for (int i = 0; i < mVolumes.size(); i++) { 
             ...
             if (!vs.equals(Environment.MEDIA_REMOVED)) {
                Slog.d(TAG, "Try to mount " + path + " share method " + method);
    ...
    public int formatVolume(String path, String type) {
        validatePermission(android.Manifest.permission.MOUNT_FORMAT_FILESYSTEMS);
        waitForReady();

        return doFormatVolume(path, type);
    }
