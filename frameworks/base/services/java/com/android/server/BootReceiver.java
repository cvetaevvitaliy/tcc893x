...
public class BootReceiver extends BroadcastReceiver {
    ...
    private void logBootEvents(Context ctx) throws IOException {
		...
		if ( (SystemProperties.getBoolean("ro.QB.enable", false))
				&& (SystemProperties.get("tcc.QB.boot.with", "not yet")
						.equals("normal"))
				&& (SystemProperties.get("persist.sys.QB.user.allow", "empty")
						.equals("empty"))) {
			 Intent clsIntent = new Intent(ctx, QBPopup.class );
             clsIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
             ctx.startActivity(clsIntent);
		}
    }
