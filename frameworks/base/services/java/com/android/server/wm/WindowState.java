...
final class WindowState implements WindowManagerPolicy.WindowState {
    ...
    public void computeFrameLw(Rect pf, Rect df, Rect of, Rect cf, Rect vf, Rect dcf) {
        ...
        boolean bAppRotation = false;
        if (mService.mAltOrientation
                && mAttrs.type != TYPE_INPUT_METHOD
                && mAttrs.type != WindowManager.LayoutParams.TYPE_NAVIGATION_BAR
                && mAttrs.type != WindowManager.LayoutParams.TYPE_STATUS_BAR
                && SystemProperties.get("persist.sys.app_rotation", "0").equals("1")) {
            bAppRotation = true;
        }

        final int pw = bAppRotation ? 526 : mContainingFrame.width();
