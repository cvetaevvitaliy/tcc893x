...
public class ExternalStorageFormatter extends Service
    ...
    public static final String FORMAT_TYPE = "com.android.internal.os.storage.FORMAT_TYPE";
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        mStorageVolume = intent.getParcelableExtra(StorageVolume.EXTRA_STORAGE_VOLUME);
				mNTFSformat = intent.getStringExtra(this.FORMAT_TYPE);
				if (mStorageVolume != null)
					Log.w(TAG, "KSJUNG - mStorageVolume.getPath() " + mStorageVolume.getPath());
        Log.w(TAG, "KSJUNG - mNTFSformat " + mNTFSformat);
    ...
    void updateProgressState() {
            Log.w(TAG, "ksjung - start updateProgressState()");
        String status = mStorageVolume == null ? 
					//"nofs" :
                Environment.getExternalStorageState() :
                mStorageManager.getVolumeState(mStorageVolume.getPath());
					Log.w(TAG, "KSJUNG - status " + status);
        final String description =  (mStorageVolume == null ?
                null : mStorageVolume.getDescription(this));
        if (Environment.MEDIA_MOUNTED.equals(status)
                || Environment.MEDIA_MOUNTED_READ_ONLY.equals(status)) {
            if (mStorageVolume != null) {
                if (description.contains(getResources().getString(R.string.storage_usb))) {
                    updateProgressDialog(R.string.progress_usb_unmounting);
                } else {
            ...
            if (mStorageVolume != null) {
                if (description.contains(getResources().getString(R.string.storage_usb))) {
                    updateProgressDialog(R.string.progress_usb_erasing);
                } else {
            ...
            if (mountService != null) {
                new Thread() {
                    ...
                    public void run() {
                        ...
                        try {
                            mountService.formatVolume(extStoragePath, mNTFSformat);
                            success = true;
                        } catch (Exception e) {
                            if (mStorageVolume != null) {
                                if (description.contains(getResources().getString(R.string.storage_usb))) {
                                    Toast.makeText(ExternalStorageFormatter.this,
                                            R.string.format_usb_error, Toast.LENGTH_LONG).show();
                                } else {
                                    Toast.makeText(ExternalStorageFormatter.this,
            ...
            if( mStorageVolume != null ) {
                if (description.contains(getResources().getString(R.string.storage_usb))) {
                    fail(R.string.media_usb_checking);
                } else {
            ...
            if( mStorageVolume != null ) {
                if (description.contains(getResources().getString(R.string.storage_usb))) {
                    fail(R.string.media_usb_shared);
                } else {
