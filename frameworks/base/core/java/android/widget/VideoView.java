...
import android.widget.Subtitle.OnSubtitleListener;
...
public class VideoView extends SurfaceView
        ...
        try {
            ...
            if (mSubtitleOn == true) {
                ...
                if (scheme != null && scheme.equals("file")) {
                    mSubtitle.start(mMediaPlayer, mUri.getPath(), mSubtitleFontSize, mSubtitle.DISPLAY_VIEW);
                } else {
                    Cursor cursor = mContentResolver.query(mUri, new String[] {"_data"}, null, null, null);
                    if (cursor != null) {
                        if (cursor.moveToFirst()) {
                            mSubtitle.start(mMediaPlayer, cursor.getString(0), mSubtitleFontSize, mSubtitle.DISPLAY_VIEW);
                        }
                        cursor.close();
                    } else {
    ...
    private void attachMediaController() {
        if (mMediaController != null) {
            ...
            {
    			String scheme = mUri.getScheme();
    			if (scheme != null && (scheme.equals("rtsp") || scheme.equals("http"))) {
            ...
            // VIEW_SUBTITLE
            if ((mSubtitle != null)
                && (mSubtitle.getSubtitleType() != Subtitle.TYPE_NONE)) {
...
*/
    ...
    // VIEW_SUBTITLE
     private Subtitle.OnSubtitleListener mSubtitleListener =

        new Subtitle.OnSubtitleListener() {
        ...
        public void onPGSCaption(int OffsetX, int OffsetY, int Srcwidth, int SrcHeight, int Dstwidth, int DstHeight, int StreamSize, int[] StreamData) {
           if (mOnSubtitleListener != null) {
              mOnSubtitleListener.onPGSCaption(OffsetX, OffsetY, Srcwidth, SrcHeight, Dstwidth, DstHeight, StreamSize, StreamData);
           }else if (mMediaController != null) {
              mMediaController.setPGSCaption(OffsetX, OffsetY, Srcwidth, SrcHeight, Dstwidth, DstHeight, StreamSize, StreamData);
           }
        }
     };
     ...
     * @param l The callback that will be run
     */
    public void setOnSubtitleListener(OnSubtitleListener l) {
	...
	if (keyCode == KeyEvent.KEYCODE_MENU) {
		...
		str_val = mMediaPlayer.getStringParameter(MediaParameterKeys.KEY_PARAMETER_GET_AUDIO_CODECTYPE);
		Log.d(TAG, "Audio Codec Type: " + str_val);

		str_val = mMediaPlayer.getStringParameter(MediaParameterKeys.KEY_PARAMETER_GET_VIDEO_CODECTYPE);
		Log.d(TAG, "Video Codec Type: " + str_val);

		int_val = mMediaPlayer.getIntParameter(MediaParameterKeys.KEY_PARAMETER_GET_VIDEO_FRAMERATE);
		Log.d(TAG, "Video Framerate: " + int_val/1000.0);

		int_val = mMediaPlayer.getIntParameter(MediaParameterKeys.KEY_PARAMETER_GET_AUDIO_SAMPLERATE);
		Log.d(TAG, "Audio Samplerate: " + int_val);

		int_val = mMediaPlayer.getIntParameter(MediaParameterKeys.KEY_PARAMETER_GET_BITRATE);
		Log.d(TAG, "Bitrate: " + int_val + " (" + int_val/1000.0 + " kbps" + ")");
	    }
	}
	*/
        ...
        if (isInPlaybackState() && isKeyCodeSupported && mMediaController != null) {
            ...
            // VIEW_SUBTITLE
            	...
            	/*
            	// Test Multi-audio selection
            	boolean ret = changeAudioTrack();
            	if (ret != true)
            		Log.d(TAG, "Can't change the audio track. ret = " + ret);
            	*/
            } else {
                toggleMediaControlsVisiblity();
            }
    ...
    public void setSubtitle(boolean onoff,int fontsize){
        if (isInPlaybackState() && mSubtitle != null && mSubtitle.getSubtitleType() != Subtitle.TYPE_NONE) {
            if (mMediaController != null) {
                ...
                if (mSubtitleFontSize != fontsize) {
                    Log.d(TAG, "mMediaController.setSubtitleFontSize " + fontsize);
