...
public final class SQLiteDatabase extends SQLiteClosable {
    ...
    public long insert(String table, String nullColumnHack, ContentValues values) {
        try {
            ...
            Log.e(TAG, "Error inserting table:" + table + " values:" + values, e);
            if( table.startsWith("KindleContent")) {
            	Cursor c = rawQuery("SELECT * FROM KindleContent WHERE key = '" + values.getAsString("key") + "'", null);
            	
            	if( c!= null && c.getCount() > 0 ) {
            		if( c != null ){ c.close(); c = null;}
            		return update(table, values, "key="+values.getAsString("key") , null);
            	}
