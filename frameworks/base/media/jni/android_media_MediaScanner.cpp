...
public:
            ...
            mSetMimeTypeMethodID(0)
    {
        ...
        if (mediaScannerClientInterface == NULL) {
            ...
            mScanDirectoryMethodID = env->GetMethodID(
            						mediaScannerClientInterface,
            						"scanDirectory",
	...
	virtual int scanDirectory(const char* path, long long lastModified,
			long long fileSize, bool isDirectory, bool noMedia) {
		ALOGV("scanDirectory: path(%s), time(%lld), size(%lld) and isDir(%d)",
				path, lastModified, fileSize, isDirectory);
		...
		jint result = mEnv->CallIntMethod(mClient, mScanDirectoryMethodID,
				pathStr, lastModified, fileSize, isDirectory, noMedia);

		mEnv->DeleteLocalRef(pathStr);
		if (checkAndClearExceptionFromCallback(mEnv, "scanDirectory") == OK)
			return result;
...
private:
    ...
    jmethodID mScanDirectoryMethodID;
