...
public class MediaPlayer implements SubtitleController.Listener
{
    ...
    static {
        ...
        // VIEW_SUBTITLE
        try {
            System.loadLibrary("TCCsubtitle");
        } catch (UnsatisfiedLinkError e) {
            Log.d("MediaPlayer", "Unable to load subtitle library!");
        }
    ...
    private static final int INVOKE_ID_SET_PLAYRATE = 7;
    private static final int INVOKE_ID_GET_PLAYRATE = 8;
     ...
     * @hide
     */
    public native void setVsync(boolean vsync);
    ...
    // VIEW_SUBTITLE
    /** @hide */
    public MediaSubtitle mMediaSubtitle;


    /**
     * Checks whether the MediaPlayer has a subtitle file.
     *
     * @param path full path to play av file
     * @param ccfb display subtitle through ccfb driver
     * @return true if currently exist, false otherwise
     * @hide
     */
    public native int openSubtitle(String path, int ccfb);

    /**
     * Gets the subtitle data.
     *
     * @param mediasubtitle to get subtitle data
     * @return the error value
     * @hide
    */
    public native boolean getSubtitle(MediaSubtitle  mediasubtitle); 

    /**
     * Seeks to specified time position.
     *
     * @param msec the offset in milliseconds from the start to seek subtitle
     * @return the error value
     * @hide
     */
    public native boolean seekSubtitle(int msec);

    /**
     * Gets class to support multi-language.
     *
     * @param mediasubtitle to get subtitle class data
     * @return the error value
     * @hide
     */
    public native boolean getSubtitleClass(MediaSubtitle mediasubtitle);

    /**
     * Sets class to support multi-language.
     *
     * @param num  the number to select classes
     * @param sequence  the sequence to display subtitles
     * @return the error value
     * @hide
     */
    public native boolean setSubtitleClass(int num, int[] sequence);
   ...
   /**
     ...
     * {@hide}
     */
    public native boolean setUIBCParameter(int key, Parcel value);
     ...
     * {@hide}
     */
    public boolean setUIBCParameter(int key, int value) {
        Parcel p = Parcel.obtain();
        p.writeInt(value);
        boolean ret = setUIBCParameter(key, p);
        p.recycle();
		Log.d(TAG, "setPatamter return key "+ key +" LINE 1616");
        return ret;
    }
    ...
    /**
     * Sets playback-rate (playback speed). 
     *  
     * @param playback-rate (playback speed). 100000 is normal speed 
     * (x1.0). The valid range of the rate is -3200000..3200000.
     * @throws IllegalStateException if called in an invalid state.
     *
     * @see android.media.MediaPlayer#getPlayRate
     */
    public void setPlayRate(int rate) throws IllegalStateException {
        ...
        try {
            request.writeInterfaceToken(IMEDIA_PLAYER);
            request.writeInt(INVOKE_ID_SET_PLAYRATE);
            request.writeInt(rate);
            invoke(request, reply);
        } finally {
            request.recycle();
            reply.recycle();
    ...
    /**
     * Returns playback-rate (playback speed). 
     *  
     * @return playback-rate (playback speed). 100000 is normal 
     * speed (x1.0).
     * @throws IllegalStateException if called in an invalid state.
     *
     * @see android.media.MediaPlayer#setPlayRate
     */
    public int getPlayRate() throws IllegalStateException {
        Parcel request = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        try {
            request.writeInterfaceToken(IMEDIA_PLAYER);
            request.writeInt(INVOKE_ID_GET_PLAYRATE);
            invoke(request, reply);
