...
public class MediaScanner
{
    ...
    private boolean debug_message = false;
    ...
    private static final int FILES_PRESCAN_PARENT_INDEX = 4;
    ...
    private boolean mNeedToScanFlag= false;
    ...
    private static class FileEntry {
        ...
        FileEntry(long rowId, String path, long lastModified, int format, long parentId) {
            mRowId = rowId;
            mPath = path;
        ...
        public String toString() {
            return " mRowId: " + mRowId + "mPath: " + mPath + " mParentId: " + mParentId + " mSeenInFileSystem: " + mSeenInFileSystem + " mFormat:" + mFormat;
        }
    }
    ...
    private HashMap<String, String> mStrageFSTypeCache;
    
    private StorageVolume mVolumeList[];

    private ArrayList<FileEntry> mPlayLists;

    private DrmManagerClient mDrmManagerClient = null;
    
    private EntryCache mEntryCache;
    ...
    private final MyMediaPreScannerClient mPreClient = new MyMediaPreScannerClient();
    ...
    private class MyMediaPreScannerClient extends MyMediaScannerClient {
    	boolean mEnableFolderPreScanFlag = false;
    	
    	private MyMediaPreScannerClient() {
    		mEnableFolderPreScanFlag = SystemProperties.getBoolean("ro.scan.folder.prescan", false);
    	}
        ...
        public void scanFile(String path, long lastModified, long fileSize,
            ...
            doScanFile(path, null, lastModified, fileSize, isDirectory, false, noMedia, false);
        }

        public int scanDirectory(String path, long lastModified, long fileSize,
                boolean isDirectory, boolean noMedia) {
            // This is the callback funtion from native codes.
            // Log.v(TAG, "scanFile: "+path);
           if(path.toLowerCase(Locale.US).endsWith("/.thumbnails")) {    /* 20130506 swhwang, don`t find ".thumbnail"  Directory */
                return 1;
            }

      		boolean bFastScanEanble = checkFastScanEnable(path);

			if ( ! bFastScanEanble ) {
				doScanFile(path, null, lastModified, fileSize, isDirectory,
						false, noMedia, false);
            ...
            try {
                ...
                if (entry != null ) {
                    if (entry.mLastModifiedChanged) {
                    	mNeedToScanFlag = true;
                    	
                    	if( mEnableFolderPreScanFlag &&  entry.mRowId == 0 ) {
							entry.mLastModified = 0;
                    		endFile(entry, false, false, false, false, false);
                    	}
                    } else if( mEntryCache != null && mEntryCache.isLeafNode(entry.mRowId) ) {
                        Log.w(TAG, path + " folder skip!");
                        return 1;
                    }
                }
        ...
        public Uri doScanFile(String path, String mimeType, long lastModified,
                long fileSize, boolean isDirectory, boolean scanAlways, boolean noMedia, boolean isSingleFileScan) {
            Uri result = null;
//            long t1 = System.currentTimeMillis();
            
            if( path.toLowerCase(Locale.US).endsWith("/.database_signature") || path.toLowerCase(Locale.US).endsWith("/.thumbnails")) {    /* 20130506 swhwang, don`t find ".thumbnail"  Directory */
                return null;
            }
            
            if (noMedia) {
             	if( mUsbConfig == null ) {
             		mUsbConfig = SystemProperties.get("persist.sys.usb.config");
             		
             		if (mUsbConfig == null || mUsbConfig.matches(".*mtp.*")) {
             			mMTPModeFlag = true;
             		} else {
             			mMTPModeFlag = false;
             		}
                ...
                if (entry != null && (entry.mLastModifiedChanged || scanAlways)) {
                	...
                	if( mEnableFolderPreScanFlag && entry.mRowId == 0 ) {
	                    ...
	                    if (noMedia) {
	                        ...
	                        if( (mMimeType != "unknown") || 
	                         (mMimeType != "video-unknown") // we ignore unsupported or unknown video codecs
	                         )
...
//            Log.v(TAG, "scanFile: " + path + " took " + (t2-t1));
    ...
    private class MyMediaScannerClient implements MediaScannerClient {
        ...
        private int mWidth;
        private int mHeight;
        public String mSkipVideoMeta = null;
        public String mSkipImageMeta = null;
        public boolean mIsSkipVideoMeta;
        public boolean mIsSkipImageMeta;
        public String mUsbConfig = null;
        public boolean mMTPModeFlag = false;
            ...
            try {
                ...
                if (entry != null && (entry.mLastModifiedChanged || scanAlways)) {
                    if (noMedia) {
                        ...
                        if( mSkipVideoMeta == null ) {
                        	mSkipVideoMeta = SystemProperties.get("tcc.solution.skipVideoMeta", "false");
                        	mIsSkipVideoMeta = SystemProperties.getBoolean("tcc.solution.skipVideoMeta", false);
                        }
                        
                        if( mSkipImageMeta == null ) {
                        	mSkipImageMeta = SystemProperties.get("tcc.solution.skipImageMeta", "false");
                        	mIsSkipImageMeta = SystemProperties.getBoolean("tcc.solution.skipImageMeta", false);
                        }
                        ...
                        if (isvideo&& !mIsSkipVideoMeta) {
                            processFile(path, mimeType, this);
                        }

                        if (isimage && !mIsSkipImageMeta) {
                        ...
                        if( end - start > 1000 ) {
                            Log.w(TAG, "parsing meta takes " +(end - start) + "ms. path is " + path ); 
                        }
                        
                        mMetaExtractTime +=(end - start); 
...
//            Log.v(TAG, "scanFile: " + path + " took " + (t2-t1));
        ...
        public void handleStringTag(String name, String value) {
            ...
            } else if (name.equalsIgnoreCase("videoheight")) {
                mHeight = parseSubstring(value, 0, 0);
            } else if (name.equalsIgnoreCase("videowidth")) {
                mWidth = parseSubstring(value, 0, 0);
            } else {
        ...
        protected void processImageFile(String path) {
            try {
        ...
        protected Uri endFile(FileEntry entry, boolean ringtones, boolean notifications,
    ...
    private boolean isMountedVolume(String path) {
    	int storage_index = -1;
    	
  		if (mVolumeList != null) {
    		for (int i = 0; i < mVolumeList.length; i++) {
    			if (path.startsWith(mVolumeList[i].getPath())) {
    				storage_index = i;
    				break;
    			}
    		}
  		...
  		String storageState = mStorageManager.getVolumeState(mVolumeList[storage_index].getPath());
    ...
    private void prescan(String[] directories, String filePath, boolean prescanFiles)
            throws RemoteException {
        if (!(prescanFiles || filePath == null)) {
            prescan(filePath, prescanFiles);
            return;
        }
        ...
        if (mDirectories == null) {
            mDirectories = new ArrayList<String>();
        } else {
            mDirectories.clear();
        }

        if (mRootDirectories == null) {
            mRootDirectories = new ArrayList<String>();
        } else {
        ...
        for (int i = 0; i < directories.length; i++) {
            if (directories[i] != null) {
                mDirectories.add(directories[i]);
                mRootDirectories.add(directories[i]);
            }
        }

        mEntryCache = new EntryCache(directories);
        
        while ( !mDirectories.isEmpty() ) {
            processDirectory(mDirectories.get(0), mPreClient);
            mDirectories.remove(0);
        }
        
        mEntryCache.flushAll();
    ...
    private boolean checkFastScanEnable(String path) {
        int storage_index = -1;
        ...
        boolean checkableStorageFlag = false;
        if (mVolumeList == null || mVolumeList.length <= storage_index || storage_index < 0)
            checkableStorageFlag = false;
        else if (mStrageFSTypeCache.get(mVolumeList[storage_index].getPath()) != null)
                checkableStorageFlag = true;
        
        return checkableStorageFlag;
    }
    
    class EntryCache {
        // hashes file path to FileEntry.
        // path should be lower case if mCaseInsensitivePaths is true
        private long mFolderCacheRowId;
        private HashMap<String, FileEntry> mFolderCache;
        private HashMap<String, FileEntry> mWFolderCache;
        private String[] mDirectories;
        MediaBulkDeleter mDeleter;

        EntryCache(String[] directories) {
            mDirectories = directories;

            if (mFolderCache == null) {
                mFolderCache = new HashMap<String, FileEntry>();
            } else {
                mFolderCache.clear();
            }

            if (mWFolderCache == null) {
                mWFolderCache = new HashMap<String, FileEntry>();
            } else {
                mWFolderCache.clear();
            }
            ...
            Uri.Builder builder = mFilesUri.buildUpon();
            builder.appendQueryParameter(MediaStore.PARAM_DELETE_DATA, "false");
            mDeleter = new MediaBulkDeleter(mMediaProvider, mPackageName, builder.build());

            makeWholeFolderCache();
        }
        
        public boolean isLeafNode(long rowId) {
            boolean isLeafNode = true;
            boolean isExistRowId = false;
            
            Iterator<FileEntry> iterator = mWFolderCache.values().iterator();
            while (iterator.hasNext()) {
                FileEntry entry = iterator.next();
            
                if( entry.mRowId == rowId )
                    isExistRowId = true;
                
                if( entry.mParentId == rowId )
                    isLeafNode = false;
            }

            if( debug_message ) {
                Log.d(TAG, "EntryCache.isLeafNode() " + rowId +" isLeafNode:" + isLeafNode + "isExistRowId:" + isExistRowId);
            }
            
            return isLeafNode && isExistRowId;
        }
        ...
        public boolean exist(String path) {
            ...
            FileEntry entry = mWFolderCache.get(key);
            
            if (entry != null) {
                entry.mSeenInFileSystem = true;
            }
            
            updateParentCache(path);

            entry = mFolderCache.get(key);

            if (entry != null) {
                entry.mSeenInFileSystem = true;
                
                if( debug_message ) {
                    Log.d(TAG, "EntryCache.exist()" + entry);
                }
                return true;
            }
            
            if( debug_message ) {
                Log.d(TAG, "EntryCache.exist() " + path + " do not exist!");
            }
        ...
        public FileEntry get(String path) {
            ...
            FileEntry wEntry = mWFolderCache.get(key);
            
            if (wEntry != null) {
                wEntry.mSeenInFileSystem = true;
            }
            
            updateParentCache(path);
            
            FileEntry entry = mFolderCache.get(key);

            if (entry != null) {
                entry.mSeenInFileSystem = true;
                
                if( debug_message ) {
                    Log.d(TAG, "EntryCache.get()" + entry);
                }
            }
            ...
            if( debug_message ) {
                Log.d(TAG, "EntryCache.get() " + path + " do not exist!");
            }
            
            return null;
        }
        
        public void setSubFolderSeenInFileSystem(long targetRowId) {
            ArrayList<Long> array = new ArrayList<Long>();
            array.add((Long)targetRowId);
            
            while( !array.isEmpty() ) {
                Iterator<FileEntry> iterator = mWFolderCache.values().iterator();
                long rowId = array.get(0);
            ...
            print_cache("set sub folder to seen in filesystem. rowId:" + targetRowId, mWFolderCache);
            
        }
        
        public void rmDir(long targetRowId) {
            ArrayList<Long> array = new ArrayList<Long>();
            ArrayList<Long> subFolderList = new ArrayList<Long>();
            
            array.add((Long)targetRowId);
            subFolderList.add((Long)targetRowId);
            
            if( debug_message ) {
                Log.d(TAG, "rmDir " + targetRowId + "====================================");
            }
            ...
            while( !array.isEmpty() ) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if( entry.mParentId == rowId ) {
                        ...
                        if( entry.mFormat == MtpConstants.FORMAT_ASSOCIATION ) {
                            array.add(entry.mRowId);
                            subFolderList.add(entry.mRowId);
                        }
                    }
                }
            ...
            while( !subFolderList.isEmpty() ) {
                long rowId = subFolderList.get(0);
                
                try {
                    mMediaProvider.delete(mPackageName, mFilesUri, "parent=" + rowId, null);
                    
                    if( debug_message ) {
                        Log.d(TAG, "delete parent= " + rowId);
                    }
            ...
            try {
                mMediaProvider.delete(mPackageName, mFilesUri, "_id=" + targetRowId, null);
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        }
        
        public void flush(HashMap<String, FileEntry> entryCache) {
            Iterator<FileEntry> iterator = entryCache.values().iterator();
            
    		StorageManager storageManager =
                    (StorageManager)mContext.getSystemService(Context.STORAGE_SERVICE);
            String[] externalStoragePaths = storageManager.getVolumePaths();
            String playlist_path = externalStoragePaths[0]+ "/Playlists";
            ...
            while (iterator.hasNext()) {
                ...
                if (!entry.mSeenInFileSystem && !MtpConstants.isAbstractObject(entry.mFormat) && !playlist_path.equals(path)) {
                    if (inScanDirectory(path, mDirectories)) {
                        // we didn't see this file in the scan directory.
                        fileMissing = true;
                    } else {
                        // the file actually a directory or other abstract
                        // object
                ...
                if (fileMissing) {
                    ...
                    if (path != null && path.startsWith("/")) {
                        ...
                        if (!exists && !MtpConstants.isAbstractObject(entry.mFormat)) {
                            // do not delete missing playlists, since they may
                            // have been
                            // modified by the user.
                            ...
                            try {
                                if (!MediaFile.isPlayListFileType(fileType)) {
                                    if (debug_message) {
                                        Log.d(TAG, "mEntryCache.flush delete "+ entry);
                                    }

                                    if( entry.mFormat == MtpConstants.FORMAT_ASSOCIATION ) {
                                        rmDir(entry.mRowId);
                                    } else {
                                        mDeleter.delete(entry.mRowId);
                                    }
        ...
        public void flushAll() {
            ...
            mEntryCache.print_cache("flushAll", mWFolderCache);
        }

        private void makeWholeFolderCache() {
            Cursor c = null;
            String where = null;
            String[] selectionArgs = null;

            where = Files.FileColumns.FORMAT + "=?";
            selectionArgs = new String[] {
                    String.valueOf(MtpConstants.FORMAT_ASSOCIATION)
            };
            
            for(int i=0; i<mDirectories.length; i++) {
            	if( mDirectories[i] == null )
            		continue;
            	
                String path = mDirectories[i];
                
                FileEntry entry = new FileEntry(0, path, 0, MtpConstants.FORMAT_ASSOCIATION, 0);
                ...
                mWFolderCache.put(key, entry);
            }

            // Build the list of files from the content provider
            try {
                // First read existing files from the files table
                c = mMediaProvider.query(mPackageName, mFilesUri, FILES_PRESCAN_PROJECTION,
                ...
                if (c != null) {
                    ...
                    while (c.moveToNext()) {
                        ...
                        int format = c.getInt(FILES_PRESCAN_FORMAT_COLUMN_INDEX);
                        long lastModified = c.getLong(FILES_PRESCAN_DATE_MODIFIED_COLUMN_INDEX);
                        long parentId = c.getLong(FILES_PRESCAN_PARENT_INDEX);
            ...
            print_cache("makeWholeFolderCache", mWFolderCache);
        }
        
        public void print_cache(String title, HashMap<String, FileEntry> cache) {
            if (debug_message) {
                Log.w(TAG, title + "============================================================");
                
                Iterator<FileEntry> iterator = cache.values().iterator();
                while (iterator.hasNext()) {
                    FileEntry entry = iterator.next();
                    Log.d(TAG, " " + entry);
                }
        ...
        private void makeFolderCache(long targetParentId) {
            Cursor c = null;
            String where = null;
            String[] selectionArgs = null;

            where = Files.FileColumns.PARENT + "=?";
            selectionArgs = new String[] {
                    String.valueOf(targetParentId)
            };
            
            if( debug_message ) {
                Log.d(TAG, "EntryCache.makeFolderCache() targetParentId:" + targetParentId + " +++++++++++++++++++++++++++++++++++++++++++++++++");
            }            
            ...
            if( debug_message ) {
                mEntryCache.print_cache("makeFolderCache " + targetParentId +" ========================================================", mWFolderCache);
            }            
            
        }

        private void updateParentCache(String path) {
            String parent = new File(path).getParent();
            ...
            if (entry == null) {
                if( debug_message ) {
                    Log.d(TAG, "EntryCache.updateCache(): " + key + " can not find in mWFolderCache!");
                }
                return;
            }
            ...
            if (entry.mRowId == mFolderCacheRowId)
                return;

            if( debug_message ) {
                Log.d(TAG, "EntryCache.updateCache(): key:" + key + " mFolderCacheRowId: " + mFolderCacheRowId + " entry.mRowId: " + entry.mRowId);
            }

            flush();

            mFolderCacheRowId = entry.mRowId;
            ...
            makeFolderCache(entry.mRowId);
        }
    }
    
    private void initialize(String volumeName) {
        debug_message = SystemProperties.getBoolean("persist.sys.scanner.print", false);
        
        if( SystemProperties.getBoolean("persist.sys.scanner.debug", false) ) {
        	Debug.waitForDebugger();
        }
        
        mNeedToScanFlag = false;
        ...
        mStorageInfoUri = Uri.parse("content://media/" + volumeName + "/storage_info");
        ...
        if (mStrageFSTypeCache == null) {
        	mStrageFSTypeCache = new HashMap<String, String>();
        } else {
        	mStrageFSTypeCache.clear();
        }
		...
		for (int i = 0; i < mVolumeList.length; i++) {
			String storageState = mStorageManager.getVolumeState(mVolumeList[i]
					.getPath());

			if (Environment.MEDIA_MOUNTED.equals(storageState)) {
				String fsType = getFileSystemType(mVolumeList[i].getPath());
                String bk_scan_state = "";
                
                Cursor c = null;
                try {
                    c = mMediaProvider.query(mPackageName, mStorageInfoUri,
                            new String[] { "bk_scan_state" }, "storage_id=?",
                            new String[] { Integer.toString(mVolumeList[i].getStorageId()) }, null, null);

                    if (c != null && c.getCount() > 0 && c.moveToFirst()) {
                        bk_scan_state = c.getString(0);
                    }
                } catch (RemoteException e) {
                    // TODO Auto-generated catch block
                    e.printStackTrace();
				...
				Log.d(TAG, mVolumeList[i].getPath() + " file system type : "
						+ fsType);

				if (bk_scan_state.equals("success") && (fsType.compareTo("ufsd") == 0 || fsType.compareTo("ext4") == 0) ) {
					mStrageFSTypeCache.put(mVolumeList[i].getPath(), fsType);
				}
			}
		}
    }
    ...
    private String getFileSystemType(String path)
 	{
         ...
         try {
             while ((line=buf.readLine())!=null) {
             	...
             	if(st.hasMoreTokens() )
             		devPath = st.nextToken();
             	
             	if(st.hasMoreTokens() )
             		mountPath = st.nextToken();

             	if(st.hasMoreTokens() )
             		fsType = st.nextToken();

             	if( mountPath.compareTo(path) == 0 ) {
             		return fsType;
             	}
             }
         } catch (IOException e) {
    ...
    public void scanDirectories(String[] directories, String volumeName) {
        try {
            ...
            if (ENABLE_BULK_INSERTS) {
                ...
                mMediaInserter = new MediaInserter(mMediaProvider, mPackageName, 1000);
            }
            
            //prescan(null, true);
            prescan(directories, null, true);
            ...
            mContext.sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_PRESCAN_FINISHED, uri));
            ...
            if( mNeedToScanFlag ) {
	            ...
	            while ( !mDirectories.isEmpty() ) {
	                if( debug_message ) {
	                    Log.d(TAG, "processDirectory: " + mDirectories.get(0));
	                }
	                
	                processDirectory(mDirectories.get(0), mClient);
	                mDirectories.remove(0);
	            }
            }
            ...
            if (true) {
                ...
                Log.d(TAG, "  scan(meta) time: " + (scan - prescan) + "ms (" + mMetaExtractTime +") \n");
                Log.d(TAG, "    postscan time: " + (end - scan) + "ms\n");
                Log.d(TAG, "  scan total time: " + (end - start) + "ms\n");
            }
    ...
    public Uri scanSingleFile(String path, String volumeName, String mimeType) {
        try {
            ...
            return mClient.doScanFile(path, mimeType, lastModifiedSeconds, file.length(),
                    false, true, MediaScanner.isNoMediaPath(path), true);
        } catch (RemoteException e) {
            Log.e(TAG, "RemoteException in MediaScanner.scanFile()", e);
            return null;
        }
        ...
        try {
            if (MediaFile.isPlayListFileType(fileType)) {
                ...
                mClient.doScanFile(path, mediaFileType.mimeType, lastModifiedSeconds, file.length(),
                    (format == MtpConstants.FORMAT_ASSOCIATION), true, isNoMediaPath(path), true);
            }
        } catch (RemoteException e) {
            Log.e(TAG, "RemoteException in MediaScanner.scanFile()", e);
        } finally {
