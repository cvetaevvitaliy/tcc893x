...
public class AudioManager {
    ...
    /*
        ...
        if ((vibrate && currentMode == AudioManager.RINGER_MODE_VIBRATE)
            ...
            Log.d(TAG, "Mute off-stream : " + stream);
        } else {
            setRingerMode(vibrate ? AudioManager.RINGER_MODE_VIBRATE
                    : AudioManager.RINGER_MODE_SILENT);
            Log.d(TAG, "Mute on-stream : " + stream);
        }
     */
        setStreamMute(STREAM_MUSIC, !isStreamMute(STREAM_MUSIC));
        adjustSuggestedStreamVolume(ADJUST_SAME, STREAM_MUSIC, FLAG_SHOW_UI | FLAG_VIBRATE);
    }
    ...
    */
     ...
     *           <var>false</var> to turn it off
     */
    public void setSPDIFPCMOn(boolean on){
        IAudioService service = getService();
        try {
            service.setSPDIFPCMOn(on);
        } catch (RemoteException e) {
            Log.e(TAG, "Dead object in setSPDIFPCMOn", e);
        }
    }

    /**
     * Checks whether the SPDIF PCM is on or off.
     *
     * @return true if SPDIF PCM is on, false if it's off
     */
    public boolean isSPDIFPCMOn() {
        IAudioService service = getService();
        try {
            return service.isSPDIFPCMOn();
        } catch (RemoteException e) {
            Log.e(TAG, "Dead object in isSPDIFPCMOn", e);
            return false;
        }
     }
    ...
    public void avrcpSupportsAbsoluteVolume(String address, boolean support) {
	 ...
	 /**
      * Sets a variable number of sampling rate values to audio hardware.
      *
      * @param rate sampling_rate value:
      *     8000, ..., 96000
      *
      */
     public void setForceSamplingRate(int rate) {
         AudioSystem.setForceSamplingRate(rate);
     }
