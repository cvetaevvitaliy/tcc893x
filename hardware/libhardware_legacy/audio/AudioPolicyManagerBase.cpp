...
void AudioPolicyManagerBase::setForceUse(AudioSystem::force_use usage, AudioSystem::forced_config config)
{
    ...
    case AudioSystem::FOR_MEDIA:
        if (config != AudioSystem::FORCE_HEADPHONES && config != AudioSystem::FORCE_BT_A2DP &&
            ...
            config != AudioSystem::FORCE_NO_BT_A2DP && config != AudioSystem::FORCE_SPDIF_PCM) {    // SPDIF PCM Planet 20121107
            ALOGW("setForceUse() invalid config %d for FOR_MEDIA", config);
            return;
        }
        mForceUse[usage] = config;
        // S: telechips		SPDIF PCM Planet 20121107 Start
		if (config == AudioSystem::FORCE_SPDIF_PCM) {
			ALOGE("setForceUse() route to %s", "SPDIF PCM");
            // request routing change if necessary
            for (size_t i = 0; i < mOutputs.size(); i++) {
                audio_io_handle_t output = mOutputs.keyAt(i);
                setOutputDevice(output, getDeviceForStrategy(STRATEGY_MEDIA, false));
            }
        }
		// E: telechips		SPDIF PCM Planet 20121107 End
        break;
...
status_t AudioPolicyManagerBase::setForceSamplingRate(uint32_t rate)
{
	ALOGE("~~~~~!!!!! AudioPolicyManagerBase.cpp setForceSamplingRate(): %d  Start !!!!!~~~~~", rate);

    String8 value;

    ALOGV("==== setForceSamplingRate: cur[%d],req[%d],  device: out[%x]in[%x],  mInputs.size()=%d,  ", 
                mOutputs.valueFor(mPrimaryOutput)->mSamplingRate, rate, 
                mAvailableOutputDevices, mAvailableInputDevices,
                mInputs.size());

	audio_io_handle_t activeInput = getActiveInput();
    if (activeInput != 0) {
        ALOGV("==== input is active...");
        return INVALID_OPERATION;
    }
	...
	/* ALSA is activate ? */
    if (mOutputs.indexOfKey(mPrimaryOutput) < 0) {
        ALOGV("setOutputDevice() unknown output %d", mPrimaryOutput);
        return NO_INIT;
    }

	/* Audio output is BT? */
    if((mOutputs.valueFor(mPrimaryOutput)->mDevice & AudioSystem::DEVICE_OUT_ALL)
    ...
    if(mOutputs.valueFor(mPrimaryOutput)->mSamplingRate == rate) {
		ALOGE("~~~~~~!!!!!!!! AudioPolicyManagerBase.cpp setForceSamplingRate() Same End !!!!!!!!!~~~~~~~~");
        return NO_ERROR;
    }
    else if (rate > 96000) {
        ALOGE("==== sampling rate is out of range. [Max Sampling rate: 96kHz]");
        return BAD_VALUE;
    }
    else if (rate < 8000) {
        //ALOGE("==== Set default sampling rate: 44.1kHz");
        if(mOutputs.valueFor(mPrimaryOutput)->mSamplingRate == 44100)
            return NO_ERROR;
        rate = 44100;
    }

    ALOGE("~~~~~!!!!! AudioPolicyManagerBase.cpp setForceSamplingRate() P001 !!!!!~~~~~");
	// set the sampling rate
    AudioParameter param = AudioParameter();
    param.addInt(String8(AudioParameter::keySamplingRate), (uint32_t)rate);
    mpClientInterface->setParameters(mPrimaryOutput, param.toString(), 0);

	// udpate sampling rate
    mOutputs.valueFor(mPrimaryOutput)->mSamplingRate = rate;

    ALOGE("~~~~~!!!!! AudioPolicyManagerBase.cpp setForceSamplingRate() P002 !!!!!~~~~~");
	return NO_ERROR;
}
                                                             ...
                                                             bool fromCache)
{
    ...
    case STRATEGY_MEDIA: {
        ...
        // S: telechips - SPDIF device setting Planet 20121204
        char value[PROPERTY_VALUE_MAX]={0};
        property_get("persist.sys.spdif_setting", value, "");
        
        if ((mForceUse[AudioSystem::FOR_MEDIA] == AudioSystem::FORCE_SPDIF_PCM) || (value[0] == '1')) {
            device2 = AudioSystem::DEVICE_OUT_SPDIF_PCM;                // SPDIF PCM Planet 20121107
        }
        // E: telechips
        ...
        // Planet 20130220 hdmi_on add Start
        if ((device2 == AUDIO_DEVICE_NONE) && (strategy != STRATEGY_SONIFICATION)) {
            // no sonification on aux digital (e.g. HDMI)
            device2 = mAvailableOutputDevices & AUDIO_DEVICE_OUT_AUX_DIGITAL;
        }
        // Planet 20130220 hdmi_on add End
        if (device2 == AUDIO_DEVICE_NONE) {
...
audio_devices_t AudioPolicyManagerBase::getDeviceForInputSource(int inputSource)
{
    ...
    case AUDIO_SOURCE_LINE_IN:
        device = AudioSystem::DEVICE_IN_LINE;
        break;
...
AudioPolicyManagerBase::device_category AudioPolicyManagerBase::getDeviceCategory(audio_devices_t device)
{
    switch(getDeviceForVolume(device)) {
        ...
        case AUDIO_DEVICE_OUT_SPDIF_PCM:    // SPDIF device setting Planet 20121204
...
const struct StringToEnum sDeviceNameToEnumTable[] = {
    ...
    STRING_TO_ENUM(AUDIO_DEVICE_OUT_SPDIF_PCM),     // SPDIF PCM Planet 20121107
