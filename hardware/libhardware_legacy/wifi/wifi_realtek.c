...
#ifdef SUPPORT_WPS_PBC
int unload_flag=0;
int wps_retry=0;
int wps_connected=0;
#define BUFF_SIZE 256
#define WPS_PBC_CMD "IFNAME=wlan0 WPS_PBC"
#define TERMINATE "IFNAME=wlan0 TERMINATE"
void* check_wps_push_button_thread(void *args)
{
	char buff;
	int ret,i;
	char reply[BUFF_SIZE];
	char event[BUFF_SIZE];
	size_t reply_len;
	int retry_num=0;
	
	ALOGD("check_wps_push_button_thread start");

	int fd = open("/dev/wps_pbc", O_RDONLY );
	if (fd < 0) {
		ALOGE("WPS: Cannot open \"%s\": %s", __func__, strerror(errno));
			return (void*)-1;
	}
	
	while(1) {
		if(unload_flag) {	
			ALOGD("WPS: check_wps_push_button_thread exit");
			return (void*)0;
		}
			
		ret = read(fd, &buff, 1);

		if( (buff == '0') && (wps_connected==0) )	{ // push button
			sleep(1);
			...
			ALOGD("WPS: wps button pushed---------------------------------");
retry:			
			ALOGD("WPS: try %d", retry_num);
			ret = wpa_ctrl_request(ctrl_conn, (const char*)WPS_PBC_CMD, strlen(WPS_PBC_CMD), &reply[0], &reply_len, NULL);
			if (ret == -2) {
				ALOGE("WPS: '%s' command timed out.\n",WPS_PBC_CMD);
			...
			for(i=0; i<130; i++) {
				...
				if(wps_connected == 1) {	// wps success				
				    if( wpa_ctrl_request(ctrl_conn, TERMINATE, strlen(TERMINATE), &reply[0], &reply_len, NULL) < 0 ) {	
				        ALOGD("WPS: TERMINATE fail!");
				    }
				    break;
				}
			sleep(1);
...
int wifi_start_supplicant(int p2p_supported)
{
    ...
    if(debug) ALOGD("%s: %s", __FUNCTION__, supplicant_name);
...
{
    char supp_status[PROPERTY_VALUE_MAX] = {'\0'};
    if(debug) ALOGD("%s: %s", __FUNCTION__, path);
...
#ifdef SUPPORT_WPS_PBC
	static pthread_t wps_push_button_thread;
	unload_flag = 0;
	if ( pthread_create(&wps_push_button_thread, NULL, check_wps_push_button_thread, NULL) < 0 )
		ALOGE("WPS: fail to create check_wps_push_button_thread");
#endif
...
int wifi_send_command(const char *cmd, char *reply, size_t *reply_len)
{
    ...
    if(debug) ALOGD("%s: %s", __FUNCTION__, cmd);
    ...
    if(debug) ALOGD("%s: %s", __FUNCTION__, reply);
    return 0;
}
...
int wifi_wait_on_socket(char *buf, size_t buflen)
{
    ...
    if(debug) ALOGD("%s: %s", __FUNCTION__, buf);
        ...
        if (match != NULL) {       
            if (match[1] == '<') {
            	...
            	if(strstr(buf,"WPS-TIMEOUT")) {
            		ALOGD("WPS: WPS-TIMEOUT");
            		wps_retry = 1;
            	}else if(strstr(buf,"WPS-FAIL")) {
            		ALOGD("WPS: WPS-FAIL");
            		wps_retry = 1;
            	}else if(strstr(buf,"WPS-CRED")) {
            		ALOGD("WPS: WPS-OK");
            	}else if(strstr(buf,"WPS-OVERLAP")) { 
            		ALOGD("WPS: WPS-OVERLAP");
                    wps_retry = 1;
            	}else if(strstr(buf,"CTRL-EVENT-CONNECTED")) {
                    ALOGD("WPS: WPS-CONNECTED");
            		wps_connected = 1;
    ...
    // check IFNAME=wlan0 Failed to initiate AP scan 'Failed to initiate AP scan'
    if(!strncmp(buf,"IFNAME=wlan0 Failed to initiate AP scan",39)) {
        ALOGE("Failed to initiate AP scan");
        //wifi_stop_supplicant(1);
        wifi_unload_driver();
        wifi_load_driver();
        //wifi_start_supplicant(1);
    }
...
const char *wifi_get_fw_path(int fw_type)
{
    ...
    if(debug) ALOGD("%s: %d", __FUNCTION__, fw_type);
    switch (fw_type) {
