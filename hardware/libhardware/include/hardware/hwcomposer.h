...
enum {
    ...
    /*
     * HWC_SKIP_MALI_FOR_VIDEO is set by HWcomposer to tell whether using mali skip routine during palying video and indicate that HWC
     * skip mali routine . instead handle that in DisplayHarware. it's not baed on each layer buf all of layer.
     */
    HWC_SKIP_MALI_FOR_VIDEO = 0x00000002,
    
    HWC_FB_FOR_VIDEO = 0x00000004,
};
...
//Telechips, ZzaU
	int (*fbTargetUpdate)(struct hwc_composer_device_1 *dev,
                buffer_handle_t primary_h, buffer_handle_t external_h,hwc_display_contents_1_t *fb_contents,hwc_display_contents_1_t *ex_contents);
