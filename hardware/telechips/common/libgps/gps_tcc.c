...
gps_state_start( GpsState*  s )
{
    ...
    ALOGD("%s",gps_idle_off);

    #if GPS_GPIO_INCLUDE
    D("HAL : gps -> ioctl : 0 -> gps module on");
...
static int
tcc_gps_init(GpsCallbacks* callbacks)
{
...
static int
tcc_gps_inject_time(GpsUtcTime time, int64_t timeReference, int uncertainty)
{
    D("%s: called", __FUNCTION__);                          // for Debug
    return 0;
}
...
static int
tcc_gps_inject_location(double latitude, double longitude, float accuracy)
{
    D("%s: called", __FUNCTION__);                          // for Debug
    return 0;
}
...
static void
tcc_gps_delete_aiding_data(GpsAidingData flags)
{
    D("%s: called", __FUNCTION__);                          // for Debug
}

//static int tcc_gps_set_position_mode(GpsPositionMode mode, int fix_frequency)
static int tcc_gps_set_position_mode(GpsPositionMode mode, GpsPositionRecurrence recurrence,
...
static const void*
tcc_gps_get_extension(const char* name)
{
