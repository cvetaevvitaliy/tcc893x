...
struct private_handle_t
{
	...
	int     share_fd; 	/*shared file descriptor for dma_buf sharing*/
	int fd_0;
	int fd_1;
	int fd_2;
	int usage;
...
#define GRALLOC_GTV_NUM_INTS 1	//for GTV

#ifdef __cplusplus
	static const int sNumInts = 23 + GRALLOC_ARM_UMP_NUM_INTS + GRALLOC_ARM_DMA_BUF_NUM_INTS + GRALLOC_GTV_NUM_INTS;
	...
	private_handle_t(int flags, int size, int base, int lock_state, ump_secure_id secure_id, ump_handle handle, int fd_0, int fd_1, int fd_2, int usage, int width, int height, int format, int bpp, int id_surface):
		share_fd(-1),
		magic(sMagic),
		flags(flags),
		size(size),
	...
	private_handle_t(int flags, int size, int base, int lock_state, int fd_0, int fd_1, int fd_2, int usage, int width, int height, int format, int bpp, int id_surface):
		share_fd(-1),
		magic(sMagic),
		flags(flags),
		size(size),
