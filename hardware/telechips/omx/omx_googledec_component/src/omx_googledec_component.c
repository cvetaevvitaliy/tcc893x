...
OMX_ERRORTYPE initDecoder(OMX_COMPONENTTYPE *openmaxStandComp) {
    ...
    if ((vpx_err = vpx_codec_dec_init((vpx_codec_ctx_t *)omx_private->mCtx, 
                                        omx_private->video_coding_type == OMX_VIDEO_CodingVP8 ? &vpx_codec_vp8_dx_algo : &vpx_codec_vp9_dx_algo, 
                                        &cfg, 
                                        0))) 
	{
...
static void SetInternalVideoParameters(OMX_COMPONENTTYPE *openmaxStandComp) {
	...
	if (video_coding_type == OMX_VIDEO_CodingVP8) 
    {
#ifndef OPENMAX1_2
    ...
    else if(video_coding_type == OMX_VIDEO_CodingVP9) 
    {
#ifndef OPENMAX1_2
		strcpy(omx_private->ports[OMX_BASE_FILTER_INPUTPORT_INDEX]->sPortParam.format.video.cMIMEType,"video/x-vnd.on2.vp9");
#endif
		omx_private->ports[OMX_BASE_FILTER_INPUTPORT_INDEX]->sPortParam.format.video.eCompressionFormat = OMX_VIDEO_CodingVP9;
...
OMX_ERRORTYPE omx_videodec_component_Init(OMX_COMPONENTTYPE *openmaxStandComp) {
	...
	if(video_coding_type == OMX_VIDEO_CodingVP8) 
    {
		return (omx_videodec_component_Constructor(openmaxStandComp, VIDEO_DEC_VP8_SW_NAME));
	}
    else if(video_coding_type == OMX_VIDEO_CodingVP9)
	{
		return (omx_videodec_component_Constructor(openmaxStandComp, VIDEO_DEC_VP9_SW_NAME));
	}
    else
       ;
...
OMX_ERRORTYPE omx_videodec_component_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,OMX_STRING cComponentName) {
	...
	OMX_U32 i;
		...
		if(!strcmp(cComponentName, VIDEO_DEC_VP8_SW_NAME)) { 
			omx_private->video_coding_type = OMX_VIDEO_CodingVP8;
		}
        else if(!strcmp(cComponentName, VIDEO_DEC_VP9_SW_NAME)) {
			omx_private->video_coding_type = OMX_VIDEO_CodingVP9;
		}
        else{
		...
		*/
		...
		inPort->sPortParam.format.video.nFrameWidth = 1920;
		inPort->sPortParam.format.video.nFrameHeight = 1080;
		...
		outPort->sPortParam.format.video.nFrameWidth = 1920;
		outPort->sPortParam.format.video.nFrameHeight = 1080;
		outPort->sPortParam.nBufferSize =  (OMX_U32) (1920*1080*3/2);
		outPort->sPortParam.format.video.xFramerate = 30;
		outPort->sPortParam.nBufferCountMin= 2;
		...
		if(omx_private->video_coding_type == OMX_VIDEO_CodingVP8) {
			omx_private->ports[OMX_BASE_FILTER_INPUTPORT_INDEX]->sPortParam.format.video.eCompressionFormat = OMX_VIDEO_CodingVP8;
		}

        if(omx_private->video_coding_type == OMX_VIDEO_CodingVP9) {
            omx_private->ports[OMX_BASE_FILTER_INPUTPORT_INDEX]->sPortParam.format.video.eCompressionFormat = OMX_VIDEO_CodingVP9;
        }
		...
		*/
		...
		if((omx_private->video_coding_type == OMX_VIDEO_CodingVP8)||(omx_private->video_coding_type == OMX_VIDEO_CodingVP9))
...
static int isPortChange(OMX_COMPONENTTYPE *openmaxStandComp, int width, int height)
{
		...
		height != omx_private->rectParm.nHeight)
	{
		...
		ALOGI(" CropInfo Changed %ld,%ld - %ldx%ld", omx_private->rectParm.nLeft, omx_private->rectParm.nTop, omx_private->rectParm.nWidth, omx_private->rectParm.nHeight);
	}
	...
	while (!nOutputFilled) {	    
        ...
        if (img != NULL) {
			...
			if(omx_private->gralloc_info.PortBuffers[OMX_BASE_FILTER_OUTPUTPORT_INDEX].BufferType == GrallocPtr) 
			{
				...
				pOutputBuffer->nFilledLen = (width * height) * 2;
	#else
...
#endif
			{
				...
				DBUG_MSG("No GrallocBuffer :: maybe Thumbnail :: VPX addr : %p/%p/%p -> %p", (const OMX_U8 *)img->planes[PLANE_Y], (const OMX_U8 *)img->planes[PLANE_U], (const OMX_U8 *)img->planes[PLANE_V], dst);
		...
		DBUG_MSG("Done OutLen %ld", pOutputBuffer->nFilledLen);
		nOutputFilled = 1;
	}
...
OMX_IN  OMX_PTR ComponentParameterStructure) {
  ...
  switch(nParamIndex) {
    case OMX_IndexParamPortDefinition:
      {
        ...
        if(eError == OMX_ErrorNone) {
		  ...
		  ALOGI(" CropInfo %ld,%ld - %ldx%ld", omx_private->rectParm.nLeft, omx_private->rectParm.nTop, omx_private->rectParm.nWidth, omx_private->rectParm.nHeight);
        }
        break;
      }
    ...
    case OMX_IndexParamStandardComponentRole:
	{
		...
		if (!strcmp((char *)pComponentRole->cRole, VIDEO_DEC_VP8_SW_ROLE)) {
			omx_private->video_coding_type = OMX_VIDEO_CodingVP8;
		}

        if (!strcmp((char *)pComponentRole->cRole, VIDEO_DEC_VP9_SW_ROLE)) {
            omx_private->video_coding_type = OMX_VIDEO_CodingVP9;
        }
		SetInternalVideoParameters(openmaxStandComp);
		break;
    ...
    case OMX_IndexParamStandardComponentRole:
      {
        ...
        if (omx_private->video_coding_type == OMX_VIDEO_CodingVP8) {
          strcpy((char *)pComponentRole->cRole, VIDEO_DEC_VP8_SW_ROLE);
        }
        if (omx_private->video_coding_type == OMX_VIDEO_CodingVP9) {
          strcpy((char *)pComponentRole->cRole, VIDEO_DEC_VP9_SW_ROLE);
        }
        break;
      }
#ifndef OPENMAX1_2
    ...
    default: /*Call the base component function*/
    {
		...
		if( nParamIndex == OMX_IndexParamPortDefinition )
		{
			...
			else
			{
				...
				DBUG_MSG("pPortParam->eColorFormat(%d)", pPortDef->format.video.eColorFormat);
			}
		}
...
OMX_ERRORTYPE omx_videodec_component_ComponentRoleEnum(
  ...
  OMX_IN OMX_U32 nIndex) {
	...
	if (nIndex == 0) {
		...
		if(omx_private->video_coding_type == OMX_VIDEO_CodingVP8) {
			strcpy((char *)cRole, VIDEO_DEC_VP8_SW_ROLE);
		}

        if(omx_private->video_coding_type == OMX_VIDEO_CodingVP9) {
            strcpy((char *)cRole, VIDEO_DEC_VP9_SW_ROLE);
        }
    ...
    case OMX_IndexConfigCommonOutputCrop:
		{
			...
			DBUG_MSG("OMX_IndexConfigCommonOutputCrop: %d,%d ~ %dx%d", rectParams->nLeft, rectParams->nTop, rectParams->nWidth, rectParams->nHeight);
	    }
		break;
	...
	case OMX_IndexConfigCommonScale:
		{
			...
			DBUG_MSG("OMX_IndexConfigScale: 0x%lx/0x%lx", scale->xWidth, scale->xHeight);
		}
		break;
  ...
  OMX_OUT OMX_INDEXTYPE* pIndexType) {
	...
	if(strcmp(cParameterName,"OMX.tcc.index.config.videoextradata") == 0) {
		...
		*pIndexType = (OMX_INDEXTYPE) NULL;
		...
		ALOGE("OMX_ErrorBadParameter  %s - %s \n", __func__, cParameterName);
		return OMX_ErrorBadParameter;
	}
#endif
...
OMX_ERRORTYPE OMX_ComponentInit(OMX_HANDLETYPE openmaxStandComp, OMX_STRING cCompontName, OMX_HANDLE_FUNC pHandleFunc)
{
	...
	if(err == OMX_ErrorNone)
	{
		...
		{
            ALOGE("ERROR: can't load gralloc using hw_get_module(%s) => err[0x%x]", GRALLOC_HARDWARE_MODULE_ID, err);
