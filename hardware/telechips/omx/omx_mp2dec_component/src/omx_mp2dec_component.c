...
OMX_ERRORTYPE OMX_ComponentInit(OMX_HANDLETYPE openmaxStandComp, OMX_STRING cCompontName, OMX_HANDLE_FUNC pHandleFunc)
{
  OMX_ERRORTYPE err = OMX_ErrorNone;
	...
	err = omx_mp2dec_component_Constructor(openmaxStandComp,cCompontName,pHandleFunc);
...
OMX_ERRORTYPE omx_mp2dec_component_Constructor(OMX_COMPONENTTYPE *openmaxStandComp, OMX_STRING cComponentName, OMX_HANDLE_FUNC pHandleFunc) 
{
	...
	*/
	...
	omx_mp2dec_component_Private->pExternalDec = pHandleFunc;
	omx_mp2dec_component_Private->OpenCodec = OpenMP2Dec;
	omx_mp2dec_component_Private->DecodeCodec = DecodeMP2;
	omx_mp2dec_component_Private->FlushCodec = FlushMP2Dec;
	omx_mp2dec_component_Private->CloseCodec = CloseMP2Dec;	
...
//	memset(&omx_mp2dec_component_Private->cdmx_out, 0x00, sizeof(cdmx_output_t));
	...
	memset(&omx_mp2dec_component_Private->stStreamInfo, 0x00, sizeof(audio_streaminfo_t));
	memset(&omx_mp2dec_component_Private->stPcmInfo, 0x00, sizeof(audio_pcminfo_t));
	memset(&omx_mp2dec_component_Private->stADecInit, 0x00, sizeof(adec_init_t));	
	
	omx_mp2dec_component_Private->iAudioProcessMode = AUDIO_NORMAL_MODE; /* decoded pcm mode */
...
static OMX_S32 OpenMP2Dec(OMX_COMPONENTTYPE *openmaxStandComp, OMX_BUFFERHEADERTYPE* inputbuffer)
{
	omx_mp2dec_component_PrivateType* pPrivate = openmaxStandComp->pComponentPrivate;
	OMX_S32 ret	= 0;
	...
	// setting stream info
	pPrivate->stStreamInfo.m_eSampleRate = pPrivate->pAudioMp2.nSampleRate;
	pPrivate->stStreamInfo.m_uiNumberOfChannel = pPrivate->pAudioMp2.nChannels;
	
	pPrivate->stADecInit.m_psAudiodecInput = &pPrivate->stStreamInfo;
	pPrivate->stADecInit.m_psAudiodecOutput = &pPrivate->stPcmInfo;

	pPrivate->stADecInit.m_unAudioCodecParams.m_unMP2.m_iDABMode = (pPrivate->iAudioProcessMode == AUDIO_BROADCAST_MODE) ? 1 : 0;
...
static OMX_S32 DecodeMP2(OMX_COMPONENTTYPE* openmaxStandComp, OMX_BUFFERHEADERTYPE* inputbuffer, OMX_BUFFERHEADERTYPE* outputbuffer)
{
	...
	if(iRet == TCAS_ERROR_INVALID_BUFSTATE)
	{
...
static OMX_S32 FlushMP2Dec(OMX_COMPONENTTYPE* openmaxStandComp)
{
	omx_mp2dec_component_PrivateType* pPrivate = openmaxStandComp->pComponentPrivate;
...
static OMX_S32 CloseMP2Dec(OMX_COMPONENTTYPE* openmaxStandComp)
{
	omx_mp2dec_component_PrivateType* pPrivate = openmaxStandComp->pComponentPrivate;
