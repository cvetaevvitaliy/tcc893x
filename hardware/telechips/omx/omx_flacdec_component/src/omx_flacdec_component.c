...
OMX_ERRORTYPE OMX_ComponentInit(OMX_HANDLETYPE openmaxStandComp, OMX_STRING cCompontName, OMX_HANDLE_FUNC pHandleFunc)
{
  OMX_ERRORTYPE err = OMX_ErrorNone;
	...
	err = omx_flacdec_component_Constructor(openmaxStandComp,cCompontName,pHandleFunc);
...
#if 1
	omx_flacdec_component_Private->pExternalDec = pHandleFunc;
	omx_flacdec_component_Private->OpenCodec = OpenFLACDec;
	omx_flacdec_component_Private->DecodeCodec = DecodeFLAC;
	omx_flacdec_component_Private->FlushCodec = FlushFLACDec;
	omx_flacdec_component_Private->CloseCodec = CloseFLACDec;
#endif
...
//	memset(&omx_flacdec_component_Private->cdmx_out, 0x00, sizeof(cdmx_output_t));
	...
	memset(&omx_flacdec_component_Private->stStreamInfo, 0x00, sizeof(audio_streaminfo_t));
	memset(&omx_flacdec_component_Private->stPcmInfo, 0x00, sizeof(audio_pcminfo_t));
	memset(&omx_flacdec_component_Private->stADecInit, 0x00, sizeof(adec_init_t));		
		
	omx_flacdec_component_Private->iAudioProcessMode = 2; /* decoded pcm mode */
...
{
	omx_flacdec_component_PrivateType* pPrivate = openmaxStandComp->pComponentPrivate;
	OMX_S32 ret	= 0;
	...
	// setting stream info
	pPrivate->stStreamInfo.m_eSampleRate = pPrivate->pAudioFlac.nSampleRate;
	pPrivate->stStreamInfo.m_uiNumberOfChannel = pPrivate->pAudioFlac.nChannels;
	...
	pPrivate->stADecInit.m_unAudioCodecParams.m_unFLAC.m_iFlacInitMode = 1;
