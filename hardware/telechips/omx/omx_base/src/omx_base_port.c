...
#ifdef HAVE_ANDROID_OS
  (*openmaxStandPort)->Port_Allocate_SecuredInBuffer = &base_port_Allocate_SecuredInBuffer;
  (*openmaxStandPort)->Port_UseBuffer2 = &base_port_UseBuffer2;
  (*openmaxStandPort)->Port_Free_SecuredInBuffer = &base_port_Free_SecuredInBuffer;
#endif
...
//ALOGI("alloc[%d] :: type[%d] header(0x%x), pBuffer(0x%x - 0x%x)", i, openmaxStandPort->bBufferStateAllocated[i], openmaxStandPort->pInternalBufferStorage[i], *pBuffer, openmaxStandPort->pInternalBufferStorage[i]->pBuffer);
...
OMX_ERRORTYPE base_port_FreeBuffer(
  ...
  for(i=0; i < openmaxStandPort->sPortParam.nBufferCountActual; i++){
    if (openmaxStandPort->bBufferStateAllocated[i] & (BUFFER_ASSIGNED | BUFFER_ALLOCATED)) {
		...
		{
			//ALOGD("free[%d] : type[%d], header(0x%x), pBuffer(0x%x - 0x%x)", i, openmaxStandPort->bBufferStateAllocated[i], openmaxStandPort->pInternalBufferStorage[i], pBuffer, openmaxStandPort->pInternalBufferStorage[i]->pBuffer);
...
#ifdef HAVE_ANDROID_OS
OMX_ERRORTYPE base_port_Allocate_SecuredInBuffer(
  omx_base_PortType *openmaxStandPort,
  OMX_BUFFERHEADERTYPE** pBuffer,
  OMX_U32 nPortIndex,
  OMX_PTR pAppPrivate,
  ...
  for(i=0; i < openmaxStandPort->sPortParam.nBufferCountActual; i++){
    if (openmaxStandPort->bBufferStateAllocated[i] == BUFFER_FREE) {
	  ...
	  if(openmaxStandPort->sPortParam.eDir == OMX_DirInput)  //ANdroid
      	openmaxStandPort->pInternalBufferStorage[i]->pBuffer = nStartAddr + (nActualSizebytes*i); 
	  else
	  	openmaxStandPort->pInternalBufferStorage[i]->pBuffer = TCC_calloc(1,nSizeBytes); 	
	...
	//  	  	ALOGE("%s %d End\n",__func__,__LINE__);
//ALOGI("alloc2[%d] :: type[%d] header(0x%x), pBuffer(0x%x - 0x%x)", i, openmaxStandPort->bBufferStateAllocated[i], openmaxStandPort->pInternalBufferStorage[i], *pBuffer, openmaxStandPort->pInternalBufferStorage[i]->pBuffer);
 ...
 * When all the bufers are done, the variable bIsEmptyOfBuffers is set to OMX_TRUE
 */
OMX_ERRORTYPE base_port_Free_SecuredInBuffer(
  omx_base_PortType *openmaxStandPort,
  OMX_U32 nPortIndex,
  OMX_BUFFERHEADERTYPE* pBuffer) {
  ...
  for(i=0; i < openmaxStandPort->sPortParam.nBufferCountActual; i++){
    if (openmaxStandPort->bBufferStateAllocated[i] & (BUFFER_ASSIGNED | BUFFER_ALLOCATED)) {
		if(pBuffer == openmaxStandPort->pInternalBufferStorage[i]) //ZzaU :: to arrange free-order
		{		
//			ALOGD("free2[%d] :: type[%d] header(0x%x), pBuffer(0x%x - 0x%x)", i, openmaxStandPort->bBufferStateAllocated[i], openmaxStandPort->pInternalBufferStorage[i], pBuffer, openmaxStandPort->pInternalBufferStorage[i]->pBuffer);
			...
			if (openmaxStandPort->bBufferStateAllocated[i] & BUFFER_ALLOCATED) {
				...
				openmaxStandPort->pInternalBufferStorage[i]->pBuffer = NULL; // do not anything.
			}
