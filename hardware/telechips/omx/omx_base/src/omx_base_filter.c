...
void* omx_base_filter_BufferMgmtFunction (void* param) {
  ...
  while(omx_base_filter_Private->state == OMX_StateIdle || omx_base_filter_Private->state == OMX_StateExecuting ||  omx_base_filter_Private->state == OMX_StatePause || 
...
#ifdef OPENMAX1_2
      (omx_base_filter_Private->state != OMX_StateLoaded && omx_base_filter_Private->state != OMX_StateReserved_0x00000000)) {
#else
...
#ifdef OPENMAX1_2
    if(omx_base_filter_Private->state == OMX_StateLoaded || omx_base_filter_Private->state == OMX_StateReserved_0x00000000) {
#else
    ...
    if(isInputBufferNeeded==OMX_FALSE && isOutputBufferNeeded==OMX_FALSE) {
	  ...
	  if(((pInputBuffer->nFlags & OMX_BUFFERFLAG_EOS) == 1) && (omx_base_filter_Private->nDelayedOutputBufferCount > 0))
		  pInputBuffer->nFlags |= OMX_BUFFERFLAG_DO_PEND_EOF;

      if (omx_base_filter_Private->BufferMgmtCallback && ((pInputBuffer->nFilledLen > 0) || (pInputBuffer->nFlags & OMX_BUFFERFLAG_DO_PEND_EOF))) {
      ...
      if(((pInputBuffer->nFlags & OMX_BUFFERFLAG_EOS) == 1) && (pInputBuffer->nFilledLen == 0) && (omx_base_filter_Private->nDelayedOutputBufferCount <= 0) ) {
        pOutputBuffer->nFlags=pInputBuffer->nFlags;
        pInputBuffer->nFlags=0;
        (*(omx_base_filter_Private->callbacks->EventHandler))
          (openmaxStandComp,
		...
		pthread_mutex_lock(&omx_base_component_Private->bPortChangeSem->mutex);
		if(PORT_IS_BEING_CHANGED(pOutPort))
		{	
			ALOGD("PORT_IS_BEING_CHANGED before !! ");
			//tsem_wait(omx_base_component_Private->bPortChangeSem);
			pthread_cond_wait(&omx_base_component_Private->bPortChangeSem->condition, &omx_base_component_Private->bPortChangeSem->mutex);
			ALOGD("PORT_IS_BEING_CHANGED after !!  ");
		}
		pthread_mutex_unlock(&omx_base_component_Private->bPortChangeSem->mutex);
...
//    } // removed by daniel
    ...
    if((isInputBufferNeeded == OMX_FALSE) && (pInputBuffer->nFilledLen == 0) && ((pInputBuffer->nFlags & OMX_BUFFERFLAG_DO_PEND_EOF) == 0)) {
      pInPort->ReturnBufferFunction(pInPort,pInputBuffer);
      inBufExchanged--;
      pInputBuffer=NULL;
      isInputBufferNeeded=OMX_TRUE;
