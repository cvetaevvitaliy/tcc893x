...
OMX_ERRORTYPE OMX_ComponentInit(OMX_HANDLETYPE openmaxStandComp, OMX_STRING cCompontName, OMX_HANDLE_FUNC pHandleFunc)
{
  OMX_ERRORTYPE err = OMX_ErrorNone;
	...
	err = omx_mp3dec_component_Constructor(openmaxStandComp,cCompontName,pHandleFunc);
...
OMX_ERRORTYPE omx_mp3dec_component_Constructor(OMX_COMPONENTTYPE *openmaxStandComp, OMX_STRING cComponentName, OMX_HANDLE_FUNC pHandleFunc) 
{
	...
	*/
	...
	omx_mp3dec_component_Private->pExternalDec = pHandleFunc;
	omx_mp3dec_component_Private->OpenCodec = OpenMP3Dec;
	omx_mp3dec_component_Private->DecodeCodec = DecodeMP3;
	omx_mp3dec_component_Private->FlushCodec = FlushMP3Dec;
	omx_mp3dec_component_Private->CloseCodec = CloseMP3Dec;
...
//	memset(&omx_mp3dec_component_Private->cdmx_out, 0x00, sizeof(cdmx_output_t));
	...
	memset(&omx_mp3dec_component_Private->stStreamInfo, 0x00, sizeof(audio_streaminfo_t));
	memset(&omx_mp3dec_component_Private->stPcmInfo, 0x00, sizeof(audio_pcminfo_t));
	memset(&omx_mp3dec_component_Private->stADecInit, 0x00, sizeof(adec_init_t));
		
	omx_mp3dec_component_Private->iAudioProcessMode = AUDIO_NORMAL_MODE; /* decoded pcm mode */
...
{
	omx_mp3dec_component_PrivateType* pPrivate = openmaxStandComp->pComponentPrivate;
	OMX_S32 ret	= 0;
	...
	// setting stream info
	pPrivate->stStreamInfo.m_eSampleRate = pPrivate->pAudioMp3.nSampleRate;
	pPrivate->stStreamInfo.m_uiNumberOfChannel = pPrivate->pAudioMp3.nChannels;
	
	pPrivate->stADecInit.m_psAudiodecInput = &pPrivate->stStreamInfo;
	pPrivate->stADecInit.m_psAudiodecOutput = &pPrivate->stPcmInfo;
	
	pPrivate->stADecInit.m_unAudioCodecParams.m_unMP321Dec.m_iLayer = 0;
	
	if(pPrivate->iExtractorType == OMX_BUFFERFLAG_EXTRACTORTYPE_TCC)
	{
		DBUG_MSG("%s , %d\n",__func__, pPrivate->cdmx_info.m_sAudioInfo.m_iFormatId);

		/* MPEG 1/2/2.5 Layer 1,2,3 Decoder */
		switch( pPrivate->cdmx_info.m_sAudioInfo.m_iFormatId )
		{
		case AV_AUDIO_MP3:
			pPrivate->stADecInit.m_unAudioCodecParams.m_unMP321Dec.m_iLayer = 3;
			break;
		case AV_AUDIO_MP2:	// for future use
			pPrivate->stADecInit.m_unAudioCodecParams.m_unMP321Dec.m_iLayer = 2;
			break;
		case AV_AUDIO_MP1:
			pPrivate->stADecInit.m_unAudioCodecParams.m_unMP321Dec.m_iLayer = 1;
			break;
		default:
			pPrivate->stADecInit.m_unAudioCodecParams.m_unMP321Dec.m_iLayer = 3;
		}
	...
	pPrivate->stADecInit.m_unAudioCodecParams.m_unMP321Dec.m_iEnableLayer = 0;	// for future use
	pPrivate->stADecInit.m_unAudioCodecParams.m_unMP321Dec.m_iDABMode = (pPrivate->iAudioProcessMode == AUDIO_BROADCAST_MODE) ? 1 : 0;	// for future use

	ret = pPrivate->pExternalDec(AUDIO_INIT, &pPrivate->iDecoderHandle, &pPrivate->stADecInit, NULL);

	pPrivate->iMinStreamSize = 32;
...
static OMX_S32 DecodeMP3(OMX_COMPONENTTYPE* openmaxStandComp, OMX_BUFFERHEADERTYPE* inputbuffer, OMX_BUFFERHEADERTYPE* outputbuffer)
{
	...
	DBUG_MSG("start: input length %d", pPrivate->stStreamInfo.m_iStreamLength);
	...
	while((pPrivate->stStreamInfo.m_iStreamLength > pPrivate->iMinStreamSize ) || ((pPrivate->stStreamInfo.m_iStreamLength > 0) && (pPrivate->iEndOfFile == 1)))
	{
		...
		/*if(iOutFrameSize + pPrivate->stPcmInfo.m_uiNumberOfChannel * pPrivate->stPcmInfo.m_uiSamplesPerChannel * sizeof(short) > outputbuffer->nAllocLen) 
		{
			ALOGE("too many sample!");
			break;
		}*/
		
		iRet = pPrivate->pExternalDec(AUDIO_DECODE, &pPrivate->iDecoderHandle, &pPrivate->stStreamInfo, &pPrivate->stPcmInfo);

		DBUG_MSG("ret %d, len %d", iRet, pPrivate->stStreamInfo.m_iStreamLength);
