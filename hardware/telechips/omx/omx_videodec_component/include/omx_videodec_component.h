...
//#define ANDROID_USE_STOREMETADATAINBUFFERS
#include <hardware/gralloc.h>
#include <hardware/hardware.h>
#ifdef ANDROID_USE_STOREMETADATAINBUFFERS
#include <hardware/HardwareAPI.h>
#include <hardware/MetadataBufferType.h>
#endif
#endif

//#define SKIP_ERROR_MB_FRAME //to skip frame which have error macroblock.
#define TS_ADJUSTMENT // to play video smooth when changing resolution.

#define VPU_BUFFER_MANAGEMENT
...
typedef enum {
        kNumInputBuffers  = 8, // 4
        kNumOutputBuffers = 4, // 2
        kNumNativeMinBuffers = 2,
}BUFFER_COUNT_NUM;
...
//	int       m_iDTS[32];								   //! Decode Timestamp (decoding order)
	OMX_TICKS m_iDTS[32];//[usec]						   //! Decode Timestamp (decoding order)
...
//	int       m_iextTimeStamp;							   //! TR(RV)
	OMX_TICKS m_iTimeStamp;	//[usec]					   //! Time Stamp
	OMX_TICKS m_iextTimeStamp;//[usec]					   //! TR(RV)
...
typedef struct VBUFFER_MANAGEMENT{
  OMX_U32 Display_out_addr; // a address for output buffer in component;
  OMX_BOOL Display_cleared;
  OMX_S32 Display_Buff_ID;
  OMX_S32 Display_index;
  OMX_S32 Keep_index; // in case of not-vsync and on-the-fly mode, it means current index displaying.
} VBUFFER_MANAGEMENT;
...
/*  cdmx_info_t cdmx_info;*/ \
  ...
  /** @param private data for VPU ringbuffer activation */\
  void* pVpuRingPrivate; \
  ...
  void* 			mSecureInMapInfo; \
  pmap_t			mUmpSecuredInPmap; \
  ...
  OMX_U32 MVC_Base_addr0; \
  OMX_U32 MVC_Base_addr1; \
  OMX_U32 MVC_Base_addr2; \
  OMX_U32 curr_width; \
  OMX_U32 curr_height; \
  OMX_BOOL bDetected_res_changed; \
  OMX_BOOL bNoOutput_after_res_changed; \
  OMX_BOOL bSet_Except_I_Frm; \
  OMX_TICKS prev_timestamp; \
  OMX_TICKS prev_interval; \
  OMX_CONFIG_SCALEFACTORTYPE scale; \
  /** @start for TimeStamp adjustment */ \
  OMX_BOOL bNeed_TS_adjustment; \
  OMX_BOOL bTopDown_adjustmemt; \
  OMX_TICKS uTS_last_output; \
  OMX_BOOL bCheck_same_index; \
  OMX_S32 iDx_last_output; \
  nsecs_t nST_last_output; \
  OMX_TICKS uTS_new_base; \
  OMX_TICKS uTS_curr_output; \
  OMX_BOOL bDRAM_16bit; \
  OMX_U32 isTry_count_for_copying; \
  OMX_BOOL bWFD_mode; \
  /** @end for TimeStamp adjustment */ \
  /** @param Secured input buffer mode flag */ \
  OMX_BOOL bSecuredInBufferMode; \
  OMX_BOOL bWFDSyncMode; \
  OMX_BOOL bMVC_mode; \
  OMX_BOOL bTEEEnable; \
  OMX_BOOL bWFDRingBuffer; \
  OMX_BOOL bWFDVPURef; \
  OMX_BOOL bWFDDisplayLV; \
  OMX_BOOL bWFDDisplayEnable; \
  OMX_BOOL bWFDErrorFrame; \
  OMX_U32  mWFD_PrevVPUPOC; \
  OMX_BOOL bWFDHDCP2SrcEnable; \
  pmap_t   mSeqBackupPmap; \
  void*    mSeqBackupMapInfo; \
  OMX_BOOL bNoBufferDelay;    \
