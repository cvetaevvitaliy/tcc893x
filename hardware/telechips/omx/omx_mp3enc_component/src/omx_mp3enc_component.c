...
#define DBUG_MSG2(msg...)	if (DEBUG2_ON) { ALOGD( ": " msg);}
#endif /* HAVE_ANDROID_OS */

static OMX_HANDLE_FUNC pEncFunc = NULL;
...
//			ALOGD( "m_iDataSize : %ld nFilledLen : %ld", omx_mp3enc_component_Private->cmux_input.m_iDataSize, omx_mp3enc_component_Private->temporary_buffer->nFilledLen);
...
//				outputbuffer->pBuffer    = omx_mp3enc_component_Private->cdk_core.m_pOutWav; // encoded data buffer
		...
		if(omx_mp3enc_component_Private->temporary_buffer->nFilledLen < 4608)
		{
//			ALOGE("22. omx_mp3enc_component_Private->temporary_buffer->nFilledLen < 4608");
			inputbuffer->nFilledLen = 0;
			omx_mp3enc_component_Private->isNewBuffer = 1;
		}
		else
		{
//			ALOGD("22. inputbuffer->nFilledLen = 1");
...
//	if((inputbuffer->nFlags & OMX_BUFFERFLAG_CODECCONFIG) && omx_mp3enc_component_Private->mp3_encode_ready == OMX_FALSE)
	...
	if(omx_mp3enc_component_Private->mp3_encode_ready == OMX_FALSE)
	{
		ALOGE(" MP3 Encoder not Initialized!!");
		return;
	}
...
#if 0
	{
		...
		ALOGD(" Length (%d)", inputbuffer->nFilledLen);
		ALOGD("+++ %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x",
  ...
  OMX_IN  OMX_PTR ComponentParameterStructure)
{
	...
	switch(nParamIndex)
	{
		...
		case OMX_IndexParamAudioPcm:
			...
			ALOGI("@@@@ :: nChannels = %d, nBitPerSample = %d, nSamplingRate = %d",
...
OMX_ERRORTYPE omx_mp3enc_component_Constructor(OMX_COMPONENTTYPE *openmaxStandComp, OMX_STRING cComponentName, OMX_HANDLE_FUNC pHandleFunc)
{
...
//    inPort->sPortParam.nBufferCountActual = 1152 * 2 * 2;
	...
	omx_mp3enc_component_Private->cb_function 	= pHandleFunc;//TCC_MP3_ENC;
...
OMX_ERRORTYPE omx_mp3enc_component_LibDeinit(omx_mp3enc_component_PrivateType* omx_mp3enc_component_Private)
{
	...
	{
		ALOGE("[DBG_MP3_ENC]  ==> Audio Dec close error\n");
...
{
	return omx_mp3enc_component_Constructor(openmaxStandComp,AUDIO_ENC_MP3_NAME,pEncFunc);
}
...
OMX_ERRORTYPE omx_mp3enc_component_MessageHandler(OMX_COMPONENTTYPE* openmaxStandComp, internalRequestMessageType *message)
{
	...
	if (message->messageType == OMX_CommandStateSet){
		if ((message->messageParam == OMX_StateExecuting) && (eCurrentState == OMX_StateIdle)) {
			...
			if(err!=OMX_ErrorNone) {
				ALOGE("In %s mp3 encoder Init Failed Error=%x\n",__func__,err);
				return err;
			}
			...
			if(err!=OMX_ErrorNone) {
				ALOGE("In %s mp3 encoder Deinit Failed Error=%x\n",__func__,err);
				return err;
			}
		}
	}
...
OMX_ERRORTYPE OMX_ComponentInit(OMX_HANDLETYPE openmaxStandComp, OMX_STRING cCompontName, OMX_HANDLE_FUNC pHandleFunc)
{
  OMX_ERRORTYPE err = OMX_ErrorNone;
	...
	pEncFunc = pHandleFunc;
	err = omx_mp3enc_component_Constructor(openmaxStandComp,cCompontName,pHandleFunc);
