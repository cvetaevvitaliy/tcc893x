...
static void mem_prepare() {
	...
	{
		ALOGE("mmap failed. fd(%d), base addr(0x%x), size(%d)", enc_private->mFd, enc_private->mVideomap.base, enc_private->mVideomap.size);
		memset(&enc_private->mVideomap, 0, sizeof(pmap_t));
...
static int mem_get_addr(unsigned int *out_virtaddr, unsigned int in_phyaddr) {
	if (enc_private->mMapInfo != MAP_FAILED) {
		*out_virtaddr = (unsigned int)((unsigned int)enc_private->mMapInfo + (in_phyaddr - enc_private->mVideomap.base));
		return 1;
	}
...
static int enc_init(tENC_PRIVATE *enc_private)
{
	...
	ALOGI("ENC Info :: %dx%d, %d fps, %d interval, %d Kbps", enc_private->gsVEncInit.m_iPicWidth, enc_private->gsVEncInit.m_iPicHeight, 
					enc_private->gsVEncInit.m_iFrameRate, enc_private->gsVEncInit.m_iKeyInterval, enc_private->gsVEncInit.m_iTargetKbps);
	
	if( (ret = venc_vpu( VENC_INIT, NULL, &enc_private->gsVEncInit, (void*)&enc_private->use_NalStartCode )) != RETCODE_SUCCESS )
	{
		ALOGE( "[Err:%d] VENC_INIT failed", ret );
...
//variable init
	...
	enc_private->gsVEncInit.m_iWFDTranscoding   = pInit->isWFDmode;
...
int ENCODER_CLOSE(void)
{
	...
	{
		if( (ret = venc_vpu( VENC_CLOSE, NULL, NULL, NULL )) != RETCODE_SUCCESS )
		{
			ALOGE( "[Err:%d] VENC_CLOSE failed", ret );
		}
		enc_private->isVPUClosed = 1;
	}
...
{	
	...
	{
		ALOGI("%d'th Restore Encode-Error", restred_count);
	...
	{
		ALOGE("Vpu already is closed because of Error!!");
		return -1;
	}
...
//Send the first output buffer such as CodecSpecificData
	if(enc_private->iConfigDataFlag == 0)
	{
		...
		{		
			...
			if( ret != RETCODE_SUCCESS )
			{
				ALOGE( "[Err:-0x%x] VENC_SEQ_HEADER failed\n", -ret );
				enc_private->isEncError = 1;
				goto ERR_PROCESS;
			}

			pOutput->outputStreamVirtAddr = enc_private->gsVEncSeqHeader.m_pSeqHeaderOut;
			pOutput->outputStreamPhyAddr = vpu_getSeqHeaderPhyAddr(enc_private->gsVEncSeqHeader.m_pSeqHeaderOut, VA);
	...
	else
	{
		...
		if( enc_private->qp_value == 0 && !(pInput->targetKbps != 0 && pInput->targetKbps != enc_private->curr_targetKbps))
		{
			...
			ALOGE("QP for I-Frame - Initial value(%d)", enc_private->gsVEncInput.m_iQuantParam);
		}
#endif	
		...
		if(pInput->targetKbps != 0 && pInput->targetKbps != enc_private->curr_targetKbps)
		{
			...
			ALOGE("Bitrate- Change(%d)", enc_private->gsVEncInput.m_iChangeTargetKbps);
#ifdef CHANGE_QP_FOR_IFRAME
			enc_private->gsVEncInput.m_iQuantParam = getInitQpND(enc_private->curr_targetKbps, enc_private->gsVEncInit.m_iPicWidth, enc_private->gsVEncInit.m_iPicHeight);
            enc_private->gsVEncInput.m_iChangeRcParamFlag = (0x10 | 0x1);
			ALOGE("QP for I-Frame - Change(%d)", enc_private->gsVEncInput.m_iQuantParam);
#endif			
		}
		else if(pInput->frameRate != 0 && pInput->frameRate != enc_private->curr_frameRate)
		{
			...
			ALOGE("FrameRate- Change(%d)", enc_private->gsVEncInput.m_iChangeFrameRate);
		}

		if(pInput->isForceIFrame){
			enc_private->gsVEncInput.request_IntraFrame = 1;
			ALOGE("IntraRefreshVOP");
		}
		else{
		...
		if( ret != RETCODE_SUCCESS )
		{
			...
			ALOGE( "[Err:-0x%x] VENC_ENCODE failed\n", -ret );
			goto ERR_PROCESS;
		}
		
		pOutput->frameLen = enc_private->gsVEncOutput.m_iBitstreamOutSize;
		pOutput->outputStreamVirtAddr = enc_private->gsVEncOutput.m_pBitstreamOut;
		pOutput->outputStreamPhyAddr = vpu_getStreamOutPhyAddr(enc_private->gsVEncOutput.m_pBitstreamOut, VA);
		...
		{
			unsigned char *p = pOutput->outputStreamVirtAddr;
		...
		if(1 == mem_get_addr(&addr_cvt, (unsigned int)vpu_getStreamOutPhyAddr(pOutput->outputStreamVirtAddr, VA)))
		{
			pOutput->outputStreamVirtAddr = (unsigned char*)addr_cvt;
		}
	}
...
int TCC_VENC_Init_H264(unsigned int *pInitParam)
{
	...
	InitParam.isWFDmode 		= pInitParam[5];
...
int TCC_VENC_Enc(unsigned int *pInputStream, unsigned int *pOutstream)																				
{
	...
	if(OutputStream.headerLen)
		Display_Stream(OutputStream.outputStreamVirtAddr,OutputStream.headerLen);
	else
		Display_Stream(OutputStream.outputStreamVirtAddr,OutputStream.frameLen);
#endif
