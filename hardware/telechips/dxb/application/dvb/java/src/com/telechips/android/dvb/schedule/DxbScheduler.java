...
public class DxbScheduler extends Activity
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		getPlayer().isDxbScheduleActivityON = true;
	...
	{
		DxbLog(I, "~~~~~~~~~~~~~~~~> onStop()");
		getPlayer().isDxbScheduleActivityON = false;
		super.onStop();
		DxbLog(I, "~~~~~~~~~~~~~~~~> onStop() finish");
	}
	...
	private void setBundleData(Bundle bundle)
	{
		...
		long tempTOT;
		if(gInformation.eventData.lUTC_baseTOT <= 0)
		{
			tempTOT	= System.currentTimeMillis();
		}
		else
		{
			tempTOT	= gInformation.eventData.lUTC_baseTOT;
		}
		gInformation.dateBase_TOT		= new Date(tempTOT);
		...
		dateCurrent_TOT	= new Date(tempTOT);
	...
	private void check_scheduleTime()
	{
		...
		if(gInformation.dbSchedule != null)
		{
			...
			int	iCount_schedule	= gInformation.curSchedule.getCount();
			gInformation.curSchedule.moveToFirst();

			if(iCount_schedule > 0)
			{
				int		iRepeat	= gInformation.curSchedule.getInt(ScheduleDB.iColumnIndex_Repeat);
				int		iCount	= gInformation.curSchedule.getInt(ScheduleDB.iColumnIndex_Count);
				long	lStart	= gInformation.curSchedule.getLong(ScheduleDB.iColumnIndex_UTC_Start);
				
				long	lCurrent	= gInformation.dateBase_TOT.getTime() + getUTC_System_gap();
				
				long	curTime;
				if(iRepeat == 1)
				{
					curTime	= lStart + (24*60*60*1000)*iCount;	//	1-daily : 24H:60M:60S:1000m
				}
				else if(iRepeat == 2)
				{
					curTime	= lStart + (7*24*60*60*1000)*iCount;	//	2-weekly : 7D:24H:60M:60S:1000m
				}
				else
				{
					curTime	= lStart;
				...
				DxbLog(D, "lCurrent = " + lCurrent);
				DxbLog(D, "curTime = " + curTime);
				if(lCurrent >= curTime)
				{
					gInformation.cScheduler.reset(gInformation.mContext, gInformation.dbSchedule, lCurrent);
				}
			}
		}
	}
...
//							", [" + gInformation.dateCurrent_System.getTime() + "]   " + gInformation.dateCurrent_System.getHours() + ":" + gInformation.dateCurrent_System.getMinutes() + ":" + gInformation.dateCurrent_System.getSeconds());
		...
		public void onClick(View v)
		{
			...
			switch(id)
			{
				...
				case R.id.schedule_add_date:
					gInformation.pickerDate.updateDate(gInformation.dateStart.getYear()+1900, gInformation.dateStart.getMonth(), gInformation.dateStart.getDate());
	...
	private void setState(STATE state)
	{
		...
		if(state == STATE.ADD || state == STATE.EDIT)
		{
			...
			{
				indexTV	= gInformation.eventData.iIndexService;
				indexRadio	= 0;
			}
			else// if(gInformation.eventData.iChannelType == 1)
			{
				indexTV	= 0;
				indexRadio	= gInformation.eventData.iIndexService;
			}
	...
	private void displayScreen()
	{
		...
		{
			if(gInformation.eventData.iMode == 0)
			{
				gInformation.dateEnd.setTime(-1);
			}
			else
			{
				gInformation.dateEnd.setTime(gInformation.dateStart.getTime() + 60*1000);
			}
	...
	private String getText_No()
	{
		...
		{
			return	String.format("%03d", tempList.get(gInformation.eventData.iIndexService).iLogical_channel);
		}
	...
	private void setChannelType(int iIndex)
	{
		...
		)
		{
			...
			if(gInformation.eventData.iChannelType == 0)
			{
				setChannelNumber(indexTV);
			}
			else
			{
				setChannelNumber(indexRadio);
			}
		}
	}
	...
	private void setMode(int iMode)
	{
		...
		if(gInformation.eventData.iMode != iMode)
		{
			...
			if(		(gInformation.eventData.iMode == 1)
				&&	(gInformation.dateEnd.getTime() == -1)
			)
			{
				gInformation.dateEnd.setTime(gInformation.dateStart.getTime() + 60*1000);
			}
	...
	private void accessDB_insert()
	{
		...
		if(gInformation.dbSchedule != null)
		{
			...
			else if(gInformation.eState == STATE.EDIT)
			{
				...
				if(		(state != Scheduler.STATE_SUCCESS)
					&&	(state != gInformation.iID_EditEvent)
				)
				{
					displayErrorMSG(state);
					return;
	...
	private void displayErrorMSG(long state)
	{
		String	sMsg	= "";
		...
		{
			int	index	= 0;
			Cursor curSchedule	= gInformation.dbSchedule.getSchedule();
			if(curSchedule != null)
			{
				int iCount	= curSchedule.getCount();
				if(iCount > 0)
				{
					...
					{
						if(state == curSchedule.getInt(ScheduleDB.iColumnIndex_ID))
						{
							index	= i+1;
							break;
						}
			...
			sMsg	= getResources().getString(R.string.this_schedule_is_conflict_with_event) + index;
			DxbLog(D, "state=" + state);
			DxbLog(D, "index=" + index);
		}
	...
	private BroadcastReceiver mReceiverIntent = new BroadcastReceiver()
	{
		...
		public void onReceive(Context context, Intent intent)
		{
			...
			{
				DxbLog(D, "if(Alert.ACTION_IS_ALERT.equals(action))");
