...
import com.telechips.android.dvb.player.ChannelManager;
...
//		int		serviceType	= cursor.getInt(iIndex_serviceType);
		...
		int scrambling_info1 = cursor.getColumnIndexOrThrow(ChannelManager.KEY_FREE_CA_MODE); //free_ca_mode in sdt
		int scrambling_info2 = cursor.getColumnIndexOrThrow(ChannelManager.KEY_SCRAMBLING); //cas descriptor in pmt
	    scrambling_info1	= cursor.getInt(scrambling_info1);
	    scrambling_info2	= cursor.getInt(scrambling_info2);
	   
	    Log.d(TAG, serviceName + " : Scramble Infromation " + scrambling_info1 +" " + scrambling_info1);

	    if((scrambling_info1 == 1 && !getPlayer().isDVB_S2()) || scrambling_info2 == 1)
