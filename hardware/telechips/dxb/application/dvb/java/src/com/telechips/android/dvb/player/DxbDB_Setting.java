...
import com.telechips.android.dvb.*;
...
public class DxbDB_Setting {
	...
	private static final String KEY_RELATIVE_VOL	= "relative_vol";
	...
	private static final String DATABASE_FEEDTABLE_CREATE =		"create table "
															...
															+	KEY_PASSWORD		+ " text, "
															+	KEY_AGE				+ " integer, "
															+	KEY_RELATIVE_VOL	+ " integer )";

	String[] mColumns = new String[] { KEY_ROWID, KEY_POSITION_TV, KEY_POSITION_RADIO, KEY_TIME_ZONE, KEY_PASSWORD, KEY_AGE, KEY_RELATIVE_VOL};
	...
	{
		DVBPlayerActivity	mContext	= (DVBPlayerActivity)mCtx;
	...
	public void loadSettings(SQLiteDatabase db)
	{
		...
		cv.put(KEY_TIME_ZONE, getPlayer().DEFAULT_VALUE_TIME_ZONE_INDEX);
		cv.put(KEY_PASSWORD, getPlayer().DEFAULT_VALUE_PASSWORD);
		cv.put(KEY_AGE, getPlayer().DEFAULT_VALUE_AGE);
		cv.put(KEY_RELATIVE_VOL, getPlayer().DEFAULT_VALUE_RELATIVE_VOL);
	...
	public int getRelativeVol() {
		Log.i(TAG, "getRelativeVol()");
		int	age;
		Cursor cursor = mDb.query(DATABASE_SETTINGTABLE, mColumns, KEY_ROWID + "=" + 1, null, null, null, null, null);
		if (cursor == null || cursor.getCount() <= 0) {
			if (cursor != null) {
	...
	public void putRelativeVol(int age) {
		ContentValues cv = new ContentValues();
		cv.put(KEY_RELATIVE_VOL, age);
		mDb.update(DATABASE_SETTINGTABLE, cv, KEY_ROWID + "=" + 1, null);
	}
}
