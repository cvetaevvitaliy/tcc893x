...
import com.telechips.android.dvb.player.dvb.TeletextData;
...
public class DxbTeletext extends ImageView implements DVBTPlayer.OnTeletextDataUpdateListener {
	...
	private boolean misTTXSubtDraw = false, misStartTeletext=false;	
	...
	public DxbTeletext(Context context)
	{
		this(context, null);
	}
	
	public DxbTeletext(Context context, DxbPlayerBase parent)
	{
		super(context);
	...
	public void setPlayer(DVBTPlayer player)
	{
		...
		if (mPlayer != null)
		{
			mPlayer.setOnTeletextDataUpdateListener(this);
		}
	}

	public void onTeletextDataUpdate(DVBTPlayer player, TeletextData teletextData)
	{
		DxbLog(D, "onTeletextDataUpdate()");
		Rendering(teletextData, false, false);
	}
	...
	{						
		...
		float scaleX = 1;
		if(fontname == "system") {
			DxbLog(D, "Set TTX font = " + fontname);
        ...
        DxbLog(D, fontname + "-" + scaleX + "-" + underline + "-" + height);
	}
	...
	public void Rendering(TeletextData pPage, boolean bErase, boolean bforceDisplay)
	{
		...
		if(updateScreen)
		{
			...
			{
				mParent.onTeletextDataUpdate(mPlayer, mBitmap);
			}
		}
	}
	...
	public void startSubtitle(int i)
	{
		misTTXSubtDraw = true;
		startTeletext();
		setTeletext_UpdatePage(cTTX.Subtitle[i].iPage);
	}
	
	public void stopSubtitle()
	{
		stopTeletext();
		misTTXSubtDraw = false;
	}

	public boolean isPlayTTX()
	{
		return misStartTeletext && !misTTXSubtDraw;
	}
	...
	public void setTeletext_UpdatePage(int pageNumber) {
		synchronized(mParent) {
			if (mPlayer == null) {
				DxbLog(E, "[setTeletext_UpdatePage] mPlayer is null");
				return;
			}
	...
	private void playTeletext(int onoff) {// 0-off, 1-on, 2-subtitle
		synchronized(mParent) {
			if (mPlayer == null) {
				DxbLog(E, "[playTeletext] mPlayer is null");
				return;
			}
