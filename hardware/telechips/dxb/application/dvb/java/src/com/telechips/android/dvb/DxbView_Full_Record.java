...
import com.telechips.android.dvb.schedule.Alarm;
	...
	public void setVisible(boolean v)
	{
		...
		if(getPlayer().getRecordState() == REC_STATE.RECORDING)
		{
			if(v)
			{
				...
				mbuStop.setVisibility(View.VISIBLE);
			...
			else
			{
				...
				mbuStop.setVisibility(View.GONE);
...
//		{
			if(getPlayer().getRecordState() == REC_STATE.STOP)
			{
				...
				{
					eRecType = recType;
					//getPlayer().setRecordState(REC_STATE.RECORDING);
					...
					{
						sFullPath_recording = DxbStorage.getPath_Device() + "/DVB_TimeShift";
						sFileName_recording = DxbStorage.getPath_Device() + "/DVB_TimeShift";
					}
					else 
					{
						sFullPath_recording =	DxbStorage.getPath_Device()
											...
											+	strServiceName
											+	CurTime_Str
											+	DxbStorage.getPath_Extension_Record();
						sFileName_recording = strServiceName + CurTime_Str + DxbStorage.getPath_Extension_Record();
					}
...
//						mbuStop.requestFocus();
			...
			else if(getPlayer().getRecordState() == REC_STATE.RECORDING)
			{
				...
				//getPlayer().setRecordState(REC_STATE.SAVE);

				if(eRecType != TYPE.ALARM)
				{
					getPlayer().setScreenOn(true);
					getPlayer().setPlayerOn(true);
				}
...
//				}
			...
			else
			{
				...
				//getPlayer().setRecordState(REC_STATE.STOP);
				
				if(!getPlayer().isScreenOn())
				{
					getPlayer().goToSleep();
				}
				if(!getPlayer().isPlayerOn())
				{
					getContext().finish();
				}
			}
