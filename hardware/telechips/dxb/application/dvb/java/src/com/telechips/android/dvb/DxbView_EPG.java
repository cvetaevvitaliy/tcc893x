													...
													OnItemClickListener, OnItemSelectedListener
{
	...
	private void setTextSize()
	{
		...
		//	detail info
		mtvName_Current.setTextSize(getTextSize(20));
		mtvDetail_Current.setTextSize(getTextSize(18));
		
		//	Navi
		mtvDate_Current.setTextSize(getTextSize(20));
	}
	...
	public void setVisible(boolean v)
	{
		...
		if (v)
		{
			...
			if(		(getPlayer().getChannelCount() > 0)
				&&	(getPlayer().getRecordState() != REC_STATE.RECORDING)
				&&	getPlayer().isSTB()
			)
			{
	...
	private void updateSpinner_Type()
	{
		...
		if(getPlayer().getChannelCount(1) > 0)
		{
		...
		else
		{
			...
			{
				spinnerAdapter_Type	= new ArrayAdapter<String>(getContext(), R.layout.dxb_spinner_25px, Items);
			}
			else
			{
				spinnerAdapter_Type	= new ArrayAdapter<String>(getContext(), R.layout.dxb_spinner_12px, Items);
			}
		}
	...
	private void updateSpinner_List()
	{
		...
		else
		{
			if(getDisplayWidth() >= 1024)
			{
				spinnerAdapter_List	= new ArrayAdapter<String>(getContext(), R.layout.dxb_spinner_25px, getPlayer().getNameList(getServiceType()));
			}
			else
			{
				spinnerAdapter_List	= new ArrayAdapter<String>(getContext(), R.layout.dxb_spinner_12px, getPlayer().getNameList(getServiceType()));
			}
		}
	...
	{
		int	iStartMJD	= cur.getInt(miIndex_startMJD);
		
		String	date	= DxbUtil.getMJD_Date(iStartMJD);
	...
	private void updateList()
	{
		...
		{
			mScheduleType = 0;
			requestEPGUpdate(miEpgUpdate_day);
			if(mepgCursor != null)
			{
				if(!mepgCursor.isClosed())
				{
			...
			mepgCursor = getPlayer().getEPG_Schedule();
...
//			getContext().startManagingCursor(c);
	...
	private void setEventData(Cursor cursor)
	{
		if(isValidateCussor(cursor))
		{
			int	startHH, startMM, durationHH, durationMM;
			int	MJD;
			long	lUTC_Start, lUTC_End;
			
			MJD			= cursor.getInt(miIndex_startMJD);
			startHH		= cursor.getInt(miIndex_startHH);
			startMM		= cursor.getInt(miIndex_startMM);
