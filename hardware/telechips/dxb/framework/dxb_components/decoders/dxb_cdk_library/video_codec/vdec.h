...
void vpu_update_sizeinfo(unsigned int format, unsigned int bps, unsigned int fps, unsigned int image_width, unsigned int image_height, void * pInst);
unsigned char *vpu_getBitstreamBufAddr(unsigned int index, void* pVdec);
int vpu_getBitstreamBufSize(void *pVdec);
unsigned char *vpu_getFrameBufVirtAddr(unsigned char *convert_addr, unsigned int base_index, void* pInst);
#endif
