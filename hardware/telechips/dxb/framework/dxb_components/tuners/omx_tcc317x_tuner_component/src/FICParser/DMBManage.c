...
#define LOG_TAG	"TCC317X_DMBMANAGE"
#include <utils/Log.h>
#include "fig.h"
#include "Dmb_Types.h"
#include "TDMBInfo.h"
...
FIC_INFO_T gFicInfo[MAX_FICPARSER_SUPPRT_BB];
unsigned char g_DMBBuffer[MAX_FICPARSER_SUPPRT_BB][DMB_BUFFER_SIZE];
...
void DAB_Buff_Init(int m_idx)
{
	int i;	
	FIC_INFO_T * pFicInfo;

	if(m_idx >= MAX_FICPARSER_SUPPRT_BB)
		return;
	
	pFicInfo = &gFicInfo[m_idx];
...
void DAB_Buffer_Alignment( int m_idx, unsigned char *Inst /** <DMB DAB 운영 버퍼를 위한 배열 주소> */)
{
	uint size=0;	
	FIC_INFO_T * pFicInfo;
...
INT32U
DMB_BufferInit(int m_idx, PINT8U Inst)
{
	DAB_Buffer_Alignment(m_idx, Inst);
	DAB_Buff_Init(m_idx);
	return 1;
}

INT32U
DMB_IsChannelUpdate(int m_idx)
{
...
INT32U
DMB_TakeChannelDBFromFIC(int m_idx, PCHANNELDB pChannel, INT32U SvcCompIndex, INT32U SubChIndex)
{
	INT16U	uiUEPval;
 ...
 */
INT32U
DMB_TakeSCINFOFromFIC(int m_idx, PSC_INFO pscinfo ,PINT8U pcaflag,INT32U ServiceId, INT32U SubChId )
{
...
PENSEMBLEDB
DMB_TakeEnsembleTreeFormFIC(int m_idx, PENSEMBLEDB pEnsemble)
{
	...
	{
		pService = DMB_GetServiceDB(nTuner);
		if(pService == NULL)
		{
			return (NULL);
		}
		...
		for( SvcCompIndex = 0; SvcCompIndex < MaxSvcCompCount; SvcCompIndex++ )
		{
			if ( pFicInfo->psSvcComponent[SvcCompIndex].SId == pFicInfo->psService[SvcIndex].SId )
			{
				for( SubChIndex = 0; SubChIndex < MaxSubChCount; SubChIndex++ )
				{
					...
					{
						pChannel = DMB_GetChannelDB(nTuner);
						...
						DMB_TakeChannelDBFromFIC(nTuner, pChannel, SvcCompIndex, SubChIndex );
						...
						pChannel->UserApplType = DMB_GetUserAppTy(nTuner, pService, pChannel);						
