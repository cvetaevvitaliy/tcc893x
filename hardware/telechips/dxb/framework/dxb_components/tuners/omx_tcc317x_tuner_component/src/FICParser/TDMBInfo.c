...
#define LOG_TAG	"TCC317X_TDMBINFO"
#include <utils/Log.h>
...
PENSEMBLEDB		g_tEnsembleDBList[MAX_FICPARSER_SUPPRT_BB];
PSERVICEDB		g_tServiceDBList[MAX_FICPARSER_SUPPRT_BB];
PCHANNELDB		g_tChannelDBList[MAX_FICPARSER_SUPPRT_BB];

unsigned int	g_tEnsembleDBMaxSize[MAX_FICPARSER_SUPPRT_BB];
unsigned int	g_tServiceDBMaxSize[MAX_FICPARSER_SUPPRT_BB];
unsigned int	g_tChannelDBMaxSize[MAX_FICPARSER_SUPPRT_BB];
...
void
DMB_InitDB(int m_idx, PDMB_INITDBINFO pInitDBInfo)
{
	INT32U i;
	...
	g_tEnsembleDBList[m_idx] = pInitDBInfo->pEnsembleDBList;
	g_tServiceDBList[m_idx] = pInitDBInfo->pServiceDBList;
	g_tChannelDBList[m_idx] = pInitDBInfo->pChannelDBList;

	g_tEnsembleDBMaxSize[m_idx] = pInitDBInfo->ensembleDBMaxSize;
	g_tServiceDBMaxSize[m_idx] = pInitDBInfo->serviceDBMaxSize;
	g_tChannelDBMaxSize[m_idx] = pInitDBInfo->channelDBMaxSize; 

	for(i=0;i<g_tEnsembleDBMaxSize[m_idx];i++)
	{
		pEnsemble = &g_tEnsembleDBList[m_idx][ i ];
		
		DMBDB_Init( pEnsemble, i );
		DMB_InitEnsembleDB( pEnsemble );
		
		if ( i != (g_tEnsembleDBMaxSize[m_idx] - 1) )
		{
			pEnsembleNext = &g_tEnsembleDBList[m_idx][ i+1 ];			
	...
	for(i=0;i<g_tServiceDBMaxSize[m_idx];i++)
	{
		pService = &g_tServiceDBList[m_idx][i];
		
		DMBDB_Init( pService, i );
		DMB_InitServiceDB( pService );

		if ( i != (g_tServiceDBMaxSize[m_idx] - 1) )
		{
			pServiceNext = &g_tServiceDBList[m_idx][ i+1 ];
	...
	for(i=0;i<g_tChannelDBMaxSize[m_idx];i++)
	{
		pChannel = &g_tChannelDBList[m_idx][i];

		DMBDB_Init( pChannel, i );
		DMB_InitChannelDB( pChannel );

		if ( i != (g_tChannelDBMaxSize[m_idx] - 1) )
		{
			pChannelNext = &g_tChannelDBList[m_idx][ i+1 ];
...
PENSEMBLEDB
DMB_GetEnsembleDB(int m_idx)
{
	return (PENSEMBLEDB) DMBDB_Alloc( &g_tEnsembleDBList[m_idx][ 0 ] );
}

PSERVICEDB
DMB_GetServiceDB(int m_idx)
{
	return (PSERVICEDB) DMBDB_Alloc( &g_tServiceDBList[m_idx][ 0 ] );
}

PCHANNELDB
DMB_GetChannelDB(int m_idx)
{
	return (PCHANNELDB) DMBDB_Alloc( &g_tChannelDBList[m_idx][ 0 ] );
}
...
PENSEMBLEDB DMB_GetEnsembleServiceNumber( PENSEMBLEDB pEnsembleDB, PINT32U pVideo, 
	PINT32U pAudio, PINT32U pData, PINT32U pAudio_Plus, PINT32U pTotal, TDMB_INFO *pINFO )
{
	PSERVICEDB	pRootServiceDB;
	PCHANNELDB	pRootChannelDB;
			...
			{	
				if ( pRootChannelDB->Type == 1 )		//dab
				{
					*pAudio = *pAudio + 1;
					...
					memcpy(pINFO[*pTotal].Ensemble_Label ,pRootEnsembleDB->Label, ENSEMBLE_LABEL);
					memcpy(pINFO[*pTotal].Service_Label ,pRootServiceDB->Label, SERVICE_LABEL);
					memcpy(pINFO[*pTotal].Channel_Label,pRootChannelDB->SCLabel, CHANNEL_LABEL);
