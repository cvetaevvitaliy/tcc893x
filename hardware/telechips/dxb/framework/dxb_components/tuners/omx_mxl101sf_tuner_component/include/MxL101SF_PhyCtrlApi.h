...
/* Macro to calculate SNR value */
#define CALCULATE_SNR(data)        ((1563 * data) - 25000)

/* Macro to calculate BER value */
#define CALCULATE_BER(avg_errors)  (UINT32)(avg_errors * 4 * 3247)

/* Macro to indicate tuner only mode */
#define MXL_TUNER_MODE         0

/* Macro to indicate tuner-demod(SOC) mode */
...
typedef struct
{
  ...
  void *PrivateDataPtr;     /* IN or OUT, pointer to private data */
...
typedef struct
{
  UINT8 I2cSlaveAddr;       /* IN, I2C Address of MxL Device */
  void *PrivateDataPtr;     /* IN or OUT, pointer to private data */
} MXL_CABLE_CFG_T, *PMXL_CABLE_CFG_T;
...
typedef struct
{
  UINT8 I2cSlaveAddr;       /* IN, I2C Address of MxL Device */
  void *PrivateDataPtr;     /* IN or OUT, pointer to private data */
} MXL_RESET_IRQ_CFG_T, *PMXL_RESET_IRQ_CFG_T;
...
typedef struct
{
  UINT8 I2cSlaveAddr;       /* IN, I2C Address of MxL Device */
  void *PrivateDataPtr;     /* IN or OUT, pointer to private data */
} MXL_DEV_MPEG_OUT_CFG_T, *PMXL_DEV_MPEG_OUT_CFG_T;
...
typedef struct
{
  UINT8 I2cSlaveAddr;       /* IN, I2C Address of MxL Device */
  void *PrivateDataPtr;     /* IN or OUT, pointer to private data */
} MXL_RESET_PEC_CFG_T, *PMXL_RESET_PEC_CFG_T;
...
typedef enum
{   
  MXL_DEV_TUNER_MODE = MXL_TUNER_MODE,      /* Device in tuner only mode */
  MXL_DEV_TUNER_DEMOD_MODE = MXL_SOC_MODE   /* Device in tuner+demodulator mode */
} MXL_DEV_MODE_E;
...
{
  UINT8 I2cSlaveAddr;             /* IN, I2C Address of MxL Device */
  MXL_DEV_MODE_E DeviceMode;      /* IN, Operational mode of MxL101SF */
  void *PrivateDataPtr;           /* IN or OUT, pointer to private data */
...
typedef struct
{
  ...
  UINT32 IF_Freq;                 /* IN, IF in Hz for non-predefined frequencies */
  void *PrivateDataPtr;           /* IN or OUT, pointer to private data */
...
{
  UINT8 I2cSlaveAddr;                       /* IN, I2C Address of MxL Device */
  UINT32 Frequency;                         /* IN, Lastest Channel RF used for tuning */
...
{
  UINT8 I2cSlaveAddr;                  /* IN, I2C Address of MxL Device */
  MXL_CHAN_SCAN_CTRL_E ChanScanCtrl;   /* IN, Max Locking level required while scaning */
  MXL_RF_TUNE_CFG_T ChanTuneCfg;       /* IN, Channel RF value, BW & TPS cell id data */
  MXL_BOOL UseExtendedChanTune;        /* IN, Enable or disable extended chan tune for scanning */
...
{
  UINT8 I2cSlaveAddr;           /* IN, I2C Address of MxL Device */
  MXL_BOOL SpectrumCfg;         /* IN, Enable/Disable inverted signal processing */
  void *PrivateDataPtr;         /* IN or OUT, pointer to private data */
