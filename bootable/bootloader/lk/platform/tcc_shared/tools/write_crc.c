...
static BOOL write_first_crc(FILE *fd, unsigned int ramdisk_size, unsigned int len)
{
    if (fd) {
        ...
        do {
            ...
            readable_size = (len - readable_size) > 1024 ? 1024 : (len - readable_size);
	...
	if (len <= MIN_IMAGE_SIZE){
        ...
        if (fwrite(headers_buf, 1, HEADERS_SIZE, fd) == HEADERS_SIZE) {
            ...
            if (write_second_crc(fd)) {
                return write_first_crc(fd, p_input_info->ramdisk_size , image_length);
            }
        }
    }
    return FALSE;
