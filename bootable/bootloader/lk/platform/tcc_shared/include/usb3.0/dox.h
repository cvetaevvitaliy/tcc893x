 ...
 *
 * The DWC_usb3 Subsystem Version 2.10a is a host and peripheral controller
 ...
 *
 * The 2.10a version supports SuperSpeed USB 3.0, and HS and FS USB 2.0.
 *
 ...
 * be shipped with the DWC_usb3 Version 2.10a controller. It specifies what
 * the software will do, not how it will accomplish these tasks.
 *
 * The driver software for DWC_usb3 Version 2.10a will support only Linux as
 ...
 * controller driver software. See the OTG 2.0/3.0 user manual for a description
 ...
 * between operating environments. However, the 2.10a version of the driver has
 * only been compiled and tested under Linux 2.6.37.1.
 *
 ...
 * That's why the PCD is considered a hardware abstraction layer.
 *
 ...
 *
 * All driver components will be developed for the Linux 2.6 operating
 ...
 * - Patches to the Linux File-Backed Storage Gadget Driver source code
 ...
 * applying any required patches will be included in the release. Standard
 * Linux components are required to run the demo software. These components
 * include:
 *
 * - File-Backed Storage Gadget Driver (This is a Function Driver that acts as
 *   a mass storage device and stores/retrieves data from the file system.)
 *
 * @subsection demo_sw_subsec Demo Software
 *
 * No need for special demo software is anticipated for the 2.10a release.
 ...
 *
 * No binaries will be distributed with the 2.10a release.
 *
 */
 ...
 * the Linux HCD framework. The OS-supplied xHCI HCD will implement the hc_driver
 * interface.
 *
 ...
 *
 * This module also defines data structures for the dwc_usb3_driver and
 * dwc_usb3_device. These structures allow the DWC_usb3 driver to comply with the
 * standard Linux driver model in which devices and drivers are registered with
 * a bus driver. This has the benefit that Linux can expose attributes of the
 * driver and device in its special sysfs file system. Users can then read or
 * write files in this file system to perform diagnostics on the driver
 ...
 * - dwc_usb3_common_remove()
 * - dwc_usb3_core_device_init()
 ...
 * - dwc_usb3_get_frame()
 * - dwc_usb3_get_link_state()
 * - dwc_usb3_set_address()
 * - dwc_usb3_suspend_usb2_phy()
 * - dwc_usb3_resume_usb2_phy()
 * - dwc_usb3_suspend_usb3_phy()
 * - dwc_usb3_resume_usb3_phy()
 * - dwc_usb3_accept_u1()
 * - dwc_usb3_accept_u2()
 * - dwc_usb3_enable_u1()
 * - dwc_usb3_enable_u2()
 ...
 * File-Backed Storage Gadget Driver). See pcd_setup() for a detailed
 * explanation.
 *
 ...
 * - handle_end_periodic_frame_intr()
 * - handle_sof_intr()
 ...
 * - dwc_usb3_pcd_ep_queue()
 * - dwc_usb3_pcd_ep_dequeue()
 * - dwc_usb3_pcd_ep_set_halt()
 * - dwc_usb3_get_frame_number()
 * - dwc_usb3_ep0_activate()
 * - dwc_usb3_ep0_out_start()
 ...
 *
 * The bulk of the hibernation code is contained in the file pcd_hiber.c. Note
 * that this is not a complete implementation; the HAPS platform on which it has
 * been tested does not provide the normal PCIe PME mechanism, instead there is
 * special logic in the custom PCIe "gasket" to handle power control and provide
 * an interrupt to awaken the core from hibernation. Therefore, the code in
 * pcd_hiber.c which handles power control and wakeup will need to be rewritten
 * for a "real" implementation. Also, since the CPU is still running while the
 * controller is in the hibernated state, code has been added to pcd_linux.c
 * to prevent any calls that would try to touch the controller hardware from
 * entering the driver while hibernation is active. A "real" implementation
 * should not require this.
 *
 * @section hiber_func_sec Hibernation Functions
 *
 * These are the functions provided by pcd_hiber.c
 *
