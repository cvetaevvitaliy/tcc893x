...
//for debugging by j-tag
	*(volatile unsigned long *)0x77000018 = 0xf0030000;
...
//Clock setting..
	...
	*(volatile unsigned long *)addr_clk(0x00001C) = 0x00200010|MEM_SRC_PLL;  // hsio bus
...
static unsigned int get_membus_ckc(unsigned int mem_freq)
{
	...
	CKC_CHANGE_ARG(CLK_RATE) = (nPLL.fpll/(2*10000));
...
//Clock setting..
	*(volatile unsigned long *)addr_clk(0x000004) = CKC_CHANGE_ARG(CKC_CTRL_VALUE);  // mem bus
	...
	i=100; while(i--);
	*(volatile unsigned long *)addr_clk(MEM_PLL_ADDR) = CKC_CHANGE_ARG(PLL_VALUE) | 0x00100000; //pll1 for mem , lock_en
	*(volatile unsigned long *)addr_clk(MEM_PLL_ADDR) |= 0x80000000; //pll1 for mem , lock_en
