    ...
    for (;;) {
        ...
        switch (chosen_item) {
            ...
            case Device::WIPE_CACHE:
                ...
                status = update_directory(INTERNAL_ROOT, INTERNAL_ROOT, &wipe_cache, device);
            ...
            case Device::APPLY_USB:
                ...
                // as scratch space).
                ensure_path_mounted(CACHE_ROOT);
                status = update_directory(USB_ROOT, USB_ROOT, &wipe_cache, device);
                if (status == INSTALL_SUCCESS && wipe_cache) {
                    ui->Print("\n-- Wiping cache (at package request)...\n");
                    if (erase_volume("/cache")) {
                        ui->Print("Cache wipe failed.\n");
                ...
                if (status >= 0) {
                    if (status != INSTALL_SUCCESS) {
                        ...
                        ui->Print("\nInstall from usb device complete.\n");
                    }
                }
                break;

            case Device::WIPE_SNAPSHOT:
                ui->ShowText(true);
                if (QUICKBOOT_SNAPSHOT) {
                  ui->Print("\n-- Wiping snapshot...\n");
                  erase_volume("/snapshot");
                  ui->Print("Snapshot wipe complete.\n");
                } else
                  ui->Print("There is no Snapshot partition in this system.\n");
                break;
	    case Device::QB_FUNC:
		do_qb_prebuilt(device);
		ui->Print("Done.\n");
		break;
        }
    }
...
main(int argc, char **argv) {
    ...
    memset(buf, 0x0, PROPERTY_VALUE_MAX);
    property_get("ro.QB.enable", buf, "");
    if (!strncmp(buf, "1", 1))
    ...
    int wipe_data = 0, wipe_cache = 0, show_text = 0, wipe_snapshot = 0;
    ...
    if (update_package != NULL) {
        ...
        if (QUICKBOOT_SNAPSHOT)
          if (wipe_snapshot && erase_volume("/snapshot")) status = INSTALL_ERROR;
        if (status != INSTALL_SUCCESS) ui->Print("Data wipe failed.\n");
    } else if (wipe_cache) {
        if (wipe_cache && erase_volume("/cache")) status = INSTALL_ERROR;
        if (QUICKBOOT_SNAPSHOT)
          if (wipe_cache && erase_volume("/snapshot")) status = INSTALL_ERROR;
...
#if 1
    if (wipe_data && wipe_cache && wipe_snapshot) {
      ensure_path_mounted(SYSTEM_ROOT);
      ensure_path_mounted(QB_INTERNAL_ROOT);
      ensure_path_unmounted(CACHE_ROOT);
      quickboot_factory_reset();
      ui->Print("QB data is written successfully!!\n");
    }
#endif
