...
void load_volume_table()
{
	...
	memset(boot_mode, NULL, PROPERTY_VALUE_MAX);
...
int ensure_path_mounted(const char* path) {
    ...
    if (v == NULL) {
        ...
        ui->Print("unknown volume for path [%s]\n", path);
        return -1;
    }
    ...
    if (result < 0) {
        ...
        ui->Print("failed to scan mounted volumes\n");
        return -1;
    }
...
#if 0
	ui->Print("v->fs_type : %s\n", v->fs_type);
	ui->Print("v->blk_device : %s\n", v->blk_device);
	ui->Print("v->mount_point : %s\n", v->mount_point);
	ui->Print("v->label : %s\n", v->label);
#endif
