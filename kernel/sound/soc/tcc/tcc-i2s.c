...
void tcc_i2s_set_clock(unsigned int ClockRate)
{
	...
	if ((pCKC->MBUSCTRL & 0x1) == 0) {
		...
		if (tcc_spdif_clk)
			clk_forced_set_rate(tcc_spdif_clk, 1000000);

		clk_forced_set_rate(pll3_clk, pll3_rate);
	}
#endif
    
#if defined(CONFIG_MEM_CLK_SYNC_MODE)
    clk_forced_set_rate(tcc_dai_clk, 0);
#endif
    clk_forced_set_rate(tcc_dai_clk, clk_rate);
	...
	if(tcc_spdif_clk)
	{
	    ...
	    clk_forced_set_rate(tcc_spdif_clk, clk_rate);
	}
#endif
