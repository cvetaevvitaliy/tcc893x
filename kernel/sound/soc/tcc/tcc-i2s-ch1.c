...
// Planet 20140129 clk_set_rate() -> clk_forced_set_rate()
...
void tcc_i2s_set_clock_ch1(unsigned int ClockRate)
{
	...
	if ((pCKC->MBUSCTRL & 0x1) == 0) {
		...
		if (tcc_spdif_clk_ch1)
			clk_forced_set_rate(tcc_spdif_clk_ch1, 1000000);

		clk_forced_set_rate(pll3_clk_ch1, pll3_rate);
	}
#endif
    
#if defined(CONFIG_MEM_CLK_SYNC_MODE)
    clk_forced_set_rate(tcc_dai_clk_ch1, 0);
#endif
    clk_forced_set_rate(tcc_dai_clk_ch1, clk_rate);
...
void tcc_spdif_set_clock_ch1(unsigned int clock_rate)
{
    ...
    clk_forced_set_rate(tcc_spdif_clk_ch1, clk_rate);
