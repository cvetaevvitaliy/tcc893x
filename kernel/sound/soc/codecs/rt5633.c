...
static int rt5633_write_index(struct snd_soc_codec *codec, unsigned int index,unsigned int value)
{
    ...
    RetVal = snd_soc_write(codec,RT5633_PRI_REG_ADD,index);
    
    if(RetVal != 0)
      return RetVal;
    
    RetVal = snd_soc_write(codec,RT5633_PRI_REG_DATA,value);
    return RetVal;    
}
...
unsigned int rt5633_read_index(struct snd_soc_codec *codec, unsigned int reg)
{
	...
	snd_soc_write(codec,RT5633_PRI_REG_ADD,reg);
	value=snd_soc_read(codec,RT5633_PRI_REG_DATA);	
...
static void rt5633_alc_enable(struct snd_soc_codec *codec,unsigned int EnableALC)
{
	if(EnableALC)
	{
		snd_soc_write(codec, 0x64,0x0a0f);
		snd_soc_write(codec, 0x65,0x0005);
		snd_soc_write(codec, 0x66,0xe080);
...
static void rt5633_update_eqmode(struct snd_soc_codec *codec, int mode)
{
	...
	{		
		snd_soc_write(codec, 0x70,HwEq_Preset[mode].HwEQCtrl);
...
static void spk_work_handler(struct work_struct *work)
{
	...
	bool is_spk = (snd_soc_read(codec, 0x4a)) & 0x10;	//detect rt5633 reg4a[4], 1'b:SPK, 0'b:HP ;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	l = (snd_soc_read(codec, RT5633_PWR_MANAG_ADD4) & (0x01 << 15)) >> 15;
	r = (snd_soc_read(codec, RT5633_PWR_MANAG_ADD4) & (0x01 << 14)) >> 14;
	...
	case SND_SOC_DAPM_POST_PMU:		
		if (l && r) 
		{
			...
			rt5633_write_mask(codec, RT5633_SPK_OUT_VOL, 0x0000, 0x8000);
			 snd_soc_write(codec, RT5633_DAC_DIG_VOL, 0x1010);
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	l = (snd_soc_read(codec, RT5633_PWR_MANAG_ADD4) & (0x01 << 11)) >> 11;
	r = (snd_soc_read(codec, RT5633_PWR_MANAG_ADD4) & (0x01 << 10)) >> 10;
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if ((l && r)&&(!hp_out_enable))
		{
			...
			hp_out_enable=1;
			snd_soc_write(codec, RT5633_DAC_DIG_VOL, 0x0000);
		}
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	l = (snd_soc_read(codec, RT5633_PWR_MANAG_ADD4) & (0x01 << 9)) >> 9;
	r = (snd_soc_read(codec, RT5633_PWR_MANAG_ADD4) & (0x01 << 8)) >> 8;
...
static int rt5633_reg_dump(struct snd_soc_codec *codec)
{
	...
	for (i = 0; i < ARRAY_SIZE(reg_addr_list); i ++) {
		val = snd_soc_read(codec, reg_addr_list[i]);	
			...
			struct snd_soc_dai *dai)
{
	...
	snd_soc_write(codec, RT5633_STEREO_AD_DA_CLK_CTRL, coeff_div[coeff].reg_val);
...
static int rt5633_hifi_codec_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	snd_soc_write(codec, RT5633_SDP_CTRL, iface);
	return 0;
}
	...
	if (rt5633->master) {
		for (i = 0; i < ARRAY_SIZE(codec_master_pll_div); i ++) {
			if ((freq_in == codec_master_pll_div[i].pll_in) && (freq_out == codec_master_pll_div[i].pll_out)) {
				snd_soc_write(codec, RT5633_PLL_CTRL, codec_master_pll_div[i].regvalue);
				rt5633_write_mask(codec, RT5633_PWR_MANAG_ADD2, PWR_PLL, PWR_PLL);
				schedule_timeout_uninterruptible(msecs_to_jiffies(20));
				snd_soc_write(codec, RT5633_GBL_CLK_CTRL, 0x0000);	
				ret = 0;
			}
		}	
	} else {
		for (i = 0; i < ARRAY_SIZE(codec_slave_pll_div); i ++) {
			if ((freq_in == codec_slave_pll_div[i].pll_in) && (freq_out == codec_slave_pll_div[i].pll_out))  {
				snd_soc_write(codec, RT5633_PLL_CTRL, codec_slave_pll_div[i].regvalue);
				rt5633_write_mask(codec, RT5633_PWR_MANAG_ADD2, PWR_PLL, PWR_PLL);
				schedule_timeout_uninterruptible(msecs_to_jiffies(20));
				snd_soc_write(codec, RT5633_GBL_CLK_CTRL, 0x6000);
				ret = 0;
			}
		}
	}
 ...
 size_t count)
{
	...
	if(!strncmp(buf, "readr", strlen("readr")))
	{
		...
		read_codec_value=snd_soc_read(rt5633_codec, read_codec_reg);
	...
	else if(!strncmp(buf, "writer", strlen("writer")))
	{
		...
		if(p)
		{
			++ p;
			val=(u32)StrtoInt(p);
			snd_soc_write(rt5633_codec, reg, val);
	...
	else if(!strncmp(buf, "writei", strlen("writei")))
	{
		...
		if(p)
		{
			++ p;
			val=(u32)StrtoInt(p);
			snd_soc_write(rt5633_codec, 0x6a, reg);
			snd_soc_write(rt5633_codec, 0x6c, val);
	...
	else if(!strncmp(buf, "readi", strlen("readi")))
	{
		p+=strlen("readi");
		read_codec_reg=(u32)StrtoInt(p);
		snd_soc_write(rt5633_codec, 0x6a, read_codec_reg);	
		read_codec_value=snd_soc_read(rt5633_codec, 0x6c);
...
static int rt5633_probe(struct snd_soc_codec *codec) 
{
	...
	snd_soc_add_codec_controls(codec, rt5633_snd_controls,
			    ...
			    const struct i2c_device_id *id)
{
	...
	rt5633 = devm_kzalloc(&i2c->dev, sizeof(struct rt5633_priv), GFP_KERNEL);
	if (rt5633 == NULL)
		return -ENOMEM;
