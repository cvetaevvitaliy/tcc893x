...
static int tca_tsif_update_changed_pids(tca_spi_handle_t *h, unsigned int *pids, unsigned int count)
{
    volatile unsigned long* PIDT;
    int i,j,reg_data, gpsb_channel = -1, pid_ch = 0;
...
//add pids
    for (i=0; i<count; i++)
    {
         if( new_pids[i] != -1)
         {
            ...
            for (j=0; j<32; j++)
            {
                PIDT = (volatile unsigned long *)tcc_p2v(HwGPSB_PIDT(j));
                reg_data =  (int)(*PIDT);
                if( (reg_data & (Hw31|Hw30|Hw29)) == 0)
                {                    
                    *PIDT = new_pids[i] & 0x1FFFFFFF;
                    BITSET(*PIDT, pid_ch);
                    printk("PIDT %d:0x%08X [ADD]\n",i, *PIDT);                   
                    new_pids[i] = -1;
                    break;
                }
            }
