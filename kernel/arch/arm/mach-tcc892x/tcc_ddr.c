/****************************************************************************
 * arch/arm/mach-tcc892x/tcc_ddr.c
...
#define MEM_SRC_PLL 1	// PLL1
#define MEM_PLL_ADDR 0x000034
#else
#define MEM_SRC_PLL 8	// PLL4
...
//	i=100; while(i--);
	...
	*(volatile unsigned long *)addr_clk(MEM_PLL_ADDR) |= 0x80000000;
	...
	i=20; while(i--);
	*(volatile unsigned long *)addr_clk(0x000004) = ((*(volatile unsigned long *)addr_clk(0x000004))&0xFFFFFFF0) | (MEM_SRC_PLL&0x0000000F);
	while((*(volatile unsigned long *)addr_clk(0x000004))&0x80000000);	// CHGREQ
	*(volatile unsigned long *)addr_clk(0x000004) = ((*(volatile unsigned long *)addr_clk(0x000004))&0xFFFFFF0F) | 0x10;
...
/*===========================================================================
FUNCTION
 - param: mem_freq (100Hz)
 - return value : MHz unit
===========================================================================*/
static unsigned int get_membus_ckc(unsigned int mem_freq, unsigned int tmp_src, unsigned int tmp_div)
{
	tPMS nPLL;
	nPLL.fpll = mem_freq*2;
	if (tcc_find_pms(&nPLL))
		return 0;
	CKC_CHANGE_ARG(CKC_CTRL_VALUE) = 0x00200000|(tmp_src&0xF)|((tmp_div<<4)&0xF0);
	CKC_CHANGE_ARG(PLL_VALUE) = ((nPLL.vsel&0x1) << 30) | ((nPLL.s&0x7) << 24) | (1 << 20) | ((nPLL.m&0x3FF) << 8) | (nPLL.p&0x3F);
	CKC_CHANGE_ARG(CLK_RATE) = (nPLL.fpll/20000);
	return (nPLL.fpll/20000);
}
...
unsigned int tcc_ddr_set_clock(unsigned int freq, unsigned int src, unsigned int div)
{
	...
	mem_freq = get_membus_ckc(freq*10, src, div);
