...
struct func_clktbl_t *gmac_clktbl = NULL;
#endif
...
#if defined(CONFIG_CLOCK_TABLE)
	gmac_clktbl = clocktable_get("gmac_clktbl");
	if (IS_ERR(gmac_clktbl))
		gmac_clktbl = NULL;
#endif
}
...
#if defined(CONFIG_CLOCK_TABLE)
		if (gmac_clktbl)
			clocktable_ctrl(gmac_clktbl, (gmac_clktbl->num_clktbls-1), CLKTBL_ENABLE);
#endif
	}
...
#if defined(CONFIG_CLOCK_TABLE)
		if (gmac_clktbl)
			clocktable_ctrl(gmac_clktbl, 0, CLKTBL_DISABLE);
#endif
