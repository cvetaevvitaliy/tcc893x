...
static int tcc_clk_show(struct seq_file *m, void *v)
{
    ...
    seq_printf(m, "cpu         : %4d MHz\n", tca_ckc_fclk_is_enabled(FBUS_CPU)?tca_ckc_getfbusctrl(FBUS_CPU)/10000:0);
    seq_printf(m, "memory bus  : %4d MHz\n", tca_ckc_fclk_is_enabled(FBUS_MEM)?(tca_ckc_getfbusctrl(FBUS_MEM)*2)/10000:0);
    seq_printf(m, "display bus : %4d MHz\n", tca_ckc_fclk_is_enabled(FBUS_DDI)?tca_ckc_getfbusctrl(FBUS_DDI)/10000:0);
    seq_printf(m, "graphic bus : %4d MHz\n", tca_ckc_fclk_is_enabled(FBUS_GPU)?tca_ckc_getfbusctrl(FBUS_GPU)/10000:0);
    seq_printf(m, "i/o bus     : %4d MHz\n", tca_ckc_fclk_is_enabled(FBUS_IO)?tca_ckc_getfbusctrl(FBUS_IO)/10000:0);
    seq_printf(m, "video bus   : %4d MHz\n", tca_ckc_fclk_is_enabled(FBUS_VBUS)?tca_ckc_getfbusctrl(FBUS_VBUS)/10000:0);
    seq_printf(m, "video codec : %4d MHz\n", tca_ckc_fclk_is_enabled(FBUS_VCORE)?tca_ckc_getfbusctrl(FBUS_VCORE)/10000:0);
    seq_printf(m, "hsio bus    : %4d MHz\n", tca_ckc_fclk_is_enabled(FBUS_HSIO)?tca_ckc_getfbusctrl(FBUS_HSIO)/10000:0);
    seq_printf(m, "smu bus     : %4d MHz\n", tca_ckc_fclk_is_enabled(FBUS_SMU)?tca_ckc_getfbusctrl(FBUS_SMU)/10000:0);
#if defined(CONFIG_CHIP_TCC8935S) || defined(CONFIG_CHIP_TCC8933S) || defined(CONFIG_CHIP_TCC8937S)
#else
    seq_printf(m, "g2d bus     : %4d MHz\n", tca_ckc_fclk_is_enabled(FBUS_G2D)?tca_ckc_getfbusctrl(FBUS_G2D)/10000:0);
    seq_printf(m, "Cortex-M3   : %4d MHz\n", tca_ckc_fclk_is_enabled(FBUS_CM3)?tca_ckc_getfbusctrl(FBUS_CM3)/10000:0);
#endif
