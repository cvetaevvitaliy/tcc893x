...
static int tcc_bt_rfkill_set_power(void *data, bool blocked)
{
	...
	if (!blocked && (bt_enable_flag == 0)) {
	    ...
	    if(system_rev == 0x7231){
			gpio_direction_output(TCC_GPD(21), 1); // bt_wake
			gpio_direction_output(TCC_GPD(16), 0); // bt_reset
			msleep(500);
			gpio_direction_output(TCC_GPD(16), 1);
	    }
		else{
			gpio_direction_output(TCC_GPD(10), 0);// bt_reset
			mdelay(100);
	        gpio_direction_output(TCC_GPD(9), 1); // output
	        gpio_direction_input(TCC_GPD(8));
	        gpio_set_value(TCC_GPB(24), 1);
			gpio_direction_output(TCC_GPD(10), 1);// bt_reset
			mdelay(100);
	    }
		...
		if(bt_reg_off_skip == 1)
		    ...
		    if(system_rev == 0x7231){
			gpio_direction_output(TCC_GPD(21), 0);
			gpio_direction_output(TCC_GPD(16), 0);
		    }else{
			gpio_set_value(TCC_GPD(10), 0);   // bt_reset
		    }
		}
	}
...
static int tcc_bluetooth_probe(struct platform_device *pdev)
{
	...
	if(system_rev == 0x9312 || system_rev == 0x9313){
	    gpio_request(TCC_GPD(8), "bt_hwake");
	    gpio_request(TCC_GPD(9), "bt_wake");
	    gpio_request(TCC_GPD(10), "bt_reset");
	    gpio_request(TCC_GPB(24), "wifi_reset");

	    gpio_direction_output(TCC_GPD(9), 1); // output
	    gpio_direction_input(TCC_GPD(8));
	    gpio_set_value(TCC_GPB(24), 1);
	}
	else if(system_rev == 0x7231){
	    gpio_request(TCC_GPD(24), "bt_hwake");
	    gpio_request(TCC_GPD(21), "bt_wake");
	    gpio_request(TCC_GPD(16), "bt_reset");
	    gpio_request(TCC_GPF(8), "wifi_reset");

	    gpio_direction_input(TCC_GPD(24));
	}
