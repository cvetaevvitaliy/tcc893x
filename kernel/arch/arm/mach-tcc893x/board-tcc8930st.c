/****************************************************************************
FileName    : kernel/arch/arm//mach-tcc893x/board-tcc8930st.c
Description : 
...
static struct platform_device tcc_composite_device = {
	.name	= "tcc_composite",
};

static void tcc_add_composite_device(void)
{
	platform_device_register(&tcc_composite_device);
}
#endif

#if defined(CONFIG_FB_TCC_COMPONENT)
static struct platform_device tcc_component_device = {
	.name	= "tcc_component",
};

static void tcc_add_component_device(void)
{
	platform_device_register(&tcc_component_device);
}
#endif
...
static struct tcc_i2c_platform_data tcc8930_core0_platform_data = {
    ...
    .core_clk_name      = "i2c0",
    .smu_i2c_flag       = 0,
    .i2c_ch_clk_rate    = 400,      /* SCL clock rate : 100kHz */
};
#endif
#if defined(CONFIG_I2C_TCC_CORE1)
static struct tcc_i2c_platform_data tcc8930_core1_platform_data = {
    ...
    .core_clk_name      = "i2c1",
    .smu_i2c_flag       = 0,
    .i2c_ch_clk_rate    = 400,      /* SCL clock rate : 400kHz */
};
#endif
#if defined(CONFIG_I2C_TCC_CORE2)
...
static struct tcc_i2c_platform_data tcc8930_core3_platform_data = {
    ...
    .core_clk_name      = "i2c3",
    .smu_i2c_flag       = 0,
    .i2c_ch_clk_rate    = 25,      /* SCL clock rate : 25kHz */
};
#endif
#if defined(CONFIG_I2C_TCC_SMU)
...
char *frame_buffer_size = NULL;
static int __init board_fbsize_setup(char *fbsize)
{
	frame_buffer_size = fbsize;

	return 1;
}
__setup("androidboot.fbsize=", board_fbsize_setup);
...
static void __init tcc8930_init_machine(void)
{
...
#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T)  // HDMI reciever wake up
	gpio_request(TCC_GPF(4), NULL);
	gpio_direction_output(TCC_GPF(4), 1);	 
	gpio_request(TCC_GPF(2), NULL);
	gpio_direction_output(TCC_GPF(2), 1);
#endif
	...
	tcc_add_nand_device();
#if defined(CONFIG_FB_TCC_COMPOSITE)
	tcc_add_composite_device();
#endif
#if defined(CONFIG_FB_TCC_COMPONENT)
	tcc_add_component_device();
#endif
...
{
	init_consistent_dma_size(TCC_CONSISTENT_DMA_SIZE);
    tcc_clk_init();
}
