...
struct func_clktbl_t *sc_clktbl = NULL;
#endif
...
void tca_sc_init_port(void)
{
	...
	sc_io.rst = TCC_GPD(0);
	sc_io.detect = TCC_GPD(3);
	sc_io.pwren = TCC_GPD(1);
	sc_io.set_volt = -1;
	...
	#if defined(CONFIG_CHIP_TCC8935)
...
void tca_sc_clock_ctrl(sDRV_UART *pstDrvSc, unsigned uEnable)
{
	...
	{
#ifdef CONFIG_CLOCK_TABLE
		sc_clktbl = clocktable_get("smartcard_clktbl");
		if (IS_ERR(sc_clktbl)) {
			sc_clktbl = NULL;
			clk_disable(gSC_clk);
			return;
		} else  {
			clocktable_ctrl(sc_clktbl, 0, CLKTBL_ENABLE);
		}
#endif
...
#ifdef CONFIG_CLOCK_TABLE
		if (sc_clktbl) {
			clocktable_ctrl(sc_clktbl, 0, CLKTBL_DISABLE);
			clocktable_put(sc_clktbl);
			sc_clktbl = NULL;
		}
#endif
		clk_disable(gSC_clk);
...
#if defined(CONFIG_CHIP_TCC8935) 
	uUartPort = 11;
#elif defined(CONFIG_CHIP_TCC8933) || defined(CONFIG_CHIP_TCC8935S) || defined(CONFIG_CHIP_TCC8933S) || defined(CONFIG_CHIP_TCC8937S)
	uUartPort = 10;
#elif defined (CONFIG_CHIP_TCC8930)
	uUartPort = 10;
#endif
