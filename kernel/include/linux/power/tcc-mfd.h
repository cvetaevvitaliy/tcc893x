#ifndef __LINUX_TCC_MFD_H_
#define __LINUX_TCC_MFD_H_

/* Unified sub device IDs for TCC */
enum {
	TCC_ID_LDO1,
	TCC_ID_LDO2,
	TCC_ID_LDO3,
...
/* TCC battery charger data */
struct power_supply_info;

struct tcc_supply_init_data {
	/* battery parameters */
...
struct tcc_funcdev_info {
	int		id;
	const char	*name;
	void	*platform_data;
};

struct tcc_platform_data {
	int num_regl_devs;
	int num_sply_devs;
	
	struct tcc_funcdev_info *regl_devs;
	struct tcc_funcdev_info *sply_devs;
};

struct tcc_mfd_chip {
	struct i2c_client	*client;
	struct device		*dev;
	struct tcc_mfd_chip_ops	*ops;
...
struct tcc_mfd_chip_ops {
	int	(*init_chip)(struct tcc_mfd_chip *);
	int	(*enable_irqs)(struct tcc_mfd_chip *, uint64_t irqs);
	int	(*disable_irqs)(struct tcc_mfd_chip *, uint64_t irqs);
	int	(*read_irqs)(struct tcc_mfd_chip *, uint64_t *irqs);
	int	(*clear_irqs)(struct tcc_mfd_chip *, uint64_t irqs);
};
...
#define POWER20_INTER_TEMPER                    (0x5E)
...
/*  TCC  */
#define TCC_STATUS_SOURCE					(1<< 0)
#define TCC_STATUS_ACUSBSH					(1<< 1)
#define TCC_STATUS_BATCURDIR				(1<< 2)
#define TCC_STATUS_USBLAVHO					(1<< 3)
#define TCC_STATUS_USBVA					(1<< 4)
#define TCC_STATUS_USBEN					(1<< 5)
#define TCC_STATUS_ACVA						(1<< 6)
#define TCC_STATUS_ACEN						(1<< 7)

#define TCC_STATUS_CHACURLOEXP				(1<<10)
#define TCC_STATUS_BATINACT					(1<<11)

#define TCC_STATUS_BATEN					(1<<13)
#define TCC_STATUS_INCHAR					(1<<14)
#define TCC_STATUS_ICTEMOV					(1<<15)

#if defined (CONFIG_REGULATOR_TCC270)
#define TCC_IC_TYPE							POWER20_IC_TYPE

#define TCC_INTEN1							POWER20_INTEN1 
#define TCC_INTEN2							POWER20_INTEN2 
#define TCC_INTEN3							POWER20_INTEN3 
#define TCC_INTEN4							POWER20_INTEN4 
#define TCC_INTEN5							POWER20_INTEN5 
#define TCC_INTSTS1							POWER20_INTSTS1
#define TCC_INTSTS2							POWER20_INTSTS2
#define TCC_INTSTS3							POWER20_INTSTS3
#define TCC_INTSTS4							POWER20_INTSTS4
#define TCC_INTSTS5							POWER20_INTSTS5

#define TCC_STATUS							POWER20_STATUS
#define TCC_MODE_CHGSTATUS					POWER20_MODE_CHGSTATUS
#define TCC_VOFF_SET							POWER20_VOFF_SET
#define TCC_OFF_CTL							POWER20_OFF_CTL
#define TCC_COULOMB_CTL						POWER20_COULOMB_CTL
#define TCC_DATA_BUFFER0					POWER20_DATA_BUFFER1
#define TCC_DATA_BUFFERC					POWER20_DATA_BUFFERC
#define TCC_DATA_NUM						12
#endif

#if defined (CONFIG_REGULATOR_TCC192)
#define TCC_IC_TYPE							POWER19_IC_TYPE

#define TCC_INTEN1							POWER19_INTEN1 
#define TCC_INTEN2							POWER19_INTEN2 
#define TCC_INTEN3							POWER19_INTEN3 
#define TCC_INTEN4							POWER19_INTEN4 
#define TCC_INTSTS1							POWER19_INTSTS1
#define TCC_INTSTS2							POWER19_INTSTS2
#define TCC_INTSTS3							POWER19_INTSTS3
#define TCC_INTSTS4							POWER19_INTSTS4

#define TCC_STATUS							POWER19_STATUS
#define TCC_MODE_CHGSTATUS					POWER19_MODE_CHGSTATUS
#define TCC_OFF_CTL							POWER19_OFF_CTL
#define TCC_COULOMB_CTL						POWER19_COULOMB_CTL
#define TCC_DATA_BUFFERC					POWER19_STATUS
#define TCC_DATA_BUFFER0					POWER19_DATA_BUFFER1
#define TCC_DATA_NUM						4
#endif

extern struct device *tcc_get_dev(void);
extern int tcc_register_notifier(struct device *dev,
		struct notifier_block *nb, uint64_t irqs);
extern int tcc_unregister_notifier(struct device *dev,
		struct notifier_block *nb, uint64_t irqs);

/* NOTE: the functions below are not intended for use outside
 * of the TCC sub-device drivers
 */
extern int tcc_write(struct device *dev, int reg, uint8_t val);
extern int tcc_writes(struct device *dev, int reg, int len, uint8_t *val);
extern int tcc_read(struct device *dev, int reg, uint8_t *val);
extern int tcc_reads(struct device *dev, int reg, int len, uint8_t *val);
extern int tcc_update(struct i2c_client *client, int reg, uint8_t val, uint8_t mask);
extern int tcc_set_bits(struct i2c_client *client, int reg, uint8_t bit_mask);
extern int tcc_clr_bits(struct i2c_client *client, int reg, uint8_t bit_mask);
extern struct i2c_client *tcc;
extern struct device *tcc_get_dev(void);
#endif /* __LINUX_PMIC_TCC_H */
