...
//	printk("DDI_I2C_Write_HDMI_IN : writing register addr : = 0x%2x, ",buffer[i]);
	}
	printk("writing register value : = ");
	for(i=reg_bytes;i<bytes;i++){
	printk("0x%02x  ",buffer[i]);
	}
	printk("\n");
#endif
	if(i2c_master_send(cam_i2c_client, buffer, bytes) != bytes)
	{
		printk("write error!!!! \n");
		return -EIO; 
	}
