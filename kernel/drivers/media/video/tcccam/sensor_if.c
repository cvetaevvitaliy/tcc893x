...
static enum image_size sensor_find_size(unsigned int width, unsigned int height)
{
	...
	|| defined(CONFIG_VIDEO_CAMERA_SENSOR_SP0A18) || defined(CONFIG_VIDEO_CAMERA_SENSOR_SP0A19) ||  defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T)
	return VGA;
	#else
	return SXGA;
	#endif
	...
	#elif defined(CONFIG_ARCH_TCC892X) || defined(CONFIG_ARCH_TCC893X)
	{
		dprintk("[TCC892x] gpio init start!\n");
			...
			// Power doesn't need it.
				...
				else if(machine_is_m805_893x()) { 				// M805_TCC8935
				...
				} else if(system_rev == 0x7230 || system_rev == 0x7231) {
			
					#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T)
						#if 0  // already wake up HDMI IN module
						gpio_request(TCC_GPF(4), NULL);
						gpio_direction_output(TCC_GPF(4), 1);	   

					
						dprintk("EP9553T init reset !!\n");
						gpio_request(TCC_GPF(2), NULL);
						gpio_direction_output(TCC_GPF(2), 1);
						#endif


						dprintk("EP9553T init Key !!\n");
						gpio_request(TCC_GPF(3), NULL);
						gpio_direction_output(TCC_GPF(3), 1);
...
int sensor_if_get_audio_samplerate(int value)
{
	return sensor_func.GetAudioSR();
}
...
int sensor_if_get_max_resolution(int index)
{
	...
	switch(tcc_sensor_info.sensor_sizes[0].width) {
		...
		ret = sensor_func.GetVideoSize();
		#if 0
		sensor_delay(3000);
		tcc_get_ep9553e_vin_fmt();
		#endif
	...
	else
	{
		...
		// if you want to control the power, this part should be coding.
				...
				if(system_rev == 0x1000) { 			// TCC8930
					...
					#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T)
						printk("EP9553T : reset high!\n");
						gpio_set_value(TCC_GPF(2), 1);
					#endif
				}
			#elif defined(CONFIG_ARCH_TCC892X)
	...
	else
	{
		...
		// if you want to control the power, this part should be coding.
				...
				if(system_rev == 0x1000) { 			// TCC8930
					...
					#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T)
						printk("EP9553T : reset low!\n");
						gpio_set_value(TCC_GPF(2), 0);
					#endif
				}
			#elif defined(CONFIG_ARCH_TCC892X)
				if(system_rev == 0x1006){
