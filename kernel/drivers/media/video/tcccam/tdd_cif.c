...
void TDD_CIF_Initialize()
{
	...
	BITSET(pGPIO_E->GPCD1, 0x000000FF);
		...
		if(system_rev == 0x7230 || system_rev == 0x7231) {
			printk("port configuration for HDMI IN!!\n");
			volatile PGPION pGPIO_B = (PGPION)tcc_p2v(HwGPIOB_BASE);
			volatile PGPION pGPIO_E = (PGPION)tcc_p2v(HwGPIOE_BASE);
			volatile PGPION pGPIO_F = (PGPION)tcc_p2v(HwGPIOF_BASE);
			volatile PGPION pGPIO_G = (PGPION)tcc_p2v(HwGPIOG_BASE);
			// HDMI-IN JH3 setting
			BITCSET(pGPIO_B->GPFN3.nREG, 0x0F0F0000, 0x00000000); 	// gpio_b28 -> cam_rst ,gpio_b30 -> VE_GPIO
			#if 1 //gpio setting : functional
			BITCSET(pGPIO_E->GPFN0.nREG, 0xFFFFFFFF, 0x33333333); 	// cam_d16 ~ cam_d23
			BITCSET(pGPIO_E->GPFN1.nREG, 0xFFFFFFFF, 0x33333333); 	// cam_d08 ~ cam_d15
			BITCSET(pGPIO_E->GPFN2.nREG, 0xFFFFFFFF, 0x33333333); 	// cam_d00 ~ cam_d07
			BITCSET(pGPIO_E->GPFN3.nREG, 0xFFFFFFFF, 0x00033333); 	// gpio_e24 -> CAM_CLK, gpio_e25 -> CAM_VS, gpio_e26 -> CAM_HS, gpio_e27 -> DE, gpio_e28 -> CAM_DE, gpio_E30~31 ->DDC_I2C(SDA, SCL)
			BITCSET(pGPIO_E->GPEN.nREG, 0x1FFFFFFF, 0x00000000);
			#else //gpio setting : normal(input mode)
			BITCSET(pGPIO_E->GPFN0.nREG, 0xFFFFFFFF, 0x00000000); 	// cam_d16 ~ cam_d23
			BITCSET(pGPIO_E->GPFN1.nREG, 0xFFFFFFFF, 0x00000000); 	// cam_d08 ~ cam_d15
			BITCSET(pGPIO_E->GPFN2.nREG, 0xFFFFFFFF, 0x00000000); 	// cam_d00 ~ cam_d07
			BITCSET(pGPIO_E->GPFN3.nREG, 0xFFFFFFFF, 0x66000000); 	// gpio_e24 -> CAM_CLK, gpio_e25 -> CAM_VS, gpio_e26 -> CAM_HS, gpio_e27 -> DE, gpio_e28 -> CAM_DE, gpio_E30~31 ->DDC_I2C(SDA, SCL)
			BITCSET(pGPIO_E->GPEN.nREG, 0x1FFFFFFF, 0x00000000);
			BITCSET(pGPIO_E->GPPE.nREG, 0xFFFFFFFF, 0x00000000);
			#endif
			// HDMI-IN JH4 setting
			BITCSET(pGPIO_F->GPFN0.nREG, 0x0FFFFFFF, 0x00000000); 	// gpio_f00 -> RX_INTb_9351, gpio_f01 -> EP94M1_INTb, gpio_f02 -> EP94M1_RSTB, gpio_f03 -> RSTb_9351
																	// gpio_f04 -> RXHPD_CTL_b, gpio_f05 -> CDS, gpio_f06 -> VBUS_OC_INTb
			//BITCSET(pGPIO_F->GPFN2.nREG, 0x000FF000, 0x000AA000);	// gpio_f19~20 ->MCU_I2C(SDA, SCL)

			BITCSET(pGPIO_G->GPFN2.nREG, 0x0000FF00, 0x00004400);	// gpio_g18~19 ->VE_I2C(SDA, SCL)
		}
	...
	volatile PGPION pGPIO_G = (PGPION)tcc_p2v(HwGPIOG_BASE);
		...
		#if defined(CONFIG_CHIP_TCC8925) || defined(CONFIG_CHIP_TCC8925S)
	...
	#endif //CONFIG_VIDEO_TCCXX_HDMI_IN		
...
void TDD_CIF_Termination(void)
{
 	...
 	#if defined(CONFIG_VIDEO_TCCXX_HDMI_IN)&& defined(CONFIG_ARCH_TCC893X)
	if(system_rev == 0x7230 || system_rev == 0x7231)
	{
		printk("port close for HDMI IN!!\n");
	}
	#else
	//	In case of CAM4
	volatile PGPION pGPIO_F = (PGPION)tcc_p2v(HwGPIOF_BASE);
