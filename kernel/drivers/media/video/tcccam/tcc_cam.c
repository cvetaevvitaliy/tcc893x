...
extern int check_int; // check interrupt
...
struct tccxxx_cif_buffer *prev_buf;
static unsigned int		prev_num;
#if defined (CONFIG_VIDEO_ATV_SENSOR_TVP5150) || defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T) 
static uint				bfield;
static uint				frm_cnt;
...
static void cif_data_init(void *priv)
{
	...
	data->cif_cfg.fmt				= RGB565;    //RGB 565 MODE
	//data->cif_cfg.fmt 			= ARGB8888
	#else
	data->cif_cfg.fmt				= M420_EVEN;    
	#endif
#else
	...
	if(data->cif_cfg.oper_mode == OPER_PREVIEW) { // preview operation.
		if(pWDMABase->uIRQSTS.nREG & VIOC_WDMA_IREQ_EOFF_MASK) {
			...
			if(skip_frm == 0 && !frame_lock) {
				...
				if(&next_buf->buf_list != &data->list && prev_buf != next_buf) { // exception process!!
					if(prev_num != prev_buf->v4lbuf.index) {
						printk("Frame num mismatch :: true num	:: %d \n", prev_num);
						printk("Frame num mismatch :: false num :: %d \n", prev_buf->v4lbuf.index);
						prev_buf->v4lbuf.index = prev_num ;
					...
					{
						unsigned int stride = ALIGNED_BUFF(data->cif_cfg.main_set.target_x, 16);
					#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T) && defined(FEATURE_WDMA_RGB_MODE)
						if(data->cif_cfg.fmt == RGB565)	prev_buf->v4lbuf.bytesused = (stride * data->cif_cfg.main_set.target_y) * 2; //rgb565
						else							prev_buf->v4lbuf.bytesused = (stride * data->cif_cfg.main_set.target_y) * 4; //Argb8888
				...
				#if defined(CONFIG_VIDEO_ATV_SENSOR_TVP5150) || defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T)//bfield 
					if((frm_cnt == 1) && (bfield == 0)) {
						...
						if(hdmi_in_Interlaced)	VIOC_VIQE_SetDeintlMode((VIQE *)pVIQEBase, 2);
						#else
				...
				if(check_int == 0)	check_int = 1; // 20131018 swhwang, for check video frame interrupt
...
#if  defined(CONFIG_VIDEO_ATV_SENSOR_TVP5150) || defined(CONFIG_VIDEO_ATV_SENSOR_RDA5888) || defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T) 
...
#elif defined(CONFIG_ARCH_TCC892X) || defined(CONFIG_ARCH_TCC893X)
	#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T) && defined(FEATURE_WDMA_RGB_MODE)
		VIOC_WDMA_SetImageBase(pWDMABase, (unsigned int)data->cif_cfg.preview_buf[frame_num].p_Y, (unsigned int)NULL, (unsigned int)NULL);
	#else
...
void cif_preview_dma_set(void)
{
	...
	#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T) && defined(FEATURE_WDMA_RGB_MODE)
		if(data->cif_cfg.fmt == RGB565)	total_off = y_offset* 2;  //rgb565
		else							total_off = y_offset* 4;  //Argb8888
	#else
	...
	for(i=0; i < data->cif_cfg.pp_num; i++) {
		...
		#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T) && defined(FEATURE_WDMA_RGB_MODE)
			data->cif_cfg.preview_buf[i].p_Cb = (unsigned int)NULL;
			data->cif_cfg.preview_buf[i].p_Cr = (unsigned int)NULL;
...
void cif_capture_dma_set(void *priv)
{
...
#endif
	{
		...
		#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T) && defined(FEATURE_WDMA_RGB_MODE)
			uv_offset = target_width*target_height;
		#else
		if(data->cif_cfg.fmt == M420_ZERO)
	...
	#if defined(CONFIG_ARCH_TCC892X) || defined(CONFIG_ARCH_TCC893X)
		#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T) && defined(FEATURE_WDMA_RGB_MODE)
			VIOC_WDMA_SetImageBase(pWDMABase, (unsigned int)data->cif_cfg.capture_buf.p_Y,  (unsigned int)NULL, (unsigned int)NULL);
...
void cif_set_port(unsigned int nUsingPort)
{
	...
	switch(nUsingPort){
		...
		case 1: //used port1
			BITCSET(pDDIConfig->CIFPORT.nREG, 0x00077777, (0 << 16) | (4 << 12) | (3 << 8) | (2 << 4) | nUsingPort);
			break;
		
		case 2: //used port2
			BITCSET(pDDIConfig->CIFPORT.nREG, 0x00077777, (0 << 16) | (4 << 12) | (3 << 8) | (1 << 4) | nUsingPort);
			break;
		
		case 3: //used port3
			BITCSET(pDDIConfig->CIFPORT.nREG, 0x00077777, (0 << 16) | (4 << 12) | (2 << 8) | (1 << 4) | nUsingPort);
			break;
...
#elif defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T) 
		printk("HDMI IN VIOC VIN Setting!!!!\n");
		VIOC_VIN_SetSyncPolarity(pVIN, ON/*Hsync*/, ON/*Vsync*/, OFF, OFF/*DE*/, ON/*gen_field_en*/, ON/*pclk*/);
		VIOC_VIN_SetCtrl(pVIN, OFF, OFF, OFF, FMT_RGB444_24BIT/*hdmi_in_vin_fmt*/, ORDER_RGB);
		VIOC_VIN_SetInterlaceMode(pVIN, hdmi_in_Interlaced, OFF);
		
#else
	#if defined(CONFIG_MACH_M805_892X) || defined(CONFIG_MACH_M805_893X)  // Because of DE signal
		#if defined(CONFIG_CHIP_TCC8925) || defined(CONFIG_CHIP_TCC8925S) || defined(CONFIG_MACH_M805_893X) 
...
int tccxxx_vioc_vin_wdma_set(VIOC_WDMA *pDMA_CH)
{
	...
	#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T) && defined(FEATURE_WDMA_RGB_MODE)
		if(data->cif_cfg.fmt == RGB565)	 fmt = VIOC_IMG_FMT_RGB565;
		else						     fmt = VIOC_IMG_FMT_ARGB8888;
...
{
	uint channel = 0, preview_ratio = 0;
	...
	if (plug_in == VIOC_SC_VIN_00) {			// PlugIn path : VIN0 - SCALE - WMIX
		...
		mh = data->cif_cfg.main_set.target_y;
			...
			mw 	= PRV_W;
				...
				mh 	= PRV_H*2;
			#elif defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T)  
				if(hdmi_in_Interlaced) 	mh 	= PRV_H*2;
				else 					mh 	= PRV_H ; 
			#else
				mh 	= PRV_H ; 
			#endif
...
int tccxxx_cif_buffer_set(struct v4l2_requestbuffers *req)
{
	...
	if(req->count == 0) {
		...
		#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T) && defined(FEATURE_WDMA_RGB_MODE)
			if(data->cif_cfg.fmt == RGB565)	buff_size = PAGE_ALIGN(y_offset*2); //rgb565
			else							buff_size = PAGE_ALIGN(y_offset*4); //argb8888
		#else
	...
	y_offset = stride * data->cif_cfg.main_set.target_y;
		...
		#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T) && defined(FEATURE_WDMA_RGB_MODE)
			uv_offset = stride * data->cif_cfg.main_set.target_y;
		#else
			uv_offset = ALIGNED_BUFF((stride/2), 16) * (data->cif_cfg.main_set.target_y/2);
		#endif
	...
	#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T) && defined(FEATURE_WDMA_RGB_MODE)
		data->cif_cfg.preview_buf[req->count].p_Cb = (unsigned int)NULL;
		data->cif_cfg.preview_buf[req->count].p_Cr = (unsigned int)NULL;
...
void tccxxx_set_camera_addr(int index, unsigned int addr, unsigned int cameraStatus)
{
	...
	if(cameraStatus == 1 /* MODE_PREVIEW */) {
		...
		#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T) && defined(FEATURE_WDMA_RGB_MODE)
			uv_offset = data->cif_cfg.main_set.target_x * data->cif_cfg.main_set.target_y;
	...
	if(cameraStatus == 1 /* MODE_PREVIEW */) {
		...
		#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T) && defined(FEATURE_WDMA_RGB_MODE)
			data->cif_cfg.preview_buf[index].p_Cb = (unsigned int)NULL;
			data->cif_cfg.preview_buf[index].p_Cr = (unsigned int)NULL;
		...
		#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T) && defined(FEATURE_WDMA_RGB_MODE)
			data->cif_cfg.capture_buf.p_Cb = (unsigned int)NULL;
			data->cif_cfg.capture_buf.p_Cr = (unsigned int)NULL;
...
int tccxxx_cif_start_stream(void)
{
	...
	ISP_SetPreview_Control(ON);
		...
		prev_buf 		= NULL;
			...
			field_cnt 	= 0;
			#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR_EP9553T)
			if(hdmi_in_Interlaced)	bUseSimpleDeIntl = ON; // if you use DEINTL_S, you have to change below variable.
			#else
...
int tccxxx_cif_set_resolution(unsigned int pixel_fmt, unsigned short width, unsigned short height)
{
	...
	else if(pixel_fmt == V4L2_PIX_FMT_RGB32) data->cif_cfg.fmt = ARGB8888;  
	else if(pixel_fmt == V4L2_PIX_FMT_RGB565) data->cif_cfg.fmt = RGB565;
