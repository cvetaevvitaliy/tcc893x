...
//added 20140214 swhwang for check to cif status
#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR)
int cur_res; //save current resolution
#endif
int check_int = 1; // check interrupt
...
#if defined(CONFIG_CLOCK_TABLE)
static struct func_clktbl_t *camera_clktbl = NULL;
#endif
...
int tcc_videobuf_querybuf(struct v4l2_buffer *buf)
{
	...
	buf->memory 				= V4L2_MEMORY_MMAP;
		...
		if(data->cif_cfg.fmt == 2)	buf->length = PAGE_ALIGN(pix.width*pix.height*2);	//rgb565	
		else							buf->length	= PAGE_ALIGN(pix.width*pix.height*4);	//Argb8888
	#else
...
int tcc_interrupt_check(int arg)
{
	#if defined(CONFIG_VIDEO_HDMI_IN_SENSOR)
	struct TCCxxxCIF *data = (struct TCCxxxCIF *)(&hardware_data);
	int value = 0;
	...
	if(data->stream_state != STREAM_OFF){
		value = sensor_if_get_max_resolution(0);
		if(cur_res != value && value != 0){
			cur_res = value;
...
int tcc_videobuf_dqbuf(struct v4l2_buffer *buf, struct file *file )
{
	...
	if(list_empty(&data->done_list)) {
		...
		if(wait_event_interruptible_timeout(data->frame_wait, data->wakeup_int == 1, msecs_to_jiffies(500)) <= 0) {
			...
			if(check_int == 1)	check_int = 0; // 20131018 swhwang, for check video frame interrupt
...
static void tcc_set_clock(unsigned char cpu_max)
{
	...
	printk("camera clock index = %d \n", index);

	#if defined(CONFIG_CHIP_TCC8925S) || defined(CONFIG_CHIP_TCC8935S) || defined(CONFIG_CHIP_TCC8933S) || defined(CONFIG_CHIP_TCC8937S)
	{
	...
	#if defined(CONFIG_USE_ISP)
	camera_clktbl = clocktable_get("isp_camera_clktbl");
	#else
		#if defined(CONFIG_ARCH_TCC892X) && defined(CONFIG_VIDEO_ENABLE_CAMERA_WITH_MAX_CLK)
		camera_clktbl = clocktable_get("normal_clktbl");
		#else
		camera_clktbl = clocktable_get("camera_clktbl");
		#endif
	#endif
	if (IS_ERR(camera_clktbl)) {
		camera_clktbl = NULL;
		printk("fail to get camera clock table. \n");
	} else {
		#if defined(CONFIG_ARCH_TCC892X) && defined(CONFIG_VIDEO_ENABLE_CAMERA_WITH_MAX_CLK)
		clocktable_ctrl(camera_clktbl, camera_clktbl->num_clktbls-1, CLKTBL_ENABLE);
		#else
		clocktable_ctrl(camera_clktbl, index, CLKTBL_ENABLE);
		#endif
	}
#else
	// TODO:
...
#if defined(CONFIG_CLOCK_TABLE)
	if (camera_clktbl) {
		clocktable_ctrl(camera_clktbl, 0, CLKTBL_DISABLE);
		clocktable_put(camera_clktbl);
		camera_clktbl = NULL;
	}
#else
	// TODO:
...
static long camera_core_do_ioctl(struct file *file, unsigned int cmd, void *arg)
{
	...
	switch (cmd) {
		...
		case VIDIOC_USER_INT_CHECK: // 20131018 swhwang, for check video frame interrupt
			return tcc_interrupt_check((int)arg);
		case VIDIOC_USER_AUDIO_SR_CHECK:
			return sensor_if_get_audio_samplerate((int)arg);
