		...
		unsigned int buflength)
{
	...
	uvc_trace(UVC_TRACE_CAPTURE, "preview memory size: %u, preview buffer size: %u, preview buffer count: %u. \n", PREVIEW_MEM_SIZE, bufsize, nbuffers);
#else
	...
	struct v4l2_buffer *v4l2_buf)
{
	...
	mutex_lock(&queue->mutex);
	if (v4l2_buf->index >= queue->count) {
		uvc_trace(UVC_TRACE_CAPTURE, "[E] Out of range index. queue->count = %d. \n", queue->count);
		ret = -EINVAL;
		goto done;
	}
