...
#include <linux/power/axp-misc.h>
...
static void axp_set_basecap(struct axp_charger *charger, int base_cap)
{
	...
	__axp_write(charger->master, AXP_DATA_BUFFER4, val);
#endif
#if defined (CONFIG_REGULATOR_AXP192)
	__axp_write(charger->master, AXP_DATA_BUFFER0, val);
#endif
}
...
static int axp_get_basecap(struct axp_charger *charger)
{
	uint8_t val;
#if defined (CONFIG_REGULATOR_AXP202)
	__axp_read(charger->master, AXP_DATA_BUFFER4, &val);
#endif
#if defined (CONFIG_REGULATOR_AXP192)
...
static void axp_set_batcap(struct axp_charger *charger,int cou)
{
	...
	temp[2] = (cou & 0x00ff);
	__axp_writes(charger->master,AXP_DATA_BUFFER2,3,temp);
#endif
#if defined (CONFIG_REGULATOR_AXP192)
	uint8_t temp[3];
	__axp_reads(charger->master, AXP_DATA_BUFFER1,2,temp);
	cou = cou/4;
	temp[0] = ((cou >> 4) | 0x80);
	temp[2] = temp[1] & 0x0f;
	temp[2] |= ((cou & 0x0f) << 4);
	temp[1] = AXP_DATA_BUFFER2;
	__axp_writes(charger->master,AXP_DATA_BUFFER1,3,temp);
#endif
}
...
static int axp_get_batcap(struct axp_charger *charger)
{
	...
	int rValue;
#if defined (CONFIG_REGULATOR_AXP202)
	__axp_reads(charger->master, AXP_DATA_BUFFER2,2,temp);
	rValue = ((temp[0] << 8) + temp[1]);

	if(rValue & 0x8000){
		return ((rValue & 0x7fff) == 0 ? 1 : (rValue & 0x7fff));
...
void axp_charging_monitor(struct axp_charger *charger)
{
	...
	/* 更新电池开路电压 */
	__axp_reads(charger->master,AXP_OCV_BUFFER0,2,v);
...
#if defined (CONFIG_REGULATOR_AXP202)
		__axp_read(charger->master, AXP_CAP,&val);
#endif
...
#if defined (CONFIG_REGULATOR_AXP202)
			__axp_write(charger->master, AXP_DATA_BUFFER5, rt_rest_cap);
...
#if defined (CONFIG_REGULATOR_AXP202)
			__axp_read(charger->master, AXP_DATA_BUFFER5, &val);
	...
	if(charger->rest_cap - pre_rest_cap){
		...
		pre_rest_cap = charger->rest_cap;
#if defined (CONFIG_REGULATOR_AXP202)
		__axp_write(charger->master,AXP_DATA_BUFFER1,charger->rest_cap | 0x80);
#endif
...
if(axp_debug){
	DBG_PSY_MSG("is on = %d\n", charger->is_on);
	DBG_PSY_MSG("ac det = %d\n", charger->ac_det);
	DBG_PSY_MSG("usb det = %d\n", charger->usb_det);
...
#if defined (CONFIG_AXP_DEBUG)
	__axp_reads(charger->master,AXP_DATA_BUFFER0,AXP_DATA_NUM,data_mm);
...
EXPORT_SYMBOL(axp_charging_monitor);
...
static void axp_battery_infoinit(struct axp_charger *charger)
{
...
#if defined (CONFIG_REGULATOR_AXP202)
	__axp_read(charger->master, AXP_CAP,&val2);
	charger->ocv_rest_cap = ((val2 & 0x7F)==127)? 100:(val2 & 0x7F);
	
	__axp_read(charger->master,AXP_DATA_BUFFER1,&val1);
	...
	__axp_read(charger->master,AXP_DATA_BUFFER0,&val1);
...
int axp_suspend(struct axp_charger *charger)
{
	/* 清irq */
	...
	/* 设置阈值时间为127分钟 */
	__axp_write(charger->master, AXP_TIMER_CTL, 0x80);
	__axp_write(charger->master, AXP_TIMER_CTL, 0x7F);

	return 0;
}
EXPORT_SYMBOL(axp_suspend);

int axp_resume(struct axp_charger *charger)
{
	int pre_rest_cap;
	int rt_rest_cap = 0;
	...
	if(charger->bat_det == 1){
		...
		__axp_read(charger->master, AXP_TIMER_CTL, &val);
...
if(axp_debug){
	...
	DBG_PSY_MSG("charger->iac = %d\n",charger->iac);
#if defined (CONFIG_REGULATOR_AXP202)
	__axp_reads(charger->master,0xbc,2,v);
	charger->ocv = ((v[0] << 4) + (v[1] & 0x0f)) * 11 /10;
#endif
	DBG_PSY_MSG("charger->ocv = %d\n",charger->ocv);
	...
	rdc = (((v[0] & 0x1F) << 8) | v[1]) * 10742 / 10000;
	DBG_PSY_MSG("rdc = %d\n",rdc);
	__axp_read(charger->master,0x8,v);
...
#if defined (CONFIG_REGULATOR_AXP202)
	/* 休眠低电关机接口 */
	__axp_read(charger->master, AXP_DATA_BUFFERA,&val);
	printk("[AXP]suspend power off val = 0x%x\n",val);
	if(val){
		axp_set_bits(charger->master,AXP_INTSTS5,0x03);
	}
	__axp_write(charger->master, AXP_DATA_BUFFERA,0x00);
#endif
