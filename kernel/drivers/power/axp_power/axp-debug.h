...
{
    uint8_t val;
	__axp_read(charger_axp->master,axp_reg_addr,&val);
			...
			struct class_attribute *attr,	const char *buf, size_t count)
{
	...
	if( tmp < 256 )
		...
		__axp_write(charger_axp->master,axp_reg_addr, val);
	}
	return count;
}
...
{
  uint8_t val[4];
	__axp_reads(charger_axp->master,axp_reg_addr,4,val);
			...
			struct class_attribute *attr,	const char *buf, size_t count)
{
	...
	if( tmp < 256 )
		...
		val[4] = (tmp >>  0)& 0xFF;
		__axp_writes(charger_axp->master,axp_reg_addr,5,val);
	}
	return count;
}
