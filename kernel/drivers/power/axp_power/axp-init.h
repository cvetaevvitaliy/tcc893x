...
extern struct axp_charger *charger_axp;
...
static void axp_clear_irq(struct axp_charger *charger)
{
	...
	irq_w[7] = AXP_INTSTS5;
	irq_w[8] = 0xff;
    __axp_writes(charger->master, AXP_INTSTS1, 9, irq_w);
#endif
#if defined (CONFIG_REGULATOR_AXP192)	
	__axp_writes(charger->master, AXP_INTSTS1, 7, irq_w);
#endif
}

#if defined (CONFIG_REGULATOR_AXP202)
int axp_get_temp(struct axp_charger *charger)
{
	uint8_t val[2];
	uint16_t tmp;
	
	__axp_reads(charger->master, POWER20_INTER_TEMPER, 2, val);
	tmp = ((val[0] << 4) | (val[1] & 0xF)) & 0xFFF;
	tmp = (tmp - 1447) /10;
	return tmp;
}
EXPORT_SYMBOL(axp_get_temp);
#endif

static int axp_get_freq(struct i2c_client *client)
{
	int  ret = 25;
	uint8_t  temp;
	__axp_read(client, AXP_ADC_CONTROL3,&temp);
	temp &= 0xc0;
	switch(temp >> 6){
		case 0:	ret = 25; break;
		case 1:	ret = 50; break;
...
{
	int ret = 0;
#if (1) //defined (CONFIG_AXP_FRQSET)
	uint8_t val;	
	if(frq >= 25 && frq <= 200){
		if(frq < 50)
			val = 0x0;
			...
			val = 0xc0;		
		ret = axp_update(charger_axp->master, AXP_ADC_CONTROL3, val, 0xC0);
	}
#endif
	return ret;
}

int axp_set_chargecurrent(int cur)
{
	int ret = 0;
#if (1) //defined (CONFIG_AXP_CHGCURSET)
	uint8_t tmp;
	if(cur == 0)
		ret = axp_clr_bits(charger_axp->master,AXP_CHARGE_CONTROL1,0x80);
	else
		ret = axp_set_bits(charger_axp->master,AXP_CHARGE_CONTROL1,0x80);
	if(ret)
		return ret;
	...
	if(cur >= 300 && cur <= 1800){
		tmp = (cur -201)/100;
		ret = axp_update(charger_axp->master, AXP_CHARGE_CONTROL1, tmp, 0x0F);
	}
#endif
...
EXPORT_SYMBOL(axp_set_chargecurrent);
...
static int axp_set_chargevoltage(int vol)
{
	...
	if(vol >=4100 && vol <=4360){
		if(vol < 4150)
			...
			val |= 3 << 5;
    	ret = axp_update(charger_axp->master, AXP_CHARGE_CONTROL1,val,0x60);
    }
#endif
	return ret;
}
...
static int axp_set_chargeendrate(int end)
{
	...
	switch (end){
		case 10: ret = axp_clr_bits(charger_axp->master, AXP_CHARGE_CONTROL1,0x10);break;
		case 15: ret = axp_set_bits(charger_axp->master, AXP_CHARGE_CONTROL1,0x10);break;
		default: break;
	}
#endif
	return ret;
...
static int axp_set_prechargetime(int pre)
{
	...
	if(pre >= 40 && pre <= 70){
		if (pre < 50)
			...
			val = 0xc0;
		ret = axp_update(charger_axp->master, AXP_CHARGE_CONTROL2,val,0xC0);
	}
#endif
...
static int axp_set_cstchargetime(int cst)
{
	...
	if(cst >= 360 && cst <= 720){
		if (cst < 480)
			...
			val = 0x03;
		ret = axp_update(charger_axp->master, AXP_CHARGE_CONTROL2,val,0x03);
	}
#endif
...
static int axp_battery_adc_set(struct axp_charger *charger)
{
	...
	/* 使能adc */
	val= 0
		| AXP_ADC_BATVOL_ENABLE
		| AXP_ADC_BATCUR_ENABLE
		| AXP_ADC_DCINCUR_ENABLE
		| AXP_ADC_DCINVOL_ENABLE
		| AXP_ADC_USBVOL_ENABLE
		| AXP_ADC_USBCUR_ENABLE
#if TS_ALERT_EN
		| AXP_ADC_TSVOL_ENABLE
#endif
		;

	ret = __axp_write(charger->master, AXP_ADC_CONTROL1, val);
	if (ret){
		printk("[AXP]Unable to enable adc1\n");
		return ret;
	}
	val= 0
#if 1 //INTEMP_MONITOR_EN
		| AXP_ADC_ICTEMP_ENABLE
#endif
		;
	ret = __axp_write(charger->master, AXP_ADC_CONTROL2, val);	
	if (ret){
		printk("[AXP]Unable to enable adc2\n");
		return ret;
	}
	val= 0
#if TS_ALERT_EN // enable battery temperature monitor
#else
		| BIT(2)
#endif
		| 0x02
		;
	axp_update(charger->master, AXP_ADC_CONTROL3, val, 0x07);
	/* 设置adc采样率 */
...
static int axp_set_pekinfo(struct platform_device *pdev)
{
	...
	powerkeydev->name = "axp-power-key";//pdev->name;
	powerkeydev->phys = pdev->name;//"m1kbd/input2";
	powerkeydev->evbit[0] = BIT(EV_KEY);
	set_bit(KEY_POWER & KEY_MAX, powerkeydev->keybit);
...
static int axp_set_pekopen(int pek)
{
	...
	if(pek >=128 && pek <=3000){	
		if (pek < 1000)
			...
			val = 0x40;
		ret = axp_update(charger_axp->master,AXP_PEK_SET,val,0xc0);
		if(ret)
...
static int axp_set_peklong(int pek)
{
	...
	if(pek >=1000 && pek <=2500){
		if (pek < 1500)
			...
			val = 0x30;
		ret = axp_update(charger_axp->master,AXP_PEK_SET,val,0x30);
		if(ret){
...
static int axp_set_pekoffen(int pek)
{
	...
	switch(pek){
		case 0: ret = axp_clr_bits(charger_axp->master, AXP_PEK_SET,0x08);break;
		case 1: ret = axp_set_bits(charger_axp->master, AXP_PEK_SET,0x08);break;
		default: ret = -1;break;
	}
	if(ret){
		printk("[AXP]Unabel to set pek long time\n");
...
static int axp_set_pekdelay(int pek)
{
	...
	switch(pek){
		case 8: ret = axp_clr_bits(charger_axp->master, AXP_PEK_SET,0x04);break;
		case 64: ret = axp_set_bits(charger_axp->master, AXP_PEK_SET,0x04);break;
		default: ret = -1;break;
	}
	if(ret){
...
static int axp_set_pekoff(int pek)
{
	...
	if(pek >=4000 && pek <=10000){	
		if (PEKOFF < 6000)
			...
			val = 0x03;
		ret = axp_update(charger_axp->master,AXP_PEK_SET,val,0x03);
		if(ret){
...
#if 0
static int axp_set_voff(int voff)
{
	int ret =0;
	uint8_t val;

	if(voff >= 2600 && voff <= 3300){
		val = ((voff - 2600) / 100) & 0x07;
		ret = axp_update(charger_axp->master,AXP_VOFF_SET,val,0x07);
		if(ret){
			printk("[AXP]Unabel to set off voltage\n");
		}
	}
...
static int axp_set_noe(int noe)
{
	...
	if(noe >=128 && noe <=3000){	
		if (noe < 1000)
			...
			val = 0x03;
		ret = axp_update(charger_axp->master,AXP_OFF_CTL,val,0x03);
		if(ret){
...
static int axp_set_otpoffen(int otp)
{
	...
	switch(otp){
		case 0: ret = axp_clr_bits(charger_axp->master, AXP_HOTOVER_CTL,0x04);break;
		case 1: ret = axp_set_bits(charger_axp->master, AXP_HOTOVER_CTL,0x04);break;
		default: ret = -1;break;
	}
	if(ret){
...
static int axp_set_opclinit(void)
{
	...
	/* 设置关机电压 */
	#if 0
	ret = axp_set_voff(VOFF);
	if(ret){
		printk("[AXP]Unable to set voff\n");
		return ret;
	}
	#endif
	/* 设置N_OE开关属性 */
...
static int axp_set_ocv(void)
{
	...
	ocv_cap[29] = 0xCF;
	ocv_cap[30] = OCVREGF;
	ret = __axp_writes(charger_axp->master, 0xC0,31,ocv_cap);
	if(ret){
...
static int axp_set_warning(int warn1,int warn2)
{
	...
	if(warn1 >= 2867200 && warn1 <= 4200000)
		...
		val = 0x68;
	/* 3.5552V--%5 close*/
	ret = __axp_write(charger_axp->master, AXP_APS_WARNING1,val);
	if(ret){
	...
	if(warn2 >= 2867200 && warn2 <= 4200000)
		...
		val = 0x5F;
	ret = __axp_write(charger_axp->master, AXP_APS_WARNING2,val);
	if(ret){
		printk("[AXP]Unable to set warning voltage 1\n");
	}
#endif
...
static int axp_set_rdcclose(void)
{
	...
	int rdc;
#if defined (CONFIG_REGULATOR_AXP202)
	axp_set_bits(charger_axp->master,AXP_CAP,0x80);
	axp_clr_bits(charger_axp->master,AXP_RDC_BUFFER0,0x80);
	/* 设置电池内阻的默认值 */
	__axp_reads(charger_axp->master,AXP_RDC_BUFFER0,2,val);
	rdc = (((val[0] & 0x1F) << 8) | val[1]) * 10742 / 10000;
	__axp_read(charger_axp->master,AXP_DATA_BUFFER0,&val[0]);
	if((((val[0] >> 7) & 0x1) == 0)||(rdc > BATRDC*3 )||(rdc < BATRDC/4)){
		rdc = (BATRDC * 10000 + 5371) / 10742;
		__axp_write(charger_axp->master,AXP_RDC_BUFFER1,rdc & 0x00FF);
		axp_update(charger_axp->master, AXP_RDC_BUFFER0, (rdc >> 8), 0x1F);
	}
	ret = axp_clr_bits(charger_axp->master,AXP_CAP,0x80);
#endif

#if defined (CONFIG_REGULATOR_AXP192)
	ret = __axp_reads(charger_axp->master,AXP_RDC_BUFFER0,2,val);
	rdc = val[1] * 2;
	if(!(val[0] & 0x08)|| rdc > BATRDC*3 || rdc < BATRDC/4){
		val[1] = (BATRDC) / 2;
		axp_set_bits(charger_axp->master, AXP_RDC_BUFFER0, 0x08);
		__axp_write(charger_axp->master,AXP_RDC_BUFFER1,val[1]);
	}
#endif
...
static int axp_usb_vollim(int vol)
{
	...
	uint8_t val;
	if(vol){
		axp_set_bits(charger_axp->master,AXP_CHARGE_VBUS, 0x40);
		if(vol >= 4000 && vol <=4700){
			val = ((vol - 4000)/100) << 3;
			ret = axp_update(charger_axp->master, AXP_CHARGE_VBUS,val,(0x7 << 3));
			if(ret)
				printk("[AXP]Unable to set usb voltage set %d mV\n",val);
		}
	}
	else{
		ret = axp_clr_bits(charger_axp->master, AXP_CHARGE_VBUS, 0x40);	
		if(ret)
...
#if defined (CONFIG_REGULATOR_AXP202)
	if(cur){
		axp_clr_bits(charger_axp->master, AXP_CHARGE_VBUS, 0x01);
		if(cur == 900){
			ret = axp_clr_bits(charger_axp->master, AXP_CHARGE_VBUS, 0x03);
			if(ret)
				printk("[AXP]Uable to set usb current set 900mA\n");
    	}
		else if (cur == 500){
			ret = axp_update(charger_axp->master, AXP_CHARGE_VBUS,0x01, 0x03);
			if(ret)
				printk("[AXP]Uable to set usb current set 500mA\n");
		}
		else if (cur == 100){
			ret = axp_update(charger_axp->master, AXP_CHARGE_VBUS,0x10, 0x03);
			if(ret)
				printk("[AXP]Uable to set usb current set 100mA\n");
		}
	}
	else
		ret = axp_set_bits(charger_axp->master, AXP_CHARGE_VBUS, 0x03);
		if(ret)
...
#if defined (CONFIG_REGULATOR_AXP192)
	if(cur){
		axp_set_bits(charger_axp->master, AXP_CHARGE_VBUS, 0x02);
		if (cur == 500){
			ret = axp_clr_bits(charger_axp->master, AXP_CHARGE_VBUS, 0x01);
			if(ret)
				printk("[AXP]Uable to set usb current set 500mA\n");
		}
		else if (cur == 100){
			ret = axp_set_bits(charger_axp->master, AXP_CHARGE_VBUS, 0x01);
			if(ret)
				printk("[AXP]Uable to set usb current set 100mA\n");
		}
	}
	else
		ret = axp_clr_bits(charger_axp->master, AXP_CHARGE_VBUS, 0x02);
		if(ret)
			printk("[AXP]Uable to set usb current set 0mA\n");
#endif
...
static int axp_battery_first_init(struct axp_charger *charger)
{
	...
	ret = axp_clr_bits(charger->master,AXP_HOTOVER_CTL,0x88);
...
static int axp_get_rdc(struct axp_charger *charger)
{
	...
	int rdc;
	uint8_t v[2];
	__axp_reads(charger->master,AXP_RDC_BUFFER0,2,v);
#if defined (CONFIG_REGULATOR_AXP202)
	rdc = (((v[0] & 0x1F) << 8) | v[1]) * 10742 / 10000;
	__axp_read(charger->master,AXP_DATA_BUFFER0,&v[0]);
#endif
...
static void axp_cal_rdc(struct axp_charger *charger)
{
	...
	if(charger->is_on && (axp_ibat_to_mA(charger->adc.ichar_res) > 200) && (charger->vbat > 3600) && (charger->disibat >= 400)){
#if defined (CONFIG_REGULATOR_AXP202)
		__axp_read(charger->master,AXP_DATA_BUFFER0,&val);
		if((((val >> 7) == 0) || (((val >> 3) & 0x01) == 0) )&& (rdc_count >= TIMER3)){
			...
			__axp_write(charger->master,AXP_RDC_BUFFER1,rdc & 0x00FF);
...
#if defined (CONFIG_REGULATOR_AXP192)
		__axp_read(charger->master,AXP_DATA_BUFFER2,&val);
		if(((((val >> 3)& 0x01) == 0) || (((val >> 0) & 0x01) == 0) )&& (rdc_count >= TIMER3)){
			rdc = axp_get_rdc(charger);
			__axp_write(charger->master,AXP_RDC_BUFFER1,(rdc / 2) & 0x00FF);
...
static int axp_get_coulomb(struct axp_charger *charger)
{
	...
	__axp_reads(charger->master, AXP_CCHAR3_RES,8,temp);
	...
	m = axp_get_freq(charger->master) * 480;
	do_div(rValue,m);
	if(rValue1 >= rValue2)
...
static int axp_get_coulomb(struct axp_charger *charger)
{
	...
	__axp_read(charger->master,AXP_IC_TYPE, &temp[0]);
	if( (temp[0] & 0x0F) == 0x03){
		ic_type = 1;
	}
	else{
	...
	if(ic_type){
		...
		m = axp_get_freq(charger->master) * 225;
		do_div(rValue,m);
	}
	else{
		rValue1 = ((temp[0] << 24) + (temp[1] << 16) + (temp[2] << 8) + temp[3]);
		...
		m = axp_get_freq(charger->master)* 3600 * 2;
		do_div(rValue,m);
	}
	if(axp_debug){
...
{
	uint8_t temp;
	__axp_read(charger->master, AXP_COULOMB_CONTROL, &temp);
	temp |= 0x20;
	temp &= 0xbf;
	__axp_write(charger->master, AXP_COULOMB_CONTROL, temp);
	temp |= 0x80;
	temp &= 0xbf;
	__axp_write(charger->master, AXP_COULOMB_CONTROL, temp);
}
...
static uint8_t axp_ocv_restcap(struct axp_charger *charger)
{
	int rdc;
	uint8_t val[2];
	__axp_reads(charger->master,AXP_RDC_BUFFER0,2,val);
	rdc = val[1] * 2;
