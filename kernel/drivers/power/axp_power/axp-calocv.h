...
#endif
#if defined (CONFIG_REGULATOR_AXP192)
		__axp_write(charger->master,AXP_DATA_BUFFER0,charger->rest_cap | 0x80);
#endif
...
static int axp_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	__axp_write(charger->master, AXP_TIMER_CTL, 0x02);
...
static int axp_resume(struct platform_device *dev)
{
	...
	if(charger->bat_det == 1){
		...
		__axp_read(charger->master, AXP_TIMER_CTL, &val1);
		pre_rest_cap = charger->rest_cap;
	...
	__axp_write(charger->master, 0x0e,0x00);
#endif
