...
static void axp_chargingstate_monitor(struct work_struct *work)
{
	...
	__axp_read(charger->master,AXP_INTSTS5,&press);
	/* 按下 */
	if(press & 0x20){
		printk("press down\n");
		input_report_key(powerkeydev, KEY_POWER, 1);
		input_sync(powerkeydev);
		__axp_write(charger->master,AXP_INTSTS5,press);
	}
...
#if defined(CONFIG_REGULATOR_AXP192)
	__axp_read(charger->master,AXP_INTSTS3,&press);
	if(press_count > 0)
		press_count--;
	/* 短按 */
	if(press & 0x2){
		...
		input_report_key(powerkeydev, KEY_POWER, 1);
		input_sync(powerkeydev);
		__axp_write(charger->master,AXP_INTSTS3,press);
		press_count = 1;
	}
