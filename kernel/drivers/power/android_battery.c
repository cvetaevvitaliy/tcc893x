...
extern int tcc_charger_init(struct tcc_charger *charger);
extern void tcc_battery_infoinit(struct tcc_charger *charger);
#endif 
...
static int android_bat_enable_charging(struct android_bat_data *battery,
					bool enable);
static int android_power_get_property(struct power_supply *ps,
				enum power_supply_property psp,
				union power_supply_propval *val);
static unsigned int check_boot_bat = 0;
				...
				union power_supply_propval *val)
{
	struct android_bat_data *battery =
		container_of(ps, struct android_bat_data, psy_ac);

	switch (psp) {
	case POWER_SUPPLY_PROP_ONLINE:
		if((ps->type = POWER_SUPPLY_TYPE_MAINS)){
			val->intval = (battery->pdata->poll_charge_source() == CHARGE_SOURCE_AC ? 1:0);
			}
		else val->intval = 0;
		break;
	default:
...
{
	struct android_bat_data *battery =
		container_of(ps, struct android_bat_data, psy_usb);

	switch (psp) {
	case POWER_SUPPLY_PROP_ONLINE:
		if((ps->type = POWER_SUPPLY_TYPE_USB) && battery->pdata->poll_charge_source)
			val->intval = (battery->pdata->poll_charge_source() == CHARGE_SOURCE_USB ? 1:0);			
		else val->intval = 0;
		break;
	default:
		return -EINVAL;
...
#ifndef CONFIG_REGULATOR_TCC270	
...
#ifdef CONFIG_REGULATOR_TCC270
		batt_temp = battery->pdata->get_temperature(&batt_temp);
#else
...
*/
	...
	if(check_boot_bat>3)
		...
		android_bat_monitor_set_alarm(battery, 1);
		tcc_battery_infoinit(charger_tcc);
		check_boot_bat++;
	}
#else
	android_bat_monitor_set_alarm(battery, FAST_POLL);
...
static __devinit int android_bat_probe(struct platform_device *pdev)
{
	...
	battery->psy_ac.properties = power_properties,
	battery->psy_ac.num_properties =  ARRAY_SIZE(power_properties),
	battery->psy_ac.get_property = android_ac_get_property,

	battery->psy_usb.name = "usb",
	battery->psy_usb.type = POWER_SUPPLY_TYPE_USB,
	battery->psy_usb.properties = power_properties,
	battery->psy_usb.num_properties =  ARRAY_SIZE(power_properties),
	...
	if (ret) {
		dev_err(battery->dev, "%s: failed to register ac supply\n",
			__func__);
		goto err_psy_bat_reg;
	}
	...
	if (ret) {
		dev_err(battery->dev, "%s: failed to register usb supply\n",
			__func__);
		goto err_psy_bat_reg;
	}
#if defined(CONFIG_REGULATOR_TCC270)
...
static int android_bat_suspend(struct device *dev)
{
...
#ifndef CONFIG_BATTERY_SET_ALARM
	alarm_cancel(&battery->monitor_alarm);
#endif
	
#if defined(CONFIG_BATTERY_SET_ALARM)
	...
	if(battery->pdata->battery_suspend)
		battery->pdata->battery_suspend(); // add by Telechips
...
static void android_bat_resume(struct device *dev)
{
	...
	if(battery->pdata->battery_resume)
		battery->pdata->battery_resume(); // add by Telechips
