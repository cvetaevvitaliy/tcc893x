...
#ifdef CONFIG_PM_RUNTIME
static void tcc_fb_runtime_suspend(struct device *dev)
{
	printk("%s:  \n", __FUNCTION__);
	console_lock();
	fb_power_state = 0;
...
static void tcc_fb_runtime_resume(struct device *dev)
{
	printk("%s:  \n", __FUNCTION__);
	
	console_lock();

	tca_fb_late_resume(dev);
...
#else
#define tcc_fb_runtiem_suspend	NULL
#define tcc_fb_runtiem_resume	NULL
#endif
...
static const struct dev_pm_ops tccfb_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(tccfb_suspend, tccfb_resume)
	SET_RUNTIME_PM_OPS(tcc_fb_rumtime_susepnd, tcc_fb_rumtime_resumt, NULL)
};
