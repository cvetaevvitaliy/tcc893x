/*
 * lcd_lp101wx1.c -- support for LP101WX1 LVDS Panel
 *
...
static int lp101wx1_probe(struct platform_device *pdev)
{
	...
	lp101wx1_panel.state = 1;
	lp101wx1_panel.dev = &pdev->dev;
	#if defined(CONFIG_ARCH_TCC892X) || defined(CONFIG_ARCH_TCC893X)
	...
	#endif
	clk_enable(lvds_clk);	
	tccfb_register_panel(&lp101wx1_panel);
	return 0;
}

static int lp101wx1_remove(struct platform_device *pdev)
{
	return 0;
}

static struct platform_driver lp101wx1_lcd = {
	.probe	= lp101wx1_probe,
	.remove	= lp101wx1_remove,
	.driver	= {
		.name	= "lp101wx1_lcd",
		.owner	= THIS_MODULE,
	},
...
static __init int lp101wx1_init(void)
{
	printk("~ %s ~ \n", __func__);
	return platform_driver_register(&lp101wx1_lcd);
}

static __exit void lp101wx1_exit(void)
{
	platform_driver_unregister(&lp101wx1_lcd);
}

subsys_initcall(lp101wx1_init);
module_exit(lp101wx1_exit);
