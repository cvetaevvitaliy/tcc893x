...
extern void axp_charger_init(struct axp_charger *charger);
...
static int axp202_pmic_probe(struct i2c_client *client, const struct i2c_device_id *i2c_id)
{
...
#if defined(CONFIG_BATTERY_ANDROID)
	if(charger_axp != NULL)
		charger_axp->master = client;
	axp_charger_init(charger_axp);
#endif	
	...
	if (client->irq) {
		...
		/* clear irq status */
		while(i2c_smbus_read_byte_data(client, AXP202_IRQ_STS1_REG) != 0)
		{
		...
		i2c_send_count = 0;
		while(i2c_smbus_read_byte_data(client, AXP202_IRQ_STS2_REG) != 0)
		{
		...
		i2c_send_count = 0;
		while(i2c_smbus_read_byte_data(client, AXP202_IRQ_STS3_REG) != 0)
		{
		...
		i2c_send_count = 0;
		while(i2c_smbus_read_byte_data(client, AXP202_IRQ_STS4_REG) != 0)
		{
		...
		i2c_send_count = 0;
		while(i2c_smbus_read_byte_data(client, AXP202_IRQ_STS5_REG) != 0)
		{
