...
static int itv_lgdt3305_activate(itv_object_t *p_this)
{
	...
	do {
		...
		if ((lg_fail(ret)) | (val == 0))
			eprintk("Fail to identify lgdt3305. reg(0x%04x)=%02x [%d]\n", LGDT3305_GEN_CTRL_2, val, reset_retry_count);
		
		msleep(50);
	} while(--reset_retry_count > 0);
	if((reset_retry_count == 0) && (val == 0))
		goto fail;
