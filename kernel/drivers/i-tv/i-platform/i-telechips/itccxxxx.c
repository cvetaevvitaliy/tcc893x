...
static void itv_platform_demod_reset(void)
{
	...
	regs = (volatile PGPION)tcc_p2v(HwGPIOB_BASE);

	BITCSET(regs->GPFN1.nREG, (Hw24-Hw20), 0);
	BITSET(regs->GPEN.nREG, Hw13);
	BITSET(regs->GPDAT.nREG, Hw13);				//TS_RST
	msleep(50);
	BITCLR(regs->GPDAT.nREG, Hw13);
	msleep(50);
	BITSET(regs->GPDAT.nREG, Hw13);
#else
	PGPION regs;
