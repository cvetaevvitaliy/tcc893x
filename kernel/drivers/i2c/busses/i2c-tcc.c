...
static int tcc_i2c_probe(struct platform_device *pdev)
{
	...
	i2c = kzalloc(sizeof(struct tcc_i2c), GFP_KERNEL);
	if (!i2c) {
		ret = -ENOMEM;
		goto err_nockl;
	}
	...
	i2c->core     = pdev->id;
	i2c->regs     = (volatile struct tcc_i2c_regs *) IO_ADDRESS(resource->start);
	i2c->IRQSTR   = resource->end;
	i2c->smu_i2c_flag  = tcc_platform_data->smu_i2c_flag;
	i2c->i2c_clk_rate  = tcc_platform_data->i2c_ch_clk_rate;
	i2c->core_clk_rate = tcc_platform_data->core_clk_rate;
	memcpy(i2c->core_clk_name, tcc_platform_data->core_clk_name, sizeof(tcc_platform_data->core_clk_name));
...
#ifdef CONFIG_PM
static int tcc_i2c_suspend(struct platform_device *pdev, pm_message_t state)
{
	struct tcc_i2c *i2c = platform_get_drvdata(pdev);
...
static int tcc_i2c_resume(struct platform_device *pdev)
{
	struct tcc_i2c *i2c = platform_get_drvdata(pdev);
