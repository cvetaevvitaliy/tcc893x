...
#define TS_IRQ 			INT_EINT2
#else
#define TS_IRQ 			INT_EI2
#endif
...
static void sitronix_ts_work(struct work_struct *work)
{
	...
	for(i=0 ; i<CHECK_FINGERS; i++)
	{
		fingerbuf[0] = 0x10; // finger register
		if ((ret = i2c_master_send(priv->client, fingerbuf, 1)) != 1)
		return -EIO;
	
		if ((ret = i2c_master_recv(priv->client, fingerbuf, 1)) != 1)
			return -EIO;
		temp += (u32)fingerbuf[0];
	}

	fingers = (temp/CHECK_FINGERS);
...
//	input->close = sitronix_ts_close;
	...
	err = request_irq(TS_IRQ, sitronix_ts_isr, IRQF_TRIGGER_FALLING | IRQF_DISABLED,
			  DEVICE_NAME, priv);
	if (err) {
