...
char M2M_Scaler_Ctrl_Detail(SCALER_TYPE *scale_img)
{
	...
	if((scale_img->src_fmt > SC_IMG_FMT_ARGB6666) && (pSrcBase1 == (unsigned int)NULL || pSrcBase2 == (unsigned int)NULL)) { // address limitation!!
	...
	if((scale_img->dest_fmt > SC_IMG_FMT_ARGB6666) && (pDstBase1 == (unsigned int)NULL || pDstBase2 == (unsigned int)NULL)) { // address limitation!!
...
{
	int ret = 0;
	unsigned char rdma_num, sc_num, wmix_num, wdma_num;
	...
	if(sc_data.dev_opened == 0)
	{
		if(sc_data.block_operating)
		{
			...
			BITCSET(pIREQConfig->uSOFTRESET.nREG[1], (1<<wdma_num), (1<<wdma_num));
			//BITCSET(pIREQConfig->uSOFTRESET.nREG[1], (1<<wmix_num), (1<<wmix_num));
			BITCSET(pIREQConfig->uSOFTRESET.nREG[0], (1<<sc_num), (1<<sc_num));
			BITCSET(pIREQConfig->uSOFTRESET.nREG[0], (1<<rdma_num), (1<<rdma_num));

			BITCSET(pIREQConfig->uSOFTRESET.nREG[0], (1<<rdma_num), (0<<rdma_num));
			BITCSET(pIREQConfig->uSOFTRESET.nREG[0], (1<<sc_num), (0<<sc_num));
			//BITCSET(pIREQConfig->uSOFTRESET.nREG[1], (1<<wmix_num), (0<<wmix_num));
			BITCSET(pIREQConfig->uSOFTRESET.nREG[1], (1<<wdma_num), (0<<wdma_num));
		#else
		if(sc_data.irq_reged)	{
...
{	
	...
	if(!sc_data.irq_reged) {
				...
				BITCSET(pIREQConfig->uSOFTRESET.nREG[1], (1<<6), (1<<6)); 	// WDMA06 reset
				BITCSET(pIREQConfig->uSOFTRESET.nREG[1], (1<<15), (1<<15)); 	// WMIX05 reset
				...
				BITCSET(pIREQConfig->uSOFTRESET.nREG[1], (1<<15), (0<<15)); 	// WMIX05 reset
				BITCSET(pIREQConfig->uSOFTRESET.nREG[1], (1<<6), (0<<6)); 	// WDMA06 reset
				...
				BITCSET(pIREQConfig->uSOFTRESET.nREG[1], (1<<3), (1<<3)); 	// WDMA03 reset
				//BITCSET(pIREQConfig->uSOFTRESET.nREG[1], (1<<13), (1<<13)); 	// WMIX03 reset
				BITCSET(pIREQConfig->uSOFTRESET.nREG[0], (1<<29), (1<<29)); 	// SCALER1 reset
				BITCSET(pIREQConfig->uSOFTRESET.nREG[0], (1<<12), (1<<12)); 	// RDMA12 reset

				BITCSET(pIREQConfig->uSOFTRESET.nREG[0], (1<<12), (0<<12)); 	// RDMA12 reset
				BITCSET(pIREQConfig->uSOFTRESET.nREG[0], (1<<29), (0<<29)); 	// SCALER1 reset
				//BITCSET(pIREQConfig->uSOFTRESET.nREG[1], (1<<13), (0<<13)); 	// WMIX03 reset
				BITCSET(pIREQConfig->uSOFTRESET.nREG[1], (1<<3), (0<<3)); 	// WDMA03 reset
