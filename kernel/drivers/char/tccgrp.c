...
static struct func_clktbl_t *overlay_clk_tbl = NULL;
#endif//
...
#if defined(CONFIG_CLOCK_TABLE)
		if(overlay_clk_tbl)
			clocktable_ctrl(overlay_clk_tbl, 0, CLKTBL_DISABLE);
#endif//
...
#if defined(CONFIG_CLOCK_TABLE)
		if(overlay_clk_tbl)
			clocktable_ctrl(overlay_clk_tbl, 0, CLKTBL_ENABLE);
#endif//
...
#if defined(CONFIG_CLOCK_TABLE)
	if (overlay_clk_tbl) {
		if(overlay_clk_tbl->count)
			clocktable_ctrl(overlay_clk_tbl, 0, CLKTBL_DISABLE);
		clocktable_put(overlay_clk_tbl);
		overlay_clk_tbl = NULL;
	}
#endif
...
#if defined(CONFIG_CLOCK_TABLE)
	overlay_clk_tbl = clocktable_get("overlay_clktbl");
	if(IS_ERR(overlay_clk_tbl)) {
		overlay_clk_tbl = NULL;
		printk(" ### %s clotable_get err %d \n",__func__,__LINE__);
	}
#endif
