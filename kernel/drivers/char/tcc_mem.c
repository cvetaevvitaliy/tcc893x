...
extern  int tcc_video_swap_vpu_frame(int idx);
...
extern  int tcc_video_ctrl_last_frame(int enable);
#endif
	...
	mutex_lock(&io_mutex);
	{
		switch(cmd)
		{
			...
			case TCC_LCDC_VIDEO_SWAP_VPU_FRAME:
				ret = tcc_video_swap_vpu_frame((int)arg);
			break;

			case TCC_LCDC_CTRL_LAST_FRAME:
				ret = tcc_video_ctrl_last_frame((int)arg);
			break;
			...
			{
				ret = 0;
				if( (int)arg < 0 || (int)arg >= MAX_VPU_INSTANCE ){
					ret = -1;
					goto Error;
				}
				ret = curr_displaying_idx[(int)arg];
			}
			break;
			...
			case TCC_VIDEO_GET_DISPLAYED_BUFFER_ID:
			{
				...
				ret = curr_displayed_buffer_id[(int)arg];
			}
			break;
#endif
