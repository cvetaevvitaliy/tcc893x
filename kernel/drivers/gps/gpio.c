...
{  
	...
	if(machine_is_m805_893x())
	{
		gpio_set_value(TCC_GPC(19), 0); // tcc892x : gpio_c(14)
	}
#endif
...
static long gps_gpio_ioctl (struct file *filp, unsigned int cmd, void *arg)  
{
	...
	{  
		...
		case 0:
			if(machine_is_m805_893x()){
				gpio_set_value(TCC_GPE(17), 1);  // tcc892x : gpio_e(14)
			}
			gps_dbg("tccxxxx : gps_gpio_on\n");
			break;	 
		case 1:
...
{  
	...
	{
		gps_dbg("GPS_PWREN on\n");
		tcc_gpio_config(TCC_GPE(17), GPIO_FN(0)); // tcc892x :gpio_e(14)
		gpio_request(TCC_GPE(17), "GPIO_PWREN");
		gpio_direction_output(TCC_GPE(17), 0);
	}
#endif
