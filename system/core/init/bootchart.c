...
#include "log.h"				// BC - For Kernel Message.
#include "property_service.h"	// BC - To get boot Mode property.
...
/* Save and Get snapshot jiffies */
long long init_jiffies=0; // TCC - Save jiffies when snapshot kernel is start.

/* Booting Mode Flag */
int bootchart_QB = 0;	// 0 : Normal Boot    1 : QuickBoot

/* Get the jiffies aftert Load snapshot image */
static void
get_log_init_jiffies(void)
{
    char  buff[65];
    int   fd, ret, len;
    ...
    if (fd >= 0) {
        ...
        buff[64] = 0;
        if (ret >= 0) {
            init_jiffies = 100LL*strtod(buff,NULL);
        }
    }
}
...
do_log_uptime(FileBuff  log)
{
    ...
    if (fd >= 0) {
        ...
        if (ret >= 0) {
			...
			/* if system is booting on QuickBoot Mode, it fixes jiffies value. */
			if (bootchart_QB) {
				jiffies = jiffies - init_jiffies;	// TCC - get QuickBoot jiffy time
				if (jiffies < 0)
					jiffies = 0;
			}
...
int   bootchart_init( void )
{
	...
	/* Check Boot Mode, and continue bootchart_init or not. */

	pval = property_get("persist.sys.QB.user.allow", tmp_prop);
	if ( pval == 0 || !strcmp(tmp_prop, "false") || !strcmp(tmp_prop, "0"))
	{		
		ERROR("Bootchart : Normal Boot Mode.\n");
		ERROR("Bootchart : BootChart START!\n");
	}
	else if ((bootchart_QB==0) && (pval!=0) && (!strcmp(tmp_prop, "1") || !strcmp(tmp_prop, "true")))
	{
		ERROR("Bootchart_init : persist.sys.QB.user.allow : %s\n", tmp_prop);
		ERROR("Bootchart : Making QuickBoot Image Mode.\n");
		ERROR("Bootchart : SKIP Bootchart.\n");
		return 0;
	}
	else if ((bootchart_QB==1) && (pval!=0) && (!strcmp(tmp_prop, "1") || !strcmp(tmp_prop, "true")))
	{
		ERROR("Bootchart : QuickBoot Mode.\n");
		ERROR("Bootchart : Bootchart START!\n");
	}
	else
	{
		ERROR("Bootchart : Unknown Boot Mode.\n");
		if (pval != 0)
			ERROR("Bootchart : persist.sys.QB.user.allow : %s\n", tmp_prop);
		pval = property_get("tcc.QB.boot.with", tmp_prop);
		if (pval == 0)
			ERROR("Bootchart : tcc.QB.boot.with : NULL\n");
		else
			ERROR("Bootchart : tcc.QB.boot.with : %s\n", tmp_prop);

		return 0;
	}

	/* Get init jiffies */
	get_log_init_jiffies();	// TCC - it is the jiffiy when the snapshot kernel is start.
