...
#if 1
	for (i=0; i<BLK_DEVICE2_NUM; i++)
	{
	  if ((p = strtok_r(NULL, delim, &save_ptr))) {
	    fstab->recs[cnt].blk_device2[i] = strdup(p);
	    //SLOGE("[ksjung] fstab->recs[cnt].blk_device2[%d] : %s\n", i, fstab->recs[cnt].blk_device2[i]);
	  } else {
	    fstab->recs[cnt].blk_device2[i] = strdup(NO_BLK_DEVICE2);
	    //SLOGE("[ksjung] fstab->recs[cnt].blk_device2[%d] : %s\n", i, fstab->recs[cnt].blk_device2[i]);
	  }
	}
#endif
...
// For telechips
int fs_mgr_is_nofuse(struct fstab_rec *fstab)
{
    return fstab->fs_mgr_flags & MF_NOFUSE;
}
