...
int CommandListener::VolumeCmd::runCommand(SocketClient *cli,
	...
	SLOGI("format argc : %d", argc);
	SLOGI("format argv[0] : %s", argv[0]);
	SLOGI("format argv[1] : %s", argv[1]);
	SLOGI("format argv[2] : %s", argv[2]);
	SLOGI("format argv[3] : %s", argv[3]);
#if 0
        if (argc < 3 || argc > 4 ||
...
#endif
	bool wipe = true;
        rc = vm->formatVolume(argv[2], argv[3], wipe);
		...
		SLOGD("usb port control on");		
		vm->usbControl(true);			
	} else if (!strcmp(argv[1], "disable")) {
		dumpArgs(argc, argv, 3);	
		SLOGD("usb port control off"); 		
		vm->usbControl(false);			
