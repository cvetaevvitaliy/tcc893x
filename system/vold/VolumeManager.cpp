...
#define MASS_STORAGE_NDD_FILE_PATH  "/sys/class/android_usb/android0/f_mass_storage/lun0/file"
#define MASS_STORAGE_EXT_FILE_PATH  "/sys/class/android_usb/android0/f_mass_storage/lun1/file"
...
int VolumeManager::formatVolume(const char *label, const char *fstype, bool wipe) {
    ...
    return v->formatVol(label, fstype, wipe);
}
...
int VolumeManager::usbControl(bool on) {
	int fd;
	char file[255] = "/sys/class/android_usb/android0/enable";

      SLOGI("usbControl %s ", file);
	  
	if (!access(file, F_OK)) {
	    SLOGI("usb turn on/off attempt when dst exists");
	}	

	if ((fd = open(file, O_WRONLY)) < 0) {
		SLOGE("Unable to open usb sys file (%s)", strerror(errno));
		return -1;
	}

	if (on) {
		SLOGI("usbControl %s on ", file);
		char flag[2] = "1";

		if (write(fd, flag, strlen( flag)) < 0) {
		    SLOGE("Unable to write to usb sys file (%s)", strerror(errno));
		    close(fd);
		    return -1;
		}
	} else {
		SLOGI("usbControl %s off ", file);		
		char flag[2] = "0";
...
int VolumeManager::unmountLoopImage(const char *id, const char *idHash,
    ...
    for (i = 1; i <= UNMOUNT_RETRIES; i++) {
        rc = umount2(mountPoint, MNT_DETACH);
        if (!rc) {
            break;
        }
...
int VolumeManager::shareVolume(const char *label, const char *method) {
    ...
    if (!strcmp(externalStorage, "/mnt/sdcard")) {

       if (!strcmp(v->getMountpoint(), "/mnt/sdcard")) {
          if ((fd = open(MASS_STORAGE_NDD_FILE_PATH, O_WRONLY)) < 0) {
          ...
          if ((fd = open(MASS_STORAGE_EXT_FILE_PATH, O_WRONLY)) < 0) {
             SLOGE("Unable to open ums lunfile (%s)", strerror(errno));
             return -1;
          }
